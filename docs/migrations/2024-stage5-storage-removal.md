...
## 2. Sekret licencji (`license_secret.key`)

1. Jeżeli plik `var/security/license_secret.key` zawiera dane w postaci base64 (format legacy), uruchom `python -m dudzian_migrate.license_secret --input var/security/license_secret.key --output var/security/license_secret.key`.
2. Narzędzie zapisze zaszyfrowany dokument powiązany z lokalnym fingerprintem oraz utworzy kopię zapasową pliku legacy (`license_secret.key.legacy`).
3. Runtime zgłasza błąd „Sekret licencji ma nieobsługiwany format 'legacy'” jeżeli wykryje niezmigrowany plik – konwersja jest obowiązkowa przed startem usług.
