...
4. Jeśli marketing korzysta z lustrzanego repozytorium S3/Git, porównaj sumę SHA256 `signal_quality/index.csv` z wersją w lustrze; w CI wykorzystaj job `marketing-bundle-parity`, który pobiera artefakty `stress-lab-report` oraz `exchange-report` (CSV + `signal_quality/index.csv`) i uruchamia `python -m scripts.audit.marketing_parity_check --stress-lab-dir artifacts/stress-lab-report/reports/stress_lab --signal-quality-index artifacts/exchange-report/reports/exchanges/signal_quality/index.csv --mirror-dir <mirror> --audit-output docs/audit/marketing_parity_report.md --json-output docs/audit/marketing_parity_report.json`. Brak parytetu hashy powoduje błąd pipeline; raport CI (`marketing-parity-report`) dołącz do zgłoszenia marketingowego i `docs/audit/paper_trading_log.md`. Pliki audytu (Markdown + JSON) są nadpisywane nawet przy błędzie walidacji, aby nie zostawiać starych wyników.
...
1. Utwórz job kontrolny (CI lub cron) uruchamiany co 12 h: `find reports/exchanges/signal_quality -name "*.json" -mtime -2 | wc -l` oraz `python scripts/export_marketing_bundle.py --destination var/marketing/benchmark --signing-key-env MARKETING_BUNDLE_HMAC --validate-only`. Jeżeli liczba plików jest mniejsza niż liczba wierszy w `reports/exchanges/<data>.csv` lub walidacja HMAC zwraca błąd, eskaluj do HyperCare i marketingu. Job `marketing-bundle-parity` pełni rolę blokerową – brak konfiguracji lustra (`MARKETING_PARITY_MIRROR_S3`/`MARKETING_PARITY_MIRROR_GIT`) traktuj jako błąd krytyczny.
