...
9. Zsynchronizuj lustrzane repozytorium (S3/Git): `aws s3 sync var/marketing/benchmark/ s3://<bucket>/benchmark/ --delete` lub `git add var/marketing/benchmark && git commit -m "Update benchmark bundle"`; po synchronizacji wykonaj `sha256sum` na `index.csv` i `manifest.json` w obu lokalizacjach i zapisz wynik w audycie. Rozbieżności oznaczają blokadę publikacji do czasu wyrównania hashy. Brak dostępnego katalogu lustra (np. `MARKETING_PARITY_MIRROR_*` wskazuje na nieistniejący bucket/branch) zapisuj jako `status=skipped` + `classification=infra_skip` + `result=non_passing` i traktuj procesowo jako blokadę publikacji do czasu odtworzenia lustrzanej kopii.
...
4. Jeśli marketing korzysta z lustrzanego repozytorium S3/Git, porównaj sumę SHA256 `signal_quality/index.csv` z wersją w lustrze; w CI wykorzystaj job `marketing-bundle-parity`, który pobiera artefakty `stress-lab-report` oraz `exchange-report` (CSV + `signal_quality/index.csv`) i uruchamia `python -m scripts.audit.marketing_parity_check --stress-lab-dir artifacts/stress-lab-report/reports/stress_lab --signal-quality-index artifacts/exchange-report/reports/exchanges/signal_quality/index.csv --mirror-dir <mirror> --audit-output docs/audit/marketing_parity_report.md --json-output docs/audit/marketing_parity_report.json`. Brak parytetu hashy powoduje błąd pipeline; przy braku konfiguracji lustra job zapisuje raport `status=skipped` (`classification=infra_skip`, `result=non_passing`) i nie wykonuje walidacji parity. Raport CI (`marketing-parity-report`) dołącz do zgłoszenia marketingowego i `docs/audit/paper_trading_log.md`. Pliki audytu (Markdown + JSON) są nadpisywane nawet przy błędzie walidacji, aby nie zostawiać starych wyników.
5. W przypadku opóźnionego potwierdzenia lustrzanego (np. region S3 z opóźnioną spójnością) oznacz release notes statusem „oczekuje na parzystość lustra” i w ciągu 24 h powtórz walidację hashy; brak zgodności po 24 h wymaga rollbacku bundla do ostatniej zweryfikowanej wersji. Job CI zapisuje wynik w `docs/audit/marketing_parity_report.md` – jeśli jest czerwony, wykonaj akcję korygującą przed publikacją.

### 9.4. Monitoring powydawniczy i rollback
1. Utwórz job kontrolny (CI lub cron) uruchamiany co 12 h: `find reports/exchanges/signal_quality -name "*.json" -mtime -2 | wc -l` oraz `python scripts/export_marketing_bundle.py --destination var/marketing/benchmark --signing-key-env MARKETING_BUNDLE_HMAC --validate-only`. Jeżeli liczba plików jest mniejsza niż liczba wierszy w `reports/exchanges/<data>.csv` lub walidacja HMAC zwraca błąd, eskaluj do HyperCare i marketingu. Job `marketing-bundle-parity` pełni rolę blokerową na poziomie procesu: brak konfiguracji lustra (`MARKETING_PARITY_MIRROR_S3`/`MARKETING_PARITY_MIRROR_GIT`) daje techniczny `infra_skip` (`result=non_passing`) i nadal blokuje publikację do czasu naprawy konfiguracji.
2. W razie wykrycia niespójności po publikacji (różne sumy SHA między lustrami lub brakujące snapshoty) wykonaj regenerację bundla z `--force-rebuild --include-signal-quality`, a poprzednią wersję przenieś do `var/audit/benchmark/<data>/superseded/` wraz z notatką audytową opisującą różnicę. Do czasu naprawy ustaw status releasu na „freeze” i nie publikuj materiałów marketingowych.
3. Jeżeli release notes zostały już podpisane, dołącz erratę z nową sumą SHA256 i timestampem walidacji HMAC; w marketingowym repo/buckecie dodaj plik `ERRATA_<data>.md` z listą zmienionych artefaktów i linkiem do nowego manifestu. Raport z joba CI (`marketing-parity-report`) podlinkuj w erracie jako dowód korekty.
4. Po rollbacku zaktualizuj `docs/audit/paper_trading_log.md` oraz wewnętrzny kanał operacyjny, wskazując przyczynę, zakres poprawki i potwierdzenie ponownej walidacji bundla. Dołącz hash z `docs/audit/marketing_parity_report.md` (lub artefaktu CI) oraz identyfikator lustra, które wymagało korekty.
5. Gdy rollback nastąpił z powodu niespójności lustra, odnotuj w audycie, który bucket/branch posiadał odchylenie oraz dołącz wynik `sha256sum` przed i po korekcie; pozwoli to na późniejszą retrospekcję SLA replikacji. Jeśli job CI wykrył różnicę, nie resetuj statusu releasu do czasu zielonego przebiegu `marketing-bundle-parity`.
6. Jeśli w trakcie walidacji bundla pojawi się nowy snapshot lub checklista (zmieniający liczbę rekordów vs. release notes), zamroź bundel w wersji z artefaktami użytymi do podpisu (`freeze`), wykonaj rebuild na kopii roboczej i dopiero po zatwierdzeniu audytowym zamień wersję w marketingu/release notes. Upewnij się, że raport CI w artefakcie `marketing-parity-report` opisuje, które pliki zmieniły hash w trakcie freeze.
7. Przy każdym rebuildzie, który dotyka release notes lub zgłoszenia marketingowego, zaktualizuj dziennik z odniesieniem do wersji manifestu (`manifest.json`/`benchmark_marketing_bundle.sig`) oraz dołącz tabelę „przed/po” (liczba snapshotów, hash `index.csv`, hash `manifest.json`). Wpis uzupełnij numerem joba CI i nazwą lustra (S3/Git), aby audyt mógł odtworzyć przebieg automatycznej walidacji.
