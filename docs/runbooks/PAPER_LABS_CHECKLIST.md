...
Current bounded mode is intentionally conservative and valid only within the script's configured duration guard (after CPR-10, explicitly allowing up to 1800 seconds for controlled validation).
A 5-minute / 300-second, 10-minute / 600-second, and 30-minute / 1800-second controlled validation run are allowed by the current guard; longer runs (for example 24h/72h) still require separate duration-bound patches and seals.
