...
- `--include-tag pipeline` wymaga obecności przynajmniej jednego z podanych tagów (możesz podawać opcję wielokrotnie lub rozdzielać tagi przecinkami) – filtr obejmuje zarówno tagi z sekcji raportu, jak i tagi scalone z metadanych strategii, datasetów oraz kontekstu. `--exclude-tag legacy` usuwa raporty z niedozwolonymi tagami.
...
- `--include-issue dq_missing_feature` zachowuje raporty zawierające określone kody problemów lub alertów (np. `dq_missing_feature`, `drift_threshold_exceeded`), natomiast `--exclude-issue legacy_issue` usuwa raporty, w których występują wykluczone kody. Argumenty można podawać wielokrotnie lub rozdzielać przecinkami; porównanie jest nieczułe na wielkość liter.
...
- `--include-engine daily_trend_momentum` zachowuje raporty powiązane ze strategiami uruchamianymi przez wskazane silniki (np. `daily_trend_momentum`, `volatility_target`), a `--exclude-engine legacy` usuwa wpisy dotyczące silników spoza aktualnego katalogu. Argumenty można przekazywać wielokrotnie lub rozdzielać przecinkami; nazwy są normalizowane do małych liter.
...
- `--include-license-tier standard` ogranicza raporty do strategii wymagających określonego poziomu licencji (np. `standard`, `enterprise`), a `--exclude-license-tier legacy` usuwa wpisy z niedozwolonym poziomem licencyjnym. Argumenty można przekazywać wielokrotnie lub rozdzielać przecinkami; wartości są normalizowane do małych liter.
- `--include-risk-class directional` pozwala skupić się na raportach strategii o wskazanych klasach ryzyka (np. `directional`, `market_making`), natomiast `--exclude-risk-class legacy` usuwa wpisy z niepożądanymi klasami. Argumenty można podawać wielokrotnie lub rozdzielać przecinkami.
- `--include-required-data ohlcv` zachowuje raporty wymagające określonych źródeł danych (np. `ohlcv`, `technical_indicators`), a `--exclude-required-data legacy` usuwa raporty zależne od nieobsługiwanych źródeł. Argumenty są nieczułe na wielkość liter i mogą być przekazywane wielokrotnie.
- `--include-exchange binance` ogranicza raporty do wskazanych giełd lub dostawców danych (np. `binance`, `kraken`), natomiast `--exclude-exchange legacy` usuwa wpisy pochodzące z wykluczonych źródeł. Argumenty można przekazywać wielokrotnie lub rozdzielać przecinkami; wartości są normalizowane do małych liter.
...
- `--include-profile conservative` ogranicza wyniki do strategii o wskazanych profilach ryzyka (np. `conservative`, `balanced`), a `--exclude-profile legacy` usuwa raporty dla profili wyłączonych z audytu. Argumenty można przekazywać wielokrotnie lub rozdzielać przecinkami; wartości są normalizowane do małych liter.
- `--include-strategy mean_reversion` pozwala filtrować raporty dotyczące konkretnych strategii (np. `mean_reversion`, `trend-follow`), natomiast `--exclude-strategy legacy` usuwa wpisy powiązane z wykluczonymi strategiami. Argumenty można podawać wielokrotnie lub rozdzielać przecinkami; nazwy są normalizowane do małych liter.
- `--include-dataset primary` ogranicza wyniki do raportów powiązanych z określonymi zbiorami danych (np. `primary`, `nightly-features`), a `--exclude-dataset legacy` usuwa wpisy dotyczące wykluczonych źródeł. Argumenty można przekazywać wielokrotnie lub rozdzielać przecinkami; wartości są normalizowane do małych liter i uwzględniają aliasy z kontekstu raportu.
- `--include-model trend_v2` pozwala filtrować raporty dla konkretnych modeli lub artefaktów (np. `trend_v2`, `mean_v1`), a `--exclude-model legacy` usuwa wpisy powiązane z wykluczonymi modelami. Argumenty można podawać wielokrotnie lub rozdzielać przecinkami; nazwy są normalizowane do małych liter.
- `--include-model-version 1.2.0` ogranicza raporty do wskazanych wersji modeli/artefaktów (np. `1.2.0`, `rc1`), natomiast `--exclude-model-version legacy` usuwa wpisy z niepożądanymi wersjami. Argumenty można przekazywać wielokrotnie lub rozdzielać przecinkami; wartości są normalizowane.
