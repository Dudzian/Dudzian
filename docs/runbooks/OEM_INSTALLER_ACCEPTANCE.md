...
| 1a. Uruchom automatyczną weryfikację fingerprintu/licencji (`python scripts/oem_provision_license.py --verify --fingerprint config/fingerprint.expected.json --fingerprint-key … --license-key … --output var/licenses/registry.jsonl`) | Security | Log CLI, `config/fingerprint.expected.json`, `var/licenses/registry.jsonl` | Skrypt kończy się kodem 0, brak błędów polityki OEM, podpisy HMAC poprawne |
