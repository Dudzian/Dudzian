...
| [ ] (Opcjonalnie) wystartuj usługę cloudową (`python scripts/run_cloud_service.py --config config/cloud/server.yaml --emit-stdout`), upewnij się, że `cloud.enabled_signed` wskazuje na świeżo podpisane `var/runtime/cloud_flag.json` + `.sig` (HMAC/Ed25519), wykonaj testowy handshake `CloudAuthService.AuthorizeClient` i potwierdź wpisy `cloud_auth_granted`/`cloud_session_*` w `logs/security_admin.log` | Inżynier Runtime + SecOps | `config/cloud/`, `var/runtime/cloud_ready.json`, `var/runtime/cloud_flag.*`, `logs/security_admin.log` | [ ] |
...
> **Uwaga:** lint layoutu kończy się błędem przy każdej pozostałości historycznych katalogów lub pliku wykonywalnym w `archive/**`. Usuń takie pliki przed kontynuowaniem procedury.
