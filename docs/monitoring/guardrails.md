...
- **Backup/restore alertów Alertmanagera** – eksport reguł i silnika
  routingu do paczki (np. `deploy/prometheus/rules/cloud_worker_alerts.yml`)
  oraz weryfikacja importu po awarii control-plane. Lista kontrolna:
  1. Zrzut aktualnych reguł do repozytorium DR (wraz z adnotacjami
     `owner/team/channel`).
  2. Odtworzenie reguł na węźle zapasowym Alertmanagera i potwierdzenie, że
     alerty `CloudWorkerHealthDegraded`/`CloudWorkerErrorCritical` są
     gotowe do odpalenia.
  3. Walidacja webhooków/receiverów HyperCare/CloudAlertService po
     przełączeniu (status HTTP 2xx oraz obecność `service=cloud-orchestrator`).
- **Przełączenie Prometheus multi-region** – w scenariuszu DR konieczne jest
  przełączenie scrape’ów `bot_cloud_health_status` i
  `bot_cloud_worker_last_error` na zapasowy Prometheus, wraz z testowym
  wyzwoleniem alertu workerów. Procedura obejmuje synchronizację reguł,
  potwierdzenie dostępności endpointów `/federate` i sanity-check metryk
  w nowym regionie (wartość `bot_cloud_health_status >= 1`,
  `bot_cloud_last_error <= 0`).
- **Walidacja `_lastError`/`_health` CloudOrchestratora** – synthetic probes
  (patrz `scripts/dr_synthetic_probes.py`) wywołują `HealthService.Check` i
  oczekują nagłówków `x-bot-cloud-health`/`x-bot-cloud-last-error`. Wynik
  zapisuje się w `reports/ci/dr_probes/` i powinien wskazywać status `running`
  oraz brak `_lastError`; w przeciwnym razie należy skorelować alerty z
  logami orchestratora. Probes powinny raportować opóźnienie RPC < 5 s
  (limit CLI `--latency-threshold-ms`) i flagę `healthOk=true`/`prometheusOk=true`.
