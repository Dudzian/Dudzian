...
```json
{
  ...
  "executable_authority": {
    ...
    "s9d_c25_m08_reconciliation_authority_disposition": {
      ...
      "production_discovery": "no production executable M0.8 accepted journal/accounting projection, CoreAcceptedObservedBalanceFactProjection, accepted reconciliation-result authority, durable reconciliation history, or owner-composed reconciliation publisher/carrier exists",
      "non_authorities_found": "bot_core.execution.paper.LedgerEntry is a paper execution DTO; bot_core.persistence.record_registry contains contract/schema metadata; AccountingRiskProjection, CoreAcceptedAccountingFactProjection, AssetReference, observed-fact validation, and reconciliation outcome computation exist only in tests/architecture/test_cryptohunter_ledger_portfolio_capital_and_pnl.py",
      ...
      "missing_upstream_dependencies": [
        "production carrier-owned accepted M0.8 journal/accounting history that derives exact projected quantity for the complete reconciliation key at as_of_utc",
