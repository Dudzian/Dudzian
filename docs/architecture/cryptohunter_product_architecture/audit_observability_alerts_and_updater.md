        ...
        "m05_historical_instrument_dependency": {
          ...
          "m05_catalog_source_policy_disposition": {
            "PAPER": {
              ...
              "instrument_catalog_discovery_policy": "LOCAL_CONTRACT_METADATA",
              "account_identity_discovery_policy": "LOCAL_SIMULATED_IDENTITY",
              "authority_status": "METADATA_MECHANISM_AVAILABLE_CATALOG_HISTORY_NOT_IMPLEMENTED",
              "external_authenticated_adapter_membership_required": "NO",
              "complete_snapshot_derivation": "NOT_AVAILABLE: the release-owned canonical metadata mechanism is implemented, but its registry intentionally has zero entries because no product-owned PAPER instrument set is approved.",
              "instrument_ids_source": "RELEASE_OWNED_SOURCE_AVAILABLE_BUT_EMPTY: only source-controlled entries in bot_core.instruments.paper_canonical_metadata can establish metadata membership; no IDs exist until product approval.",
              ...
              "core_owned_catalog_path": "METADATA_SOURCE_ONLY: stable restart-resolvable product metadata exists, but M0.5 InstrumentCatalogSnapshot acceptance/history authority is intentionally NOT_IMPLEMENTED.",
              ...
              "nearest_missing_prerequisite": "MISSING_APPROVED_CANONICAL_PAPER_INSTRUMENT_ENTRIES",
              "canonical_metadata_discovery": {
                ...
                "catalog_acceptance_disposition": "NOT_IMPLEMENTED: the metadata trust boundary is not InstrumentCatalogSnapshot acceptance or accepted history; zero approved entries prevents snapshot derivation.",
                ...
                "external_adapter_authentication_required": "NO: PAPER uses LOCAL_CONTRACT_METADATA; the remaining content blocker is product approval of canonical instrument entries.",
                "production_artifact": "bot_core.instruments.paper_canonical_metadata; source-controlled and packaged with the release, with no caller-selected load path.",
                "membership_rule": "Only inclusion in the private release-owned tuple establishes canonical metadata membership. Every included entry is validated before indexing. A fingerprint binds the full ordered entry projection for integrity and cannot enroll raw metadata.",
                ...
                "restart_and_version_binding": "Repeated imports/loads resolve source identity cryptohunter.product.paper-instrument-metadata and version 1.0.0 with stable entries. The SHA-256 binds source ID, source version, content status, and every ordered entry field including nested AssetReference and explicit nulls; future approved metadata requires a source-controlled version change.",
                "stable_id_policy": "Future entries must carry product-approved stable instrument_id and catalog_snapshot_id values; IDs are never generated at process start. Identity changes require explicit version evolution under frozen M0.5 rules.",
