...
```json
{
  ...
  "upstream_binding_manifest": [
    {
      ...
      "sha256": "bb92436a67abe42975d763b06717962009d3f4bc8a6066b227f34c8e7d178e9b",
  ...
  "executable_authority": {
    ...
    "s9d_c25_m08_reconciliation_authority_disposition": {
      ...
      "production_discovery": "production search found schemas/registry metadata and non-authoritative runtime DTO/event paths, but no genuine production M0.5 accepted historical Instrument membership authority and no production M0.7 accepted canonical Order/lifecycle projection deriving accepted_fill_ids_by_order_id; R1A self-enrolling substitutes were removed",
      ...
      "missing_upstream_dependencies": [
        "production genuine M0.5 accepted historical Instrument membership authority",
        "production M0.7 accepted canonical Order and lifecycle projection deriving accepted_fill_ids_by_order_id and validating Fill progression",
        "production M0.7 accepted SUBMIT_ORDER command context required for capital reservation accounting",
        "production M0.7 accepted terminal lifecycle/event context required for capital release accounting",
        "production internal_transfer authority remains a separate reconciliation-relevant M0.8 dependency",
      ...
      "match_authority": "UNAVAILABLE; MATCH cannot be produced until both independent accepted authority sides exist and exact quantities compare equal",
      "reconciliation_divergence_alertstore_adapter": "NOT_STARTED",
      "reconciliation_divergence_status": "OPEN_SOURCE_AUTHORITY",
      "source_producer_authenticity": "OPEN",
      "s9d": "OPEN",
      "updater": "NOT_STARTED",
      "m010": "CLOSED",
      "c24_r1": "CLOSED",
      "internal_accounting_authority": "M0.8_INTERNAL_ACCOUNTING_AUTHORITY_BLOCKED_UPSTREAM_SOURCE_MEMBERSHIP",
      "internal_accounting_authority_boundary": "CoreAcceptedAccountingFactProjection binds deposit/withdrawal economics; raw M0.7 Full Fill receives exact structural/fingerprint validation only. Fill journal publication remains unsupported until genuine M0.5 historical Instrument membership, M0.7 canonical Order/lifecycle history, and reservation/FIFO authority paths exist",
      ...
      "m07_full_fill_authority": "M07_FULL_FILL_AUTHORITY_BLOCKED_M05_HISTORICAL_INSTRUMENT_AND_M07_LIFECYCLE_AUTHORITY",
      "m07_full_fill_authority_boundary": {
        "structural_implementation": "bot_core/execution/m07_fill_validation.py validates exact frozen fields, canonical encodings, fee semantics and independently recomputes fill_fingerprint_sha256",
        ...
        "m05_historical_instrument_dependency": {
          ...
          "discovery_scope": [
            "all production bot_core modules",
            "runtime process topology and CoreHost composition",
            "runtime and ExchangeManager adapter registries plus dynamic plugin/config loading",
            "M0.3 CoreHost authority and M0.4 ProductCapabilities contracts",
            "M0.5 Instrument and InstrumentCatalogSnapshot machine contract and executable oracle",
            "M0.6 downstream adapter-family Instrument/Catalog projections",
            "persistence kernel record registry, carriers, restore and authenticity components",
            "S9C/S9D source-producer authenticity and accepted-observation authorities"
          ],
          "genuine_production_authority": "NOT_FOUND",
          ...
          "missing_upstream_anchor": "POLICY_SPECIFIC: PAPER requires a Core-owned canonical local-contract-metadata Catalog/history producer; TESTNET requires independently authenticated adapter-source membership before adapter snapshot acceptance.",
          "forbidden_substitutes": [
          ...
          "production_catalog_source_authority_discovery": {
            "source_identity_authenticator": {
              ...
              "evidence": "NOT_REQUIRED_AS_EXTERNAL_ADAPTER_MEMBERSHIP for PAPER LOCAL_CONTRACT_METADATA; NOT_FOUND for TESTNET ADAPTER_SNAPSHOT_REQUIRED"
            },
            "catalog_snapshot_acceptor": {
              "status": "NOT_FOUND",
              "evidence": "no production ingestion component implements either frozen PAPER LOCAL_CONTRACT_METADATA derivation or TESTNET independently sourced ADAPTER_SNAPSHOT_REQUIRED acceptance"
            },
            "accepted_catalog_carrier_history": {
              "status": "NOT_FOUND",
              "evidence": "no production carrier retains immutable accepted Catalog snapshots, monotonic membership revisions, replay conflicts, predecessor membership or historical resolution"
            },
            "scope_binding_evidence": {
              "status": "SCHEMA_ONLY_NOT_AUTHORITY",
              "fields": [
                "exchange_id",
                "environment",
                "market_type",
            ...
            "independent_non_caller_mintable_anchor": {
              "status": "NOT_FOUND",
              "evidence": "PAPER has no Core-owned immutable local metadata producer/history; TESTNET has no independently authenticated adapter-source membership; M0.3/M0.4, config, fingerprints, persistence DTOs and S9C/S9D observation authorities do not fill either policy-specific gap"
            }
          },
          "existing_adapter_registration_disposition": "NOT_AUTHORITY_FOR_ADAPTER_SNAPSHOT_REQUIRED_SOURCE_PROVENANCE: public register_adapter_factory and register_native_adapter accept caller-selected identifiers/factories and mutable configuration provenance; this finding applies to TESTNET ADAPTER_SNAPSHOT_REQUIRED and does not invalidate PAPER LOCAL_CONTRACT_METADATA.",
          "catalog_authority_stage_disposition": "BLOCKED_POLICY_SPECIFIC: neither the PAPER local-metadata producer/history path nor the TESTNET authenticated adapter-snapshot path exists; Catalog accept/resolve/history/restore APIs remain intentionally absent.",
          "enabled_frozen_exchange_policy_matrix": [
            {
            ...
            },
            {
              ...
              "capability_discovery_policy": "ADAPTER_SNAPSHOT_REQUIRED",
              "instrument_catalog_discovery_policy": "ADAPTER_SNAPSHOT_REQUIRED",
              "account_identity_discovery_policy": "PRIVATE_READINESS_CHECK_REQUIRED"
            }
          ],
          "m05_catalog_source_policy_disposition": {
            "PAPER": {
              "exchange_id": "paper_simulated_venue",
              "supported_environments": [
                "PAPER"
              ...
              "instrument_catalog_discovery_policy": "LOCAL_CONTRACT_METADATA",
              "account_identity_discovery_policy": "LOCAL_SIMULATED_IDENTITY",
              "authority_status": "FROZEN_POLICY_CONFLICT_DYNAMIC_EXCHANGE_CATALOG_REQUIRES_EXPLICIT_M05_MIGRATION",
              "external_authenticated_adapter_membership_required": "PRODUCT_TARGET_YES_FROZEN_PAPER_POLICY_NO: LOCAL_CONTRACT_METADATA binds paper_simulated_venue/paper_simulation_adapter_family and defines no authenticated real-exchange lineage.",
              "complete_snapshot_derivation": "NOT_AVAILABLE: the frozen PAPER source is release-owned LOCAL_CONTRACT_METADATA; it cannot dynamically derive a full selected real-exchange catalog.",
              "instrument_ids_source": "FROZEN_CURRENT_PATH_ONLY: release-owned local entries could establish PAPER membership, but product intent rejects a static pair whitelist and the tuple remains empty.",
              "metadata_immutability": "Build-time immutable internal record types and a private release-owned tuple expose read-only enumeration/resolution. Every tuple entry is frozen-M0.5-compatible validated before indexing; there is no public nominal constructor, caller path, config/env/CLI override, merge, writer, adapter registration, or plugin injection surface.",
              "core_owned_catalog_path": "NOT_IMPLEMENTED: no trusted real-exchange ingestion/canonicalization authority exists, and the frozen PAPER identity/source policy cannot represent the requested lineage.",
              ...
              "nearest_missing_prerequisite": "EXPLICIT_M05_SOURCE_VENUE_EXECUTION_ENVIRONMENT_IDENTITY_MIGRATION",
              "canonical_metadata_discovery": {
                "build_time_immutable_production_metadata": "AVAILABLE_RELEASE_OWNED_EMPTY_INSTRUMENT_REGISTRY",
                "build_time_values_found": [
                  "source_id=cryptohunter.product.paper-instrument-metadata",
                  "source_version=1.0.0",
                  "content_status=NO_APPROVED_CANONICAL_PAPER_INSTRUMENT_ENTRIES",
                  "approved_entry_count=0"
                ],
                "runtime_derived_metadata": "PaperBackend load_markets/MarketRules delegation is neither genuine source authentication nor a frozen Catalog acceptance authority; raw results must never be accepted directly.",
                ...
                "catalog_acceptance_disposition": "NOT_IMPLEMENTED: zero entries remain; more importantly, dynamic selected-exchange canonicalization is outside frozen PAPER LOCAL_CONTRACT_METADATA semantics.",
                ...
                "external_adapter_authentication_required": "PRODUCT TARGET REQUIRES IT, BUT FROZEN PAPER DOES NOT DEFINE IT; this mismatch requires an explicit M0.5 migration rather than a workaround.",
                "production_artifact": "bot_core.instruments.paper_canonical_metadata remains an empty fallback/static local-metadata mechanism under the frozen contract, not the future dynamic PAPER source.",
                "membership_rule": "Current frozen fallback membership can arise only from the private release-owned tuple. Product intent does not require or permit filling it as a pair-by-pair approval whitelist.",
                "caller_extension": "IMPOSSIBLE_BY_API: CoreConfig.instrument_universes, exchange_symbols, YAML, CLI, environment, adapters, plugins, fixtures, raw mappings and matching self-hashes are not accepted inputs.",
                "restart_and_version_binding": "The empty release projection is restart-stable. Future dynamic catalogs require durable accepted immutable snapshots, predecessor lineage and exact historical resolution rather than release-bound replacement.",
                "stable_id_policy": "Frozen IDs are Core-owned prefixed UUIDv7 values, so a migration needs a durable authority mapping the immutable venue identity tuple to one stable instrument_id; refresh must resolve that map, never mint per refresh or use an ordinary hash.",
                "asset_reference_policy": "Future entries contain full explicit AssetReference values for base, quote and settlement as applicable; contract_value_currency is a nullable scalar string. Venue symbols are never parsed to derive assets or canonical display codes.",
                "approved_entry_count": 0,
                "frozen_fields_without_production_owned_source_meaning": "EMPTY_MEANS_SCHEMA_COVERAGE_COMPLETE_ONLY: the release mechanism can represent and validate every frozen field; it does not claim approved values or entries, whose count remains zero.",
                "production_validation": "AVAILABLE: import-time private validator enforces exact PAPER venue/adapter scope, frozen market/instrument pairs, AssetReference structure, canonical decimals, positive metadata_version, timestamp graph, derivative conditional rules, unique instrument_id and immutable identity tuple assumptions before _BY_ID construction.",
                "contract_value_currency_type": "nullable scalar non-empty string for derivatives; never AssetReference, exactly matching frozen validate_instrument_record."
              }
            },
            "TESTNET": {
              "exchange_id": "generic_testnet_venue",
              ...
              "closed_registry_sufficiency": "INSUFFICIENT: the frozen registry fixes legal identity/scope and adapter_family_id but supplies neither an adapter-produced snapshot nor evidence that a concrete producer owns that family.",
              "snapshot_source_requirement": "A full Catalog snapshot must be produced through the frozen generic_testnet_adapter_family path; a matching string or locally hashed raw Catalog is not source evidence.",
              "runtime_registry_forgery": "POSSIBLE_AS_DI_DECLARATION: ordinary callers can register a chosen factory/exchange_id, so runtime registry presence cannot authenticate ownership of the frozen adapter family.",
              "minimal_independent_evidence": "Durable Core-owned exact-scope membership authenticating the snapshot producer as generic_testnet_adapter_family before Catalog acceptance; the design remains intentionally unspecified in this discovery.",
              "nearest_missing_prerequisite": "CORE_OWNED_DURABLE_AUTHENTICATED_ADAPTER_SOURCE_MEMBERSHIP"
            },
            "LIVE": {
              "frozen_exchange_entry_status": "NO_FROZEN_ENABLED_LIVE_EXCHANGE_ENTRY",
              "authority_status": "NOT_APPLICABLE_TO_CURRENT_FROZEN_REGISTRY",
              "evidence": "LIVE exists in the environment registry, but no ENABLED frozen exchange entry supports LIVE."
            }
          },
          "paper_catalog_product_decision_discovery": {
            "status": "NOT_IMPLEMENTED",
            "product_decision": {
              "static_product_owned_paper_pair_whitelist_required": "NO",
              "full_trusted_exchange_catalog_is_availability_source": "YES",
              "catalog_membership_implies_trading_authority": "NO",
              "new_listing_without_software_release_target": "YES",
              "paper_release_owned_entries_must_remain_empty": "YES"
            },
            "frozen_contract_compatibility": {
              "dynamic_model_legal_without_contract_change": "NO",
              "conflict": "PAPER is closed to exchange_id=paper_simulated_venue, environment=PAPER, source_adapter_family_id=paper_simulation_adapter_family and LOCAL_CONTRACT_METADATA. Real venues are not enabled PAPER registry entries, and no source-venue lineage field/authenticator exists.",
              "model_a_legal": "NO: rewriting real venue identity/asset namespace/adapter family as paper_simulated_venue destroys provenance; retaining lineage is not expressible in the frozen Instrument/Catalog shape.",
              "model_b_legal": "NO: a real exchange_id cannot pair with PAPER because frozen enabled registry entries do not support that tuple; runtime configuration cannot extend the closed registry.",
              "minimal_migration": "Explicitly revise M0.5 to separate source venue/catalog identity from execution environment, register supported trusted real exchange families, define authenticated source-family/version provenance and acceptance, and preserve all existing fail-closed validation."
            },
            "discovery_answers": {
            ...
            "exchange_catalog_ingestion": {
              "status": "NOT_IMPLEMENTED",
              "required": "Authenticate exchange identity, adapter family and version independently of caller claims; ingest a complete non-partial venue/market-type snapshot with source provenance and timestamps."
            },
            "catalog_acceptance": {
              "status": "NOT_IMPLEMENTED",
              "required": "Core validates trusted AssetReference metadata without symbol parsing, capabilities, completeness, snapshot identity/hash, monotonic predecessor and append-only accepted history; partial/stale snapshots fail activation and never overwrite history."
            },
            "manual_trading_universe": {
              "status": "NOT_IMPLEMENTED",
              "flow": [
                "accepted Catalog query/search/filter",
                "user submits instrument_id selection intent",
                "Core resolves every ID in explicit accepted VALID fresh source snapshots",
                "Core creates a new immutable TradingUniverse version",
                "activation still passes account/capability/strategy/risk/execution gates"
              ],
              "authority": "Selection cannot create Instrument or Catalog membership; unknown IDs fail closed and GUI is not authority."
            },
            "autonomous_trading_universe": {
              "status": "NOT_IMPLEMENTED",
              "flow": [
                "accepted Catalog",
                "policy-derived Eligible Universe",
                "Candidate Ranking",
                "new versioned Active Autonomous Selection"
              ],
              "authority": "Selector cannot mutate Catalog, self-enroll symbols, or maintain an authority-bypassing mutable whitelist."
            },
            "autonomous_candidate_ranking": {
              "status": "NOT_IMPLEMENTED",
              "constraints": "No thresholds or scoring are specified. Ranking consumes only eligible accepted members and grants neither Instrument identity nor execution/risk authority."
            },
            "execution_authority_separation": {
              "status": "NOT_IMPLEMENTED",
              "invariant": "accepted Catalog member AND eligible AND selected AND strategy-compatible AND M0.9 risk-approved AND kill-switch-permitted AND execution-lease-authorized AND M0.7-lifecycle-valid; autonomous ranking alone is never sufficient."
            },
            "catalog_lifecycle": {
              "snapshot_version": "monotonic immutable accepted version",
              "predecessor": "exact prior accepted snapshot reference",
              "new_listing": "new member in a later complete accepted snapshot; stable identity map reused",
              "changed_metadata": "new immutable metadata version linked through accepted history",
              "delisting": "status transition/absence semantics must be explicitly migrated; never hard delete historical membership",
              "stale_catalog": "not eligible for new activation",
              "partial_catalog": "retained/rejected per frozen rules but never ACTIVE source",
              "restart": "durable accepted snapshot, membership, identity map and lineage restore before resolution",
              "historical_resolution": "exact snapshot_id plus instrument_id/metadata_version remains resolvable"
            },
            "market_types": {
              "catalog_capability": [
                "SPOT",
              ...
              "rule": "Exchange offering does not imply TradingUniverse, edition, strategy, risk or execution eligibility."
            },
            "testnet_disposition": "UNCHANGED: ADAPTER_SNAPSHOT_REQUIRED and its authenticated provenance blocker remain open; a future authenticator may be shared only after explicit architecture design.",
            "live_disposition": "UNCHANGED_NOT_ENABLED",
            "global_status_invariants": {
              "C25": "BLOCKED",
              "S9D": "OPEN"
            },
            "explicit_m05_source_venue_execution_environment_identity_migration_design": {
              "status": "DESIGN_ONLY_NOT_IMPLEMENTED",
              "decision_id": "EXPLICIT_M05_SOURCE_VENUE_EXECUTION_ENVIRONMENT_IDENTITY_MIGRATION",
              "scope": "M0.2 Instrument identity-dimensions plus M0.5 source-venue/execution-separation migration design; M0.5 authoritative cutover depends on the explicit M0.2 identity migration. No production authority or downstream contract is changed.",
              "chosen_identity_model": {
                "model": "TWO_LEVEL_SOURCE_PRODUCT_AND_WORKSPACE_INSTRUMENT_WITH_MODEL_2_STYLE_EXECUTION_REFERENCE",
                "decision": "Source venue owns the logical source-product key and shared metadata fact; each canonical frozen-domain Instrument is a projection owned by exactly one Workspace. PAPER eligibility references that Workspace Instrument, account/backend and TradingUniverse and never creates a second PAPER Instrument in that Workspace.",
                "comparison": "exact bytes for each component; no trim, case-fold, Unicode normalization or symbol parsing",
                "environment_in_identity": false,
                "namespace_rule": "source_exchange_id is the source-product venue namespace boundary; any source_market_namespace_id is source-owned and never execution_environment.",
                "why_minimal": "Shared authenticated source facts feed separate Workspace-owned Instrument projections, preserving frozen ownership, provenance and history without PAPER/LIVE copies inside a Workspace.",
                "source_product_identity": {
                  ...
                  "optional_discriminator": "source_market_namespace_id only when a venue exposes collision-capable product namespaces",
                  "scope": "Venue-owned logical product key; globally reusable as source fact and independent of Workspace and execution_environment.",
                  "entity_status": "Not the canonical frozen-domain Instrument and does not itself mint instrument_id."
                },
                "workspace_instrument_identity": {
                  "tuple": [
                    "workspace_id",
                    "source_exchange_id",
                    "market_type",
                  ...
                  "optional_discriminator": "source_market_namespace_id exactly when present in source-product identity",
                  "mapping": "workspace_id + immutable source-product identity -> canonical instrument_id",
                  "ownership": "Exactly one Workspace parent per canonical Instrument; workspace_id is immutable.",
                  "cross_workspace_rule": "The same source product in two Workspaces receives two distinct instrument_ids; source-product equivalence is not canonical Instrument identity equivalence; foreign Instrument resolution fails closed."
                }
              },
              "evaluated_alternatives": [
                {
                  "model": "MODEL_1",
                  "disposition": "CHOSEN_WITH_WORKSPACE_PROJECTION_CORRECTION",
                  "reason": "Source venue owns source-product identity and metadata, while frozen M0.2 ownership requires a distinct canonical Instrument projection in each Workspace."
                },
                {
                  "model": "MODEL_2",
                  "disposition": "CHOSEN_AS_BINDING_PATTERN_NOT_NEW_ENTITY",
                  "reason": "A PAPER reference is useful only as execution eligibility/binding to source_instrument_id; it must not become another Instrument identity."
                },
                {
                  "model": "MODEL_3",
                  "disposition": "REJECTED",
                  "reason": "Keeping a PAPER-owned Instrument while adding source fields preserves two identities and ambiguous exchange_id semantics; frozen stability does not require that duplication and legacy records are handled explicitly."
                }
              ],
              "source_execution_separation": {
                "source_venue": "Real exchange identity that defines instrument and venue asset namespaces.",
                "catalog_provenance": "Authenticated producer membership plus immutable accepted snapshot lineage; independent of execution.",
                "execution_context": "ExchangeAccount/ExecutionRoute environment PAPER, TESTNET or LIVE plus backend identity and authorization; never mutates Instrument.",
                "paper_binding_required_fields": [
                ...
                "paper_binding_rules": [
                  "execution_environment must equal PAPER",
                  "source Instrument and catalog retain real source_exchange_id",
                  "source Instrument, TradingUniverse, ExchangeAccount and ExecutionRoute all resolve to the same workspace_id",
                  "TradingUniverse version must contain exact source_instrument_id and bind exact accepted source_catalog_snapshot_id",
                  "account/backend/route policies must permit PAPER for the selected source exchange and market type",
                  "Catalog membership and ranking grant no account, route, risk, lease, lifecycle or execution authority"
                ],
                "workspace_projection": "A Workspace-owned Instrument exact-binds one source-product key, one accepted upstream source snapshot/version and immutable metadata provenance; reuse of the source fact does not share instrument_id.",
                "paper_execution_flow": [
                  "trusted real-venue source product / AcceptedSourceCatalogSnapshot",
                  "WorkspaceCatalogProjection and Workspace-owned Instrument",
                  "same-Workspace TradingUniverse",
                  "same-Workspace/Portfolio PAPER ExchangeAccount",
                  "PAPER ExecutionRoute/backend"
                ]
              },
              "exchange_registry_migration": {
                "no_exchange_entries_added_by_this_design": true,
                "real_exchange_identity_schema": {
                  "required_fields": [
                    "exchange_id",
                    "display_name",
                    "status",
                  ...
                  "identity_rules": [
                    "exchange_id is Core-owned stable real venue identity, never display/config/plugin/class name",
                    "aliases are migration/read aliases only and never durable IDs",
                    "registry membership does not authenticate a producer and does not enable execution"
                  ]
                },
                "adapter_family_schema": {
                  "required_fields": [
                    "adapter_family_id",
                    "exchange_id",
                    "source_capabilities",
                  ...
                  "rule": "Family compatibility is descriptive; source authority additionally requires authenticated producer membership."
                },
                "source_catalog_policy": {
                  "fields": [
                    "allowed_adapter_family_ids",
                    "supported_source_market_types",
                    "completeness_evidence_policy",
                    "freshness_policy"
                  ],
                  "meaning": "Where trusted metadata may exist and under which acceptance rules; contains no execution grant."
                },
                "account_execution_policy": {
                  "fields": [
                    "allowed_execution_environments",
                    "account_identity_policy_by_environment",
                    "execution_backend_policy_by_environment"
                  ],
                  "meaning": "Where accounts/routes/execution may exist; independent of metadata availability."
                },
                "supported_environments_disposition": "Rename/reinterpret only on account/execution records as allowed_execution_environments. Do not use it for metadata; supported_source_market_types plus source_catalog_policy own metadata availability."
              },
              "paper_simulated_venue_disposition": {
                "future_role": [
                  "PAPER_EXECUTION_BACKEND_IDENTITY",
                  "OPTIONAL_SYNTHETIC_EXCHANGE_ACCOUNT_PROVIDER",
                  "SIMULATION_ENGINE_IDENTITY"
                ],
                "instrument_source_identity_owner": false,
                "status_as_instrument_owner": "DEPRECATED_LEGACY_READ_ONLY",
                "rule": "It may identify who simulates execution, but may not own or rewrite real venue Instrument, AssetReference or Catalog provenance."
              },
              "asset_reference_model": {
                "canonical_asset": "Venue-independent canonical asset identity when explicitly resolved by trusted mapping; no symbol-derived inference.",
                "venue_asset_reference": "Immutable reference containing canonical_asset_id when known, source_exchange_id asset namespace, exact venue asset code and metadata version/status.",
                "source_namespace_rule": "BTC on a real venue retains that source_exchange_id namespace in PAPER, TESTNET and LIVE.",
                "paper_rule": "PAPER binds balances/simulation to venue AssetReferences; it creates no paper_simulated_venue canonicalization and no cross-venue asset equivalence.",
                "scope_separation": "A venue AssetReference/source asset fact may be source-global; that fact does not globalize the Workspace-owned Instrument entity or permit a cross-Workspace Instrument reference."
              },
              "catalog_snapshot_contract": {
                ...
                "scope": "Upstream snapshot: source_exchange_id + market_type. Workspace membership projection: workspace_id + accepted_source_catalog_snapshot_id. execution_environment is forbidden from source provenance and fingerprint input.",
                "immutability": "Every accepted snapshot, ordered/set-canonicalized membership, timestamps, predecessor and producer provenance is immutable and historically resolvable.",
                "fingerprint": "Domain-separated canonical encoding exact-binds every required semantic field except the fingerprint itself; catalog_snapshot_id is Core-owned and is not derived by pretending a hash is UUIDv7.",
                "source_snapshot_identity": "Exact upstream venue/adapter snapshot identifier or authenticated retrieval identity; absence is explicit, never synthesized from execution environment.",
                "two_level_model": {
                  "AcceptedSourceCatalogSnapshot": {
                    ...
                    "ownership": "Shared immutable authenticated upstream fact; no execution_environment and no Workspace Instrument membership.",
                    "members": "source-product identities plus exact metadata versions/provenance"
                  },
                  "WorkspaceCatalogProjection": {
                    ...
                    "ownership": "Workspace-owned accepted projection/membership compatible with frozen M0.5 Catalog use.",
                    "members": "Only instrument_ids whose Instrument.workspace_id exactly equals projection.workspace_id and which exact-bind a member source product/version from the referenced upstream snapshot.",
                    "cross_workspace": "DENIED; a foreign Workspace instrument_id cannot enter membership even for an equivalent source product."
                  },
                  "flow": [
                    "AcceptedSourceCatalogSnapshot",
                    "WorkspaceCatalogProjection",
                    "Workspace-owned Instrument"
                  ]
                },
                "upstream_source_required_fields": [
                  "catalog_snapshot_id",
                  "source_exchange_id",
                  "source_adapter_family_id",
                  ...
                  "source_catalog_snapshot_identity",
                  "market_type",
                  "accepted_at_utc",
                  "observed_at_utc",
                  "effective_at_utc",
                  "stale_after_utc",
                  "previous_catalog_snapshot_id",
                  "completeness_status",
                  "completeness_evidence",
                  "content_fingerprint_sha256",
                  "member_source_product_versions"
                ],
                "workspace_projection_required_fields": [
                  "workspace_catalog_projection_id",
                  "workspace_id",
                  "accepted_source_catalog_snapshot_id",
                  "accepted_at_utc",
                  "previous_workspace_catalog_projection_id",
                  "member_instrument_ids",
                  "content_fingerprint_sha256"
                ]
              },
              "source_authenticator": {
                "architecture_name": "AcceptedSourceProducerMembership",
                "authority_owner": "Core",
                "trust_root": "Core-owned, durable, versioned and independently installed acceptance set; not adapter self-description, runtime DI registration, plugin manifest or user configuration.",
                "exact_binding_fields": [
                  ...
                  "valid_from_utc",
                  "valid_until_utc",
                  "release_or_code_provenance",
                  "membership_generation"
                ],
                "acceptance_rule": "Snapshot acceptance resolves one current membership and exact scope before catalog validation, then persists the membership identity/generation with the accepted snapshot for historical proof.",
                "minting_rule": "Only Core release/security authority can mint/revoke membership; caller, config, plugin and adapter cannot.",
                "plugin_layers": {
                  "installed_adapter": "Code is discoverable/loadable only.",
                  "configured_adapter": "Instance has user/runtime settings only.",
                  "authenticated_source_producer": "Separately matches accepted Core membership and exact release/capability scope."
                },
                "dynamic_plugin_claim_rule": "A plugin claiming an official exchange_id remains unauthenticated and cannot publish canonical venue snapshots unless its concrete release has separately accepted membership."
              },
              "stable_identity_map": {
                "mapping": "workspace_id + immutable source-product identity tuple -> Core-owned canonical instrument_id",
                "durability": "Persisted atomically per Workspace with first accepted projection appearance and restored before Workspace snapshot/history resolution.",
                "creation": "First accepted appearance for a given Workspace reserves a new Core-generated prefixed UUIDv7; refresh in that Workspace resolves it, while another Workspace must allocate a distinct ID.",
                "metadata_revision": "Creates a new immutable metadata_version under the same instrument_id.",
                "symbol_reuse": "Exact tuple never silently rebinds. If the venue proves continuity, retain ID; if it proves a distinct product/venue identity epoch, extend the durable mapping with an explicit Core-owned identity_epoch/venue_product_id discriminator and mint a new ID; ambiguity or collision rejects acceptance.",
                "prohibitions": [
                  "no per-refresh mint",
                  "no config/plugin supplied ID",
                  "no ordinary deterministic hash represented as canonical UUIDv7",
                  "no tuple rewrite or inferred alias"
                ],
                "historical_resolution": "instrument_id + exact metadata_version resolves an immutable accepted version and its source snapshot; missing version fails closed with no current-metadata fallback.",
                "workspace_invariants": [
                  "one immutable workspace_id owns each instrument_id",
                  "same source product in Workspace A and Workspace B maps to instrument_A and instrument_B where instrument_A != instrument_B",
              ...
              "dynamic_listing_delisting_and_completeness": {
                "new_listing": "A later immutable accepted COMPLETE snapshot may add tuples, allocate stable IDs once and leave prior snapshots unchanged; no CryptoHunter release is required.",
                "complete_proof_required": true,
                "completeness_evidence": "Authenticated producer attests full enumeration for exact source_exchange_id + market_type scope, with retrieval/page/cursor completion evidence and Core validation of scope, chronology, predecessor and anomaly policy. A count or adapter boolean alone is insufficient.",
                "partial_policy": "PARTIAL may be retained as diagnostic/history but cannot become ACTIVE membership source, remove members, infer delisting, activate a universe or replace last accepted COMPLETE baseline.",
                "delisting_rule": "Explicit trusted trading_status=DELISTED/SUSPENDED is authoritative when accepted. Absence may imply DELISTED only in a later accepted COMPLETE snapshot whose policy explicitly declares exhaustive membership and whose completeness evidence passes; otherwise absence has no membership-removal effect.",
                "anomaly_rule": "Large/unexpected removal triggers policy validation/manual or independent confirmation requirement and fails closed; it never downgrades completeness automatically into accepted deletion.",
                "history": "Never hard delete Instrument, metadata version, snapshot membership, orders, fills or audit references."
              },
              "trading_universe_compatibility": {
                "manual_flow": [
                  "accepted source Catalog",
                  "user selection intent",
                  "exact instrument_id and snapshot resolution",
                  "immutable TradingUniverse version"
                ],
                "autonomous_flow": [
                  "accepted source Catalog",
                  "policy-derived Eligible Universe",
                  "ranking",
                  "immutable/versioned Active Selection"
                ],
                "copy_instrument": false,
                "authority": "Both store references to canonical source Instruments/snapshots. Selection, eligibility and ranking never mint identity or execution authority.",
                "workspace_binding": "TradingUniverse remains an ExchangeAccount child and every selected Instrument must satisfy Instrument.workspace_id == TradingUniverse/ExchangeAccount.workspace_id.",
                "cross_workspace_selection": "DENIED_FAIL_CLOSED even when the foreign Instrument binds the same source-product identity or upstream Catalog fact."
              },
              "testnet_live_disposition": {
                "TESTNET": "Separation removes false environment from source identity. The same AcceptedSourceProducerMembership shape may later authenticate a distinct testnet or real-venue producer release/capability, but existing TESTNET blocker remains OPEN and no account/source equivalence is asserted.",
                "LIVE": "Schema is compatible with later LIVE accounts/routes while LIVE remains disabled and unchanged; no LIVE authority is enabled by this design."
              },
              "required_schema_changes": [
                {
                  "target": "M0.2 canonical Instrument identity_dimensions",
                  "classification": "SCHEMA_CONTRACT_MIGRATION_REQUIRED",
                  "change": "Keep parent Workspace unchanged; replace exchange_id/environment/market_type/venue_symbol dimensions with source_exchange_id/market_type/venue_symbol plus only an explicitly frozen optional source_market_namespace_id rule. execution environment remains account/route/order/lease context.",
                  "compatibility": "REQUIRED_BEFORE_AUTHORITATIVE_M0.5_CUTOVER; NOT_IMPLEMENTED"
                },
                {
                  "target": "M0.5 Instrument and identity index",
                  "change": "separate global source-product key from Workspace-owned canonical identity map: workspace_id + source-product tuple -> instrument_id; remove environment from both",
                  "compatibility": "M0.2 Workspace ownership UNCHANGED; identity migration required for legacy PAPER source semantics"
                },
                {
                  "target": "M0.5 AssetReference",
                  "change": "bind venue namespace to source_exchange_id independently of execution",
                  "compatibility": "SCHEMA_MIGRATION_REQUIRED"
                },
                {
                  "target": "M0.5 Exchange Registry",
                  "change": "split source catalog/market metadata policy from account/execution environment policy",
                  "compatibility": "SCHEMA_MIGRATION_REQUIRED"
                },
                {
                  "target": "M0.5 CatalogSnapshot",
                  "change": "replace exchange/environment scope with exact source/provenance/completeness contract",
                  "compatibility": "SCHEMA_MIGRATION_REQUIRED"
                },
                {
                  "target": "M0.5 execution eligibility",
                  "change": "add source Instrument + snapshot + TradingUniverse + account/backend/route PAPER binding",
                  "compatibility": "ADDITIVE_NEW_BINDING"
                },
                {
                  "target": "M0.11 persistence",
                  "change": "declare durable identity map, producer membership, accepted snapshots and metadata history plus explicit legacy migration",
                  "compatibility": "SCHEMA_MIGRATION_REQUIRED_NOT_IMPLEMENTED"
                }
              ],
              "downstream_impact_audit": [
                {
                  "target": "M0.2 Instrument parent",
                  "classification": "UNCHANGED",
                  "impact": "Instrument retains exactly one Workspace parent; canonical instrument_id is never shared across Workspaces."
                },
                {
                  "target": "M0.2 Workspace->Instrument",
                  "classification": "UNCHANGED",
                  "impact": "The one_to_many_catalog relationship remains authoritative. Shared source-product facts do not change domain entity ownership and no M0.2 ownership migration is required."
                },
                {
                  "target": "M0.2 Instrument identity_dimensions",
                  "classification": "SCHEMA_CONTRACT_MIGRATION_REQUIRED",
                  "impact": "Frozen dimensions are exchange_id/environment/market_type/venue_symbol. Target removes environment and uses source_exchange_id; Workspace parent remains unchanged. Required before M0.5 cutover."
                },
                {
                  "target": "M0.6 workspace binding",
                  "classification": "UNCHANGED",
                  "impact": "Instrument.workspace_id must exactly equal ExchangeAccount.workspace_id; foreign Workspace Instrument is TRUSTED_CONTEXT_INVALID; historical universe workspace binding remains mandatory."
                },
                {
                  "target": "M0.6 Instrument exchange/environment equality",
                  "classification": "MIGRATED_CANONICAL_1.44.0",
                  "impact": "InstrumentProjection currently carries exchange_id and environment, environment registry binds InstrumentProjection.environment, and route/catalog checks require exact exchange/environment equality. Migrate Instrument source comparisons to source identity and retain execution comparisons on account/route; same-Workspace TRUSTED_CONTEXT_INVALID invariant is unchanged."
                },
                {
                  "target": "exchange_id",
                  "classification": "DANGEROUS_IDENTITY_ASSUMPTION",
                  "impact": "Currently overloads source venue and environment-enabled execution venue. Migrate Instrument/Catalog uses to source_exchange_id; preserve ExchangeAccount/ExecutionRoute exchange binding as execution policy context."
                },
                {
                  "target": "environment",
                  "classification": "SEMANTIC_REINTERPRETATION",
                  "impact": "Unchanged for account, route, order and lease execution context; forbidden from source Instrument/Catalog identity."
                },
                {
                  "target": "source_adapter_family_id",
                  "classification": "SCHEMA_MIGRATION_REQUIRED",
                  "impact": "Remains metadata lineage on Instrument but must bind AcceptedSourceProducerMembership and source Catalog, not execution adapter."
                },
                {
                  "target": "instrument_id",
                  "classification": "UNCHANGED",
                  "impact": "Core-owned durable Workspace-scoped ID and historical references remain; equivalent source products across Workspaces have distinct IDs and legacy IDs never deduplicate across Workspace."
                },
                {
                  "target": "catalog_snapshot_id",
                  "classification": "SCHEMA_MIGRATION_REQUIRED",
                  "impact": "ID/reference remains, while snapshot scope/provenance and completeness fields change; historical exact lookup remains mandatory."
                },
                {
                  "target": "ExchangeAccount",
                  "classification": "SEMANTIC_REINTERPRETATION",
                  "impact": "Remains environment-bound execution/account identity. PAPER provider may be simulation backend while selected source Instruments belong to real venues; current same-exchange Instrument checks must migrate."
                },
                {
                  "target": "TradingUniverse",
                  "classification": "SCHEMA_MIGRATION_REQUIRED",
                  "impact": "Current parent/account same-Workspace rule remains; version references only Workspace-owned Instruments backed by accepted source facts and a separate execution eligibility binding without copying metadata."
                },
                {
                  "target": "ExecutionRoute",
                  "classification": "SCHEMA_MIGRATION_REQUIRED",
                  "impact": "Current execution adapter == Instrument source adapter assumption is dangerous. Route binds execution backend/account/environment; source adapter lineage remains Catalog-only."
                },
                {
                  "target": "OrderIntent",
                  "classification": "SEMANTIC_REINTERPRETATION",
                  "impact": "Keep instrument_id, metadata_version, universe/account/route references; environment derives from execution context, never Instrument identity. Exact source snapshot provenance must remain reachable. Every resolved Instrument must belong to the fact/account execution Workspace; equivalent source identity never permits a foreign Workspace instrument_id."
                },
                {
                  "target": "Order",
                  "classification": "SEMANTIC_REINTERPRETATION",
                  "impact": "Historical instrument/version and route/account references remain; any copied exchange/environment equality against Instrument must use source-versus-execution roles explicitly. Every resolved Instrument must belong to the fact/account execution Workspace; equivalent source identity never permits a foreign Workspace instrument_id."
                },
                {
                  "target": "Fill",
                  "classification": "SEMANTIC_REINTERPRETATION",
                  "impact": "Keep exact instrument_id + metadata_version with order/route provenance; never current metadata fallback or PAPER venue rewrite. Every resolved Instrument must belong to the fact/account execution Workspace; equivalent source identity never permits a foreign Workspace instrument_id."
                },
                {
                  "target": "M0.8 ledger",
                  "classification": "DANGEROUS_IDENTITY_ASSUMPTION",
                  "impact": "Economic account/environment scope remains execution-bound, while instrument/AssetReference valuation provenance is source-bound; paper_simulated_venue asset namespaces require migration or legacy isolation. Every resolved Instrument must belong to the fact/account execution Workspace; equivalent source identity never permits a foreign Workspace instrument_id."
                },
                {
                  "target": "M0.9 lease",
                  "classification": "UNCHANGED_WITH_EXPLICIT_INPUT_ROLE",
                  "impact": "Lease remains account/execution-scope authority and does not authorize source Catalog membership; validators must stop expecting Instrument.environment equality. Every resolved Instrument must belong to the fact/account execution Workspace; equivalent source identity never permits a foreign Workspace instrument_id."
                }
              ],
              "compatibility_decision": {
                "fully_additive_possible": false,
                "answer": "NO for canonical identity semantics; YES only for staged transport.",
                "reason": "Adding source_exchange_id while retaining paper_simulated_venue/environment in canonical Instrument identity leaves two competing identities. A staged additive release may populate new fields and bindings, but authoritative cutover changes the tuple and validators and is identity-breaking for ambiguous legacy PAPER records.",
                "cutover_rule": "New authoritative records use only migrated semantics. Old semantics are never accepted into new Catalog/universe/execution authority merely because both field sets are present."
              },
              "legacy_paper_records": {
                "default": "LEGACY_READ_ONLY_QUARANTINE",
                "automatic_mapping": false,
                "trusted_mapping_only": "Migration is allowed only when durable accepted evidence exact-binds the old instrument_id/metadata_version to one source identity tuple and snapshot provenance without collision; migration record preserves old ID/reference lineage or issues an explicit alias according to the future M0.11 plan.",
                "unknown_source": "FAIL_CLOSED_CANNOT_INFER; retain for audit/read/historical accounting only and deny new Catalog membership, universe activation and execution.",
                "prohibition": "Never infer source exchange from venue_symbol, asset code, config defaults, account, plugin claim or current market availability.",
                "cross_workspace_dedupe": "FORBIDDEN: two legacy Instrument IDs owned by different Workspaces are never merged merely because source-product tuple or symbol matches."
              },
              ...
              "m02_m06_compatibility": {
                "M0.2": {
                  ...
                  "instrument_identity_dimensions": {
                    "status": "MIGRATED_CANONICAL_1.44.0",
                    "current": [
                      "source_exchange_id",
                      "market_type",
                      "venue_symbol"
                    ...
                    "authoritative_m05_cutover_blocked_until_migrated": false
                  }
                },
                "M0.6": {
                  "same_workspace_invariant": "Instrument.workspace_id == ExchangeAccount.workspace_id remains required",
                  "foreign_instrument_disposition": "TRUSTED_CONTEXT_INVALID",
                  "historical_universe_workspace_binding": "UNCHANGED_REQUIRED",
                  "migration_dependency_required": false,
                  "same_workspace_status": "UNCHANGED",
                  "instrument_environment_exchange_checks": "MIGRATED_CANONICAL_1.44.0_SOURCE_EXECUTION_ROLES_SEPARATED",
                  "exact_evidence": [
                    "InstrumentProjection exact fields contain source_exchange_id and omit exchange_id/environment",
                    "TradingUniverse source reference resolves WorkspaceCatalogProjection then AcceptedSourceCatalogSnapshot",
                    "account and ExecutionRoute retain exact execution environment/backend binding"
                  ],
                  "migration_rule": "MIGRATED: source validation uses Instrument/WorkspaceCatalogProjection/AcceptedSourceCatalogSnapshot; execution validation uses account/route/backend/environment."
                }
              },
              "m02_identity_dimensions_migration_dependency": {
                "status": "MIGRATED_CANONICAL_1.44.0",
                "exact_frozen_evidence": {
                  "artifact": "canonical_domain_vocabulary.json",
                  "entity": "Instrument",
                  "parent": "Workspace",
                  "identity_dimensions": [
                  ...
                  "oracle": "tests/architecture/test_cryptohunter_canonical_domain_vocabulary.py::test_corrected_ownership_and_cardinality"
                },
                "ownership_disposition": "UNCHANGED: parent Workspace and Workspace -> Instrument one_to_many_catalog remain frozen.",
                "target_identity_dimensions": [
                  "source_exchange_id",
                  "market_type",
                  "venue_symbol"
                ],
                "optional_namespace_requirement": "If source_market_namespace_id is required for a venue, M0.2 schema must explicitly permit it as a conditional identity dimension or replace the fixed dimensions with an explicit SourceProductIdentity reference; the migration must choose and freeze one representation before implementation.",
                "chosen_minimal_representation": "Use source_exchange_id, market_type and venue_symbol as M0.2 Instrument identity_dimensions inside the existing Workspace parent scope. Add source_market_namespace_id only through an explicit schema rule when proven necessary; do not invent it per adapter/config.",
                "parent_scope_rule": "workspace_id remains parent/scope identity and is not duplicated in M0.2 identity_dimensions; the durable full lookup key still combines parent workspace_id with the dimensions.",
                "semantic_change": [
                  "environment is removed from canonical Instrument identity dimensions",
                  "source_exchange_id replaces overloaded execution exchange_id semantics for Instrument source identity",
                  "execution environment remains on account, route, order and lease context"
                ],
                "cutover_dependency": "REQUIRED_BEFORE_AUTHORITATIVE_M0.5_CUTOVER",
                "dependency_satisfied": true
              },
              "identity_failure_example": {
                ...
                "execution_contexts": [
                  "PAPER",
                  "LIVE_DISABLED_IDENTITY_EXAMPLE_ONLY"
                ],
                "old_m02_result": "(binance, PAPER, SPOT, BTCUSDT) != (binance, LIVE, SPOT, BTCUSDT), incorrectly minting execution-environment copies.",
                "target_result": "One Workspace A Instrument bound to (binance, SPOT, BTCUSDT); PAPER and future LIVE use distinct account/route/execution contexts, not distinct Instrument identity.",
                "live_disposition": "DISABLED_UNCHANGED; this example defines identity semantics only and grants no LIVE capability."
              }
            },
            "canonical_m02_m05_m06_contract_migration": {
              "status": "CLOSED",
              "predecessor": "1.43.0",
              "contract_identity": "1.44.0",
              "scope": "Canonical schema and executable-oracle migration only; no runtime source ingestion or Catalog acceptance authority",
              "global_status_invariants": {
                "C25": "BLOCKED",
                "S9D": "OPEN"
              }
...
```json
{
  ...
  "accepted_source_producer_membership_authority": {
    ...
    "catalog_runtime_acceptance": "IMPLEMENTED_CURRENT_TREE_PENDING_DEPLOYMENT_ACCEPTANCE",
    ...
    "trusted_time_authority": {
      "production_symbol": "bot_core.instruments.core_time.PRODUCTION_CORE_CLOCK / ProductionCoreClock.now_utc",
      "construction_owner": "SourceProducerMembershipAuthority.__init__; production constructor accepts only SQLiteMembershipCarrier and stores no replaceable clock field",
      "production_authority_type": "bot_core.instruments.source_producer_membership.SourceProducerMembershipAuthority",
      "test_authority_type": "bot_core.instruments.testing_core_time.TestSourceProducerMembershipAuthority",
      "test_clock_backed_authority_is_not_production_authority": true,
      "canonical_graph_rejects_test_authority": true,
      "production_authority_clock_swap_supported": false,
      "admission_methods_accept_caller_timestamps": false,
      "future_catalog_runtime_acceptor_clock_semantics": "same PRODUCTION_CORE_CLOCK or an explicitly equivalent frozen Core time authority"
    },
    "caller_supplied_admission_timestamp_forbidden": true,
    "grant_authority_admission_chronology": {
      ...
      "rules": [
        ...
        "ProductionCoreClock.now_utc is read after BEGIN IMMEDIATE; admission APIs accept no timestamp",
    ...
    "durable_authority_provenance": {
      "production_store_domain": "cryptohunter.source_producer_membership.production.v1",
      "test_store_domain": "cryptohunter.source_producer_membership.test.v1",
      "domain_is_immutable": true,
      "production_rejects_test_store": true,
    ...
    "catalog_runtime_acceptance_authority": {
      ...
      "runtime_composition_symbol": "bot_core.instruments.catalog_runtime_composition.compose_catalog_runtime_acceptance",
      "runtime_composition_boundary": "Core startup composition accepts only exact absolute, non-aliasing Catalog-state and receipt-metadata paths; it constructs the exact production membership, receipt and Catalog authorities, but never provisions receipt custody or admits a producer as a startup side effect",
      "runtime_composition_status": "IMPLEMENTED_CURRENT_TREE_NOT_FINAL_ACCEPTANCE",
      "physical_location_separation": "CANONICAL_PATH_AND_EXISTING_CROSS_NAMESPACE_FILE_IDENTITY_AND_SQLITE_WAL_NAMESPACE",
      "physical_alias_policy": "Catalog-state and receipt-metadata canonical pathnames differ; when both targets exist os.path.samefile must be false, thereby rejecting hardlinks and other distinct pathnames for one physical file",
      "sqlite_sidecar_namespace": "Both production SQLite authorities execute PRAGMA journal_mode=WAL; neither main database pathname may equal the other main pathname plus -wal or -shm, in either direction",
      "sqlite_sidecar_physical_separation": "Every existing Catalog main/-wal/-shm object must have different physical file identity from every existing receipt main/-wal/-shm object; this rejects main-to-opposite-sidecar and sidecar-to-sidecar hardlinks or symlinks without deleting legitimate distinct sidecars",
      "sqlite_sidecar_symlink_policy": "DENY_ANY_PREEXISTING_WAL_OR_SHM_SYMLINK_ENTRY_INCLUDING_DANGLING",
      "parent_directory_race_boundary": "CORE_OWNED_NOT_WRITABLE_BY_UNPRIVILEGED_ONLINE_PRINCIPALS",
      "path_validation_to_construction_race": "Host root and principals able to replace entries in the Core-owned parent directories are outside this local composition boundary; deployment must deny those directories to unprivileged online principals, so no openat2-style framework is required here",
      "next_c25_lifecycle_deployment_contract": {
        "status": "IMPLEMENTED_CURRENT_TREE_PENDING_DEPLOYMENT_ACCEPTANCE",
        "contract_id": "M0.12-S9D-C25-CATALOG-RUNTIME-LIFECYCLE-DEPLOYMENT-V1",
        "owner": "CoreHost is the sole runtime owner and retains both authorities for its process lifetime",
        "composition_hook": "after M0.3 single-instance lock and protected startup recovery succeed, before Catalog readiness publication and before any source-Catalog network fetch",
        "configuration_source": "protected deployment-owned CoreHost launch configuration with exact keys catalog_state_path and receipt_metadata_path; environment variables, working-directory-relative paths, plugin metadata and caller factories are forbidden",
        "path_default_policy": "NO_DEFAULTS; both absolute paths are mandatory and are qualified by CatalogRuntimeAuthorityPaths",
        "directory_ownership": "dedicated CoreHost service identity; administrative deployment principal may create and repair, unprivileged online principals have no write authority",
        "file_ownership": "dedicated CoreHost service identity after provisioning; administrative deployment principal may create only during explicit offline provisioning",
        "posix_directory_mode": "0700",
        "posix_file_mode": "0600",
        "non_posix_acl_policy": "equivalent deny-by-default ACL granting the CoreHost service identity and administrative deployment principal only",
        "storage_creation_authority": "explicit offline installer/bootstrap maintenance operation executed by the administrative deployment principal",
        "provisioning_authority": "explicit offline security provisioning operation; never CoreHost startup composition",
        "runtime_reopen_authority": "CoreHost may only reopen already-created storage and already-provisioned custody/membership",
        "startup_missing_database": "BLOCK_CATALOG_RUNTIME_ACCEPTANCE; do not invoke composition and do not create either database",
        "startup_missing_receipt_custody": "BLOCK_CATALOG_RUNTIME_ACCEPTANCE; no key generation or provisioning fallback",
        "startup_missing_producer_admission": "compose may reopen authorities, but Catalog fetch/readiness remains BLOCKED and no membership grant is synthesized",
        "startup_corrupt_authority": "fail closed, publish no Catalog readiness, perform no fetch, preserve evidence for operator recovery",
        "restart_behavior": "repeat the same qualification and exact replay; never provision, admit, truncate, replace or migrate implicitly",
        "crash_behavior": "rely only on frozen SQLite WAL/FULL and authority replay semantics; an unacknowledged operation may retry, while acknowledged state must replay exactly",
        "repeated_start_idempotency": "same protected configuration and unchanged durable state reopen the same authorities without new keys, grants, receipts, snapshots or network activity",
        "authority_lifetime": "constructed once per successful CoreHost lifetime after recovery; closed with CoreHost and never shared with UI/plugin process ownership",
        "one_shot_fetch_owner": "CoreHost alone may explicitly invoke fetch_catalog_once after composition, custody qualification and current producer admission; composition itself never fetches",
        "background_scheduler_dependency": "NOT_REQUIRED_FOR_C25_COMPLETION; scheduling is a later separately frozen runtime policy",
        "trading_universe_auto_selection_dependency": "NOT_REQUIRED_FOR_C25_COMPLETION; Catalog acceptance grants no TradingUniverse selection authority",
        "acceptance_evidence": [
          "executable startup-order test proving composition occurs after lock/recovery and before readiness/fetch",
          "deployment permission qualification for both parent directories and database files on every supported platform",
          "missing database/custody/admission and corrupt replay fail-closed matrix",
          "restart, crash-recovery and repeated-start idempotency tests",
          "exact production-type and no-side-effect composition tests",
          "one-shot fetch ownership test proving only CoreHost invokes the accepted release binding"
        ],
        "status_transition": "PARTIAL_PROPOSED_UNACCEPTED_1.46.0 -> IMPLEMENTED_CURRENT_TREE_PENDING_DEPLOYMENT_ACCEPTANCE only after every acceptance_evidence item passes and an independent security review accepts this frozen contract",
        "c25_unblocking_condition": "all frozen lifecycle/deployment behavior is implemented and executable acceptance evidence passes; until then C25 remains BLOCKED",
        "s9d_advancement_condition": "C25 unblocking alone does not close S9D; advancement additionally requires every remaining S9D completion item in the M0.12 machine contract"
      },
      ...
      "existing_store_open": "an existing catalog_authority_metadata marker requires all five durable Catalog tables and exact heads; missing tables fail CATALOG_AUTHORITY_STORAGE_MISSING and are never recreated",
