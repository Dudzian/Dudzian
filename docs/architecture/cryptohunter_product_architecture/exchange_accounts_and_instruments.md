...
Catalog semantics are split into `AcceptedSourceCatalogSnapshot` with independent COMPLETE/PARTIAL completeness and VALID/REJECTED acceptance (scope `source_exchange_id + market_type`) and `WorkspaceCatalogProjection` (scope `workspace_id + accepted_source_catalog_snapshot_id`). `TradingUniverse.source_catalog_snapshot_ids` is retained as a transport-compatibility name, but its exact semantic typedef is `WorkspaceCatalogProjectionId[]` (`wcat_*`), never `AcceptedSourceCatalogSnapshotId[]` (`ascat_*`). The executable trusted context therefore keeps `workspace_catalog_projections_by_id` and `accepted_source_catalog_snapshots_by_id` as disjoint maps; legacy `catalogs_by_id` is read-only and cannot activate a canonical universe. `validate_activate_trading_universe` and the shared membership validator delegate to the single `validate_trading_universe_source_chain` implementation. Current activation requires a structurally valid source with `acceptance_status=VALID`, `completeness_status=COMPLETE` and `effective_at_utc <= now_utc < stale_after_utc`; PARTIAL, REJECTED and stale records remain structurally/history-valid but are activation-ineligible. A projection admits only Instruments owned by its Workspace and exact-bound to an upstream source tuple and `source_metadata_version_id`; historical lookup exact-selects an ordered `instrument_history_by_id` entry by Instrument ID, metadata version and source snapshot without current fallback. PAPER additionally needs an explicit source-exchange/market permission; matching only Workspace and market type fails closed. PARTIAL never deletes membership, infers delisting, replaces a COMPLETE baseline, or activates a universe. Exhaustive absence additionally requires authenticated producer membership, accepted COMPLETE evidence, exact scope, and policy permission. Runtime ingestion, the writer/acceptor, and `AcceptedSourceProducerMembership` remain `NOT_IMPLEMENTED`; registry/config/adapter/plugin/manifest/local hashes cannot mint authority.
