...
M0.3 owns external membership existence/semantics, exact scope, genesis, monotonic PREPARE/FINALIZE/ABORT and non-authority rules. M0.11 owns local evidence production, matching StateStore metadata, durable transactions, backup, restore validation/orchestration, migrations, failure registry and history-only recovery, but cannot mint M0.3 membership. Existing M0.10 security, M0.9 risk/lease, M0.4 capability and M0.8 accounting ownership remains unchanged. Canonical startup order remains unchanged; freshness reconciliation stays subordinate to store integrity, recovery and readiness.
