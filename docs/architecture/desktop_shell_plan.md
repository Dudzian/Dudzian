...
* Sekcja `runtime.metrics_service` w `config/core.yaml` ustawia host/port serwera telemetrii, rozmiar historii (`history_size`), aktywność log sinka (`log_sink`), parametry eksportu JSONL (`jsonl_path`, `jsonl_fsync`), źródło archiwalnego tokenu (`auth_token_env`/`auth_token_file`), ścieżkę logu alertów UI (`ui_alerts_jsonl_path`) oraz klucz `ui_alerts_risk_profile`.  Loader normalizuje nazwę profilu do małych liter i waliduje ją względem sekcji `risk_profiles`, dzięki czemu runtime i narzędzia telemetryczne otrzymują spójne metadane audytowe.
