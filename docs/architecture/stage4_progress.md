...
- **Aktualizacja utrzymaniowa:** uelastyczniono `watch_metrics_stream`, aby zmienne środowiskowe mogły zerować limity (`NONE`/`NULL`) lub przywracać wartości domyślne (`DEFAULT`) bez naruszania mapowania flag CLI – odzwierciedlono to w regresyjnych testach jednostkowych.
