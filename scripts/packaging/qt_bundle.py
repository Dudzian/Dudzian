...
def _detect_protobuf_dir() -> str | None:
    ...
    detected = _probe_protobuf_dir(
        Path(homebrew_prefix) / "opt" / "protobuf" if homebrew_prefix else None
    )
    if detected:
        return detected
...
def preflight(qt_prefix: str | None, required_modules: Sequence[str]) -> Path:
    ...
    if not detected_qt_prefix:
        raise RuntimeError(
            "Nie wykryto instalacji Qt (brak zmiennych QT_ROOT_DIR/Qt6_DIR/Qt_DIR/QTDIR ani parametru --qt-prefix)"
        )
    ...
    if missing_modules:
        raise RuntimeError(
            "Brakuje modułów Qt w instalacji: "
...
def configure_and_build(
    ...
    *,
    ...
    if protobuf_dir_override and not protobuf_root_override:
        ...
        if protobuf_dir:
            logger.info(
                "Detected Protobuf CMake directory from Protobuf_ROOT override: %s", protobuf_dir
            )
            extra_args.append(f"-DProtobuf_DIR={protobuf_dir}")
        else:
            logger.info(
                "Could not derive Protobuf_DIR from Protobuf_ROOT override: %s",
                protobuf_root_override,
            )
...
def archive_bundle(
    install_dir: Path, output_dir: Path, *, bundle_name: str, platform_id: str
) -> Path:
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-dir", type=Path, default=Path("ui"), help="Katalog źródłowy projektu Qt"
    )
    parser.add_argument("--build-dir", type=Path, default=Path("ui/build"), help="Katalog build")
    parser.add_argument(
        "--install-dir", type=Path, default=None, help="Prefiks instalacji (opcjonalny)"
    )
    parser.add_argument(
        "--artifact-dir", type=Path, default=Path("artifacts"), help="Katalog wynikowych archiwów"
    )
    ...
    parser.add_argument(
        "--extra-cmake", action="append", default=[], help="Dodatkowe argumenty cmake"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    modules_env = os.environ.get(
        "QT_WINDOWS_MODULES" if platform_id == "windows" else "QT_DESKTOP_MODULES", ""
    )
