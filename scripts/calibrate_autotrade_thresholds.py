...
_SUPPORTED_SIGNAL_THRESHOLD_METRICS = frozenset(
    _normalize_metric_key(name) for name in SUPPORTED_SIGNAL_THRESHOLD_METRICS
)
_SUPPORTED_THRESHOLD_METRICS = frozenset(
    set(_SUPPORTED_SIGNAL_THRESHOLD_METRICS)
    | {_normalize_metric_key("risk_score")}
)
_ABSOLUTE_THRESHOLD_METRICS = frozenset(_SUPPORTED_SIGNAL_THRESHOLD_METRICS)
_SUPPORTED_THRESHOLD_CANONICAL = {
    _normalize_metric_key(name): name
    for name in (*SUPPORTED_SIGNAL_THRESHOLD_METRICS, "risk_score")
}
...
def _looks_like_autotrade_record(line: str) -> bool:
    ...
    if not isinstance(payload, Mapping):
        return False
    indicative_keys = {"timestamp", "summary", "detail", "decision", "status"}
    return any(key in payload for key in indicative_keys)
    ...
    try:
        with path.open("r", encoding="utf-8") as handle:
            ...
            if second_line is None:
                return _looks_like_autotrade_record(first_line)
    except OSError:
        return False
...
def _stream_autotrade_object(
    ...
    *,
    ...
    first_pair = True
    entries_field_seen = False
    yielded_records = False
    while True:
        stream.skip_whitespace()
        char = stream.peek_char()
        if char is None:
        ...
        stream.skip_whitespace()
        if key == "entries":
            entries_field_seen = True
            char = stream.peek_char()
            if char != "[":
                raise SystemExit("Niepoprawny JSON: pole 'entries' musi być tablicą")
            stream.consume_char("[")
            stream.compact()
            for record in _stream_autotrade_array(stream, since=since, until=until):
                yielded_records = True
                yield record
        else:
    ...
    if not entries_field_seen and not yielded_records:
        raise SystemExit(
            "Niepoprawny JSON: obiekt eksportu nie zawiera pola 'entries' ani rekordów autotradera"
        )
    stream.skip_whitespace()
    if stream.peek_char() is not None:
        stream.skip_whitespace()
...
def _build_threshold_config(report: Mapping[str, object]) -> dict[str, object]:
    ...
    if isinstance(schema, str) and schema:
        metadata["generated_from"] = schema

    generated_at = report.get("generated_at")
    if isinstance(generated_at, str) and generated_at:
        metadata["generated_at"] = generated_at

    filters_payload = report.get("filters")
    if isinstance(filters_payload, Mapping):
        since_raw = filters_payload.get("since")
        until_raw = filters_payload.get("until")
        if isinstance(since_raw, str) and since_raw:
            metadata["since"] = since_raw
        if isinstance(until_raw, str) and until_raw:
            metadata["until"] = until_raw

    suggestion_percentile = report.get("suggestion_percentile")
    if isinstance(suggestion_percentile, (int, float)) and math.isfinite(
        suggestion_percentile
    ):
        metadata["suggestion_percentile"] = float(suggestion_percentile)

    sources_payload = report.get("sources")
    sample_counts: dict[str, int] = {}
    if isinstance(sources_payload, Mapping):
        for key in ("journal_events", "autotrade_entries"):
            value = sources_payload.get(key)
            if isinstance(value, (int, float)) and math.isfinite(value):
                sample_counts[key] = int(value)
        if sample_counts:
            metadata["sample_counts"] = sample_counts

        group_approx_raw = sources_payload.get("approximation_metrics_list")
        if isinstance(group_approx_raw, Iterable):
            normalized_group_entries: dict[
                tuple[str, str, str], dict[str, str]
            ] = {}
            for entry in group_approx_raw:
                if not isinstance(entry, Mapping):
                    continue
                exchange_value = entry.get("primary_exchange")
                strategy_value = entry.get("strategy")
                metric_value = entry.get("metric")
                exchange_display = _normalize_string(exchange_value)
                strategy_display = _normalize_string(strategy_value)
                metric_display = _normalize_string(metric_value)
                if not exchange_display or not strategy_display or not metric_display:
                    continue
                exchange_canonical = _canonicalize_identifier(exchange_display)
                strategy_canonical = _canonicalize_identifier(strategy_display)
                metric_canonical = _normalize_metric_key(metric_display)
                canonical_metric_display = _SUPPORTED_THRESHOLD_CANONICAL.get(
                    metric_canonical, metric_display
                )
                key = (exchange_canonical, strategy_canonical, metric_canonical)
                if key in normalized_group_entries:
                    continue
                normalized_group_entries[key] = {
                    "primary_exchange": exchange_display,
                    "strategy": strategy_display,
                    "metric": canonical_metric_display,
                }
            if normalized_group_entries:
                metadata["approximation_metrics_list"] = [
                    normalized_group_entries[key]
                    for key in sorted(normalized_group_entries.keys())
                ]

        global_approx_raw = sources_payload.get("approximation_global_metrics_list")
        if isinstance(global_approx_raw, Iterable):
            normalized_global_entries: dict[str, str] = {}
            for metric in global_approx_raw:
                if not isinstance(metric, str):
                    continue
                metric_display = metric.strip()
                ...
                if metric_canonical not in normalized_global_entries:
                    normalized_global_entries[metric_canonical] = canonical_metric_display
            if normalized_global_entries:
                metadata["approximation_global_metrics_list"] = [
                    normalized_global_entries[key]
                    for key in sorted(normalized_global_entries.keys())
                ]

        limit_field_mapping: dict[str, tuple[str, ...]] = {
            "max_freeze_events": ("raw_freeze_events_truncated_groups",),
            "max_raw_values": (
                "raw_values_truncated_groups",
                "raw_values_omitted_total",
            ),
            "max_group_samples": (
                "group_samples_truncated_metrics",
                "group_samples_omitted_total",
            ),
            "max_global_samples": (
                "global_samples_truncated_metrics",
                "global_samples_omitted_total",
            ),
        }

        for limit_key, counter_keys in limit_field_mapping.items():
            limit_value = sources_payload.get(limit_key)
            if isinstance(limit_value, (int, float)) and math.isfinite(limit_value):
                metadata[limit_key] = int(limit_value)
            for counter_key in counter_keys:
                counter_value = sources_payload.get(counter_key)
                if isinstance(counter_value, (int, float)) and math.isfinite(
                    counter_value
                ):
                    metadata[counter_key] = int(counter_value)

        clamp_payload = sources_payload.get("clamped_values")
        if isinstance(clamp_payload, Mapping):
            sanitized_clamp: dict[str, dict[str, int]] = {}
            for scope_key in ("group", "global"):
                scope_payload = clamp_payload.get(scope_key)
                if not isinstance(scope_payload, Mapping):
                    continue
                scoped_values: dict[str, int] = {}
                for clamp_key in ("regular", "absolute", "metrics_with_regular_clamp", "metrics_with_absolute_clamp"):
                    clamp_value = scope_payload.get(clamp_key)
                    if isinstance(clamp_value, (int, float)) and math.isfinite(clamp_value):
                        scoped_values[clamp_key] = int(clamp_value)
                if scoped_values:
                    sanitized_clamp[scope_key] = scoped_values
            if sanitized_clamp:
                metadata["clamped_values"] = sanitized_clamp
        ...
        for counter_key in (
            ...
            if counter_key in metadata:
                continue
            counter_value = sources_payload.get(counter_key)
            if isinstance(counter_value, (int, float)) and math.isfinite(counter_value):
                metadata[counter_key] = int(counter_value)
    ...
    if isinstance(groups_payload, Iterable):
        for entry in groups_payload:
            if not isinstance(entry, Mapping):
                continue
            exchange_raw = _normalize_string(entry.get("primary_exchange"))
            strategy_raw = _normalize_string(entry.get("strategy"))
            exchange_canonical = _canonicalize_identifier(exchange_raw)
            strategy_canonical = _canonicalize_identifier(strategy_raw)
            if "unknown" in {exchange_canonical, strategy_canonical}:
                continue
            ...
            for metric_name, payload in metrics.items():
                ...
                if canonical_name == "risk_score":
                    # per-strategia progi risk_score są obsługiwane wyłącznie w sekcji mapowania reżimu
                    continue
            ...
            if not suggestions:
                continue
            exchange_map = strategy_thresholds.setdefault(exchange_canonical, {})
            metrics_map = exchange_map.get(strategy_canonical, {})
            metrics_map.update(suggestions)
            exchange_map[strategy_canonical] = metrics_map
    ...
    if strategy_thresholds:
        ...
        for exchange in sorted(strategy_thresholds):
            ...
            if not filtered_strategies:
                continue
            ordered_strategy_thresholds[exchange] = filtered_strategies
    ...
    if auto_trader_config:
        config_payload["auto_trader"] = auto_trader_config
