...
def build_report() -> dict[str, Any]:
    ...
    sections: dict[str, dict[str, Any]] = {
        ...
        "data_source_market_feed": {
            ...
            "recommended_next_step": "Keep the read-only market data contract, scenario market context, scenario decision dry-run artifact, local artifact audit trail, and local context/artifact/audit bundle plus fail-closed bundle boundary/export refusal as partial/static-local evidence: the local scenario runner can carry deterministic read-only market context and produce deterministic local context/artifact/audit bundle plus fail-closed bundle boundary/export refusal that generates no orders/decisions, market context is in-memory/static-local fixture only, READ_ONLY_MARKET_FETCH is preview-safe, and account/balance/credentials/order/fill/live side effects remain blocked; no scoring, no recommendation, no strategy engine, AI/model inference, DecisionEnvelope integration, TradingController integration, file export, serialization export, cloud sink, external export, real market adapter/fetch, app runtime loop, UI integration, or testnet/sandbox adapter exists yet.",
        },
        ...
        "ai_decision_governor": {
            ...
            "gaps": [
                ...
                "Local scenario runner can produce a deterministic local context/artifact/audit bundle plus fail-closed bundle boundary/export refusal after paper scenario execution, including scenario state, optional in-memory/static-local market context summary, decision context, dry-run artifact, local artifact audit trail, no-action markers, no-export/no-cloud markers, and blocked engine integration markers.",
                "Dry-run artifact and local bundle are context-only/static-local and generate no orders/decisions; they are not scoring, recommendation, strategy engine, AI/model inference path, DecisionEnvelope integration, TradingController integration, file export, serialization export, cloud sink, external export, or engine handoff.",
                "Market context is in-memory/static-local fixture only; no real market adapter/fetch, app runtime loop, UI integration, testnet/sandbox adapter, file export, serialization export, cloud sink, external export, or engine handoff exists yet.",
            ],
            "recommended_next_step": "Keep ai_decision_governor partial/static-local: local scenario runner can produce deterministic context-only dry-run decision artifact and local in-memory audit trail, dry-run artifact audit trail is static-local only and generates no orders/decisions, no scoring, no recommendation, no strategy engine, no AI/model inference, no DecisionEnvelope integration, no TradingController integration, market context is in-memory/static-local fixture only, READ_ONLY_MARKET_FETCH is preview-safe, and account/balance/credentials/order/fill/live side effects remain blocked; no real market adapter/fetch, app runtime loop, UI integration, testnet/sandbox adapter, file export, serialization export, cloud sink, external export, or engine handoff exists yet.",
        },
        "paper_terminal_order_lifecycle": {
            ...
            "gaps": [
                ...
                "Local scenario runner can produce deterministic context-only dry-run decision artifact, local in-memory artifact audit trail, and deterministic local context/artifact/audit bundle plus fail-closed bundle boundary/export refusal after execution; dry-run artifact and bundle are context-only/static-local and generates no orders/decisions with no scoring, no recommendation, no strategy engine, AI/model inference, DecisionEnvelope integration, or TradingController integration.",
            ...
            "recommended_next_step": "Keep the local composition proof partial: paper spine + portfolio reducer + audit journal are wired together locally, a deterministic in-memory local scenario fixture runner now drives PaperPreviewFlow, and the runner can produce a context-only dry-run decision artifact plus local in-memory audit trail and local context/artifact/audit bundle plus fail-closed bundle boundary/export refusal with no generated orders/decisions, but there is no scoring, no recommendation, no strategy engine, no AI/model inference, no DecisionEnvelope integration, no TradingController integration, no app runtime loop, no UI integration, no file loader/export, no file export, no serialization export, no cloud sink, no external export, no engine handoff, no testnet/sandbox adapter, no real market adapter/fetch, and no live exchange/order/account side effects.",
        },
        ...
        "alerts_telemetry_audit": {
            ...
            "gaps": [
                ...
                "Local scenario runner can produce a deterministic context-only dry-run artifact audit trail and fail-closed bundle boundary/export refusal as local/in-memory/static-local evidence only; it is not cloud telemetry, file export, serialization export, external export, engine handoff, strategy evaluation, AI/model inference, scoring, recommendation, DecisionEnvelope integration, or TradingController integration, and it generates no orders/decisions.",
                "Testnet/read-only market feed still missing; no live exchange/order/account side effects are introduced or proven here.",
            ],
            "recommended_next_step": "Keep local paper audit/alerts consumer partial: local composition proof wires paper spine + portfolio reducer + audit journal locally, and a deterministic in-memory scenario runner plus static/local bundle boundary/export refusal exists, but there is no app runtime loop, no UI integration, no file loader/export, no serialization export, no cloud sink, no external export, no engine handoff, no scoring, no recommendation, no DecisionEnvelope integration, no TradingController integration, no testnet/read-only market feed, and no live exchange/order/account side effects.",
        },
