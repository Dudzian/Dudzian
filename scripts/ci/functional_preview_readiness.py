    ...
    sections: dict[str, dict[str, Any]] = {
        ...
        "data_source_market_feed": {
            ...
            "gaps": [
                ...
                "Local scenario runner can produce deterministic context-only dry-run decision artifact and local in-memory artifact audit trail after execution; dry-run artifact is context-only and generates no orders/decisions, with no scoring, no recommendation, no strategy engine, AI/model inference, DecisionEnvelope integration, or TradingController integration.",
            ...
            "recommended_next_step": "Keep the read-only market data contract, scenario market context, and scenario decision dry-run artifact and local artifact audit trail as partial/static-local evidence: the local scenario runner can carry deterministic read-only market context and produce deterministic context-only dry-run decision artifact that generates no orders/decisions, market context is in-memory/static-local fixture only, READ_ONLY_MARKET_FETCH is preview-safe, and account/balance/credentials/order/fill/live side effects remain blocked; no scoring, no recommendation, no strategy engine, AI/model inference, DecisionEnvelope integration, TradingController integration, real market adapter/fetch, app runtime loop, UI integration, testnet/sandbox adapter, cloud sink, or external export exists yet.",
        },
        ...
        "ai_decision_governor": {
            ...
            "gaps": [
                ...
                "Local scenario runner can produce a deterministic context-only dry-run decision artifact after paper scenario execution, including scenario state, optional in-memory/static-local market context summary, paper snapshot counts, risk placeholders, no-action markers, and blocked engine integration markers.",
                "Dry-run artifact is context-only and generates no orders/decisions; it is not a strategy engine, AI/model inference path, DecisionEnvelope integration, or TradingController integration.",
