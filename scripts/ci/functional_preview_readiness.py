...
def build_report() -> dict[str, Any]:
    ...
    runtime_service_gap = (
        ...
        "service snapshot history contract keeps a bounded immutable in-memory history of already-built read API evidence; history does not collect in background, run loops, serialize/export/write files/open sockets, or execute commands; refusal executor proof remains non-executing; read API remains immutable in-memory projection only; wrapper remains single-shot/static-local; no app runtime loop; no UI binding; no controller handoff; no decision engine; no export/cloud/serialization; no generated decisions/orders; no real market adapter/fetch; no testnet/sandbox adapter; BLOK B local runtime service wrapper is contract-complete for static-local evidence; closure audit aggregates service snapshot, boundary matrix, lifecycle command contract, read API, read API boundary/no-export matrix, refusal executor proof, and bounded in-memory history; closure audit confirms ready_for_block_c=True; closure audit keeps ready_for_ui_runtime_integration=False, ready_for_decision_engine=False, ready_for_export=False, ready_for_live=False; integration gate remains blocked; wrapper remains single-shot/static-local; no app runtime loop; no UI binding; no controller handoff; no decision engine; no export/cloud/serialization; no generated decisions/orders; no real market adapter/fetch; no testnet/sandbox adapter; BLOK C now maps PreviewReadOnlyBindingSnapshot into controlled read-only UI state values copied from static-local BLOK B closure evidence; source smoke confirms a read-only visible summary for BLOK B closure / BLOK C binding; state does not expose commands/actions/export/live/testnet/account/secret handles; QML/source proof remains read-only; UI proof does not start runtime loop, execute commands, submit orders, serialize/export, fetch live/testnet data, read accounts/secrets, or mutate runtime state; integration gate remains blocked; ready_for_ui_runtime_integration remains false; next step should be read-only UI state boundary/refusal proof or controlled QML value consumption, still without actions."
    )
    read_model_gap = (
