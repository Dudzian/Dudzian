...
def build_report() -> dict[str, Any]:
    ...
    sections: dict[str, dict[str, Any]] = {
        "preview_mode_contract": {
            ...
            "gaps": [
                ...
                "Local scenario runner can carry deterministic read-only market context from in-memory/static-local fixtures and can produce a deterministic local context/artifact/audit bundle without treating it as live account/balance/order access.",
            ],
        ...
        "data_source_market_feed": {
            ...
            "recommended_next_step": "Keep the read-only market data contract, scenario market context, scenario decision dry-run artifact, local artifact audit trail, and local context/artifact/audit bundle as partial/static-local evidence: the local scenario runner can carry deterministic read-only market context and produce deterministic local context/artifact/audit bundle that generates no orders/decisions, market context is in-memory/static-local fixture only, READ_ONLY_MARKET_FETCH is preview-safe, and account/balance/credentials/order/fill/live side effects remain blocked; no scoring, no recommendation, no strategy engine, AI/model inference, DecisionEnvelope integration, TradingController integration, file export, cloud sink, external export, real market adapter/fetch, app runtime loop, UI integration, or testnet/sandbox adapter exists yet.",
        },
        ...
        "ai_decision_governor": {
            ...
            "gaps": [
                ...
                "Local scenario runner can produce a deterministic local context/artifact/audit bundle after paper scenario execution, including scenario state, optional in-memory/static-local market context summary, decision context, dry-run artifact, local artifact audit trail, no-action markers, no-export/no-cloud markers, and blocked engine integration markers.",
                "Dry-run artifact and local bundle are context-only/static-local and generate no orders/decisions; they are not scoring, recommendation, strategy engine, AI/model inference path, DecisionEnvelope integration, TradingController integration, file export, cloud sink, or external export.",
        ...
        "paper_terminal_order_lifecycle": {
            ...
            "gaps": [
                ...
                "Local scenario runner can produce deterministic context-only dry-run decision artifact, local in-memory artifact audit trail, and deterministic local context/artifact/audit bundle after execution; dry-run artifact and bundle are context-only/static-local and generates no orders/decisions with no scoring, no recommendation, no strategy engine, AI/model inference, DecisionEnvelope integration, or TradingController integration.",
            ...
            "recommended_next_step": "Keep the local composition proof partial: paper spine + portfolio reducer + audit journal are wired together locally, a deterministic in-memory local scenario fixture runner now drives PaperPreviewFlow, and the runner can produce a context-only dry-run decision artifact plus local in-memory audit trail and local context/artifact/audit bundle with no generated orders/decisions, but there is no scoring, no recommendation, no strategy engine, no AI/model inference, no DecisionEnvelope integration, no TradingController integration, no app runtime loop, no UI integration, no file loader/export, no cloud sink, no external export, no testnet/sandbox adapter, no real market adapter/fetch, and no live exchange/order/account side effects.",
        },
        ...
        "alerts_telemetry_audit": {
            ...
            "gaps": [
                ...
                "Local scenario runner can produce a deterministic context-only dry-run artifact audit trail as local/in-memory/static-local evidence only; it is not cloud telemetry, file export, external export, strategy evaluation, AI/model inference, scoring, recommendation, DecisionEnvelope integration, or TradingController integration.",
    ...
    payload = {
        ...
        "scope": "FUNCTIONAL-PREVIEW-3.10 local paper event spine, portfolio reducer, local audit/alerts consumer, local composition proof, deterministic in-memory local scenario fixture runner, read-only market data contract unit evidence, static/local scenario-level read-only market context evidence, context-only paper scenario decision-context/dry-run artifact contract evidence, local in-memory dry-run artifact audit-trail evidence, and deterministic local context/artifact/audit bundle contract evidence; no runtime loop, UI integration, file loader/export, secrets, real market fetches, live account access, cloud/export sink, external export, or live order I/O executed",
        "sections": sections,
    }
