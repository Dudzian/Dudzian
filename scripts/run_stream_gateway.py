...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if stream_cfg:
        if getattr(stream_cfg, "retry_after", None) is not None:
            gateway_kwargs["retry_after"] = float(stream_cfg.retry_after)
        if getattr(stream_cfg, "state_ttl", None) is not None:
            gateway_kwargs["state_ttl"] = float(stream_cfg.state_ttl)
    gateway = StreamGateway(**gateway_kwargs)
