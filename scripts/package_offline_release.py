...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version", required=True, help="Wersja release'u umieszczona w manifescie"
    )
    parser.add_argument("--output", required=True, help="Ścieżka docelowa archiwum tar.gz")
    parser.add_argument(
        "--models-dir", default="data/models", help="Katalog ze zserializowanymi modelami"
    )
    parser.add_argument(
        "--strategies-dir", default="data/strategies", help="Katalog ze strategiami"
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.manifest_output:
        _write_manifest(
            Path(args.manifest_output).expanduser(), manifest
        )  # pragma: no cover - ścieżki usera
