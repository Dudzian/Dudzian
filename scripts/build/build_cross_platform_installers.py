...
def parse_args(argv: List[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", default=str(ROOT / "dist"), help="Destination directory for artifacts."
    )
    parser.add_argument(
        "--profile-linux", default=str(DEFAULT_PROFILES["linux"]), help="Custom Linux profile path."
    )
    parser.add_argument(
        "--profile-macos", default=str(DEFAULT_PROFILES["macos"]), help="Custom macOS profile path."
    )
    parser.add_argument(
        "--profile-windows",
        default=str(DEFAULT_PROFILES["windows"]),
        help="Custom Windows profile path.",
    )
    parser.add_argument(
        "--skip-linux", action="store_true", help="Skip building the Linux installer."
    )
    parser.add_argument(
        "--skip-macos", action="store_true", help="Skip building the macOS installer."
    )
    parser.add_argument(
        "--skip-windows", action="store_true", help="Skip building the Windows installer."
    )
    parser.add_argument(
        "extra", nargs=argparse.REMAINDER, help="Additional arguments passed to underlying scripts."
    )
