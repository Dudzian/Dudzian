...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Maksymalna dopuszczalna luka czasowa (w minutach) pomiędzy końcem okna smoke a ostatnią świecą w cache. "
            "Większa luka zakończy test błędem."
        ),
    )
    parser.add_argument(
        "--smoke-min-ok-ratio",
        ...
        default=None,
        help=(
            "Minimalny udział poprawnych wpisów (0-1) wymagany w manifestach danych. "
            "Niższa wartość zakończy smoke test błędem."
        ),
    )
    parser.add_argument(
        "--smoke-note",
        default=None,
        help=(
            "Dowolna notatka operatora dołączana do summary.json, summary.txt oraz alertu "
            "smoke testu."
        ),
    )
    parser.add_argument(
...
def _render_smoke_summary(*, summary: Mapping[str, object], summary_sha256: str) -> str:
    ...
    if isinstance(data_checks, Mapping):
        ...
        if isinstance(manifest_info, Mapping):
            ...
            if isinstance(manifest_summary, Mapping):
                summary_status = str(manifest_summary.get("status", "unknown")).upper()
                ok_ratio_value = manifest_summary.get("ok_ratio")
                if ok_ratio_value is not None:
                    try:
                        ratio_text = f"{float(ok_ratio_value) * 100:.2f}%"
                    except (TypeError, ValueError):  # pragma: no cover - fallback na tekst
                        ratio_text = str(ok_ratio_value)
                    data_lines.append(
                        f"Manifest – status agregowany: {summary_status} (ok_ratio {ratio_text})"
                    )
                else:
                    data_lines.append(
                        f"Manifest – status agregowany: {summary_status}"
                    )
                data_lines.append(
                ...
                thresholds_payload = manifest_summary.get("thresholds")
                if isinstance(thresholds_payload, Mapping) and thresholds_payload:
                    threshold_parts: list[str] = []
                    max_gap_threshold = thresholds_payload.get("max_gap_minutes")
                    if max_gap_threshold is not None:
                        try:
                            threshold_parts.append(
                                f"max_gap≤{float(max_gap_threshold):.2f} min"
                            )
                        except (TypeError, ValueError):  # pragma: no cover - fallback
                            threshold_parts.append(f"max_gap≤{max_gap_threshold}")
                    min_ok_threshold = thresholds_payload.get("min_ok_ratio")
                    if min_ok_threshold is not None:
                        try:
                            threshold_parts.append(
                                f"ok_ratio≥{float(min_ok_threshold) * 100:.2f}%"
                            )
                        except (TypeError, ValueError):  # pragma: no cover - fallback
                            threshold_parts.append(f"ok_ratio≥{min_ok_threshold}")
                    if threshold_parts:
                        data_lines.append(
                            "Manifest – progi: " + ", ".join(threshold_parts)
                        )
                ...
                interval_breakdown = manifest_summary.get("by_interval")
                if isinstance(interval_breakdown, Mapping) and interval_breakdown:
                    fragments: list[str] = []
                    for interval_name, payload in sorted(interval_breakdown.items()):
                        ok_value = _as_int(payload.get("ok")) or 0
                        total_value = _as_int(payload.get("total")) or 0
                        ratio_value = payload.get("ok_ratio")
                        ratio_text: str | None
                        if isinstance(ratio_value, (int, float)):
                            ratio_text = f"{float(ratio_value) * 100:.2f}%"
                        elif ratio_value is not None:
                            ratio_text = str(ratio_value)
                        else:
                            ratio_text = None
                        fragment = f"{interval_name}: {ok_value}/{total_value}"
                        if ratio_text:
                            fragment += f" ({ratio_text})"
                        fragments.append(fragment)
                    if fragments:
                        data_lines.append("Manifest – interwały: " + ", ".join(fragments))
                symbol_breakdown = manifest_summary.get("by_symbol")
                if isinstance(symbol_breakdown, Mapping) and symbol_breakdown:
                    problematic: list[str] = []
                    for symbol, payload in sorted(symbol_breakdown.items()):
                        error_value = _as_int(payload.get("error")) or 0
                        if not error_value:
                            continue
                        total_value = _as_int(payload.get("total")) or 0
                        ratio_value = payload.get("ok_ratio")
                        if isinstance(ratio_value, (int, float)):
                            ...
                            ratio_text = None
                        fragment = f"{symbol}: błędne wpisy {error_value}/{total_value}"
                        if ratio_text:
                            fragment += f" (ok_ratio {ratio_text})"
                        problematic.append(fragment)
                    if problematic:
                        data_lines.append(
                            "Manifest – symbole z problemami: " + "; ".join(problematic[:5])
                        )
    ...
    note_value = summary.get("note")
    note_line: str | None = None
    if isinstance(note_value, str):
        stripped_note = note_value.strip()
        if stripped_note:
            compact_note = " ".join(part.strip() for part in stripped_note.splitlines() if part.strip())
            if compact_note:
                note_line = f"Notatka operatora: {compact_note}"
        ...
        for candidate in effective_intervals:
            ...
            for symbol in symbols:
                ...
                if last_timestamp < end_ms:
                    gap_ms = max(0, end_ms - last_timestamp)
                    gap_minutes = gap_ms / 60_000
                    if max_gap_minutes is not None and gap_minutes <= max_gap_minutes:
                        allowed_missing = int((gap_ms + candidate_tick_ms - 1) // candidate_tick_ms)
                    else:
                        issues.append(
                            (
                                str(symbol),
                                (
                                    "ostatnia świeca {last_ts} < wymaganego końca {end_ms} (brakuje {gap:.2f} min)"
                                ).format(
                ...
                effective_required = max(1, candidate_required - allowed_missing)

                if row_count < effective_required:
                    issues.append(
                        (
                            str(symbol),
                            f"za mało świec ({row_count} < {effective_required})",
                        )
                    )
                    continue
                ...
                if coverage < effective_required:
                    issues.append(
                        (
                            str(symbol),
                            f"pokrycie obejmuje {coverage} świec (wymagane {effective_required})",
                        )
                    )
                    continue
                ...
                interval_map[str(candidate)] = {
                    ...
                    "required_bars": int(effective_required),
...
def _verify_manifest_coverage(
    *,
    ...
    for symbol in symbols:
        ...
        for normalized in ordered_normalized:
            ...
            if last_ts is None:
                ...
                gap_minutes = (end_ms - last_ts) / 60_000
                if max_gap_minutes is not None and gap_minutes <= max_gap_minutes:
                    pass
                else:
                    issues.append(
                        f"{status.symbol}/{status.interval}: ostatnia świeca w manifescie ({last_ts}) < wymaganego końca ({end_ms}) – luka {gap_minutes:.2f} min"
                    )
    ...
    if max_gap_minutes is not None:
        ...
        summary_payload["max_gap_threshold_minutes"] = threshold_value
    ...
    if min_ok_ratio is not None:
        ...
        summary_payload["min_ok_ratio_threshold"] = threshold_ratio
        ...
        if ok_ratio is not None:
            ...
            if ok_ratio_value is not None and ok_ratio_value < threshold_ratio:
                issues.append(
                    "Udział poprawnych wpisów w manifescie spadł poniżej wymaganego progu"
                )
        else:
            ...
            if isinstance(total_entries, (int, float)) and threshold_ratio > 0:
                if float(total_entries) <= 0:
                    issues.append(
                        "Manifest nie zawiera wpisów wymaganych dla smoke testu (aktywny próg ok_ratio)"
                    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.paper_smoke:
        ...
        max_gap_minutes = args.smoke_max_gap_minutes
        min_ok_ratio = args.smoke_min_ok_ratio
        if max_gap_minutes is None or min_ok_ratio is None:
            data_quality_cfg = getattr(pipeline.bootstrap.environment, "data_quality", None)
            if data_quality_cfg is not None:
                if max_gap_minutes is None:
                    candidate_gap = getattr(data_quality_cfg, "max_gap_minutes", None)
                    if candidate_gap is not None:
                        try:
                            max_gap_minutes = float(candidate_gap)
                        except (TypeError, ValueError):
                            _LOGGER.warning(
                                "Niepoprawna wartość data_quality.max_gap_minutes=%r – pomijam ustawienie.",
                                candidate_gap,
                            )
                            max_gap_minutes = None
                if min_ok_ratio is None:
                    candidate_ratio = getattr(data_quality_cfg, "min_ok_ratio", None)
                    if candidate_ratio is not None:
                        try:
                            min_ok_ratio = float(candidate_ratio)
                        except (TypeError, ValueError):
                            _LOGGER.warning(
                                "Niepoprawna wartość data_quality.min_ok_ratio=%r – pomijam ustawienie.",
                                candidate_ratio,
                            )
                            min_ok_ratio = None

        if min_ok_ratio is not None and not 0 <= float(min_ok_ratio) <= 1:
            _LOGGER.error("--smoke-min-ok-ratio musi mieścić się w przedziale 0-1")
            return 1
        data_checks: Mapping[str, object] | None = None
        try:
        ...
        operator_note = (
            args.smoke_note.strip() if isinstance(args.smoke_note, str) else None
        )
        ...
        if operator_note:
            body += f" Notatka operatora: {operator_note}"
        ...
        if operator_note:
            context_payload["operator_note"] = operator_note
