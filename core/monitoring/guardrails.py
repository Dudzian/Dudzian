...
from .metrics import AsyncIOMetricSet, ComplianceMetricSet, RetrainingMetricSet
...
class AsyncIOGuardrails:
    ...
    def __init__(
        ...
        *,
        ...
        compliance_metrics: ComplianceMetricSet | None = None,
    ) -> None:
    ...
    def handle_monitoring_event(self, event: MonitoringEvent) -> None:
        ...
        if isinstance(event, ComplianceViolation):
            severity = str(event.severity or "warning").lower()
            labels = {
                "environment": self._environment,
                "rule": event.rule_id,
                "severity": severity,
            }
            self._compliance_metrics.violations_total.inc(labels=labels)

            metadata = dict(event.metadata or {})
            tokens = [f"rule={event.rule_id}", f"severity={severity}"]
            for key, value in sorted(metadata.items()):
                tokens.append(f"{key}={value}")
            message = "COMPLIANCE " + " ".join(tokens)
            ...
            self._emit_ui_event("compliance_violation", severity or "warning", payload)
