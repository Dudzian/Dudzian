...
class AssetReference:
    ...
    def validate(self) -> None:
        if any(
            type(item) is not str or not item for item in asdict(self).values()
        ) or self.mapping_status not in {"EXACT", "EXPLICIT_ALIAS"}:
            raise AccountingAuthorityError("TRUSTED_CONTEXT_FAILURE")
...
class AccountingFactCarrier(Protocol):
    ...
    def compare_and_swap(
        self, expected_revision: int, state: AtomicAccountingFactState
    ) -> None: ...
...
class CoreAcceptedAccountingFactProjection:
    ...
    @classmethod
    def compose(
        cls, carrier: AccountingFactCarrier, *, content_membership: CoreAcceptedContentAuthority
    ) -> tuple[CoreAcceptedAccountingFactProjection, _AccountingFactWriter]:
        authority = cls(carrier, content_membership=content_membership)
        return authority, _AccountingFactWriter(authority)

    def __init__(
        self, carrier: AccountingFactCarrier, *, content_membership: CoreAcceptedContentAuthority
    ) -> None:
    ...
    def _validate_payload(payload: object) -> tuple[dict[str, object], str]:
        ...
        if source_type not in SUPPORTED_ECONOMIC_SOURCES:
            raise AccountingAuthorityError("UNSUPPORTED_ACCOUNTING_SEMANTICS")
        if (
            not _id(item["audit_event_id"], "evt")
            or not _id(item["workspace_id"], "ws")
            or not _id(item["portfolio_id"], "port")
            or not _id(item["exchange_account_id"], "xacc")
        ...
        expected_kind = (
            "EXTERNAL_CONTRIBUTION" if source_type == "deposit" else "EXTERNAL_WITHDRAWAL"
        )
    ...
    def _validate_state(self, state: object, *, require_content: bool) -> None:
        if (
            not isinstance(state, AtomicAccountingFactState)
        ...
        for record in state.accepted:
            ...
            try:
                payload, computed = self._validate_payload(
                    json.loads(record.canonical_payload_json)
                )
            ...
            if require_content and (
                binding is None or binding.content_fingerprint_sha256 != computed
            ):
                raise AccountingAuthorityError("CORRUPT_ACCOUNTING_SOURCE_AUTHORITY")
    ...
    def _accept(self, payload: Mapping[str, object]) -> AcceptedAccountingFact:
        ...
        with self._carrier.authority_fence():
            ...
            prior = next(
                (entry for entry in state.accepted if entry.audit_event_id == identity), None
            )
            ...
            self._carrier.compare_and_swap(
                ...
                AtomicAccountingFactState(state.store_revision + 1, state.accepted + (record,)),
            )
            return record
...
class AccountingAuthority:
    @classmethod
    def compose(
        cls, carrier: AccountingCarrier, *, source_authority: CoreAcceptedAccountingFactProjection
    ) -> tuple[AccountingAuthority, _AccountingWriter]:
        authority = cls(carrier, source_authority=source_authority)
        return authority, _AccountingWriter(authority)

    def __init__(
        self, carrier: AccountingCarrier, *, source_authority: CoreAcceptedAccountingFactProjection
    ) -> None:
    ...
    def _posting_projections(source: AcceptedAccountingFact) -> list[dict[str, object]]:
        ...
        if source.source_type == "deposit":
            roles = (
                ("OWNED_AVAILABLE", "DEBIT", "ASSET_RECEIVED"),
                ("EXTERNAL_CAPITAL", "CREDIT", "CAPITAL_CLASSIFIED"),
            )
        elif source.source_type == "withdrawal":
            roles = (
                ("EXTERNAL_CAPITAL", "DEBIT", "CAPITAL_CLASSIFIED"),
                ("OWNED_AVAILABLE", "CREDIT", "ASSET_PAID"),
            )
        else:
            raise AccountingAuthorityError("UNSUPPORTED_ACCOUNTING_SEMANTICS")
        return [
            ...
            for index, (role, direction, posting_role) in enumerate(roles)
        ]

    @staticmethod
    def _ledger_id(batch_fingerprint: str, append_sequence: int) -> str:
        raw = list(
            hashlib.sha256(f"{batch_fingerprint}:{append_sequence}".encode()).hexdigest()[:32]
        )
        raw[12], raw[16] = "7", "8"
        value = "".join(raw)
    ...
    def _entries(cls, source: AcceptedAccountingFact, start: int) -> tuple[LedgerEntry, ...]:
        ...
        return tuple(
            LedgerEntry(
                cls._ledger_id(fingerprint, start + index),
                posting["workspace_id"],
                posting["portfolio_id"],
                posting["environment"],
                posting["exchange_account_id"],
                None,
                dict(posting["asset_reference"]),
    ...
    def _validate_state(self, state: object, *, validate_upstream: bool) -> None:
        if (
            not isinstance(state, AtomicAccountingState)
            or type(state.store_revision) is not int
            or state.store_revision != len(state.accepted_batches)
            or type(state.accepted_batches) is not tuple
            or type(state.journal) is not tuple
        ):
        ...
        for accepted in state.accepted_batches:
            if (
                not isinstance(accepted, AcceptedAccountingBatch)
                or accepted.accounting_source_identity in seen_sources
                or accepted.first_append_sequence != cursor
                or accepted.entry_count <= 0
            ):
                raise AccountingAuthorityError("CONTRACT_INCONSISTENT")
            seen_sources.add(accepted.accounting_source_identity)
            source = (
                self._sources.resolve(accepted.accounting_source_identity)
            ...
            if (
                not entries
                or entries != expected
                or accepted.entry_count != len(expected)
                or accepted.batch_fingerprint_sha256 != expected[0].batch_fingerprint_sha256
                or accepted.source_type != source.source_type
                or accepted.accounting_source_fingerprint_sha256 != source.source_fingerprint_sha256
                or accepted.accounting_rule_version != ACCOUNTING_RULE_VERSION
            ):
                raise AccountingAuthorityError("CONTRACT_INCONSISTENT")
            for entry in entries:
                ...
                if entry.account_role in OWNED_ROLES:
                    key = (
                        ...
                        tuple(entry.asset_reference.items()),
                        entry.account_role,
                    )
                    amount = _quantity(entry.quantity)
                    balances[key] = balances.get(key, Fraction()) + (
                        amount if entry.direction == "DEBIT" else -amount
                    )
    ...
    def _accept_source(self, identity: str) -> AcceptedAccountingBatch:
        ...
        with self._carrier.authority_fence():
            ...
            prior = next(
                (
                    item
                    for item in state.accepted_batches
                    if item.accounting_source_identity == identity
                ),
                None,
            )
            if prior is not None:
            ...
            accepted = AcceptedAccountingBatch(
                source.source_type,
                identity,
                source.source_fingerprint_sha256,
                ACCOUNTING_RULE_VERSION,
                entries[0].batch_fingerprint_sha256,
                len(state.journal) + 1,
                len(entries),
            )
            replacement = AtomicAccountingState(
                state.store_revision + 1,
                state.accepted_batches + (accepted,),
    ...
    def resolve_internal_quantity(
        ...
        *,
        ...
        """
        if (
            not _id(workspace_id, "ws")
            or not _id(portfolio_id, "port")
            or environment not in ENVIRONMENTS
            or not _id(exchange_account_id, "xacc")
            or not isinstance(asset_reference, AssetReference)
        ):
            raise AccountingAuthorityError("TRUSTED_CONTEXT_FAILURE")
        ...
        with self._carrier.authority_fence():
            ...
            selected = tuple(
                ...
                and entry.portfolio_id == portfolio_id
                and entry.environment == environment
                and entry.exchange_account_id == exchange_account_id
                and dict(entry.asset_reference) == asdict(asset_reference)
                and entry.account_role in OWNED_ROLES
            )
            if not selected:
