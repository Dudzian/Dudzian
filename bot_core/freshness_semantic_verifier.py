...
class PostgreSQLVerifierConnection:
    ...
    def __post_init__(self) -> None:
        if type(self.socket_directory) is not str or not os.path.isabs(self.socket_directory):
            ...
            try:
                ...
                if exc.sqlstate in {"22023", "28000"}:
                    raise FreshnessSemanticVerificationError(
                        "invalid retained credential selector"
                    ) from exc
                raise FreshnessVerifierUnavailable(
                    "retained credential resolver unavailable"
                ) from exc
        if row is None:
        ...
        *,
        ...
        with self.connection.connect() as database:
            try:
                ...
                if exc.sqlstate in {"22023", "28000"}:
                    raise FreshnessSemanticVerificationError(
                        "invalid retained predecessor selector"
                    ) from exc
                raise FreshnessVerifierUnavailable(
                    "retained predecessor resolver unavailable"
                ) from exc
...
def _signature(value: object, name: str) -> bytes:
    ...
    if len(decoded) != 64 or base64.urlsafe_b64encode(decoded).rstrip(b"=").decode("ascii") != text:
...
class FreshnessSemanticVerifier:
    ...
    def verify_and_prepare(
        ...
        try:
            proposer = _object(
                parse_canonical_json(proposer_authentication),
                PROPOSER_AUTHENTICATION_FIELDS,
                "proposer authentication",
            )
            document = _object(
                parse_canonical_json(authoritative_document),
                DOCUMENT_FIELDS,
            ...
            return self._verified_prepare(
                proposer, document, payload, receipt, authoritative_document, finalization_receipt
            ).preparation_id
    ...
    def verify_exact_candidate(
        ...
        document = _object(
            parse_canonical_json(authoritative_document), DOCUMENT_FIELDS, "authoritative document"
        )
        receipt = _object(parse_canonical_json(finalization_receipt), RECEIPT_FIELDS, "receipt")
        payload = _object(document["payload"], DOCUMENT_PAYLOAD_FIELDS, "document payload")
        return self._verified_prepare(
    ...
    def _verified_prepare(
        ...
        if (
            _integer(proposer["schema_version"], "proposer schema version", 1) != 1
            or _integer(payload["schema_version"], "document schema version", 1) != 1
            or _integer(receipt["schema_version"], "receipt schema version", 1) != 1
        ):
        ...
        predecessor_generation = _integer(
            payload["predecessor_generation"], "predecessor_generation"
        )
        ...
        if (
            proposer["environment"] != environment
            or proposer["trust_domain"] != trust_domain
            or proposer["signed_or_authenticated_document_digest"] != document_digest
        ):
        ...
        proposer_unsigned = {
            k: v for k, v in proposer.items() if k != "authentication_tag_or_signature"
        }
        proposer_preimage = PROPOSER_DOMAIN + canonical_json_bytes(proposer_unsigned)
        _verify(
            proposer_credential.public_key,
        ...
        finalization_key_version = _integer(
            payload["freshness_authority_key_version"], "finalization key version", 1
        )
        ...
        _verify(
            finalization_credential.public_key,
        ...
        self._receipt_bindings(
            ...
            finalization_key_id,
            finalization_key_version,
            payload["finalization_request_id"],
        )
        receipt_unsigned = {
            k: v for k, v in receipt.items() if k != "authentication_tag_or_signature"
        }
        ...
        preparation_without_id: dict[str, object] = {
            ...
            "authoritative_document_authentication_digest": hashlib.sha256(
                document_preimage
            ).hexdigest(),
            "receipt_id": receipt["receipt_id"],
            "receipt_canonical_digest": receipt_digest,
    ...
    @staticmethod
    def _credential(
        value: RetainedVerificationCredential,
        environment: str,
        trust_domain: str,
        authority_id: str,
        role: str,
        ...
        if (
            type(value) is not RetainedVerificationCredential
            or (
                value.environment,
                value.trust_domain,
                value.authority_id,
            ...
            != (environment, trust_domain, authority_id, role, key_id, key_version)
            or type(value.lifecycle_generation) is not int
            or value.lifecycle_generation < 1
            or type(value.public_key) is not bytes
            or len(value.public_key) != 32
        ):
    ...
    def _receipt_bindings(
        ...
        expected = {
            ...
            "accepted_document_digest": document_digest,
        ...
        _integer(receipt["exact_predecessor_generation"], "receipt predecessor generation")
        _integer(receipt["accepted_generation"], "receipt accepted generation", 1)
        _integer(receipt["freshness_authority_key_version"], "receipt finalization key version", 1)
        ...
        while stop_after is None or handled < stop_after:
            ...
            with connection:
                ...
                try:
                    ...
                    inputs = tuple(
                        _decode_ipc_bytes(request[name], name)
                        for name in (
                            "proposer_authentication",
                            "authoritative_document",
                            "finalization_receipt",
