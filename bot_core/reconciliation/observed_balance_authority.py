...
FACT_FIELDS = frozenset({
    "workspace_id", "portfolio_id", "environment", "exchange_account_id",
    "asset_reference", "observed_quantity", "as_of_utc", "source_id",
...
class RawAssetReference:
    ...
    def from_mapping(cls, value: object) -> RawAssetReference:
        ...
        if any(type(item) is not str or not item for item in asdict(result).values()) or result.mapping_status not in MAPPING_STATUSES:
            raise ObservedBalanceAuthorityError("TRUSTED_CONTEXT_FAILURE")
        return result
...
class _ObservedBalancePublisher:
    ...
    def publish(self, payload: Mapping[str, object], *, semantics: str) -> AcceptedObservedBalanceFact:
...
class CoreAcceptedObservedBalanceFactProjection:
    ...
    @classmethod
    def compose(cls, carrier: ObservedBalanceCarrier) -> tuple[CoreAcceptedObservedBalanceFactProjection, _ObservedBalancePublisher]:
    ...
    def _validate_payload(payload: object) -> tuple[dict[str, object], RawAssetReference, str]:
        ...
        if not _identity(item["workspace_id"], "ws") or not _identity(item["portfolio_id"], "port") or not _identity(item["exchange_account_id"], "xacc") or type(item["source_id"]) is not str or item["environment"] not in ENVIRONMENTS:
            raise ObservedBalanceAuthorityError("TRUSTED_CONTEXT_FAILURE")
        asset = RawAssetReference.from_mapping(item["asset_reference"])
        if type(item["observed_quantity"]) is not str or DECIMAL.fullmatch(item["observed_quantity"]) is None:
            raise ObservedBalanceAuthorityError("TRUSTED_CONTEXT_FAILURE")
        ...
        if type(item["source_fingerprint_sha256"]) is not str or SHA256.fullmatch(item["source_fingerprint_sha256"]) is None or item["source_fingerprint_sha256"] != computed:
            raise ObservedBalanceAuthorityError("TRUSTED_CONTEXT_FAILURE")
    ...
    def _record(cls, payload: object, semantics: object) -> AcceptedObservedBalanceFact:
        ...
        membership_fingerprint = cls._membership_fingerprint(
            item["source_id"], fingerprint, semantics,  # type: ignore[arg-type]
        )
        return AcceptedObservedBalanceFact(
            item["source_id"], fingerprint, item["workspace_id"], item["portfolio_id"],
            item["environment"], item["exchange_account_id"], asset,
            item["observed_quantity"], item["as_of_utc"], semantics,
            membership_fingerprint, canonical,
        )  # type: ignore[arg-type]

    def _validate_state(self, state: object) -> None:
        if not isinstance(state, AtomicObservedBalanceState) or type(state.store_revision) is not int or state.store_revision != len(state.accepted) or type(state.accepted) is not tuple:
            raise ObservedBalanceAuthorityError("CORRUPT_OBSERVED_BALANCE_AUTHORITY")
        seen: set[str] = set()
        for position, record in enumerate(state.accepted, 1):
            if not isinstance(record, AcceptedObservedBalanceFact) or record.source_id in seen or position > state.store_revision:
                raise ObservedBalanceAuthorityError("CORRUPT_OBSERVED_BALANCE_AUTHORITY")
            seen.add(record.source_id)
            try:
                restored = self._record(json.loads(record.canonical_observed_fact_json), record.semantics)
    ...
    def _publish(self, payload: Mapping[str, object], *, semantics: str) -> AcceptedObservedBalanceFact:
        record = self._record(payload, semantics)
        with self._carrier.authority_fence():
            state = self._carrier.read()
            self._validate_state(state)
            prior = next((item for item in state.accepted if item.source_id == record.source_id), None)
            if prior is not None:
                if prior != record:
                    raise ObservedBalanceAuthorityError("OBSERVED_BALANCE_IDENTITY_CONFLICT")
                return prior
            replacement = AtomicObservedBalanceState(state.store_revision + 1, state.accepted + (record,))
            self._validate_state(replacement)
            self._carrier.compare_and_swap(state.store_revision, replacement)
            return record

    def resolve(self, source_id: str, *, expected_fingerprint: str | None = None) -> AcceptedObservedBalanceFact | None:
        with self._carrier.authority_fence():
            state = self._carrier.read()
            self._validate_state(state)
            record = next((item for item in state.accepted if item.source_id == source_id), None)
            if record is not None and expected_fingerprint is not None and record.source_fingerprint_sha256 != expected_fingerprint:
                raise ObservedBalanceAuthorityError("TRUSTED_CONTEXT_FAILURE")
            return record

    def consume_accepted(self, source_id: str, expected_fingerprint: str, consumer: Callable[[AcceptedObservedBalanceFact], T]) -> T:
