...
class LicenseValidationResult:
    ...
    def to_context(self) -> dict[str, Any]:
        ...
        if self.revocation_actor:
            context["revocation_actor"] = self.revocation_actor
    ...
    if isinstance(value, Mapping):
        for key in ("license_id", "id", "value", "license"):
            ...
            if isinstance(raw, Mapping):
                candidate = _coerce_license_identifier(raw)
            elif isinstance(raw, Sequence) and not isinstance(raw, (str, bytes, bytearray)):
                candidate = None
                for item in raw:
                    candidate = _coerce_license_identifier(item)
                    if candidate:
                        break
            else:
...
def _normalize_reason(value: Any) -> str | None:
    if isinstance(value, Mapping):
        for key in ("reason", "cause", "message", "comment", "details"):
            raw = value.get(key)
            if isinstance(raw, Mapping):
                candidate = _normalize_reason(raw)
            elif isinstance(raw, Sequence) and not isinstance(raw, (str, bytes, bytearray)):
                candidate = None
                for item in raw:
                    candidate = _normalize_reason(item)
                    if candidate:
                        break
            else:
                candidate = _normalize_text(raw)
...
def _normalize_actor(value: Any) -> str | None:
    if isinstance(value, Mapping):
        for key in (
            ...
            if candidate:
                return candidate
        for key in ("details", "meta", "metadata", "info", "context"):
            nested = value.get(key)
            if nested is not None:
                candidate = _normalize_actor(nested)
                if candidate:
                    return candidate
        return None
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        for item in value:
            candidate = _normalize_actor(item)
            if candidate:
                return candidate
        return None
    return _normalize_text(value)
...
def _coerce_revocation_entry(
    ...
    *,
    ...
    if isinstance(value, Mapping):
        ...
        aggregated_actor: str | None = None
        for item in value:
            item_id, item_reason, item_revoked_at, item_actor = _coerce_revocation_entry(
            ...
            if item_actor and not aggregated_actor:
                aggregated_actor = item_actor
        return (
    ...
    if actor is None and isinstance(value, Mapping):
        actor = _normalize_actor(value)

    return license_id, reason, revoked_at, actor


def _extract_revocation_entries(
    container: Any,
    ...
    def _register(
        ...
        existing_actor: str | None = None
        if license_id in entries:
            existing_reason, existing_revoked_at, existing_actor = entries[license_id]
        new_reason = existing_reason or reason
        new_revoked_at = existing_revoked_at or revoked_at
        new_actor = existing_actor or actor
        entries[license_id] = (new_reason, new_revoked_at, new_actor)
...
def _parse_revocation_document(
    ...
    *,
    errors: list[str],
    warnings: list[str],
) -> tuple[dict[str, tuple[str | None, str | None, str | None]], str | None]:
    ...
    revoked_entries: dict[str, tuple[str | None, str | None, str | None]] = {}
...
def validate_license(
    *,
    ...
    revocation_actor: str | None = None
    ...
    if not license_file.exists():
        ...
        return LicenseValidationResult(
            ...
            revocation_actor=revocation_actor,
            errors=errors,
            warnings=warnings,
            payload=None,
            ...
            try:
                ...
                if revoked_entries:
                    if license_id and license_id in revoked_entries:
                        entry_reason, entry_revoked_at, entry_actor = revoked_entries[license_id]
                        ...
                        if entry_actor and not revocation_actor:
                            revocation_actor = entry_actor
                        errors.append(
