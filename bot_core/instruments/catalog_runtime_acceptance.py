...
CATALOG_ADMISSION_MEMBERSHIP_COMMITMENT_DOMAIN = (
    "CRYPTOHUNTER_M0_12_CATALOG_MEMBERSHIP_COMMITMENT_V1"
)
...
def _valid_normalized_product(value: object) -> bool:
    ...
    if (
        ...
        or not all(
            type(value.get(name)) is str and value[name] and _ID.fullmatch(value[name])
...
_RELEASE_PRODUCERS = MappingProxyType(
    {"core_release_1_46_binance_spot": _BinanceSpotCatalogProducer}
)
...
def _normalize_binance(raw: object) -> _Observation | None:
    ...
    try:
        ...
        for item in symbols:
            ...
            if (
                not all(
                    type(v) is str and v and _ID.fullmatch(v) for v in (symbol, base, quote, status)
                )
                or symbol in seen
            ):
                return None
            ...
            for entry in filters:
                if (
                    ...
                    or type(entry.get("filterType")) is not str
                    or entry["filterType"] in by_type
                ):
                    return None
            ...
            min_notional = (
                None if notional_filter is None else _decimal(notional_filter.get("minNotional"))
            )
            ...
            products.append(
                {
                    "venue_symbol": unicodedata.normalize("NFC", symbol),
                    "market_type": "SPOT",
                    "base_asset": unicodedata.normalize("NFC", base),
                    "quote_asset": unicodedata.normalize("NFC", quote),
        ...
        evidence = MappingProxyType(
            {
                "policy": "BINANCE_EXCHANGE_INFO_ATOMIC_V1",
...
class CatalogRuntimeAcceptanceAuthority:
        ...
        with self._connect() as db:
            try:
                ...
                if marker_exists:
                    ...
                    db.execute(
                        "CREATE TABLE catalog_authority_metadata(singleton INTEGER PRIMARY KEY CHECK(singleton=1), authority_domain TEXT NOT NULL, schema_version INTEGER NOT NULL)"
                    )
                    db.execute(
                        "CREATE TABLE source_metadata_versions(source_metadata_sequence INTEGER PRIMARY KEY, source_metadata_version_id TEXT UNIQUE NOT NULL, source_exchange_id TEXT NOT NULL, market_type TEXT NOT NULL, venue_symbol TEXT NOT NULL, metadata_version INTEGER NOT NULL, introduced_by_snapshot_id TEXT NOT NULL, canonical_record TEXT NOT NULL, previous_metadata_digest TEXT NOT NULL, metadata_record_digest TEXT UNIQUE NOT NULL, UNIQUE(source_exchange_id,market_type,venue_symbol,metadata_version))"
                    )
                    db.execute(
                        "CREATE TABLE source_metadata_authority_head(singleton INTEGER PRIMARY KEY CHECK(singleton=1), committed_sequence INTEGER NOT NULL, committed_digest TEXT NOT NULL, last_metadata_version_id TEXT)"
                    )
                    db.execute(
                        "CREATE TABLE accepted_catalog_snapshots(snapshot_sequence INTEGER PRIMARY KEY, accepted_source_catalog_snapshot_id TEXT UNIQUE NOT NULL, source_exchange_id TEXT NOT NULL, market_type TEXT NOT NULL, upstream_snapshot_or_retrieval_id TEXT NOT NULL, normalized_content_sha256 TEXT NOT NULL, canonical_record TEXT NOT NULL, previous_digest TEXT NOT NULL, record_digest TEXT UNIQUE NOT NULL, UNIQUE(source_exchange_id,market_type,upstream_snapshot_or_retrieval_id))"
                    )
                    db.execute(
                        "CREATE TABLE catalog_snapshot_admission_receipts(accepted_source_catalog_snapshot_id TEXT PRIMARY KEY, catalog_admission_receipt_id TEXT UNIQUE NOT NULL, catalog_commitment_sha256 TEXT NOT NULL, membership_commitment_sha256 TEXT NOT NULL, receipt_mac TEXT NOT NULL)"
                    )
                    db.execute(
                        "CREATE TABLE catalog_authority_head(singleton INTEGER PRIMARY KEY CHECK(singleton=1), committed_sequence INTEGER NOT NULL, committed_digest TEXT NOT NULL, last_snapshot_id TEXT)"
                    )
                    db.execute("INSERT INTO catalog_authority_metadata VALUES(1,?,1)", (_DOMAIN,))
                    db.execute(
                        "INSERT INTO source_metadata_authority_head VALUES(1,0,?,NULL)", ("0" * 64,)
                    )
    ...
    def _snapshot_digest(
        ...
        membership_commitment: str = "",
        receipt_mac: str = "",
    ) -> str:
        envelope = _canonical(
            {
                "snapshot_sequence": sequence,
                "canonical_ascat": json.loads(canonical),
    ...
    def _replay(self, db: sqlite3.Connection) -> tuple[dict[str, Any], ...]:
        ...
        snapshot_head = db.execute(
            "SELECT committed_sequence,committed_digest,last_snapshot_id FROM catalog_authority_head WHERE singleton=1"
        ).fetchone()
        ...
        for expected, row in enumerate(snapshot_rows, 1):
            ...
            terminal = (
                None
                if grant is None
                else membership_state.terminal_by_membership.get(
            ...
            cutoff = (
                ...
                if terminal is None
                    ...
                    _time(terminal.effective_at_utc), _time(terminal.authority_admitted_at_utc)
                )
            )
            identity = {
                name: raw[name]
            ...
            if (
                ...
                or grant.producer_generation != raw["source_producer_generation"]
                or grant.content_fingerprint != raw["source_producer_membership_fingerprint"]
                or not _exact(grant, identity)
                or _time(grant.effective_at_utc) > accepted_at
                or _time(grant.authority_admitted_at_utc) > accepted_at
                or (cutoff is not None and cutoff <= accepted_at)
            ):
                raise ValueError("CORRUPT_CATALOG_AUTHORITY")
            ...
            if (
                receipt is None
                or not self._receipts.verify(receipt)
            ...
            snapshots_by_id[identifier] = raw
            scope_heads[key] = identifier
            previous = digest
        if snapshot_head[1] != previous or snapshot_head[2] != (
            snapshots[-1]["accepted_source_catalog_snapshot_id"] if snapshots else None
        ):
            raise ValueError("CORRUPT_CATALOG_AUTHORITY")

        metadata_head = db.execute(
            "SELECT committed_sequence,committed_digest,last_metadata_version_id FROM source_metadata_authority_head WHERE singleton=1"
        ).fetchone()
        metadata_rows = db.execute(
            "SELECT source_metadata_sequence,source_metadata_version_id,source_exchange_id,market_type,venue_symbol,metadata_version,introduced_by_snapshot_id,canonical_record,previous_metadata_digest,metadata_record_digest FROM source_metadata_versions ORDER BY source_metadata_sequence"
        ).fetchall()
        ...
        for expected, row in enumerate(metadata_rows, 1):
            ...
            if (
                sequence != expected
                or row_previous != previous
                or digest != self._metadata_digest(sequence, introduced_id, canonical, previous)
                or metadata_id in metadata_by_id
                or set(raw) != _METADATA_FIELDS
                or type(metadata_id) is not str
                or not metadata_id.startswith("smeta_")
                or raw.get("source_metadata_version_id") != metadata_id
                or not all(type(item) is str and bool(item) for item in key)
                or (raw.get("source_exchange_id"), raw.get("market_type"), raw.get("venue_symbol"))
                != key
                or type(raw.get("metadata_version")) is not int
                or raw["metadata_version"] < 1
                or raw["metadata_version"] != version
                or version != (1 if prior is None else prior[0] + 1)
                or raw.get("previous_source_metadata_version_id") != expected_predecessor
                or (expected_predecessor is not None and type(expected_predecessor) is not str)
                or not _valid_normalized_product(raw.get("normalized_metadata"))
                or raw["normalized_metadata"]["venue_symbol"] != symbol
                or raw["normalized_metadata"]["market_type"] != market_type
                or type(raw.get("content_fingerprint")) is not str
                or raw.get("content_fingerprint")
                != _digest(
                    _METADATA_FP_DOMAIN, raw, {"source_metadata_version_id", "content_fingerprint"}
                )
                or type(introduced_id) is not str
                or introducing is None
                or effective is None
                or effective > _time(introducing["effective_at_utc"])
                or (prior is not None and effective < prior[2])
            ):
                raise ValueError("CORRUPT_CATALOG_AUTHORITY")
            references = [
                member
                for member in introducing["member_source_product_metadata_versions"]
                if member["source_metadata_version_id"] == metadata_id
            ]
            if (
                len(references) != 1
                or (
                    references[0]["source_exchange_id"],
                    references[0]["market_type"],
                    references[0]["venue_symbol"],
                )
                != key
                or key[:2] != (introducing["source_exchange_id"], introducing["market_type"])
            ):
                raise ValueError("CORRUPT_CATALOG_AUTHORITY")
            metadata_by_id[metadata_id] = raw
            metadata_heads[key] = (version, metadata_id, effective)
            previous = digest
        if metadata_head[1] != previous or metadata_head[2] != (
            metadata_rows[-1][1] if metadata_rows else None
        ):
            raise ValueError("CORRUPT_CATALOG_AUTHORITY")
        for snapshot in snapshots:
            ...
            for member in snapshot["member_source_product_metadata_versions"]:
                ...
                if (
                    metadata is None
                    or (
                        metadata["source_exchange_id"],
                        metadata["market_type"],
                        metadata["venue_symbol"],
                    )
                    != key
                    or key[:2] != (snapshot["source_exchange_id"], snapshot["market_type"])
                ):
                    raise ValueError("CORRUPT_CATALOG_AUTHORITY")
            ...
            if (
                ...
                or evidence["product_count"]
                != len(snapshot["member_source_product_metadata_versions"])
    ...
    def fetch_catalog_once(self, release_binding: object) -> AcceptedSourceCatalogSnapshot | None:
        ...
        producer_type = (
            _RELEASE_PRODUCERS.get(release_binding) if type(release_binding) is str else None
        )
    ...
    def _accept_locked(
        ...
        for grant in state.grants.values():
            ...
            if (
                _exact(grant, producer.identity)
                and grant.producer_generation == producer.generation
                and _time(grant.effective_at_utc) <= when
                and _time(grant.authority_admitted_at_utc) <= when
                and (cutoff is None or when < cutoff)
            ):
        ...
        existing_row = db.execute(
            "SELECT canonical_record,normalized_content_sha256 FROM accepted_catalog_snapshots WHERE source_exchange_id=? AND market_type=? AND upstream_snapshot_or_retrieval_id=?",
            (membership.source_exchange_id, membership.market_type, observation.retrieval_id),
        ).fetchone()
        ...
        if existing_row is not None:
            ...
            snapshot = AcceptedSourceCatalogSnapshot(
                **{
                    **existing,
                    "member_source_product_metadata_versions": tuple(
                        existing["member_source_product_metadata_versions"]
                    ),
                }
            )
        ...
        for product in observation.products:
            ...
            history = db.execute(
                "SELECT canonical_record FROM source_metadata_versions WHERE source_exchange_id=? AND market_type=? AND venue_symbol=? ORDER BY metadata_version",
                key,
            ).fetchall()
            previous_record = json.loads(history[-1][0]) if history else None
            if previous_record is not None and previous_record["normalized_metadata"] == dict(
                product
            ):
                ...
                material = {
                    "source_exchange_id": key[0],
                    "market_type": key[1],
                    "venue_symbol": key[2],
                    "metadata_version": version,
                    "previous_source_metadata_version_id": None
                    if previous_record is None
                    else previous_record["source_metadata_version_id"],
                    "normalized_metadata": dict(product),
                ...
                pending_metadata.append(
                    (
                        ...
                        version,
                        {
                            "source_metadata_version_id": metadata_id,
                            **material,
        ...
        previous_snapshot = next(
            (
                item["accepted_source_catalog_snapshot_id"]
                for item in reversed(snapshots)
                if (item["source_exchange_id"], item["market_type"]) == scope
            ),
            None,
        )
        stale = (
            (when + timedelta(seconds=producer.freshness_seconds))
            .isoformat(timespec="microseconds")
            .replace("+00:00", "Z")
        )
        material = {
            "source_exchange_id": scope[0],
            "market_type": scope[1],
            "source_adapter_family_id": membership.source_adapter_family_id,
            "source_adapter_implementation_id": membership.source_adapter_implementation_id,
            "source_adapter_release_id": membership.source_adapter_release_id,
            "source_adapter_version": membership.source_adapter_version,
            "accepted_source_producer_membership_id": membership.accepted_source_producer_membership_id,
            "source_producer_generation": membership.producer_generation,
            "source_producer_membership_fingerprint": membership.content_fingerprint,
            "upstream_snapshot_or_retrieval_id": observation.retrieval_id,
            "observed_at_utc": observation.observed_at_utc,
            "effective_at_utc": now,
            "stale_after_utc": stale,
            "previous_snapshot_id": previous_snapshot,
            "completeness_status": "COMPLETE",
            "completeness_evidence": dict(observation.completeness_evidence),
            "acceptance_status": "VALID",
            "member_source_product_metadata_versions": members,
        }
        ...
        if (
            set(record) != SOURCE_FIELDS
            or not validate_accepted_source_catalog_snapshot(record)
            or any(set(member) != SOURCE_MEMBER_FIELDS for member in members)
        ):
        ...
        if pending_metadata:
            db.execute(
                "UPDATE source_metadata_authority_head SET committed_sequence=?,committed_digest=?,last_metadata_version_id=? WHERE singleton=1",
                (metadata_sequence, metadata_previous, pending_metadata[-1][0]),
            )
        canonical = _canonical(record)
        head = db.execute(
            "SELECT committed_digest FROM catalog_authority_head WHERE singleton=1"
        ).fetchone()[0]
        db.execute(
        ...
        db.execute(
            "INSERT INTO accepted_catalog_snapshots VALUES(?,?,?,?,?,?,?,?,?)",
            (
                sequence,
                snapshot_id,
                *scope,
                observation.retrieval_id,
                content_hash,
                canonical,
                head,
                digest,
        ...
        db.execute(
            "UPDATE catalog_authority_head SET committed_sequence=?,committed_digest=?,last_snapshot_id=? WHERE singleton=1",
            (sequence, digest, snapshot_id),
        )
        storage = {
            "accepted_source_catalog_snapshot_id": snapshot_id,
            ...
            return tuple(
                AcceptedSourceCatalogSnapshot(
                    **{
                        **raw,
                        "member_source_product_metadata_versions": tuple(
                            raw["member_source_product_metadata_versions"]
                        ),
                    }
                )
                for raw in self._replay(db)
            )

    def metadata_history(
        self, source_exchange_id: object, market_type: object, venue_symbol: object
    ) -> tuple[SourceProductMetadataVersion, ...]:
...
__all__ = [
    ...
    "SourceProductMetadataVersion",
    "catalog_admission_catalog_commitment",
