...
class RiskSnapshot:
    """Normalized snapshot of GUI risk controls and runtime configuration."""
...
def snapshot_from_app(app: Any) -> RiskSnapshot:
    """Extract a :class:`RiskSnapshot` instance from a Trading GUI like object."""
...
def compute_default_notional(snapshot: RiskSnapshot, *, default_notional: float) -> float:
    """Compute paper order notional based on risk profile limits."""
...
def format_decimal(value: float, *, decimals: int = 2, fallback: str = "0") -> str:
    """Format numeric values for GUI inputs."""
...
def build_risk_limits_summary(snapshot: RiskSnapshot) -> Optional[str]:
    """Return summary of risk limits exposed by the runtime profile."""
...
def apply_runtime_risk_context(
    ...
    *,
    ...
    """Load risk profile from runtime bootstrap and synchronize GUI widgets."""
    ...
    if resolved_name and hasattr(gui, "risk_profile_name"):
        ...
        except Exception:  # pragma: no cover - GUI object rejected the attribute
            if logger:
...
def refresh_runtime_risk_context(
    ...
    *,
    ...
    """Recompute derived GUI hints after runtime profile changed in place."""
