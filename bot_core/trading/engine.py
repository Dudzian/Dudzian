...
class PortfolioManager:
    ...
    def _apply_turnover_limit(
        ...
        if target_weights.size == 0 or previous_weights is None:
            return target_weights

        turnover_limit: Optional[float] = None
        enforce_turnover = False
        if max_turnover is not None:
            turnover_candidate = float(max_turnover)
            if np.isfinite(turnover_candidate) and turnover_candidate >= 0.0:
                turnover_limit = turnover_candidate
                enforce_turnover = True
        ...
        if not enforce_turnover and not has_asset_limits:
            return target_weights
