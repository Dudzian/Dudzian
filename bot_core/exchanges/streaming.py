...
class LocalLongPollStream(Iterable[StreamBatch]):
    ...
    def _decode_payload(
        ...
        if not encoding_values:
            return payload
        data = payload
        # RFC 7231 określa, że nagłówki Content-Encoding wymieniają kodowania
        # w kolejności ich nakładania na reprezentację. Aby uzyskać pierwotne
        # dane, należy je zdejmować w odwrotnej kolejności.
