...
class LiveExecutionRouter(ExecutionService):
    ...
    async def _process_order(
        ...
        route_id = route_name or "default"
            ...
            while attempt <= max_retries:
                ...
                try:
                    ...
                    self._m_orders_total.inc(
                        labels={"exchange": exchange_name, "route": route_name or "default"}
                    )
                    if breaker:
        ...
        attempts_rec.append({"status": "failed", "latency_s": f"{elapsed:.6f}", "queue_wait_s": f"{queue_wait:.6f}"})
