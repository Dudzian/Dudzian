...
def _id(value: object, prefix: str) -> bool:
    return (
        type(value) is str and value.startswith(prefix + "_") and _ID.fullmatch(value) is not None
    )
...
def _validate_fields(item: dict[str, object], *, v2: bool) -> None:
    ...
    if (
        not snapshot_valid
        or not _id(item["fill_id"], "fill")
        or not _id(item["order_id"], "ord")
        or not _id(item["workspace_id"], "ws")
        or not _id(item["portfolio_id"], "port")
        or not _id(item["exchange_account_id"], "xacc")
        or not _id(item["instrument_id"], "instr")
        or not _id(item["execution_route_id"], "xroute")
        or not _enum(item["environment"], ("PAPER", "TESTNET", "LIVE"))
        or type(item["exchange_id"]) is not str
        or not item["exchange_id"]
        or type(item["venue_trade_id"]) is not str
        or not item["venue_trade_id"]
    ...
    if (
        not legal_fee
        or not _sha256(item["fill_fingerprint_sha256"])
