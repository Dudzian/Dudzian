...
def summarize_by_interval(statuses: Iterable[CoverageStatus]) -> Mapping[str, Mapping[str, object]]:
    """Zwraca zagregowane pokrycie danych w podziale na interwały."""

    breakdown: dict[str, dict[str, object]] = {}
    for status in statuses:
        normalized = normalize_interval_token(status.interval) or status.interval
        ...
        if status.issues:
            bucket["error"] = int(bucket["error"]) + 1
        else:
            bucket["ok"] = int(bucket["ok"]) + 1

        manifest_status = status.manifest_entry.status or "unknown"
        manifest_counts: dict[str, int] = bucket.setdefault("manifest_status_counts", {})
        manifest_counts[manifest_status] = manifest_counts.get(manifest_status, 0) + 1
        ...
        if gap is not None:
            ...
            if numeric_gap is not None:
                current_best = bucket.get("_worst_gap_value")
                if current_best is None or numeric_gap > float(current_best):
                    bucket["_worst_gap_value"] = numeric_gap
                    bucket["worst_gap"] = {
                        "symbol": status.symbol,
                        "interval": status.interval,
                        "gap_minutes": numeric_gap,
                    }

    for payload in breakdown.values():
        payload.pop("_worst_gap_value", None)
        total = int(payload.get("total", 0))
        ok = int(payload.get("ok", 0))
        errors = int(payload.get("error", 0))
        if total:
            payload["ok_ratio"] = ok / total
...
def summarize_by_symbol(statuses: Iterable[CoverageStatus]) -> Mapping[str, Mapping[str, object]]:
    """Zwraca zagregowane pokrycie danych w podziale na symbole."""
