...
from bot_core.observability.metrics import DataFeedMetricSet, MetricsRegistry, get_data_feed_metrics
...
class CachedOHLCVSource(DataSource):
    ...
    _metrics: DataFeedMetricSet | None = field(default=None, init=False, repr=False)
    ...
    def fetch_ohlcv(self, request: OHLCVRequest) -> OHLCVResponse:
        ...
        if should_hit_upstream:
            ...
            upstream_started_at = time.perf_counter() if metrics is not None else None
            upstream_response = self._fetch_upstream_response(upstream_request, columns)
            if metrics is not None and upstream_started_at is not None:
                metrics.observe_fetch_latency(
                    symbol=request.symbol,
                    interval=request.interval,
                    source="upstream",
                    latency_seconds=max(0.0, time.perf_counter() - upstream_started_at),
                )
        ...
        if snapshot_fetcher is not None:
            snapshot_started_at = time.perf_counter() if metrics is not None else None
            try:
        ...
        if metrics is not None and snapshot_fetcher is not None and snapshot_started_at is not None:
            metrics.observe_fetch_latency(
                symbol=request.symbol,
                interval=request.interval,
                source="snapshot",
                latency_seconds=max(0.0, time.perf_counter() - snapshot_started_at),
            )

        if metrics is not None:
            missing = self._estimate_missing_candles(request, filtered)
            metrics.report_missing_candles(
                symbol=request.symbol,
                interval=request.interval,
                missing=missing,
            )
    ...
    def _get_metrics(self) -> DataFeedMetricSet | None:
        registry = self.metrics_registry or get_global_metrics_registry()
        if registry is None:
            return None
        if self._metrics is None or self._metrics.registry is not registry:
            self._metrics = get_data_feed_metrics(registry)
        return self._metrics

    def _estimate_missing_candles(
        self, request: OHLCVRequest, rows: Sequence[Sequence[float]]
    ) -> int:
        if not rows:
            return 0
        try:
            interval_ms = interval_to_milliseconds(request.interval)
        except (KeyError, ValueError):  # pragma: no cover - nieznane interwały
            return 0
        if interval_ms <= 0:
            return 0

        timestamps = sorted({float(row[0]) for row in rows if row})
        if len(timestamps) < 2:
            return 0

        expected = int((timestamps[-1] - timestamps[0]) // interval_ms) + 1
