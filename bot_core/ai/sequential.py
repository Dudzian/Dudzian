...
def _burke(values: Sequence[float]) -> float:
    if not values:
        return 0.0
    cumulative = 0.0
    peak = 0.0
    ...
    if not squared_drawdowns:
        return max(-5.0, min(5.0, mean_return))
    rms_drawdown = math.sqrt(sum(squared_drawdowns) / len(squared_drawdowns))
    if rms_drawdown <= 1e-12:
        if mean_return <= 0.0:
            return 0.0
        return 5.0
    ratio = mean_return / rms_drawdown
    if not math.isfinite(ratio):
        return 0.0
    return max(-5.0, min(5.0, ratio))
...
class SequentialTrainingReport:
    ...
    heuristic_burke: Mapping[str, float]
...
class SequentialTrainingPipeline:
    ...
    def train_offline(
        ...
        *,
        ...
        if heuristic_burke:
            metadata["heuristics_burke"] = {
                name: float(value) for name, value in heuristic_burke.items()
            }
        ...
        return SequentialTrainingReport(
            ...
            heuristic_burke=dict(heuristic_burke),
        )
...
class SequentialOnlineScorer:
    ...
    def __init__(
        ...
        *,
        ...
        heuristic_burke: Mapping[str, float] | None = None,
    ) -> None:
        self._model = model
        registry = build_heuristic_registry(heuristics)
        self._heuristics = select_heuristics(heuristic_names, registry=registry)
        ...
        self._heuristic_burke = {
            name: float(value)
            for name, value in (heuristic_burke or {}).items()
            if name in self._heuristics
        }
    ...
    def score(self, features: Mapping[str, float]) -> OnlineScoringResult:
        ...
        for name, confidence in self._heuristic_confidence.items():
            ...
            if name in self._heuristic_burke:
                diagnostics[f"heuristic_burke.{name}"] = float(
                    self._heuristic_burke.get(name, 0.0)
                )
    ...
    def _heuristic_probability(self, prediction: float) -> float:
        ...
        for name in self._heuristics:
            ...
            burke_factor = self._burke_factor(self._heuristic_burke.get(name))
            weighted_confidence += (
                weight
                * bounded
                * trend_factor
    ...
    @staticmethod
    def _burke_factor(value: float | None) -> float:
        if value is None or not math.isfinite(value):
            return 1.0
        clipped = max(-3.0, min(3.0, value))
        adjusted = 1.0 + 0.08 * clipped
        return max(0.6, min(1.6, adjusted))
