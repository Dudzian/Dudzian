...
@dataclass(frozen=True)
class ActivationTagStats:
    """Podsumowanie wykorzystania tagów strategii w historii aktywacji."""

    total: int
    activations_with_tags: int
    tag_counts: Mapping[str, int]
    tags_by_regime: Mapping[MarketRegime, Mapping[str, int]]
    fallback_tag_counts: Mapping[str, int]
    blocked_tag_counts: Mapping[str, int]


@dataclass(frozen=True)
class ActivationCapabilityStats:
    """Podsumowanie wymagań zdolności (capabilities) wykorzystywanych przez presety."""

    total: int
    activations_with_capabilities: int
    capability_counts: Mapping[str, int]
    capabilities_by_regime: Mapping[MarketRegime, Mapping[str, int]]
    fallback_capability_counts: Mapping[str, int]
    blocked_capability_counts: Mapping[str, int]


@dataclass(frozen=True)
class ActivationRiskStats:
    """Agregacja klas ryzyka używanych podczas aktywacji presetów."""

    total: int
    activations_with_risk: int
    risk_counts: Mapping[str, int]
    risk_by_regime: Mapping[MarketRegime, Mapping[str, int]]
    fallback_risk_counts: Mapping[str, int]
    blocked_risk_counts: Mapping[str, int]
...
@dataclass(frozen=True)
class ActivationRecommendationStats:
    """Podsumowanie rekomendacji strategii zwracanych przez orchestratora decyzji."""
    ...
    recommendation_by_regime: Mapping[MarketRegime, Mapping[str, int]]
...
class StrategyRegimeWorkflow:
    ...
    def activation_recommendation_stats(
        self,
        *,
        limit: int | None = None,
    ) -> ActivationRecommendationStats:
        """Agreguje rekomendacje strategii generowane podczas aktywacji."""
        ...
        if total == 0:
            empty_counts: Mapping[str, int] = MappingProxyType({})
            empty_nested: Mapping[MarketRegime, Mapping[str, int]] = MappingProxyType({})
            return ActivationRecommendationStats(
                total=0,
                ...
                recommendation_counts=empty_counts,
                recommendation_by_regime=empty_nested,
            )
        ...
        matched_candidates = 0
        fallback_recommendations = 0
        recommendation_counts: MutableMapping[str, int] = {}
        recommendation_by_regime: MutableMapping[MarketRegime, MutableMapping[str, int]] = {}
        ...
        for activation in entries:
            ...
            regime_bucket = recommendation_by_regime.setdefault(activation.regime, {})
            regime_bucket[normalized] = regime_bucket.get(normalized, 0) + 1

            preset_names = self._preset_strategy_names(activation.preset)
            if normalized.lower() in preset_names:
                matched_presets += 1

            candidate_names = {
                candidate.strategy.strip().lower()
                for candidate in activation.decision_candidates
                if candidate.strategy.strip()
            }
            if normalized.lower() in candidate_names:
                matched_candidates += 1

            if activation.used_fallback:
                fallback_recommendations += 1

        unmatched_recommendations = max(0, with_recommendation - matched_presets)
        ...
        return ActivationRecommendationStats(
            ...
            recommendation_counts=_freeze_counts(recommendation_counts),
            recommendation_by_regime=_freeze_nested(recommendation_by_regime),
        )

    def activation_tag_stats(
        self,
        *,
        limit: int | None = None,
    ) -> ActivationTagStats:
        """Agreguje wykorzystanie tagów strategii dla aktywacji workflow."""
        ...
        if total == 0:
            empty_counts: Mapping[str, int] = MappingProxyType({})
            empty_nested: Mapping[MarketRegime, Mapping[str, int]] = MappingProxyType({})
            return ActivationTagStats(
                total=0,
                activations_with_tags=0,
                tag_counts=empty_counts,
                tags_by_regime=empty_nested,
                fallback_tag_counts=empty_counts,
                blocked_tag_counts=empty_counts,
            )
        ...
        def _activation_tags(activation: RegimePresetActivation) -> tuple[str, ...]:
            metadata = activation.version.metadata
            tags = _normalize(metadata.get("tags", ())) if isinstance(metadata, Mapping) else ()
            if tags:
                return tags
            preset = activation.preset
            if isinstance(preset, Mapping):
                preset_tags = _normalize(preset.get("tags", ()))
                if preset_tags:
                    return preset_tags
                ...
                if isinstance(strategies, Iterable):
                    ...
                    for entry in strategies:
                        if not isinstance(entry, Mapping):
                            continue
                        for tag in _normalize(entry.get("tags", ())):
        ...
        activations_with_tags = 0
        tag_counts: MutableMapping[str, int] = {}
        tags_by_regime: MutableMapping[MarketRegime, MutableMapping[str, int]] = {}
        fallback_counts: MutableMapping[str, int] = {}
        blocked_counts: MutableMapping[str, int] = {}

        for activation in entries:
            tags = _activation_tags(activation)
            if not tags:
                continue

            activations_with_tags += 1
            regime_bucket = tags_by_regime.setdefault(activation.regime, {})
            for tag in tags:
                tag_counts[tag] = tag_counts.get(tag, 0) + 1
                regime_bucket[tag] = regime_bucket.get(tag, 0) + 1
                if activation.used_fallback:
                    fallback_counts[tag] = fallback_counts.get(tag, 0) + 1
                if activation.blocked_reason:
                    blocked_counts[tag] = blocked_counts.get(tag, 0) + 1
        ...
        return ActivationTagStats(
            total=total,
            activations_with_tags=activations_with_tags,
            tag_counts=_freeze_counts(tag_counts),
            tags_by_regime=_freeze_nested(tags_by_regime),
            fallback_tag_counts=_freeze_counts(fallback_counts),
            blocked_tag_counts=_freeze_counts(blocked_counts),
        )

    def activation_capability_stats(
        self,
        *,
        limit: int | None = None,
    ) -> ActivationCapabilityStats:
        """Agreguje wykorzystanie capabilities wymaganych przez aktywowane presety."""
        ...
        if total == 0:
            empty_counts: Mapping[str, int] = MappingProxyType({})
            empty_nested: Mapping[MarketRegime, Mapping[str, int]] = MappingProxyType({})
            return ActivationCapabilityStats(
                total=0,
                activations_with_capabilities=0,
                capability_counts=empty_counts,
                capabilities_by_regime=empty_nested,
                fallback_capability_counts=empty_counts,
                blocked_capability_counts=empty_counts,
            )
        ...
        def _activation_capabilities(activation: RegimePresetActivation) -> tuple[str, ...]:
            metadata = activation.version.metadata
            capabilities = (
                _normalize(metadata.get("capabilities", ()))
            ...
            preset = activation.preset
            if isinstance(preset, Mapping):
                preset_capabilities = _normalize(preset.get("capabilities", ()))
                if preset_capabilities:
                ...
                if isinstance(strategies, Iterable):
                    ...
                    for entry in strategies:
                        if not isinstance(entry, Mapping):
                            continue
                        capability_values = entry.get("capabilities")
                        for capability in _normalize(capability_values):
                            if capability not in collected:
                                collected.append(capability)
                        single_capability = entry.get("capability")
                        for capability in _normalize(single_capability):
                            if capability not in collected:
                                collected.append(capability)
                    if collected:
                        return tuple(collected)
        ...
        activations_with_capabilities = 0
        capability_counts: MutableMapping[str, int] = {}
        capabilities_by_regime: MutableMapping[MarketRegime, MutableMapping[str, int]] = {}
        fallback_counts: MutableMapping[str, int] = {}
        blocked_counts: MutableMapping[str, int] = {}

        for activation in entries:
            capabilities = _activation_capabilities(activation)
            if not capabilities:
                continue

            activations_with_capabilities += 1
            regime_bucket = capabilities_by_regime.setdefault(activation.regime, {})
            for capability in capabilities:
                capability_counts[capability] = capability_counts.get(capability, 0) + 1
                regime_bucket[capability] = regime_bucket.get(capability, 0) + 1
                if activation.used_fallback:
                    fallback_counts[capability] = fallback_counts.get(capability, 0) + 1
                if activation.blocked_reason:
                    blocked_counts[capability] = blocked_counts.get(capability, 0) + 1
        ...
        return ActivationCapabilityStats(
            total=total,
            activations_with_capabilities=activations_with_capabilities,
            capability_counts=_freeze_counts(capability_counts),
            capabilities_by_regime=_freeze_nested(capabilities_by_regime),
            fallback_capability_counts=_freeze_counts(fallback_counts),
            blocked_capability_counts=_freeze_counts(blocked_counts),
        )

    def activation_risk_stats(
        self,
        *,
        limit: int | None = None,
    ) -> ActivationRiskStats:
        """Agreguje klasy ryzyka wykorzystywane przez aktywacje workflow."""
        ...
        if total == 0:
            empty_counts: Mapping[str, int] = MappingProxyType({})
            empty_nested: Mapping[MarketRegime, Mapping[str, int]] = MappingProxyType({})
            return ActivationRiskStats(
                total=0,
                activations_with_risk=0,
                risk_counts=empty_counts,
                risk_by_regime=empty_nested,
                fallback_risk_counts=empty_counts,
                blocked_risk_counts=empty_counts,
            )
        ...
        def _activation_risk(activation: RegimePresetActivation) -> tuple[str, ...]:
            metadata = activation.version.metadata
            risks = _normalize(metadata.get("risk_classes", ())) if isinstance(metadata, Mapping) else ()
            if risks:
                return risks
            preset = activation.preset
            if isinstance(preset, Mapping):
                preset_risks = _normalize(preset.get("risk_classes", ()))
                if preset_risks:
                    return preset_risks
                strategies = preset.get("strategies", ())
                if isinstance(strategies, Iterable):
                    ...
                    for entry in strategies:
                        if not isinstance(entry, Mapping):
                            continue
                        for risk in _normalize(entry.get("risk_classes", ())):
                            if risk not in collected:
                                collected.append(risk)
                    if collected:
                        return tuple(collected)
            return ()

        activations_with_risk = 0
        risk_counts: MutableMapping[str, int] = {}
        risk_by_regime: MutableMapping[MarketRegime, MutableMapping[str, int]] = {}
        fallback_counts: MutableMapping[str, int] = {}
        blocked_counts: MutableMapping[str, int] = {}

        for activation in entries:
            risks = _activation_risk(activation)
            if not risks:
                continue
            activations_with_risk += 1
            regime_bucket = risk_by_regime.setdefault(activation.regime, {})
            for risk in risks:
                risk_counts[risk] = risk_counts.get(risk, 0) + 1
                regime_bucket[risk] = regime_bucket.get(risk, 0) + 1
                if activation.used_fallback:
                    fallback_counts[risk] = fallback_counts.get(risk, 0) + 1
                if activation.blocked_reason:
                    blocked_counts[risk] = blocked_counts.get(risk, 0) + 1
        ...
        return ActivationRiskStats(
            total=total,
            activations_with_risk=activations_with_risk,
            risk_counts=_freeze_counts(risk_counts),
            risk_by_regime=_freeze_nested(risk_by_regime),
            fallback_risk_counts=_freeze_counts(fallback_counts),
            blocked_risk_counts=_freeze_counts(blocked_counts),
        )
    ...
    def _preset_strategy_names(self, preset: Mapping[str, object]) -> set[str]:
        names: set[str] = set()
        preset_name = str(preset.get("name", "")).strip()
        if preset_name:
            names.add(preset_name.lower())
