...
class JsonRpcServer:
    ...
    def run(self) -> None:
        ...
        while not self._stopping.is_set():
            ...
            self.flush_ready()
            try:
                readable, _, _ = select.select([stdin], [], [], self._poll_interval)
            ...
            try:
                ...
                self._emit({"error": {"message": f"invalid-json: {exc}"}})
                continue
            if not isinstance(request, dict):
                self._emit({"error": {"message": "invalid-request"}})
                continue
            try:
                self.submit(request)
                ...
                identifier = request.get("id")
                self._emit({"id": identifier, "error": {"message": str(exc)}})
    ...
    def _notify_runtime_stopping(self) -> None:
        ...
        self._emit({"event": "runtime-stopping"})

    def _deliver(self, payload: Dict[str, Any]) -> None:
        try:
            self._results.put_nowait(payload)
        except queue.Full:
            _LOG.warning("Result queue full - emitting payload directly")
            self._emit(payload)
