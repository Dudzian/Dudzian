...
def _patch_portfolio_cycle_result_defaults() -> None:
    """Zapewnia wsteczną kompatybilność sygnatury PortfolioCycleResult.

    Historycznie portfolio_stress było opcjonalne; nowsze wersje mogą wymagać
    tego argumentu. Dla ścieżek archiwalnych HyperCare liberalizujemy
    konstruktor, aby domyślnie używał pustej mapy.
    """

    try:
        from bot_core.portfolio import hypercare as portfolio_hypercare
    except Exception:  # pragma: no cover - zależne od dystrybucji
        return

    result_cls = getattr(portfolio_hypercare, "PortfolioCycleResult", None)
    if result_cls is None:
        return

    original_init = getattr(result_cls, "__init__", None)
    if original_init is None:
        return

    defaults = getattr(original_init, "__defaults__", ()) or ()
    if defaults:  # już posiada wartości domyślne
        return
    ...
    result_cls.__init__ = _patched_init  # type: ignore[assignment]


_patch_portfolio_cycle_result_defaults()


def _resolve_composite_expectations(profile_name: str) -> Mapping[str, Any]:
    """Pobiera oczekiwania SLO2, respektując ewentualne aliasy/publiczne monkeypatche."""

    public_module = sys.modules.get("bot_core.runtime.stage6_hypercare")
    if public_module is None:
        try:
            public_module = importlib.import_module("bot_core.runtime.stage6_hypercare")
        except Exception:
            public_module = None

    if public_module is not None:
        func = getattr(public_module, "get_observability_composite_expectations", None)
        if callable(func):
            return func(profile_name)

    return get_observability_composite_expectations(profile_name)
...
class Stage6HypercareCycle:
    ...
    def _run_observability(self) -> MutableMapping[str, Any]:
        ...
        if slo_payload is not None:
            ...
            if profile_name:
                try:
                    expectations = _resolve_composite_expectations(profile_name)
