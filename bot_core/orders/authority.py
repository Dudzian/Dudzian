...
_CONTRACT_PATH = (Path(__file__).parents[2] / "docs/architecture/cryptohunter_product_architecture" / "commands_events_order_lifecycle_and_idempotency.json")
...
_ID = re.compile(r"^(?P<prefix>[a-z]+)_[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$")
...
class UpstreamAuthorityUnavailable(OrderAuthorityError):
    def __init__(self) -> None:
        super().__init__("TRUSTED_CONTEXT_FAILURE", "Workspace, ExchangeAccount, Instrument, and execution-route authorities unavailable")
...
def _validate_timestamp(value: Any, code: str) -> None:
    if type(value) is not str or not _TIMESTAMP.fullmatch(value) or unicodedata.normalize("NFC", value) != value:
...
def _validate_field(name: str, value: Any, schema: Mapping[str, Any], code: str) -> None:
    ...
    if kind == "id":
        ...
        if type(value) is not str or unicodedata.normalize("NFC", value) != value or (kind == "non_empty_string" and not value):
            raise OrderAuthorityError(code, f"invalid {name}")
    elif kind == "enum":
        values = schema.get("values", CONTRACT["event_contract"][schema["registry"]] if "registry" in schema else ())
        ...
        if type(value) is not str or not _DECIMAL.fullmatch(value) or (code == "MALFORMED_REQUEST" and value == "0"):
...
def _closed_validate(value: Mapping[str, Any], fields: list[str], schemas: Mapping[str, Any], nullable: list[str], code: str) -> None:
...
def validate_order_command(request: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    _closed_validate(request, spec["request_fields"], spec["field_schemas"], spec["nullable_fields"], "MALFORMED_REQUEST")
...
def validate_command_outcome(outcome: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    for name in ("command_id",):
        _validate_field(name, outcome[name], {"type": "id", "prefix": "cmd"}, "CONTRACT_INCONSISTENT")
    if type(outcome["operation_type"]) is not str or outcome["operation_type"] not in CONTRACT["command_registry"]:
        raise OrderAuthorityError("CONTRACT_INCONSISTENT", "invalid outcome operation")
    _validate_field("request_fingerprint_sha256", outcome["request_fingerprint_sha256"], {"type": "sha256_hex"}, "CONTRACT_INCONSISTENT")
    if type(outcome["outcome"]) is not str or outcome["outcome"] not in CONTRACT["command_outcome_contract"]["outcomes"]:
        raise OrderAuthorityError("CONTRACT_INCONSISTENT", "invalid command outcome")
    if outcome["denial_code"] is not None and (type(outcome["denial_code"]) is not str or outcome["denial_code"] not in CONTRACT["failure_taxonomy"]):
    ...
    for name, prefix in (("order_id", "ord"), ("accepted_audit_event_id", "evt")):
        if outcome[name] is not None:
            _validate_field(name, outcome[name], {"type": "id", "prefix": prefix}, "CONTRACT_INCONSISTENT")
...
def command_fingerprint_sha256(request: Mapping[str, Any]) -> str:
    ...
    value = {k: v for k, v in request.items() if k not in CONTRACT["idempotency_contract"]["fingerprint_excluded_fields"]}
...
def validate_order_event(event: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    _closed_validate(event, envelope["fields"], envelope["field_schemas"], envelope["nullable_fields"], "MALFORMED_EVENT")
    ...
    _closed_validate(event["safe_payload"], spec["safe_payload_fields"], spec["field_schemas"], spec["nullable_fields"], "MALFORMED_EVENT")
...
_RECONCILE = CONTRACT["order_lifecycle"]["dynamic_target_rules"]["RESOLVE_TRUSTED_RECONCILIATION_FACT"]["trusted_fact_targets"]
_REQUIRED_TABLES = frozenset({"authority_marker", "authority_heads", "command_journal", "event_journal"})
...
class OrderAuthority:
    ...
    def __init__(self, database: str | Path) -> None:
        ...
        try:
            ...
            raise AuthorityCorrupt("durable Order authority bootstrap rejected corrupt state") from exc
    ...
    def resolve_current(self, order_id: str) -> Mapping[str, Any] | None:
        ...
        with self._lock:
            ...
            return _freeze({"order_id": order_id, "aggregate_version": len(history), "state": state, "pre_request_state": pre})
    ...
    def resolve_event(self, order_id: str, aggregate_version: int) -> Mapping[str, Any] | None:
        ...
        with self._lock:
            self._verify_integrity()
            row = self._db.execute("SELECT canonical_event FROM event_journal WHERE order_id=? AND aggregate_version=?", (order_id, aggregate_version)).fetchone()
    ...
    def history(self, order_id: str) -> tuple[Mapping[str, Any], ...]:
        ...
        with self._lock:
            self._verify_integrity()
            return tuple(_freeze(json.loads(row["canonical_event"])) for row in self._history_rows(order_id))
    ...
    def _bootstrap_or_verify(self) -> None:
        ...
        if marker:
            ...
            row = self._db.execute("SELECT domain,purpose,algorithm,custody_handle,schema_version FROM authority_marker WHERE singleton=1").fetchone()
            if row is None or tuple(row)[:3] != (self.domain, self.authenticity_purpose, ORDER_AUTHENTICITY_ALGORITHM) or row[4] != 2:
        ...
        INSERT INTO authority_marker VALUES(1,'""" + self.domain + """','""" + self.authenticity_purpose + """','""" + ORDER_AUTHENTICITY_ALGORITHM + """','""" + self._custody_handle + """',2);
        INSERT INTO authority_heads VALUES(1,0,'""" + _ZERO + """','""" + _ZERO + """',0,'""" + _ZERO + """','""" + _ZERO + """');
        COMMIT;
        """)
        self._verify_integrity()

    def _authenticated_commitment(self, kind: str, fields: list[Any]) -> str:
        payload = self.authenticity_purpose.encode("ascii") + b"\x00" + self.domain.encode("utf-8") + b"\x00" + kind.encode("ascii") + b"\x00" + _canonical(fields).encode("utf-8")
    ...
    def _verify_integrity(self) -> None:
        try:
            ...
            raise
        except (OrderAuthorityError, sqlite3.Error, KeyError, TypeError, ValueError, AssertionError) as exc:
    ...
    def _verify_integrity_raw(self) -> None:
        ...
        marker = self._db.execute("SELECT domain,purpose,algorithm,custody_handle,schema_version FROM authority_marker WHERE singleton=1").fetchall()
        if len(marker) != 1 or tuple(marker[0]) != (self.domain, self.authenticity_purpose, ORDER_AUTHENTICITY_ALGORITHM, self._custody_handle, 2):
        ...
        for row in self._db.execute("SELECT * FROM command_journal ORDER BY command_sequence"):
            command_count += 1
            if row["command_sequence"] != command_count or row["previous_digest"] != command_head or row["previous_authenticated_commitment"] != command_auth:
                raise AuthorityCorrupt("command ordering or previous commitment mismatch")
            request = json.loads(row["canonical_request"]); outcome = json.loads(row["canonical_outcome"])
            validate_command_outcome(outcome); validate_order_command(request)
            fingerprint = command_fingerprint_sha256(request)
            expected_scope = _canonical([request[k] for k in CONTRACT["idempotency_contract"]["scope"]])
            if _canonical(request) != row["canonical_request"] or fingerprint != row["command_fingerprint"] or request["command_id"] != row["command_id"] or row["scope_key"] != expected_scope:
                raise AuthorityCorrupt("command canonical payload, scope, or fingerprint mismatch")
            if outcome["command_id"] != request["command_id"] or outcome["operation_type"] != request["operation_type"] or outcome["request_fingerprint_sha256"] != fingerprint:
                raise AuthorityCorrupt("command outcome is not bound to request")
            if request["operation_type"] == "SUBMIT_ORDER" and outcome["order_id"] != request["order_id"]:
                raise AuthorityCorrupt("command outcome order identity mismatch")
            data = [row["command_sequence"], row["command_id"], row["scope_key"], row["canonical_request"], row["command_fingerprint"], row["canonical_outcome"], row["previous_digest"]]
            command_head = _digest(data)
            auth_fields = [row["command_sequence"], row["command_id"], row["scope_key"], row["canonical_request"], row["command_fingerprint"], row["canonical_outcome"], row["previous_authenticated_commitment"]]
            expected_mac = self._authenticated_commitment("COMMAND", auth_fields)
            if command_head != row["record_digest"] or not hmac.compare_digest(expected_mac, row["admission_mac"]):
        ...
        for row in self._db.execute("SELECT * FROM event_journal ORDER BY event_sequence"):
            event_count += 1
            if row["event_sequence"] != event_count or row["previous_digest"] != event_head or row["previous_authenticated_commitment"] != event_auth:
                raise AuthorityCorrupt("event ordering or previous commitment mismatch")
            event = json.loads(row["canonical_event"]); validate_order_event(event)
            if (_canonical(event) != row["canonical_event"] or event["audit_event_id"] != row["audit_event_id"] or event["order_id"] != row["order_id"] or event["aggregate_version"] != row["aggregate_version"] or event["event_fingerprint_sha256"] != row["event_fingerprint"]):
                raise AuthorityCorrupt("event indexed envelope mismatch")
            data = [row["event_sequence"], row["audit_event_id"], row["order_id"], row["aggregate_version"], row["canonical_event"], row["event_fingerprint"], row["previous_digest"]]
            event_head = _digest(data)
            auth_fields = [row["event_sequence"], row["audit_event_id"], row["order_id"], row["aggregate_version"], row["canonical_event"], row["event_fingerprint"], row["previous_authenticated_commitment"]]
            expected_mac = self._authenticated_commitment("EVENT", auth_fields)
            if event_head != row["record_digest"] or not hmac.compare_digest(expected_mac, row["admission_mac"]):
                raise AuthorityCorrupt("event public digest or authenticated admission mismatch")
            event_auth = row["admission_mac"]
            by_order.setdefault(row["order_id"], []).append(row); events_by_id[event["audit_event_id"]] = event
        actual_heads = (heads[0]["command_count"], heads[0]["command_head"], heads[0]["command_authenticated_head"], heads[0]["event_count"], heads[0]["event_head"], heads[0]["event_authenticated_head"])
        if actual_heads != (command_count, command_head, command_auth, event_count, event_head, event_auth):
            raise AuthorityCorrupt("authority head rollback or mismatch")
        for rows in by_order.values(): self._replay(rows)
        ...
        planned_ids = {event_id for event_id, event in events_by_id.items() if event["event_type"] == "ORDER_PLANNED"}
        if planned_ids != referenced_plans:
            raise AuthorityCorrupt("orphan or multiply-bound ORDER_PLANNED event")

    @staticmethod
    def _verify_submit_order_plan_binding(request: Mapping[str, Any], outcome: Mapping[str, Any], event: Mapping[str, Any]) -> None:
        ...
        for name in ("environment", "workspace_id", "portfolio_id", "exchange_account_id", "instrument_id", "execution_route_id", "correlation_id", "causation_id"):
    ...
    def _history_rows(self, order_id: str) -> list[sqlite3.Row]:
        return list(self._db.execute("SELECT * FROM event_journal WHERE order_id=? ORDER BY aggregate_version", (order_id,)))

    @staticmethod
    def _fill_identity(event: Mapping[str, Any]) -> tuple[str, tuple[str, str, str, str], tuple[str, ...]]:
        payload = event["safe_payload"]
        external = (event["environment"], event["exchange_account_id"], event["exchange_id"], payload["venue_trade_id"])
        economics = tuple(str(event[name]) for name in ("order_id", "environment", "workspace_id", "portfolio_id", "exchange_account_id", "exchange_id", "instrument_id", "execution_route_id")) + (payload["venue_trade_id"], payload["cumulative_executed_quantity"])
        return payload["fill_id"], external, economics

    def _fill_duplicate_decision(self, rows: list[sqlite3.Row], candidate: Mapping[str, Any]) -> str | None:
    ...
    def _replay(self, rows: list[sqlite3.Row]) -> tuple[str, str | None]:
        state: str | None = None; pre: str | None = None
        scope: tuple[Any, ...] | None = None
        ordered: Decimal | None = None; cumulative = Decimal("0")
        seen_fills: dict[str, tuple[str, ...]] = {}; seen_external: dict[tuple[str, str, str, str], tuple[str, ...]] = {}
        ...
        for expected, row in enumerate(rows, 1):
            ...
            if scope is None:
                scope = current_scope; order_id = event["order_id"]
            ...
            if event["event_type"] == "ORDER_PLANNED":
                ...
                if new_cumulative <= cumulative or new_cumulative > ordered:
                    raise OrderAuthorityError("FILL_PROGRESSION_CONFLICT")
                if event["event_type"] == "ORDER_PARTIALLY_FILLED" and not (Decimal("0") < new_cumulative < ordered):
                    raise OrderAuthorityError("FILL_PROGRESSION_CONFLICT")
                if event["event_type"] == "ORDER_FILLED" and new_cumulative != ordered:
                    raise OrderAuthorityError("FILL_PROGRESSION_CONFLICT")
                seen_fills[fill_id] = economics; seen_external[external] = economics; cumulative = new_cumulative
            state, pre = self._next_state(state, pre, event)
        if state is None:
            raise OrderAuthorityError("INVALID_LIFECYCLE_TRANSITION")
        return state, pre

    def _next_state(self, state: str | None, pre: str | None, event: Mapping[str, Any]) -> tuple[str, str | None]:
        ...
        if rule == "RESTORE_PRE_REQUEST_STATE":
            if pre not in ("ACKNOWLEDGED", "PARTIALLY_FILLED"): raise OrderAuthorityError("INVALID_LIFECYCLE_TRANSITION")
            return pre, None
        if rule == "RESOLVE_TRUSTED_RECONCILIATION_FACT":
            target = _RECONCILE.get(event["safe_payload"]["trusted_fact_kind"])
            if target is None: raise OrderAuthorityError("TRUSTED_CONTEXT_FAILURE")
            return target, None
    ...
    def _validate_id(value: Any, prefix: str) -> None:
        match = _ID.fullmatch(value) if type(value) is str else None
        if match is None or match.group("prefix") != prefix: raise OrderAuthorityError("MALFORMED_REQUEST", "invalid identity")
...
def require_production_order_authority(authority: object) -> OrderAuthority:
    ...
    if type(authority) is not OrderAuthority:
        raise OrderAuthorityError("TRUSTED_CONTEXT_FAILURE", "exact production OrderAuthority required")
    return authority
...
class TestOrderAuthority(OrderAuthority):
    ...
    def submit_order(self, request: Mapping[str, Any], *, exchange_id: str = "test-exchange", recorded_at_utc: str = "2024-01-01T00:00:00Z", audit_event_id: str | None = None) -> Mapping[str, Any]:
        validated = validate_order_command(request)
        if request["operation_type"] != "SUBMIT_ORDER": raise OrderAuthorityError("MALFORMED_REQUEST")
        ...
        with self._lock:
            ...
            try:
                self._verify_integrity()
                old = self._db.execute("SELECT command_fingerprint,canonical_outcome FROM command_journal WHERE command_id=?", (request["command_id"],)).fetchone()
                if old:
                    if old[0] != fingerprint: raise OrderAuthorityError("IDEMPOTENCY_CONFLICT")
                    self._db.execute("COMMIT")
                    return _freeze(json.loads(old[1]))
                if audit_event_id is None: audit_event_id = "evt_" + request["command_id"][4:]
                event = self._planned_event(request, exchange_id, recorded_at_utc, audit_event_id)
                outcome = {"command_id":request["command_id"],"operation_type":"SUBMIT_ORDER","request_fingerprint_sha256":fingerprint,"outcome":"ACCEPTED","denial_code":None,"order_id":request["order_id"],"accepted_audit_event_id":audit_event_id,"recorded_at_utc":recorded_at_utc}
    ...
    def append_event(self, event: Mapping[str, Any]) -> str:
        ...
        with self._lock:
            ...
            try:
                self._verify_integrity()
                old = self._db.execute("SELECT event_fingerprint FROM event_journal WHERE audit_event_id=?", (event["audit_event_id"],)).fetchone()
                if old:
                    if old[0] != event["event_fingerprint_sha256"]: raise OrderAuthorityError("EVENT_IDENTITY_CONFLICT")
                    self._db.execute("COMMIT"); return "REPLAY_SUCCESS"
                rows = self._history_rows(event["order_id"]); current = len(rows)
                if event["event_type"] in ("ORDER_PARTIALLY_FILLED", "ORDER_FILLED"):
                    ...
                    if duplicate == "REPLAY_SUCCESS":
                        self._db.execute("COMMIT"); return duplicate
                    if duplicate == "FILL_IDENTITY_CONFLICT":
                        raise OrderAuthorityError(duplicate)
                if event["aggregate_version"] <= current: raise OrderAuthorityError("STALE_EVENT")
                if event["aggregate_version"] > current + 1: raise OrderAuthorityError("EVENT_VERSION_GAP")
                if not rows and event["event_type"] != "ORDER_PLANNED": raise OrderAuthorityError("INVALID_LIFECYCLE_TRANSITION")
                if rows:
                    first = json.loads(rows[0]["canonical_event"])
                    for field in CONTRACT["event_contract"]["envelope_schema"]["scope_fields"]:
                        if event[field] != first[field]: raise OrderAuthorityError("ORDER_SCOPE_MISMATCH")
                    state, pre = self._replay(rows); self._next_state(state, pre, event)
                    if event["event_type"] in ("ORDER_PARTIALLY_FILLED", "ORDER_FILLED"):
                        ...
                        for row in rows:
                            ...
                            if prior["event_type"] in ("ORDER_PARTIALLY_FILLED", "ORDER_FILLED"):
                                previous = Decimal(prior["safe_payload"]["cumulative_executed_quantity"])
                        cumulative = Decimal(event["safe_payload"]["cumulative_executed_quantity"])
                        if cumulative <= previous or cumulative > ordered:
                            raise OrderAuthorityError("FILL_PROGRESSION_CONFLICT")
                        if event["event_type"] == "ORDER_PARTIALLY_FILLED" and not (Decimal("0") < cumulative < ordered):
                            raise OrderAuthorityError("FILL_PROGRESSION_CONFLICT")
                        if event["event_type"] == "ORDER_FILLED" and cumulative != ordered:
                            raise OrderAuthorityError("FILL_PROGRESSION_CONFLICT")
                self._insert_event(event); self._db.execute("COMMIT"); return "ACCEPTED"
            except Exception:
                self._db.execute("ROLLBACK"); raise

    def _insert_command(self, request: Mapping[str, Any], fingerprint: str, scope: str, outcome: Mapping[str, Any]) -> None:
        head = self._db.execute("SELECT command_count,command_head,command_authenticated_head FROM authority_heads WHERE singleton=1").fetchone(); seq=head[0]+1
        values=[seq,request["command_id"],scope,_canonical(dict(request)),fingerprint,_canonical(dict(outcome)),head[1]]; digest=_digest(values)
        auth_fields=[seq,request["command_id"],scope,values[3],fingerprint,values[5],head[2]]
        mac=self._authenticated_commitment("COMMAND",auth_fields)
        self._db.execute("INSERT INTO command_journal VALUES(?,?,?,?,?,?,?,?,?,?)", (*values,digest,head[2],mac))
        self._db.execute("UPDATE authority_heads SET command_count=?,command_head=?,command_authenticated_head=? WHERE singleton=1",(seq,digest,mac))

    def _insert_event(self, event: Mapping[str, Any]) -> None:
        head=self._db.execute("SELECT event_count,event_head,event_authenticated_head FROM authority_heads WHERE singleton=1").fetchone(); seq=head[0]+1
        canonical=_canonical(dict(event)); values=[seq,event["audit_event_id"],event["order_id"],event["aggregate_version"],canonical,event["event_fingerprint_sha256"],head[1]]; digest=_digest(values)
        auth_fields=[seq,event["audit_event_id"],event["order_id"],event["aggregate_version"],canonical,event["event_fingerprint_sha256"],head[2]]
        mac=self._authenticated_commitment("EVENT",auth_fields)
        self._db.execute("INSERT INTO event_journal VALUES(?,?,?,?,?,?,?,?,?,?)",(*values,digest,head[2],mac))
        self._db.execute("UPDATE authority_heads SET event_count=?,event_head=?,event_authenticated_head=? WHERE singleton=1",(seq,digest,mac))

    @staticmethod
    def _planned_event(request: Mapping[str, Any], exchange_id: str, timestamp: str, audit_event_id: str) -> dict[str, Any]:
        event={"audit_event_id":audit_event_id,"event_type":"ORDER_PLANNED","order_id":request["order_id"],"aggregate_version":1,"correlation_id":request["correlation_id"],"causation_id":request["causation_id"],"command_id":request["command_id"],"environment":request["environment"],"workspace_id":request["workspace_id"],"portfolio_id":request["portfolio_id"],"exchange_account_id":request["exchange_account_id"],"exchange_id":exchange_id,"instrument_id":request["instrument_id"],"execution_route_id":request["execution_route_id"],"occurred_at_utc":timestamp,"safe_payload":{"side":request["side"],"order_type":request["order_type"],"quantity":request["quantity"]},"event_fingerprint_sha256":""}
        event["event_fingerprint_sha256"]=event_fingerprint_sha256(event); validate_order_event(event); return event
