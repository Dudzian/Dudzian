...
def _mapping_payload(value: object) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise SecretHandoffError("handoff carrier payload must be an object")
    return value
...
def validate_secret_handoff_snapshot(
    ...
    for carrier in (*snapshot.current_records, *snapshot.immutable_history):
        if carrier.representation_name in _SECRET_HANDOFF_REPRESENTATIONS:
            handoff_id = _mapping_payload(carrier.payload).get("handoff_id")
    ...
    for handoff_id in sorted(identities):
        ...
        if descriptor is None:
            raise SecretHandoffError("handoff family requires exactly one immutable descriptor")
...
def _validate_secret_handoff_view(
    ...
    *,
    ...
    descriptors = tuple(
        ...
        and _mapping_payload(item.payload).get("handoff_id") == handoff_id
    )
    ...
    if descriptors:
        ...
        if descriptor.handoff_id != handoff_id or carrier != handoff_descriptor_carrier(descriptor):
    ...
    history = tuple(_mapping_payload(item.payload) for item in history_records)
    current = None if current_record is None else _mapping_payload(current_record.payload)
    if descriptor is None:
    ...
    def _preflight(
        ...
        *,
        ...
        try:
            ...
            raise SecretHandoffError("protected handoff preflight recovery failed") from exc
        ...
        if fresh is None or fresh.metadata != recovered:
            raise SecretHandoffError("StateStore changed after protected handoff preflight")
