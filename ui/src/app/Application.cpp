...
void Application::notifyOverlayUsage(int activeCount, int allowedCount, bool reduceMotionActive) {
    ...
    if (m_lastOverlayState.has_value() && m_lastOverlayState.value().active == state.active &&
        m_lastOverlayState.value().allowed == state.allowed &&
        m_lastOverlayState.value().reduceMotion == state.reduceMotion) {
        return;
    }
...
void Application::reportReduceMotionTelemetry(bool enabled) {
    ...
    if (m_lastReduceMotionReported.has_value() && m_lastReduceMotionReported.value() == enabled) {
        return;
    }
    m_lastReduceMotionReported = enabled;
    const int overlayActive = m_lastOverlayState.has_value() ? m_lastOverlayState->active : 0;
    const int overlayAllowed = m_lastOverlayState.has_value() ? m_lastOverlayState->allowed : m_guard.maxOverlayCount;
    m_telemetry->setWindowCount(m_windowCount);
    m_telemetry->reportReduceMotion(m_guard, enabled, m_latestFpsSample, overlayActive, overlayAllowed);
}
