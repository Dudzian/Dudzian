...
void StrategyWorkbenchController::setReady(bool ready)
{
    if (m_ready == ready)
        return;
    m_ready = ready;
...
void StrategyWorkbenchController::invalidateWorkbenchData()
{
    ...
    clearList(m_catalogEngines);
    clearList(m_catalogDefinitions);
    clearMap(m_catalogMetadata);
    clearMap(m_catalogBlockedDefinitions);
    clearMap(m_catalogRegimeTemplates);

    if (catalogChanged)
        Q_EMIT catalogChanged();

    const bool validationChanged = !m_lastValidation.isEmpty();
    if (validationChanged) {
        m_lastValidation.clear();
        Q_EMIT lastValidationChanged();
    }

    const bool historyChanged = !m_presetHistory.isEmpty();
    if (historyChanged) {
