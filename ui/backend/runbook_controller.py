...
if importlib.util.find_spec("PySide6") is None:  # pragma: no cover - brak Qt
    QObject = Property = Signal = Slot = None  # type: ignore[assignment]
else:  # pragma: no cover - zależne od środowiska
...
def _load_metadata(directory: Path) -> dict[str, dict[str, Iterable[Any]]]:
    if not directory.exists():
        return {}

    if importlib.util.find_spec("yaml") is None:  # pragma: no cover - brak PyYAML
        raise RuntimeError("PyYAML required for runbook metadata")
    ...
    for path in sorted(directory.glob("*.yml")):
        ...
        actions = tuple(
            _parse_action_entry(identifier, entry, path) for entry in raw.get("automatic_actions", []) if entry
        )
        payload[identifier] = {
...
def _parse_action_entry(runbook_id: str, entry: Mapping[str, Any], origin: Path) -> RunbookAction | None:
...
if QObject is not None:
    ...
    class RunbookController(QObject):
        ...
        def _load_runbooks(self, directory: str | Path | None) -> MutableMapping[str, RunbookMetadata]:
            ...
            metadata_map = _load_metadata(metadata_dir)
        ...
        def _parse_action(self, runbook_id: str, entry: Mapping[str, Any], origin: Path) -> RunbookAction | None:
            return _parse_action_entry(runbook_id, entry, origin)
    ...
    class RunbookController:
        """Placeholder gdy brak PySide6."""

        def __init__(self, *_: object, **__: object) -> None:
            raise RuntimeError("PySide6 required for RunbookController")
...
__all__ = ["RunbookController", "RunbookAction", "RunbookMetadata", "_load_metadata"]
