...
private Q_SLOTS:
    ...
    void clearsLastErrorWhenPathsChange();
    void autoRefreshesWhenConfigPathMaterializesWithinNestedDirectories();
    void clearsStateWhenConfigurationChanges();
    void clearsStateWhenPythonExecutableCleared();
    void tracksReadyStateAcrossLifecycle();
};
...
void StrategyWorkbenchControllerTest::clearsLastErrorWhenPathsChange()
{
    QTemporaryDir dir;
    QVERIFY2(dir.isValid(), "Temporary directory not available");
    ...
    const QString missingConfig = dir.filePath(QStringLiteral("missing-core.yaml"));
    controller.setConfigPath(missingConfig);
    QVERIFY(!controller.refreshCatalog());
    QVERIFY(!controller.lastError().isEmpty());
    ...
    const QString missingScript = dir.filePath(QStringLiteral("missing-bridge.py"));
    controller.setScriptPath(missingScript);
    QVERIFY(!controller.refreshCatalog());
    QVERIFY(!controller.lastError().isEmpty());
...
void StrategyWorkbenchControllerTest::clearsStateWhenConfigurationChanges()
{
    QTemporaryDir dir;
    QVERIFY2(dir.isValid(), "Temporary directory not available");
    ...
    QVERIFY(controller.refreshCatalog());
    QVERIFY(!controller.catalogDefinitions().isEmpty());

    const QVariantMap validationResult = controller.validatePreset(QVariantMap{{QStringLiteral("preset"), QVariantMap{}}});
    QVERIFY(!validationResult.isEmpty());
    controller.savePresetSnapshot(validationResult);
    QCOMPARE(controller.presetHistory().size(), 1);

    QSignalSpy catalogSpy(&controller, &StrategyWorkbenchController::catalogChanged);
    QSignalSpy validationSpy(&controller, &StrategyWorkbenchController::lastValidationChanged);
    QSignalSpy historySpy(&controller, &StrategyWorkbenchController::presetHistoryChanged);

    const QString missingConfigPath = dir.filePath(QStringLiteral("missing/core.yaml"));
    controller.setConfigPath(missingConfigPath);

    QVERIFY(controller.catalogEngines().isEmpty());
    QVERIFY(controller.catalogDefinitions().isEmpty());
    QVERIFY(controller.catalogMetadata().isEmpty());
    QVERIFY(controller.catalogBlockedDefinitions().isEmpty());
    QVERIFY(controller.catalogRegimeTemplates().isEmpty());
    QVERIFY(controller.lastValidation().isEmpty());
    QVERIFY(controller.presetHistory().isEmpty());
...
void StrategyWorkbenchControllerTest::clearsStateWhenPythonExecutableCleared()
{
    QTemporaryDir dir;
    QVERIFY2(dir.isValid(), "Temporary directory not available");
    ...
    controller.setPythonExecutable(QString());
    ...
    QVERIFY(!controller.refreshCatalog());
    QVERIFY(controller.lastError().contains(QStringLiteral("Nie ustawiono interpretera Pythona")));
}

void StrategyWorkbenchControllerTest::tracksReadyStateAcrossLifecycle()
{
    QTemporaryDir dir;
    QVERIFY2(dir.isValid(), "Temporary directory not available");
    ...
    QSignalSpy readySpy(&controller, &StrategyWorkbenchController::readyChanged);
    ...
    const int baseline = readySpy.count();

    controller.setPythonExecutable(pythonWrapper);

    QVERIFY(readySpy.wait(4000));
    QVERIFY(controller.ready());
    QVERIFY(readySpy.count() >= baseline + 1);
}
