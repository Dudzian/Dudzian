...
class MockAppController : public QObject {
    ...
    Q_PROPERTY(QVariantMap executionDiagnostics READ executionDiagnostics NOTIFY executionDiagnosticsChanged)
...
public:
    ...
    QVariantMap executionDiagnostics() const { return m_executionDiagnostics; }
    void setExecutionDiagnostics(const QVariantMap& diagnostics)
    {
        if (m_executionDiagnostics == diagnostics)
            return;
        m_executionDiagnostics = diagnostics;
        Q_EMIT executionDiagnosticsChanged();
    }
    ...
    void setAutomationRules(const QVariantList& rules)
    {
        if (m_automationRules == rules)
            return;
        m_automationRules = rules;
        Q_EMIT automationRulesChanged();
    }
    ...
    Q_INVOKABLE QVariantMap executionDiagnosticsSnapshot() const { return m_executionDiagnostics; }
...
Q_SIGNALS:
    ...
    void executionDiagnosticsChanged();
...
private:
    ...
    QVariantMap m_executionDiagnostics;
...
public:
    ...
    Q_INVOKABLE QVariantList scenarioTestsSnapshot() const { return m_scenarioTests; }
    Q_INVOKABLE QVariantList stressTestResultsSnapshot() const { return m_scenarioTests; }
    Q_INVOKABLE QVariantList automationRulesSnapshot() const { return m_automationRules; }
...
private:
    ...
    QVariantMap  m_marketSentiment;
    QVariantList m_newsHeadlines;
    QVariantList m_automationRules;
    QVariantList m_scenarioTests;
...
void StrategyWorkbenchE2ETest::shouldExposeLiveDataFromControllers()
{
    ...
    QVariantList automationRules;
    QVariantMap firstRule;
    firstRule.insert(QStringLiteral("id"), QStringLiteral("AUTO-1"));
    firstRule.insert(QStringLiteral("name"), QStringLiteral("Stop-loss guard"));
    firstRule.insert(QStringLiteral("description"), QStringLiteral("Close positions when drawdown exceeds 6%."));
    firstRule.insert(QStringLiteral("type"), QStringLiteral("risk"));
    firstRule.insert(QStringLiteral("trigger"), QStringLiteral("Drawdown > 6%"));
    firstRule.insert(QStringLiteral("action"), QStringLiteral("Close positions"));
    firstRule.insert(QStringLiteral("enabled"), true);
    firstRule.insert(QStringLiteral("successRate"), 0.93);
    firstRule.insert(QStringLiteral("errorCount"), 0);
    firstRule.insert(QStringLiteral("lastTriggeredAt"), QStringLiteral("2024-05-17T20:00:00Z"));
    firstRule.insert(QStringLiteral("critical"), true);
    automationRules.append(firstRule);

    QVariantMap secondRule;
    secondRule.insert(QStringLiteral("id"), QStringLiteral("AUTO-2"));
    secondRule.insert(QStringLiteral("name"), QStringLiteral("Rebalance drift"));
    secondRule.insert(QStringLiteral("description"), QStringLiteral("Rebalance allocations when deviation exceeds 5%."));
    secondRule.insert(QStringLiteral("type"), QStringLiteral("portfolio"));
    secondRule.insert(QStringLiteral("trigger"), QStringLiteral("Weight drift > 5%"));
    secondRule.insert(QStringLiteral("action"), QStringLiteral("Rebalance segments"));
    secondRule.insert(QStringLiteral("enabled"), true);
    secondRule.insert(QStringLiteral("successRate"), 0.88);
    secondRule.insert(QStringLiteral("errorCount"), 1);
    secondRule.insert(QStringLiteral("lastTriggeredAt"), QStringLiteral("2024-05-18T07:45:00Z"));
    automationRules.append(secondRule);

    QVariantMap thirdRule;
    thirdRule.insert(QStringLiteral("id"), QStringLiteral("AUTO-3"));
    thirdRule.insert(QStringLiteral("name"), QStringLiteral("Maintenance pause"));
    thirdRule.insert(QStringLiteral("description"), QStringLiteral("Disable automation when connectivity drops."));
    thirdRule.insert(QStringLiteral("type"), QStringLiteral("connectivity"));
    thirdRule.insert(QStringLiteral("trigger"), QStringLiteral("Connectivity loss"));
    thirdRule.insert(QStringLiteral("action"), QStringLiteral("Pause scheduler"));
    thirdRule.insert(QStringLiteral("enabled"), false);
    thirdRule.insert(QStringLiteral("successRate"), 0.97);
    thirdRule.insert(QStringLiteral("errorCount"), 0);
    thirdRule.insert(QStringLiteral("lastTriggeredAt"), QStringLiteral("2024-05-10T03:00:00Z"));
    thirdRule.insert(QStringLiteral("critical"), true);
    automationRules.append(thirdRule);

    strategyController.setAutomationRules(automationRules);
    ...
    QVariantMap executionDiagnostics;
    executionDiagnostics.insert(QStringLiteral("provider"), QStringLiteral("OMS Simulator"));
    executionDiagnostics.insert(QStringLiteral("avgLatencyMs"), 42.0);
    executionDiagnostics.insert(QStringLiteral("p95LatencyMs"), 72.0);
    executionDiagnostics.insert(QStringLiteral("maxLatencyMs"), 128.0);
    executionDiagnostics.insert(QStringLiteral("fillRate"), 0.902);
    executionDiagnostics.insert(QStringLiteral("rejectRate"), 0.033);
    executionDiagnostics.insert(QStringLiteral("slippageBps"), 2.9);
    executionDiagnostics.insert(QStringLiteral("lastUpdated"), QStringLiteral("2024-05-18T09:00:00Z"));
    executionDiagnostics.insert(QStringLiteral("notes"), QStringLiteral("Stable performance during morning session."));
    QVariantList incidents;
    QVariantMap firstIncident;
    firstIncident.insert(QStringLiteral("type"), QStringLiteral("reject"));
    firstIncident.insert(QStringLiteral("message"), QStringLiteral("Order rejected on Deribit - limit exceeded"));
    firstIncident.insert(QStringLiteral("timestamp"), QStringLiteral("2024-05-17T21:35:00Z"));
    firstIncident.insert(QStringLiteral("resolved"), true);
    incidents.append(firstIncident);
    QVariantMap secondIncident;
    secondIncident.insert(QStringLiteral("type"), QStringLiteral("latency"));
    secondIncident.insert(QStringLiteral("message"), QStringLiteral("Latency spike above 120 ms"));
    secondIncident.insert(QStringLiteral("timestamp"), QStringLiteral("2024-05-17T17:04:00Z"));
    secondIncident.insert(QStringLiteral("resolved"), true);
    incidents.append(secondIncident);
    executionDiagnostics.insert(QStringLiteral("recentIncidents"), incidents);
    appController.setExecutionDiagnostics(executionDiagnostics);
    ...
    const QVariantList automationLive = viewModel->property("automationRules").toList();
    QCOMPARE(automationLive.size(), 3);
    QCOMPARE(automationLive.first().toMap().value(QStringLiteral("enabled")).toBool(), true);
    const int disabledAutomationRules = [&automationLive]() {
        int count = 0;
        for (const QVariant& ruleVariant : automationLive) {
            const QVariantMap map = ruleVariant.toMap();
    ...
    const QVariantMap executionLive = viewModel->property("executionDiagnostics").toMap();
    QCOMPARE(executionLive.value(QStringLiteral("provider")).toString(), QStringLiteral("OMS Simulator"));
    QVERIFY(executionLive.value(QStringLiteral("fillRate")).toDouble() > 0.85);
    QVERIFY(!executionLive.value(QStringLiteral("recentIncidents")).toList().isEmpty());
    ...
    QCOMPARE(compliance.value(QStringLiteral("disabledAutomationRules")).toInt(), disabledAutomationRules);
    ...
    const double automationRulesPenalty = qMin(static_cast<double>(disabledAutomationRules) * 0.02, 0.1);
    const double scenarioPenalty = qMin(static_cast<double>(scenarioFailures) * 0.05, 0.2);
    const double expectedComplianceScore = qBound(0.0, baseline - exposurePenalty - breachPenalty - alertPenalty - automationPenalty - automationRulesPenalty - scenarioPenalty, 1.0);
...
void StrategyWorkbenchE2ETest::shouldSwitchDemoMode()
{
    ...
    const QVariantList demoAutomation = viewModel->property("automationRules").toList();
    QVERIFY(demoAutomation.size() >= 3);
    const int demoDisabledRules = [&demoAutomation]() {
        int count = 0;
        for (const QVariant& ruleVariant : demoAutomation) {
            const QVariantMap map = ruleVariant.toMap();
            if (!map.value(QStringLiteral("enabled")).toBool())
                ++count;
        }
    ...
    QVariantMap demoExecution = viewModel->property("executionDiagnostics").toMap();
    QCOMPARE(demoExecution.value(QStringLiteral("provider")).toString(), QStringLiteral("OMS Simulator"));
    QVERIFY(demoExecution.value(QStringLiteral("fillRate")).toDouble() > 0.9);
    const QString demoInitialDiagnosticsTimestamp = demoExecution.value(QStringLiteral("lastUpdated")).toString();
    ...
    QCOMPARE(demoCompliance.value(QStringLiteral("disabledAutomationRules")).toInt(), demoDisabledRules);
    ...
    const double demoAutomationRulesPenalty = qMin(static_cast<double>(demoDisabledRules) * 0.02, 0.1);
    const double demoScenarioPenalty = qMin(static_cast<double>(demoScenarioFailures) * 0.05, 0.2);
    const double demoExpectedScore = qBound(0.0, demoBaseline - demoExposurePenalty - demoBreachPenalty - demoAlertPenalty - demoAutomationPenalty - demoAutomationRulesPenalty - demoScenarioPenalty, 1.0);
    ...
    demoExecution = viewModel->property("executionDiagnostics").toMap();
    QVERIFY(demoExecution.value(QStringLiteral("lastUpdated")).toString() != demoInitialDiagnosticsTimestamp);
    QVERIFY(demoExecution.value(QStringLiteral("recentIncidents")).toList().first().toMap().value(QStringLiteral("type")).toString().compare(QStringLiteral("refresh"), Qt::CaseInsensitive) == 0);
