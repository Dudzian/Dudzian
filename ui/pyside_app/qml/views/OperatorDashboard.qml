...
Components.StyledScrollView {
    ...
    ColumnLayout {
        ...
        GridLayout {
            ...
            Components.PreviewCard { objectName: "operatorDashboardBestScannerOpportunity"; designSystem: root.designSystem; title: qsTr("Best scanner opportunity"); description: qsTr("%1 • candidates %2 • safe preview scanner").arg(previewState.scannerBestOpportunity).arg(previewState.scannerCandidateCount); Layout.fillWidth: true }
            ...
            Components.PreviewCard { objectName: "operatorDashboardFeed"; designSystem: root.designSystem; title: qsTr("Last AI/governor decision"); description: previewState.lastGovernorDecision; Layout.fillWidth: true; Components.IconButton { designSystem: root.designSystem; text: qsTr("Wyjaśnij ostatnią decyzję"); helpText: previewState.tooltipText("Explain decision"); onClicked: previewState.openDecisionExplainDrawer(previewState.decisionPreviewRows.length > 0 ? previewState.decisionPreviewRows[0] : null) } }
