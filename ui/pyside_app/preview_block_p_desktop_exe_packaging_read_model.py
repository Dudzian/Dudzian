...
def _output_integrity(payload: dict[str, Any]) -> bool:
    ...
    if not all(
        all(type(row[key]) is str and bool(row[key]) for row in payload[name])
        and len([row[key] for row in payload[name]]) == len({row[key] for row in payload[name]})
        for name, key in families
    ):
        return False
    requirements = payload["requirement_contract_read_model_rows"]
    blockers = payload["blocker_read_model_rows"]
    scopes = payload["scope_contract_read_model_rows"]
    evidence = payload["evidence_read_model_rows"]
    acceptance = payload["acceptance_rule_read_model_rows"]
    ...
    contract_ids = {row["contract_requirement_id"] for row in requirements}
    return (
        len([row["contract_requirement_id"] for row in requirements])
        == len({row["contract_requirement_id"] for row in requirements})
        and all(
            type(row["contract_requirement_id"]) is str and bool(row["contract_requirement_id"])
            for row in requirements
        )
        and len([row["contract_clause_id"] for row in blockers])
        == len({row["contract_clause_id"] for row in blockers})
        and all(
            type(row["contract_clause_id"]) is str and bool(row["contract_clause_id"])
            for row in blockers
        )
        and _links_valid(scopes, "source_unresolved_blocker_ids", blocker_ids)
        and _links_valid(scopes, "contract_clause_ids", clause_ids)
        and _links_valid(scopes, "required_evidence_ids", evidence_ids)
        and _links_valid(requirements, "source_unresolved_condition_ids", blocker_ids)
        and _links_valid(requirements, "contract_clause_ids", clause_ids)
        and _links_valid(requirements, "required_evidence_ids", evidence_ids)
        and _links_valid(blockers, "source_affected_scope_ids", scope_ids)
        and _links_valid(blockers, "contract_requirement_ids", contract_ids)
        and _links_valid(blockers, "required_evidence_ids", evidence_ids)
        and all(
            type(row["blocker_id"]) is str and row["blocker_id"] in blocker_ids for row in evidence
        )
        and _links_valid(acceptance, "required_blocker_ids", blocker_ids)
        and _links_valid(acceptance, "required_evidence_ids", evidence_ids)
    )


def _source_referential_integrity_valid(validity: dict[str, bool]) -> bool:
    return all(
        validity[key]
        for key in (
...
def build_preview_block_p_desktop_exe_packaging_read_model() -> dict[str, Any]:
    ...
    accepted = (
        ...
        and _source_referential_integrity_valid(valid)
    )
    domains = [
    ...
    preservation = {
        ...
        "referential_integrity_preserved": accepted,
    ...
    summary = {
        ...
        "contract_definitions_complete": accepted,
        "contract_satisfied": False,
        "build_ready": False,
        "packaging_authorized": False,
        "build_authorized": False,
    ...
    overview = {
        ...
        if valid["packaging_contract_summary"]
        else "",
        ...
        "next_required_source_only_step": NEXT_STEP if valid["packaging_contract_summary"] else "",
    }
    decision = {
    ...
    evidence = {
        ...
        **{
        ...
        "source_referential_integrity_valid": _source_referential_integrity_valid(valid),
