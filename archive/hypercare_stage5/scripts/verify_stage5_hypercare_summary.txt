...
from bot_core.runtime.stage5_hypercare import (  # noqa: E402
...
def _load_key(args: argparse.Namespace) -> bytes | None:
    ...
    provided = [name for name in (inline, file_path, env_name) if name]
    if len(provided) > 1:
        raise ValueError("Podaj klucz HMAC jako wartość, plik lub zmienną środowiskową (jedną opcję)")
    ...
    if inline:
        ...
        if os.name != "nt":  # pragma: no cover - specyficzne dla systemów unixowych
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Weryfikuje raport hypercare Stage5 wraz z podpisem HMAC.",
    )
    parser.add_argument("summary", help="Ścieżka do raportu Stage5 hypercare (JSON)")
    parser.add_argument(
        "--signature",
    ...
    parser.add_argument("--hmac-key-env", help="Zmienna środowiskowa z kluczem HMAC")
    return parser


def _serialize_result(result: Stage5HypercareVerificationResult) -> dict[str, object]:
    return {
