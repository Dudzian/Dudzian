...
def test_auto_trader_lifecycle_with_guardrails_and_recalibrations() -> None:
    ...
    audit_log = trader._decision_audit_log
    base_audit = datetime(2024, 1, 4, tzinfo=timezone.utc)
    audit_log.record(
        "strategy_selected",
        "BTCUSDT",
        mode="paper",
        payload={"decision_id": "AUD-1", "strategy": "trend_following"},
        timestamp=base_audit,
    )
    audit_log.record(
        "order_submitted",
        "BTCUSDT",
        mode="paper",
        payload={"decision_id": "AUD-1", "order_id": "ORD-1"},
        metadata={"source": "auto_trader"},
        timestamp=base_audit + timedelta(seconds=1),
    )
    audit_log.record(
        "order_filled",
        "BTCUSDT",
        mode="paper",
        payload={"decision_id": "AUD-2"},
        risk_snapshot={"equity": 100_500.0},
        timestamp=base_audit + timedelta(seconds=2),
    )
    ...
    audit_snapshot = snapshot["decision_audit"]
    assert audit_snapshot["summary"]["count"] >= 3
    assert audit_snapshot["summary"]["decision_ids"]["AUD-1"] >= 2
    assert audit_snapshot["summary"]["with_risk_snapshot"] >= 1
    assert any(entry.get("decision_id") == "AUD-2" for entry in audit_snapshot["recent"])
...
def test_auto_trader_guardrail_timeline_for_24_7_regime_rotation() -> None:
    ...
    audit_log = trader._decision_audit_log
    audit_base = base_timestamp + timedelta(days=2)
    audit_log.record(
        "strategy_selected",
        "BTCUSDT",
        mode="live",
        payload={"decision_id": "AUD-LIVE-1", "strategy": "mean_reversion"},
        timestamp=audit_base,
    )
    audit_log.record(
        "order_submitted",
        "BTCUSDT",
        mode="live",
        payload={"decision_id": "AUD-LIVE-1"},
        timestamp=audit_base + timedelta(minutes=5),
    )
    ...
    audit_snapshot = snapshot["decision_audit"]
    assert audit_snapshot["summary"]["count"] >= 2
    assert audit_snapshot["summary"]["decision_ids"]["AUD-LIVE-1"] == 2
    assert len(audit_snapshot["recent"]) >= 2
