...
def test_filter_audit_reports_by_tags_collects_nested_metadata() -> None:
    reports = (
        {"strategy": {"metadata": {"tags": ["Primary", "Alpha"]}}, "category": "strategy"},
        {"context": {"strategy": {"metadata": {"tags": ["Legacy"]}}}, "category": "drop-legacy"},
        {"dataset": {"metadata": {"tags": "Secondary"}}, "category": "dataset"},
        {"context": {"dataset": {"metadata": {"tags": ["Aux"]}}}, "category": "context-dataset"},
        {"context": {"metadata": {"tags": ["Global"]}}, "category": "context"},
    )

    filtered_include = filter_audit_reports_by_tags(reports, include=("alpha", "secondary"))
    categories = {entry["category"] for entry in filtered_include}
    assert categories == {"strategy", "dataset"}

    filtered_context = filter_audit_reports_by_tags(reports, include=("global",))
    assert filtered_context == ({"context": {"metadata": {"tags": ["Global"]}}, "category": "context"},)

    filtered_exclude = filter_audit_reports_by_tags(reports, exclude=("legacy", "aux"))
    categories = {entry["category"] for entry in filtered_exclude}
    assert "drop-legacy" not in categories
    assert "context-dataset" not in categories
...
def test_normalize_report_engine_handles_inputs() -> None:
    assert normalize_report_engine(" Trend_D1 ") == "trend_d1"
    assert normalize_report_engine("LEGACY") == "legacy"
    assert normalize_report_engine(" ") is None
    assert normalize_report_engine(None) is None


def test_normalize_report_issue_code_handles_inputs() -> None:
    assert normalize_report_issue_code("  dq_missing_feature  ") == "dq_missing_feature"
    assert normalize_report_issue_code("ALERT-001") == "alert-001"
    assert normalize_report_issue_code(42) == "42"
    assert normalize_report_issue_code(" ") is None
    assert normalize_report_issue_code(None) is None
...
def test_filter_audit_reports_by_issue_code() -> None:
    reports = (
        {"issues": ["DQ_MISSING_FEATURE", "secondary"], "category": "keep-include"},
        {
            "summary": {"issues": [{"code": "drift_threshold_exceeded"}]},
            "category": "keep-summary",
        },
        {
            "context": {"metadata": {"issues": [{"identifier": "legacy_issue"}]}},
            "category": "drop-exclude",
        },
        {"category": "no-issues"},
    )

    filtered_include = filter_audit_reports_by_issue_code(
        reports,
        include=("dq_missing_feature", "drift_threshold_exceeded"),
    )
    categories = {entry.get("category") for entry in filtered_include}
    assert categories == {"keep-include", "keep-summary"}

    filtered_exclude = filter_audit_reports_by_issue_code(
        reports, exclude=("legacy_issue",)
    )
    categories = {entry.get("category") for entry in filtered_exclude}
    assert "drop-exclude" not in categories
    assert categories == {"keep-include", "keep-summary", "no-issues"}

    filtered_both = filter_audit_reports_by_issue_code(
        reports,
        include=("dq_missing_feature",),
        exclude=("dq_missing_feature",),
    )
    assert filtered_both == ()
...
def test_filter_audit_reports_by_engine() -> None:
    reports = (
        {"engine": "Trend_D1", "category": "keep-direct"},
        {"context": {"strategy": {"engine": "legacy"}}, "category": "drop-context"},
        {"strategy": {"engine": "trend_d1"}, "category": "keep-strategy"},
        {"strategy": {"metadata": {"engine": "legacy"}}, "category": "drop-metadata"},
        {"dataset": {"metadata": {"engine": "trend_d1"}}, "category": "keep-dataset"},
        {"metadata": {"engine": "trend_d1"}, "category": "keep-metadata"},
        {"category": "missing"},
    )

    filtered_include = filter_audit_reports_by_engine(
        reports, include=("trend_d1",)
    )
    categories = {entry.get("category") for entry in filtered_include}
    assert categories == {"keep-direct", "keep-strategy", "keep-dataset", "keep-metadata"}

    filtered_exclude = filter_audit_reports_by_engine(
        reports, exclude=("legacy",)
    )
    categories = {entry.get("category") for entry in filtered_exclude}
    assert "drop-context" not in categories
    ...
    filtered_both = filter_audit_reports_by_engine(
        reports, include=("trend_d1",), exclude=("trend_d1",)
    )
    assert filtered_both == ()
