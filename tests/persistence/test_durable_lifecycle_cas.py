...
def _migration_registry() -> MigrationRegistry:
    ...
    return MigrationRegistry(((definition, lambda snapshot: snapshot),))
...
def test_secret_descriptor_conflict_and_illegal_outcome_fail_closed(tmp_path: Path) -> None:
    with SQLiteStateStore(tmp_path / "state.db") as store:
        boundary = Boundary(record("UNINITIALIZED"))
        _initialize(store, boundary)
        coordinator = DurableSecretHandoffLifecycleCoordinator(store, _protected(store, boundary))
...
def test_post_commit_lost_ack_reopens_as_zero_mutation_duplicate(tmp_path: Path) -> None:
    path = tmp_path / "state.db"
    boundary = Boundary(record("UNINITIALIZED"))
    with SQLiteStateStore(path) as store:
        _initialize(store, boundary)
...
def test_duplicate_fails_closed_when_external_current_mismatches(tmp_path: Path) -> None:
    with SQLiteStateStore(tmp_path / "state.db") as store:
        boundary = Boundary(record("UNINITIALIZED"))
        _initialize(store, boundary)
        coordinator = DurableMigrationLifecycleCoordinator(
...
def test_prepare_rejects_verified_target_schema_before_external_prepare(tmp_path: Path) -> None:
    with SQLiteStateStore(tmp_path / "state.db") as store:
        boundary = Boundary(record("UNINITIALIZED"))
