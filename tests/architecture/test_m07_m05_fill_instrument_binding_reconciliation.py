...
def validate(machine: dict[str, object]) -> None:
    ...
    assert (
        machine["current_m07_fill_binding"]["schema_field_count"] == len(fill["fact_fields"]) == 19
    )
    assert (
        machine["current_m07_fill_binding"]["stale_required_matches"]
        == fill["instrument_binding"]["required_matches"]
    )
    assert machine["current_m07_fill_binding"]["valid_direct_instrument_matches"] == [
        "workspace_id"
    ]
    assert not {"exchange_id", "environment"} & fields
    assert machine["current_m07_fill_binding"]["forbidden_direct_instrument_matches"] == [
        "exchange_id",
        "environment",
    ]
    assert instrument["identity_dimensions"] == [
        "source_exchange_id",
        "market_type",
        "venue_symbol",
    ]
    ...
    assert (
        machine["current_m05_instrument_identity"][
            "exchange_id_equals_source_exchange_id_assumption"
        ]
        == "FORBIDDEN"
    )
    ...
    assert (
        machine["workspace_catalog_projection_dependency"]["runtime_authority"] == "NOT_AVAILABLE"
    )
    ...
    assert (
        ascat["runtime_writer"]
        == dependency["contract_declared_runtime_writer"]
        == "NOT_IMPLEMENTED"
    )
    assert (
        dependency["contract_metadata_interpretation"]
        == "HISTORICAL_ONLY_NOT_CURRENT_PRODUCTION_AVAILABILITY_PROOF"
    )
    assert dependency["current_production_catalog_runtime_authority"] == "AVAILABLE"
    assert dependency["authority"] == "ACCEPTED_AVAILABLE"
    assert (
        dependency["authority_symbol"]
        == "bot_core.instruments.catalog_runtime_acceptance.CatalogRuntimeAcceptanceAuthority"
    )
    ...
    assert (
        m06["canonical_source_catalog_projection_chain"]["runtime_acceptance"] == "NOT_IMPLEMENTED"
    )
...
def test_current_production_catalog_authority_boundary_and_dependencies_exist() -> None:
    ...
    assert list(
        inspect.signature(CatalogRuntimeAcceptanceAuthority.fetch_catalog_once).parameters
...
@pytest.mark.parametrize(
    "mutation",
    [
        lambda d: d["current_m07_fill_binding"].update(
            valid_direct_instrument_matches=["workspace_id", "exchange_id"]
        ),
        lambda d: d["current_m07_fill_binding"].update(
            valid_direct_instrument_matches=["workspace_id", "environment"]
        ),
        lambda d: d["current_m05_instrument_identity"].update(
            exchange_id_equals_source_exchange_id_assumption="ALLOWED"
        ),
        ...
        lambda d: d["accepted_source_catalog_dependency"].update(
            need_to_implement_new_catalog_runtime_authority=True
        ),
        lambda d: d["catalog_authority_invariant"].update(
            competing_catalog_runtime_authority_forbidden=False
        ),
        lambda d: d["catalog_authority_invariant"].update(
            catalog_admission_receipt_authority_required=False
        ),
