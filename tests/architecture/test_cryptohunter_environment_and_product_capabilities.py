...
DOC = (
    ROOT
    / "docs/architecture/cryptohunter_product_architecture/environment_and_product_capabilities.json"
)
MD = (
    ROOT
    / "docs/architecture/cryptohunter_product_architecture/environment_and_product_capabilities.md"
)
ARCH = ROOT / "docs/architecture/cryptohunter_product_architecture/README.md"
VOCAB = (
    ROOT / "docs/architecture/cryptohunter_product_architecture/canonical_domain_vocabulary.json"
)
PROC = (
...
REQ_GATES = [
    ...
    "IPC_COMMAND_VALIDATION",
    "PRODUCT_CAPABILITY_POLICY",
    "LIVE_ACCESS_GRANT_POLICY",
    "CREDENTIAL_PROFILE_RESOLUTION",
    "SECRET_READ_OR_DECRYPT",
    "ENDPOINT_RESOLUTION",
    "ADAPTER_FACTORY",
    "PRIVATE_CONNECTION_CREATION",
    "RUNTIME_ENVIRONMENT_ACTIVATION",
    "STRATEGY_ROUTE_BINDING",
    "ORDER_INTENT_ACCEPTANCE",
    "EXECUTION_COMMAND_SUBMISSION",
    "RECONNECT",
    "RECOVERY",
    "RECONCILIATION_WITH_EXTERNAL_VENUE",
]
GATE_AUDIT = {
...
STAGE_BY_RESULT = {
    "STRUCTURE_ACCEPTED": "DOCUMENT_STRUCTURE_VALIDATION",
    "STRUCTURE_REJECTED": "DOCUMENT_STRUCTURE_VALIDATION",
    "HASHES_ACCEPTED": "CANONICAL_HASH_VALIDATION",
    "HASHES_REJECTED": "CANONICAL_HASH_VALIDATION",
    "HASH_VALIDATION_NOT_REACHED": "CANONICAL_HASH_VALIDATION",
    "REGISTRY_ACCEPTED": "REGISTRY_POLICY_VALIDATION",
    "REGISTRY_REJECTED": "REGISTRY_POLICY_VALIDATION",
    "REGISTRY_VALIDATION_NOT_REACHED": "REGISTRY_POLICY_VALIDATION",
    "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED": "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION",
    "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED": "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION",
    "CRYPTOGRAPHIC_VERIFICATION_NOT_IMPLEMENTED_IN_M0_4": "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION",
    "CRYPTOGRAPHIC_VERIFICATION_NOT_REACHED": "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION",
    "EXPIRY_REVOCATION_ACCEPTED": "EXPIRY_AND_REVOCATION_VALIDATION",
    "EXPIRY_REVOCATION_REJECTED": "EXPIRY_AND_REVOCATION_VALIDATION",
    "EXPIRY_REVOCATION_NOT_REACHED": "EXPIRY_AND_REVOCATION_VALIDATION",
    "SNAPSHOT_CREATED": "VALIDATED_SNAPSHOT_CREATION",
    "SNAPSHOT_NOT_CREATED": "VALIDATED_SNAPSHOT_CREATION",
}
SPECIFIC = {
    ...
    "PERSISTED_STATE_RESTORE": [
        "persisted_active_environment",
        "persisted_last_successful_environment",
        "persisted_capabilities_identity",
    ],
    "IPC_COMMAND_VALIDATION": ["command_type", "client_role", "authorization_context"],
    "PRODUCT_CAPABILITY_POLICY": [
        "capability_trust_state",
        "capability_set",
        "environment_capability",
    ],
    "LIVE_ACCESS_GRANT_POLICY": [
        "live_access_grant_id_or_absence",
        "live_access_grant_validation_state",
    ],
    "CREDENTIAL_PROFILE_RESOLUTION": [
    ...
    "SECRET_READ_OR_DECRYPT": [
        "credential_profile_id",
        "credential_scope",
        "secret_access_purpose",
    ],
    "ENDPOINT_RESOLUTION": [
        "requested_endpoint_class",
        "exchange_capabilities",
        "exchange_account_id",
    ],
    "ADAPTER_FACTORY": [
        "requested_adapter_class",
        "exchange_capabilities",
        "requested_endpoint_class",
    ],
    "PRIVATE_CONNECTION_CREATION": [
        "exchange_account_id",
        "credential_profile_id",
        "resolved_endpoint_class",
    ],
    "RUNTIME_ENVIRONMENT_ACTIVATION": [
        "environment_readiness_state",
        "kill_switch_state",
        "previous_active_environment",
    ],
    ...
    "RECONNECT": [
        "previous_connection_environment",
        "exchange_account_id",
        "credential_scope",
        "endpoint_class",
    ],
...
def reject_numbers_in_signed_payload(value: object) -> None:
    ...
    if isinstance(value, list):
        for item in value:
            reject_numbers_in_signed_payload(item)
        return
    if isinstance(value, dict):
...
def capability_set_hash(capability_set: list[str]) -> str:
    return sha_bytes(
        jcs_current_schema_canonicalize(sorted(capability_set, key=utf16_code_unit_sort_key))
    )
...
def build_envelope(payload: dict, algorithm: str = "alg", key: str = "key") -> dict:
    ...
    header = {
        "signature_schema_version": SIG_VERSION,
    ...
    structural_signature_fixture = base64url_no_padding(
        hashlib.sha256(jcs_current_schema_canonicalize(signature_input)).digest()
    )
    return {
        "capability_payload": payload,
        "signature_header": header,
        "signed_payload_hash": signed_payload_hash,
        "signature": structural_signature_fixture,
    }
...
def cryptographic_signature_verification(envelope: dict, registry_result: Mapping) -> dict:
    try:
        if (
            not isinstance(registry_result, Mapping)
            or registry_result.get("stage_result") != "REGISTRY_ACCEPTED"
        ):
            return _stage_result("CRYPTOGRAPHIC_VERIFICATION_NOT_REACHED")
        context = _context_from_envelope(envelope)
        if not _complete_clean_evidence(
            registry_result, "REGISTRY_POLICY_VALIDATION", "REGISTRY_ACCEPTED", context
        ):
            return _stage_result(
                "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED",
                "PREVIOUS_STAGE_EVIDENCE_INVALID",
                "INVALID_SIGNATURE",
                "PRODUCT_CAPABILITIES_INVALID_SIGNATURE",
                context,
                registry_result.get("stage_evidence_id")
        ...
        return _stage_result(
            ...
            context=context,
            predecessor_stage_evidence_id=registry_result["stage_evidence_id"],
        )
    except Exception:
        return _stage_result(
            "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED",
...
def _valid_registry_container(value: object) -> bool:
    return isinstance(value, (set, frozenset, list, tuple)) and all(
...
def steps(flow=None):
    return {
        step["step_id"]: step for step in (flow or DATA["live_activation_dialog_flow"])["steps"]
    }
...
def run_path(events: list[str], flow=None):
    ...
    while True:
        ...
        if step["step_kind"] == "ACTION":
            ...
            if used >= len(events):
                raise ValueError("missing decision event")
            event = events[used]
            used += 1
        matches = [t for t in transitions if t["event"] == event]
        if len(matches) != 1:
            raise ValueError("unknown or ambiguous event")
...
def validate_graph(flow):
    ...
    for step in flow["steps"]:
        ...
        if step["step_kind"] == "ACTION":
            ...
            assert all(
                transition["destination_step_id"] in ids for transition in step["transitions"]
            )
        else:
    ...
    assert set(flow["terminal_outcomes"]) == {
        "CANCELLED",
...
def test_scope_and_basic_contract():
    ...
    assert "environment_and_product_capabilities" not in MAIN.read_text(encoding="utf-8")
    assert "## Status M0.4 — closed" in ARCH.read_text(encoding="utf-8")
    assert [env["environment_id"] for env in DATA["execution_environments"]] == [
        "PAPER",
        "TESTNET",
        "LIVE",
    ]
...
def test_timestamp_profile_preserves_nanoseconds_and_rejects_bad_formats():
    ...
    assert {
        ...
        for key in [
            ...
            "timezone_policy",
            "fractional_seconds_policy",
            "leap_second_policy",
        ]
    } == {
        "timestamp_format": "RFC3339_UTC",
        "timestamp_profile_id": "RFC3339_UTC_ZULU_SECONDS",
        "timezone_policy": "UTC_Z_REQUIRED",
        "fractional_seconds_policy": "OPTIONAL_1_TO_9_DIGITS",
...
def test_expiry_interpreter_positive_negative_and_nanosecond_boundaries():
    ...
    base = {
        "non_expiring": False,
        "issued_at_utc": "2025-01-01T00:00:00.000000001Z",
        "expires_at_utc": "2025-01-01T00:00:00.000000002Z",
    }
    ...
    malformed = [
        ...
        {
            "non_expiring": False,
            "expires_at_utc": "2025-01-01T00:00:00Z",
            "issued_at_utc": "2025-01-01T00:00:00Z",
        },
        {
            "non_expiring": False,
...
def test_current_schema_jcs_vectors_and_signed_payload_type_policy():
    assert (
        DATA["signature_contract"]["signed_payload_json_type_policy"]["number"]
        == "FORBIDDEN_IN_CURRENT_SCHEMA"
    )
    for numeric in [{"n": 1}, {"n": 1.0}, {"nested": {"n": 1.5}}, {"a": ["x", 2]}]:
        try:
            jcs_current_schema_canonicalize(numeric)
            raise AssertionError(numeric)
        except TypeError:
            pass
        try:
            reject_numbers_in_signed_payload(numeric)
            raise AssertionError(numeric)
        except ValueError:
            pass
    assert jcs_current_schema_canonicalize(
        {"\ue000": "bmp", "\U00010000": "supplementary"}
    ) == '{"𐀀":"supplementary","":"bmp"}'.encode("utf-8")
    assert (
        jcs_current_schema_canonicalize({"s": 'line\nquote"backslash\\'})
        == b'{"s":"line\\nquote\\"backslash\\\\"}'
    )
    assert jcs_current_schema_canonicalize(
        {"b": [True, None, "x"], "a": "x"}
    ) == jcs_current_schema_canonicalize({"a": "x", "b": [True, None, "x"]})
...
def test_single_authoritative_document_schema_registry_and_recursive_no_duplicates():
    ...
    assert schemas["signature_input_schema"]["canonical_object_shape"] == {
        "signature_header": "<full_signature_header>",
        "signed_payload_hash": "<signed_payload_hash>",
    }
    required_paths, shape_paths, full_copies = (
        [],
        [],
        {"payload": 0, "header": 0, "envelope": 0, "sig_input": 0},
    )
        ...
        if isinstance(value, dict):
            for key, child in value.items():
                if key == "required_fields":
                    required_paths.append(path + (key,))
                if key == "canonical_object_shape":
                    shape_paths.append(path + (key,))
                walk(child, path + (key,))
        elif isinstance(value, list):
            if value == CANONICAL_PAYLOAD:
                full_copies["payload"] += 1
            if value == HEADER:
                full_copies["header"] += 1
            if value == ENVELOPE:
                full_copies["envelope"] += 1
            if value == SIG_INPUT:
                full_copies["sig_input"] += 1
    ...
    assert shape_paths == [
        (
            "ProductCapabilities",
            "document_schemas",
            "signature_input_schema",
            "canonical_object_shape",
        )
    ]
    for container in [DATA["ProductCapabilities"]["contract_schema"], DATA["signature_contract"]]:
        for ref in [
            "capability_payload_schema_ref",
            "signature_header_schema_ref",
            "signed_document_envelope_schema_ref",
            "signature_input_schema_ref",
        ]:
...
def _payload(capabilities_id: str = "cap"):
    ...
    return {
        ...
        "expires_at_utc": None,
        "non_expiring": True,
        "capability_set": list(policy["capability_set"]),
        "environment_capabilities": copy.deepcopy(policy["environment_capabilities"]),
        "feature_flags": copy.deepcopy(policy["feature_flags"]),
        "capability_set_hash": "",
        "source": policy["source"],
        "fail_closed_policy": "SAFE_LOCAL_ONLY",
        "revocation_reference": "caprev_test1234",
    }


def _evaluate_expiry_and_revocation_policy(
    envelope: dict, current_utc: str, revocation_registry: dict | None
) -> dict:
    ...
    if expiry == "EXPIRED":
        return {
            "policy_result": "EXPIRY_REVOCATION_REJECTED",
            "diagnostic_reason_code": "EXPIRED",
            "mapped_trust_state": "EXPIRED",
            "mapped_denial_code": "PRODUCT_CAPABILITIES_EXPIRED",
            "audit_event": "PRODUCT_CAPABILITIES_REJECTED",
        }
    if expiry != "VALID_EXPIRY":
        return {
            "policy_result": "EXPIRY_REVOCATION_REJECTED",
            "diagnostic_reason_code": "UNSUPPORTED_EXPIRY",
    ...
    if revocation_registry is None:
        ...
        return {
            "policy_result": mapping["stage_result"],
            "diagnostic_reason_code": mapping["diagnostic_reason_code"],
            "mapped_trust_state": mapping["mapped_trust_state"],
            "mapped_denial_code": mapping["mapped_denial_code"],
            "audit_event": mapping.get("audit_event"),
        }
    if "revocation_status" in payload:
        mapping = policy["status_mappings"]["STATUS_UNKNOWN"]
        return {
            "policy_result": mapping["stage_result"],
            "diagnostic_reason_code": "DOCUMENT_SUPPLIED_REVOCATION_STATUS_FORBIDDEN",
            "mapped_trust_state": mapping["mapped_trust_state"],
            "mapped_denial_code": mapping["mapped_denial_code"],
            "audit_event": mapping.get("audit_event"),
        }
    ...
    return {
        "policy_result": mapping["stage_result"],
        "diagnostic_reason_code": mapping.get("diagnostic_reason_code"),
        "mapped_trust_state": mapping["mapped_trust_state"],
        "mapped_denial_code": mapping["mapped_denial_code"],
        "audit_event": mapping.get("audit_event"),
    }
...
def test_separated_pipeline_hash_registry_crypto_and_version_regressions():
    ...
    arbitrary = copy.deepcopy(envelope)
    arbitrary["signature"] = base64url_no_padding(b"arbitrary-asymmetric-signature-shaped-fixture")
    assert _validate_parsed_document_structure(arbitrary)["stage_result"] == "STRUCTURE_ACCEPTED"
    assert (
        validate_canonical_hashes(arbitrary, _validate_parsed_document_structure(arbitrary))[
            "stage_result"
        ]
        == "HASHES_ACCEPTED"
    )
        ...
        cryptographic_signature_verification(
            ...
            validate_registry_policy(
                ...
                validate_canonical_hashes(
                    arbitrary, _validate_parsed_document_structure(arbitrary)
                ),
                {"alg"},
                {"key"},
            ),
    ...
    bad_hash["capability_payload"]["capability_set_hash"] = "0" * 64
    assert _validate_parsed_document_structure(bad_hash)["stage_result"] == "STRUCTURE_ACCEPTED"
    assert (
        validate_canonical_hashes(bad_hash, _validate_parsed_document_structure(bad_hash))[
            "stage_result"
        ]
        == "HASHES_REJECTED"
    )
    registry = validate_registry_policy(
        envelope, validate_canonical_hashes(envelope, structure), {"other"}, {"key"}
    )
    assert (
        registry["stage_result"] == "REGISTRY_REJECTED"
        and registry["diagnostic_reason_code"] == "UNKNOWN_SIGNATURE_ALGORITHM"
    )
    assert (
        create_validated_snapshot(
            envelope,
            structure,
            validate_canonical_hashes(envelope, structure),
            _stage_result("REGISTRY_ACCEPTED"),
            cryptographic_signature_verification(envelope, _stage_result("REGISTRY_ACCEPTED")),
            _stage_result("EXPIRY_REVOCATION_NOT_REACHED"),
        )["stage_result"]
        != "SNAPSHOT_CREATED"
    )
    for target, field, reason in [
        ("capability_payload", "schema_version", "UNKNOWN_CAPABILITY_PAYLOAD_SCHEMA_VERSION"),
        ("signature_header", "signature_schema_version", "UNKNOWN_SIGNATURE_SCHEMA_VERSION"),
    ]:
        bad = copy.deepcopy(envelope)
        bad[target][field] = "unknown.version"
        result = _validate_parsed_document_structure(bad)
        assert (
            result["stage_result"] == "STRUCTURE_REJECTED"
            and result["diagnostic_reason_code"] == reason
        )
    bad = copy.deepcopy(envelope)
    bad["capability_payload"]["schema_registry"] = "self supplied"
...
def test_dialog_graph_determinism_and_exactly_once_audits():
    ...
    for mutate in ["action_two", "decision_duplicate", "duplicate_terminal", "missing_terminal"]:
        ...
        if mutate == "action_two":
            bad["steps"][0]["transitions"].append(
                {"event": "X", "destination_step_id": "desktop_shell_opens_activation_dialog"}
            )
        elif mutate == "decision_duplicate":
            bad["steps"][2]["transitions"][1]["event"] = "CANCEL"
        elif mutate == "duplicate_terminal":
            bad["steps"][6]["terminal_outcome"] = "CANCELLED"
        elif mutate == "missing_terminal":
            bad["terminal_outcomes"].remove("CANCELLED")
        try:
            validate_graph(bad)
...
def test_gate_mapping_no_bypass_and_defensive_validation():
    assert DATA["gate_execution_invariants"] == [
        "NO_OR_BYPASS_BETWEEN_GATES",
        "ALL_APPLICABLE_GATES_REQUIRED",
        "EARLIER_DENIAL_FORBIDS_LATER_SIDE_EFFECTS",
        "DIRECT_LAYER_INVOCATION_REVALIDATES_OWN_GATE",
        "LATER_LAYERS_DO_NOT_ASSUME_EARLIER_VALIDATION",
        "DENIED_ENVIRONMENT_NEVER_COMMITS_ACTIVE_ENVIRONMENT",
    ...
    assert (
        ...
        == set(REQ_GATES)
        == set(all_gates)
        == set(DATA["live_gate_audit_event_map"])
    )
...
def test_preserved_environment_capability_trust_endpoint_and_cross_contracts():
    ...
    assert any(
        entity["canonical_name"] == "LiveAccessGrant"
        for entity in json.loads(VOCAB.read_text(encoding="utf-8"))["entity_kinds"]
    )
    assert DATA["cross_contract_references"]["active_environment_mutation_authority"] == "CoreHost"
    assert any(
        "only CoreHost applies mutable trading state mutations" in invariant
        for invariant in json.loads(PROC.read_text(encoding="utf-8"))["invariants"]
    )
    for group in [
        DATA["audit_payload_contract"]["required_field_ids"],
        DATA["audit_payload_contract"]["optional_fields"],
        DATA["audit_payload_contract"]["result_fields"],
        DATA["audit_payload_contract"]["forbidden_fields"],
        DATA["persistence_and_recovery"]["persistable_fields"],
    ]:
        for identifier in group:
            assert re.fullmatch(r"^[a-z][a-z0-9_]*$", identifier), identifier
    assert (
        re.search(r"AKIA[0-9A-Z]{16}|BEGIN .*PRIVATE KEY|sk-[A-Za-z0-9]{20,}", json.dumps(DATA))
        is None
    )
...
def _context_from_envelope(envelope: dict) -> dict:
    ...
    return {
        "validation_context_id": sha_bytes(
            ("cryptohunter.validation_context.v1:" + fingerprint).encode()
        ),
...
def _evidence_id(evidence: dict) -> str:
    ...
    return sha_bytes(
        jcs_current_schema_canonicalize({field: evidence.get(field) for field in fields})
    )


def _base_stage_evidence(
    result_code: str,
    reason: str | None = None,
    trust: str | None = None,
    denial: str | None = None,
    context: dict | None = None,
    predecessor_stage_evidence_id: str | None = None,
    fixture_availability: str | None = None,
) -> dict:
    evidence = {
...
def _stage_result(
    ...
    context: dict | None = None,
    predecessor_stage_evidence_id: str | None = None,
    fixture_token: object | None = None,
) -> dict:
    if result_code in _ACCEPTED_RESULTS:
        return _base_stage_evidence(
            _ACCEPTED_FAIL_CLOSED[result_code],
            "ACCEPTED_RESULT_REQUIRES_STAGE_ISSUER",
            trust or "UNSUPPORTED_SCHEMA",
            denial or "PRODUCT_CAPABILITIES_UNSUPPORTED_SCHEMA",
            context,
            predecessor_stage_evidence_id,
        )
    return _base_stage_evidence(
        result_code, reason, trust, denial, context, predecessor_stage_evidence_id
    )
...
def _complete_clean_evidence(
    evidence: Mapping, expected_stage_id: str, expected_result: str, context: dict, predecessor=None
) -> bool:
    return False
...
def _same_context(evidence: Mapping, context: dict) -> bool:
    return isinstance(evidence, Mapping) and all(
        evidence.get(key) == context.get(key)
        for key in DATA["validation_context_contract"]["context_fields"]
    )
...
def _evidence_id_is_valid(evidence: Mapping) -> bool:
    return (
        ...
        and isinstance(evidence.get("stage_evidence_id"), str)
        and HEX64.fullmatch(evidence["stage_evidence_id"])
        and evidence["stage_evidence_id"] == _evidence_id(dict(evidence))
    )
...
def _canonical_hashes_revalidated(envelope: dict) -> bool:
    try:
        payload = envelope["capability_payload"]
        return payload["capability_set_hash"] == capability_set_hash(
            payload["capability_set"]
        ) and envelope["signed_payload_hash"] == sha_bytes(jcs_current_schema_canonicalize(payload))
    except Exception:
        return False
...
def validate_field(value: object, contract: dict) -> bool:
    ...
    if typ == "STRING":
        if not isinstance(value, str):
            return False
        if contract.get("non_empty") and value == "":
            return False
        if contract.get("format") == "LOWERCASE_HEX" and not HEX64.fullmatch(value):
            return False
        if "exact_length" in contract and len(value) != contract["exact_length"]:
            return False
        if "allowed_values" in contract and value not in contract["allowed_values"]:
            return False
        if "timestamp_profile_ref" in contract:
            try:
                ...
                return False
        if contract.get("format") == "BASE64URL_NO_PADDING" and not validate_base64url(value):
            return False
        return True
    if typ == "STRING_OR_NULL":
        ...
        if not isinstance(value, str):
            return False
        if contract.get("non_empty_when_string") and value == "":
            return False
        if contract.get("format") == "CAPABILITY_REVOCATION_REFERENCE_OR_NULL" and not re.fullmatch(
            r"caprev_[A-Za-z0-9_-]{8,64}", value
        ):
            return False
        return True
    if typ == "BOOLEAN":
    ...
    if typ == "ARRAY":
        if not isinstance(value, list) or (contract.get("non_empty") and not value):
            return False
        if contract.get("item_type") == "STRING" and not all(
            isinstance(item, str) for item in value
        ):
            return False
        if contract.get("unique") and len(set(value)) != len(value):
            return False
        if contract.get(
            "canonical_sort_order_ref"
        ) == "UTF16_CODE_UNIT_LEXICOGRAPHIC" and value != sorted(
            value, key=utf16_code_unit_sort_key
        ):
            return False
        return True
    if typ == "OBJECT":
        ...
        if keys and set(value) != set(keys):
            return False
        if contract.get("key_type") == "STRING" and not all(isinstance(k, str) for k in value):
            return False
        if contract.get("value_type") == "BOOLEAN" and not all(
...
def validate_nested_environment_capabilities(value: object) -> bool:
    ...
    for env, schema_ref in refs.items():
        ...
        if not isinstance(value[env], dict) or set(value[env]) != set(schema["required_fields"]):
            return False
        for field, contract in schema["field_contracts"].items():
            if not validate_field(value[env][field], contract):
                return False
            if (
                "current_edition_value" in contract
                and value[env][field] != contract["current_edition_value"]
            ):
                return False
    return True
...
def validate_feature_flags(value: object) -> bool:
    registry = DATA["ProductCapabilities"]["document_schemas"][
        "current_schema_feature_flag_registry"
    ]
    return (
        isinstance(value, dict)
        and set(value) == set(registry["required_flags"])
        and all(isinstance(v, bool) for v in value.values())
        and all(
            value[k] == registry["current_edition_values"][k] for k in registry["required_flags"]
        )
    )
...
def parse_raw_json_without_duplicates(raw: str | bytes):
    ...
    if isinstance(raw, bytes):
        try:
            ...
            raise ValueError("malformed utf-8") from exc
    allowed_ws = {" ", "\t", "\n", "\r"}
    start = 0
...
def validate_raw_document_structure(
    raw_document: str | bytes, version_registry=DEFAULT_VERSION_REGISTRY
) -> dict:
...
def test_signature_validation_stages_and_registry_mapping_are_data_driven():
    ...
    assert pipeline["validation_stages"] == [
        "DOCUMENT_STRUCTURE_VALIDATION",
        "CANONICAL_HASH_VALIDATION",
        "REGISTRY_POLICY_VALIDATION",
        "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION",
        "EXPIRY_AND_REVOCATION_VALIDATION",
        "VALIDATED_SNAPSHOT_CREATION",
    ]
    ...
    for reason in contract["diagnostic_reason_codes"]:
        expected = (
            ("UNSUPPORTED_SCHEMA", "PRODUCT_CAPABILITIES_UNSUPPORTED_SCHEMA")
            if reason
            in {
                "REGISTRY_INPUT_DOCUMENT_INVALID",
                "REGISTRY_REFERENCE_TYPE_INVALID",
                "REGISTRY_CONTAINER_INVALID",
            }
            else ("INVALID_SIGNATURE", "PRODUCT_CAPABILITIES_INVALID_SIGNATURE")
        )
    ...
    accepted = validate_registry_policy(
        env,
        validate_canonical_hashes(env, _validate_parsed_document_structure(env)),
        {"alg"},
        {"key"},
    )
    ...
    for algs, keys, reason in [
        ...
        outcome = validate_registry_policy(
            env,
            validate_canonical_hashes(env, _validate_parsed_document_structure(env)),
            algs,
            keys,
...
def test_utf16_capability_set_order_and_fixed_hash_regression():
    assert (
        DATA["signature_contract"]["capability_set_sort_order_id"]
        == "UTF16_CODE_UNIT_LEXICOGRAPHIC"
    )
    ...
    assert (
        capability_set_hash(capability_set)
        == "e8bdee294d4a756532cd1660a49d7d99325bb04ec58c236f78b94ff2718d31de"
    )
...
def test_field_contracts_are_complete_and_malformed_types_rejected():
    ...
    assert set(schemas["signature_input_schema"]["field_contracts"]) == {
        "signature_input",
        "signature_header",
        "signed_payload_hash",
    }
    ...
    mutations = {
        "capabilities_id_array": lambda e: e["capability_payload"].__setitem__(
            "capabilities_id", []
        ),
        ...
        "non_expiring_string": lambda e: e["capability_payload"].__setitem__(
            "non_expiring", "true"
        ),
        "capability_set_string": lambda e: e["capability_payload"].__setitem__(
            "capability_set", "A"
        ),
        "capability_set_non_string": lambda e: e["capability_payload"].__setitem__(
            "capability_set", ["A", 1]
        ),
        "env_caps_null": lambda e: e["capability_payload"].__setitem__(
            "environment_capabilities", None
        ),
        "env_caps_array": lambda e: e["capability_payload"].__setitem__(
            "environment_capabilities", []
        ),
        "missing_environment": lambda e: e["capability_payload"].__setitem__(
            "environment_capabilities", {"PAPER": {}, "TESTNET": {}}
        ),
        "extra_environment": lambda e: e["capability_payload"].__setitem__(
            "environment_capabilities", {"PAPER": {}, "TESTNET": {}, "LIVE": {}, "DEMO": {}}
        ),
        "feature_flags_array": lambda e: e["capability_payload"].__setitem__("feature_flags", []),
        "feature_flag_string": lambda e: e["capability_payload"].__setitem__(
        ...
        "fail_closed_other": lambda e: e["capability_payload"].__setitem__(
            "fail_closed_policy", "OPEN"
        ),
        "algorithm_array": lambda e: e["signature_header"].__setitem__(
...
def test_execution_environment_matrix_endpoint_trust_safe_local_and_recovery_contracts():
    ...
    assert (
        edition["TESTNET"]["visible"]
        and edition["TESTNET"]["selectable"]
        and edition["TESTNET"]["executable"]
        and edition["TESTNET"]["requires_readiness_gates"]
        and not edition["TESTNET"]["locked"]
    )
    assert (
        edition["LIVE"]["visible"]
        and edition["LIVE"]["locked"]
        and not edition["LIVE"]["selectable"]
        and not edition["LIVE"]["executable"]
        and edition["LIVE"]["activation_dialog_available"]
        and edition["LIVE"]["denial_code"] == "LIVE_BLOCKED_BY_EDITION"
    )
    pc_policy = DATA["ProductCapabilities"]["current_edition_capability_policy"]
    assert (
        pc_policy["live_allowed_in_current_edition"] is False
        and pc_policy["environment_capabilities"]["LIVE"]["private_execution_allowed"] is False
    )
    ...
    assert all(
        cls in DATA["endpoint_policy"]["endpoint_classes"]
        for env in envs.values()
        for cls in env["endpoint_classes"]
    )
    assert DATA["endpoint_policy"]["testnet_to_live_fallback_allowed"] is False
    expected_trust = [
        "VALID",
    ...
    assert (
        safe["read_or_decrypt_exchange_secrets"] is False
        and safe["testnet_private_execution_allowed"] is False
        and safe["live_allowed"] is False
        and safe["paper_local_simulation_may_remain_available"] is True
    )
    ...
    assert (
        "core_commits_active_environment"
        in DATA["runtime_environment_activation_gate_order"]["order"]
    )
...
def test_closed_environment_capability_schemas_and_feature_flag_registry():
    valid = build_envelope(_payload())
    env_mutations = {
        "empty_paper": lambda e: e["capability_payload"]["environment_capabilities"].__setitem__(
            "PAPER", {}
        ),
        "empty_testnet": lambda e: e["capability_payload"]["environment_capabilities"].__setitem__(
            "TESTNET", {}
        ),
        "empty_live": lambda e: e["capability_payload"]["environment_capabilities"].__setitem__(
            "LIVE", {}
        ),
        "missing_nested": lambda e: e["capability_payload"]["environment_capabilities"]["LIVE"].pop(
            "private_execution_allowed"
        ),
        "extra_nested": lambda e: e["capability_payload"]["environment_capabilities"][
            "LIVE"
        ].__setitem__("unknown_live_unlock", True),
        "string_boolean": lambda e: e["capability_payload"]["environment_capabilities"][
            "PAPER"
        ].__setitem__("local_execution_allowed", "true"),
        "live_private_true": lambda e: e["capability_payload"]["environment_capabilities"][
            "LIVE"
        ].__setitem__("private_execution_allowed", True),
    }
    ...
    assert registry["required_flags"] == [
        "live_activation_dialog",
        "live_execution",
        "testnet_execution",
    ]
    assert registry["current_edition_values"] == {
        "live_activation_dialog": True,
        "live_execution": False,
        "testnet_execution": True,
    }
    flag_mutations = {
        "fictional_x": lambda e: e["capability_payload"].__setitem__("feature_flags", {"x": True}),
        "unknown_live_unlock": lambda e: e["capability_payload"]["feature_flags"].__setitem__(
            "unknown_live_unlock", True
        ),
        "live_execution_true": lambda e: e["capability_payload"]["feature_flags"].__setitem__(
            "live_execution", True
        ),
        "missing_live_execution": lambda e: e["capability_payload"]["feature_flags"].pop(
            "live_execution"
        ),
        "string_boolean": lambda e: e["capability_payload"]["feature_flags"].__setitem__(
            "testnet_execution", "true"
        ),
    }
...
def test_version_policy_and_revocation_interpreter():
    ...
    for target, field in [
        ...
        ("signature_header", "signature_schema_version"),
    ]:
        for value in ["", "unknown", [], None]:
            ...
            assert _validate_parsed_document_structure(bad)["stage_result"] == "STRUCTURE_REJECTED"
    assert (
        _evaluate_expiry_and_revocation_policy(
            envelope, "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
        )["policy_result"]
        == "EXPIRY_REVOCATION_ACCEPTED"
    )
    revoked = _evaluate_expiry_and_revocation_policy(
        envelope, "2026-01-01T00:00:00Z", {"caprev_test1234": "REVOKED"}
    )
    assert (
        revoked["policy_result"],
        revoked["mapped_trust_state"],
        revoked["mapped_denial_code"],
    ) == ("EXPIRY_REVOCATION_REJECTED", "REVOKED", "PRODUCT_CAPABILITIES_REVOKED")
    unknown_ref = _evaluate_expiry_and_revocation_policy(
        envelope, "2026-01-01T00:00:00Z", {"other": "NOT_REVOKED"}
    )
    assert (unknown_ref["policy_result"], unknown_ref["diagnostic_reason_code"]) == (
    ...
    assert (unavailable["policy_result"], unavailable["diagnostic_reason_code"]) == (
        "EXPIRY_REVOCATION_REJECTED",
        "REVOCATION_REGISTRY_UNAVAILABLE",
    )
    empty_registry = _evaluate_expiry_and_revocation_policy(envelope, "2026-01-01T00:00:00Z", {})
    assert (empty_registry["policy_result"], empty_registry["diagnostic_reason_code"]) == (
        "EXPIRY_REVOCATION_REJECTED",
        "REVOCATION_REGISTRY_UNAVAILABLE",
    )
    injected = copy.deepcopy(envelope)
    injected["capability_payload"]["revocation_status"] = "NOT_REVOKED"
    assert (
        _evaluate_expiry_and_revocation_policy(
            injected, "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
        )["diagnostic_reason_code"]
        == "DOCUMENT_SUPPLIED_REVOCATION_STATUS_FORBIDDEN"
    )
    expired = build_envelope(
        {**_payload(), "non_expiring": False, "expires_at_utc": "2025-01-01T00:00:01Z"}
    )
    expired_outcome = _evaluate_expiry_and_revocation_policy(
        expired, "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
    )
    assert (
        expired_outcome["policy_result"],
        expired_outcome["diagnostic_reason_code"],
        expired_outcome["mapped_trust_state"],
        expired_outcome["mapped_denial_code"],
    ) == ("EXPIRY_REVOCATION_REJECTED", "EXPIRED", "EXPIRED", "PRODUCT_CAPABILITIES_EXPIRED")
...
def test_raw_json_policy_rejects_malformed_utf8_nonfinite_duplicates_and_trailing_data():
    ...
    for raw in bad_inputs:
        try:
            ...
            assert (
                DATA["raw_json_parse_policy"]["canonicalization_after_parse_error_allowed"] is False
            )
...
def test_pipeline_regression_hash_registry_expiry_revocation_and_snapshot_boundaries():
    ...
    assert (
        structure["stage_result"] == "STRUCTURE_ACCEPTED"
        and hashes["stage_result"] == "HASHES_ACCEPTED"
    )
    bad_hash = copy.deepcopy(envelope)
    bad_hash["signed_payload_hash"] = "0" * 64
    assert _validate_parsed_document_structure(bad_hash)["stage_result"] == "STRUCTURE_ACCEPTED"
    assert (
        validate_canonical_hashes(bad_hash, _validate_parsed_document_structure(bad_hash))[
            "stage_result"
    ...
    crypto = cryptographic_signature_verification(
        envelope, validate_registry_policy(envelope, hashes, {"alg"}, {"key"})
    )
    ...
    assert (
        DATA["signature_validation_pipeline"]["test_functions_may_return_VALID_trust_state"]
        is False
    )
...
def run_pipeline(
    ...
    *,
    ...
    algorithms={"alg"},
    keys={"key"},
    current_utc="2026-01-01T00:00:00Z",
    ...
    document = (
        ...
        if structure.get("stage_result") == "STRUCTURE_ACCEPTED"
        else None
    )
    ...
    expiry = validate_expiry_revocation_stage(
        document or {}, crypto, current_utc, revocation_registry
    )
    snapshot = create_validated_snapshot(
        document or {}, structure, hashes, registry, crypto, expiry
    )
    return {
        "structure": structure,
        "hashes": hashes,
...
def test_raw_document_entrypoint_is_the_only_safe_pipeline_entry():
    ...
    assert (
        validate_raw_document_structure(raw.encode("utf-8"))["stage_result"] == "STRUCTURE_ACCEPTED"
    )
    for bad in [
    ...
    assert (
        validate_raw_document_structure(parsed_by_last_key_wins)["stage_result"]
        == "STRUCTURE_REJECTED"
    )
...
def test_stage_result_registry_is_closed_and_all_outputs_registered():
    ...
    required = {
        ...
        "HASHES_ACCEPTED",
        "HASHES_REJECTED",
        "HASH_VALIDATION_NOT_REACHED",
        "REGISTRY_ACCEPTED",
        "REGISTRY_REJECTED",
        "REGISTRY_VALIDATION_NOT_REACHED",
        "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED",
        "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED",
        "CRYPTOGRAPHIC_VERIFICATION_NOT_IMPLEMENTED_IN_M0_4",
    ...
    returned_literals = set(re.findall(r'stage_result\("([A-Z0-9_]+)"', source)) | set(
        re.findall(r'"stage_result": "([A-Z0-9_]+)"', source)
    )
...
def test_stage_prerequisites_not_reached_and_snapshot_prerequisites():
    envelope = build_envelope(_payload())
    assert (
        validate_canonical_hashes(envelope, _stage_result("STRUCTURE_REJECTED"))["stage_result"]
        == "HASH_VALIDATION_NOT_REACHED"
    )
    assert (
    ...
    assert (
        validate_expiry_revocation_stage(
            envelope,
            _stage_result("CRYPTOGRAPHIC_VERIFICATION_NOT_IMPLEMENTED_IN_M0_4"),
            "2026-01-01T00:00:00Z",
            {"caprev_test1234": "NOT_REVOKED"},
        )["stage_result"]
        == "EXPIRY_REVOCATION_NOT_REACHED"
    )
    structure, hashes, registry, crypto, accepted = _accepted_future_evidence_chain(envelope)
    rejected = validate_expiry_revocation_stage(
        envelope, crypto, "2026-01-01T00:00:00Z", {"caprev_test1234": "REVOKED"}
    )
    assert (
        create_validated_snapshot(envelope, structure, hashes, registry, crypto, accepted)[
            "stage_result"
        ]
        == "SNAPSHOT_CREATED"
    )
    for combo in [
        ...
        (
            ...
            _stage_result(
                "HASHES_REJECTED",
                context=_context_from_envelope(envelope),
                predecessor_stage_evidence_id=structure["stage_evidence_id"],
            ),
            registry,
            crypto,
            accepted,
        ...
        (
            ...
            _stage_result(
                "REGISTRY_REJECTED",
                context=_context_from_envelope(envelope),
                predecessor_stage_evidence_id=hashes["stage_evidence_id"],
            ),
            crypto,
            accepted,
        ),
        (
            ...
            hashes,
            registry,
            cryptographic_signature_verification(envelope, registry),
            accepted,
        ),
    ]:
...
def test_revocation_reference_format_and_invalid_registry_status_fail_closed():
    ...
    for value in [None, "caprev_12345678", "caprev_ABCDEFGHIJKLMNOPQRSTUVWXYZ_0123456789-abcd"]:
        ...
        good["capability_payload"]["revocation_reference"] = value
        good = build_envelope(good["capability_payload"])
    ...
    for value in [
        ...
        bad = copy.deepcopy(envelope)
        bad["capability_payload"]["revocation_reference"] = value
        assert _validate_parsed_document_structure(bad)["stage_result"] == "STRUCTURE_REJECTED"
    for status in ["BROKEN", None, True, {}, []]:
        outcome = _evaluate_expiry_and_revocation_policy(
            envelope, "2026-01-01T00:00:00Z", {"caprev_test1234": status}
        )
    ...
    assert (
        DATA["capability_revocation_policy"]["status_mappings"]["NOT_REVOKED"].get("audit_event")
        == "PRODUCT_CAPABILITIES_REVOCATION_CHECK_PASSED"
    )
    assert (
        DATA["capability_revocation_policy"]["status_mappings"]["NOT_REVOKED"].get("audit_event")
        != "PRODUCT_CAPABILITIES_VALIDATED"
    )
...
def test_missing_version_registry_and_state_machine_paths():
    ...
    raw = raw_document(envelope)
    assert (
        validate_raw_document_structure(raw, DEFAULT_VERSION_REGISTRY)["stage_result"]
        == "STRUCTURE_ACCEPTED"
    )
    assert (
        validate_raw_document_structure(raw, {"payload": {"unknown"}, "signature": {SIG_VERSION}})[
            "diagnostic_reason_code"
        ]
        == "UNKNOWN_CAPABILITY_PAYLOAD_SCHEMA_VERSION"
    )
    assert validate_raw_document_structure(raw, None) == _stage_result(
        "STRUCTURE_REJECTED",
        "MISSING_SCHEMA_VERSION_REGISTRY",
        "UNSUPPORTED_SCHEMA",
        "PRODUCT_CAPABILITIES_UNSUPPORTED_SCHEMA",
    )
    assert (
        validate_raw_document_structure(raw, {"payload": set(), "signature": set()})[
            "diagnostic_reason_code"
        ]
        == "MISSING_SCHEMA_VERSION_REGISTRY"
    )
    override = copy.deepcopy(envelope)
    override["capability_payload"]["schema_registry"] = {"payload": [PAYLOAD_VERSION]}
    assert (
        _validate_parsed_document_structure(override)["diagnostic_reason_code"]
        == "PAYLOAD_SCHEMA_REGISTRY_OVERRIDE_FORBIDDEN"
    )
    happy = run_pipeline(raw, revocation_registry={"caprev_test1234": "NOT_REVOKED"})
    assert [
        happy[k]["stage_result"]
        for k in ["structure", "hashes", "registry", "crypto", "expiry_revocation", "snapshot"]
    ] == [
        "STRUCTURE_ACCEPTED",
        "HASHES_ACCEPTED",
        "REGISTRY_ACCEPTED",
    ...
    bad_hash_doc = copy.deepcopy(envelope)
    bad_hash_doc["signed_payload_hash"] = "0" * 64
    bad_hash = run_pipeline(raw_document(bad_hash_doc))
    assert [
        bad_hash[k]["stage_result"]
        for k in ["structure", "hashes", "registry", "crypto", "expiry_revocation", "snapshot"]
    ] == [
        "STRUCTURE_ACCEPTED",
        "HASHES_REJECTED",
    ...
    assert (
        unknown_alg["registry"]["stage_result"] == "REGISTRY_REJECTED"
        and unknown_alg["crypto"]["stage_result"] == "CRYPTOGRAPHIC_VERIFICATION_NOT_REACHED"
    )
    future_ok = _run_future_contract_pipeline(
        raw, revocation_registry={"caprev_test1234": "NOT_REVOKED"}
    )
    assert future_ok["snapshot"]["stage_result"] == "SNAPSHOT_CREATED"
    future_revoked = _run_future_contract_pipeline(
        raw, revocation_registry={"caprev_test1234": "REVOKED"}
    )
    assert (
        future_revoked["expiry_revocation"]["stage_result"] == "EXPIRY_REVOCATION_REJECTED"
        and future_revoked["snapshot"]["stage_result"] == "SNAPSHOT_NOT_CREATED"
    )
...
def test_registry_stage_never_returns_structure_stage_result_for_malformed_input():
    malformed_documents = [
        {
            "capability_payload": {"signature_algorithm_id": "alg"},
            "signature_header": {"signature_algorithm_id": "alg", "key_id": "key"},
        },
        {"capability_payload": {}, "signature_header": None},
        {"capability_payload": []},
        {},
    ]
    issued_hash = validate_canonical_hashes(
        build_envelope(_payload()), _validate_parsed_document_structure(build_envelope(_payload()))
    )
...
def test_each_stage_helper_owns_only_its_declared_stage_outputs():
    ...
    assert {result["stage_result"] for result in structure_results} <= _stage_outputs(
        "DOCUMENT_STRUCTURE_VALIDATION"
    )
    ...
    hash_results = [
        accepted_hashes,
        validate_canonical_hashes(
            bad_hash_doc, validate_raw_document_structure(raw_document(bad_hash_doc))
        ),
        validate_canonical_hashes(envelope, _stage_result("STRUCTURE_REJECTED")),
    ]
    assert {result["stage_result"] for result in hash_results} <= _stage_outputs(
        "CANONICAL_HASH_VALIDATION"
    )
    ...
    registry_results = [
        ...
        validate_registry_policy(
            {"capability_payload": {"key_id": "payload-key"}, "signature_header": {}},
            accepted_hashes,
            {"alg"},
            {"key"},
        ),
    ...
    assert {result["stage_result"] for result in registry_results} <= _stage_outputs(
        "REGISTRY_POLICY_VALIDATION"
    )
    ...
    assert {result["stage_result"] for result in crypto_results} <= _stage_outputs(
        "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION"
    )
    ...
    assert {result["stage_result"] for result in expiry_results} <= _stage_outputs(
        "EXPIRY_AND_REVOCATION_VALIDATION"
    )
    ...
    snapshot_results = [
        create_validated_snapshot(
            envelope,
            _stage_result("STRUCTURE_ACCEPTED"),
            _stage_result("HASHES_ACCEPTED"),
            _stage_result("REGISTRY_ACCEPTED"),
            _stage_result("CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED"),
            _stage_result("EXPIRY_REVOCATION_ACCEPTED"),
        ),
        create_validated_snapshot(
            envelope,
            _stage_result("STRUCTURE_ACCEPTED"),
            _stage_result("HASHES_REJECTED"),
            _stage_result("REGISTRY_ACCEPTED"),
            _stage_result("CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED"),
            _stage_result("EXPIRY_REVOCATION_ACCEPTED"),
        ),
    ]
    assert {result["stage_result"] for result in snapshot_results} <= _stage_outputs(
        "VALIDATED_SNAPSHOT_CREATION"
    )
...
def test_stage_result_registry_has_exact_global_and_per_stage_ownership():
    ...
    normal = run_pipeline(
        raw_document(build_envelope(_payload())),
        revocation_registry={"caprev_test1234": "NOT_REVOKED"},
    )
    normal_results = {
        normal[key]["stage_result"]
        for key in ["structure", "hashes", "registry", "crypto", "expiry_revocation", "snapshot"]
    }
    future_only = {
        result
        for result, spec in pipeline["stage_result_registry"].items()
        if spec.get("availability") == "FUTURE_ONLY"
    }
...
def test_current_edition_signed_payload_policy_and_capability_registry_binding():
    ...
    pc_policy = DATA["ProductCapabilities"]["current_edition_capability_policy"]
    assert (
        signed_policy["edition_id"]
        == DATA["current_product_edition"]["edition_id"]
        == pc_policy["edition_id"]
        == "CRYPTOHUNTER_TESTNET_EDITION"
    )
    assert (
        signed_policy["capability_set"]
        == pc_policy["capability_set"]
        == capability_registry["current_schema_allowed_capability_ids"]
    )
    assert (
        signed_policy["environment_capabilities_ref"]
        == "ProductCapabilities.current_edition_capability_policy.environment_capabilities"
    )
    assert (
        signed_policy["feature_flags_ref"]
    ...
    mutations = {
        ...
        "unknown_capability": lambda p: p.__setitem__(
            "capability_set",
            ["LIVE_VISIBLE_LOCKED_ONLY", "PAPER_LOCAL_SIMULATION", "UNKNOWN_CAPABILITY"],
        ),
        ...
        "future_live_capability": lambda p: p.__setitem__(
            "capability_set",
            [
                "LIVE_PRIVATE_EXECUTION",
                "PAPER_LOCAL_SIMULATION",
                "TESTNET_PRIVATE_EXECUTION_AFTER_READINESS",
            ],
        ),
        "wrong_order": lambda p: p.__setitem__(
            "capability_set", list(reversed(p["capability_set"]))
        ),
        "duplicate": lambda p: p.__setitem__(
            "capability_set", p["capability_set"] + [p["capability_set"][0]]
        ),
    }
    for name, mutate in mutations.items():
        payload = _payload()
        ...
        envelope = (
            ...
            if set(payload) == set(CANONICAL_PAYLOAD)
            else {
                "capability_payload": payload,
                "signature_header": valid["signature_header"],
                "signed_payload_hash": valid["signed_payload_hash"],
                "signature": valid["signature"],
            }
        )
        assert (
            _validate_parsed_document_structure(envelope)["stage_result"] == "STRUCTURE_REJECTED"
        ), name
...
def test_document_bound_validation_context_blocks_cross_document_evidence_mixing():
    ...
    expiry_a = validate_expiry_revocation_stage(
        envelope_a, crypto_a, "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
    )
    expiry_b = validate_expiry_revocation_stage(
        envelope_b, crypto_b, "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
    )
    assert validate_canonical_hashes(envelope_a, structure_b)["stage_result"] == "HASHES_REJECTED"
    assert (
        validate_registry_policy(envelope_b, hashes_a, {"alg"}, {"key"})["stage_result"]
        == "REGISTRY_REJECTED"
    )
    assert (
        cryptographic_signature_verification(envelope_b, registry_a)["stage_result"]
        == "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED"
    )
    assert (
        validate_expiry_revocation_stage(
            envelope_b, crypto_a, "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
        )["stage_result"]
        == "EXPIRY_REVOCATION_REJECTED"
    )
    assert (
        create_validated_snapshot(
            envelope_b, structure_a, hashes_a, registry_a, crypto_a, expiry_a
        )["stage_result"]
        == "SNAPSHOT_NOT_CREATED"
    )
    for field in ["validation_context_id", "capabilities_id", "signed_payload_hash"]:
        ...
        assert (
            validate_registry_policy(envelope_a, tampered, {"alg"}, {"key"})["stage_result"]
            == "REGISTRY_REJECTED"
        )
    assert (
        create_validated_snapshot(
            envelope_a, structure_a, hashes_a, registry_a, crypto_a, expiry_b
        )["stage_result"]
        == "SNAPSHOT_NOT_CREATED"
    )
...
def test_future_crypto_fixture_is_private_context_bound_and_not_public_pipeline_switch():
    ...
    assert (
        DATA["signature_validation_pipeline"]["normal_pipeline_allows_force_future_crypto"] is False
    )
    assert (
        DATA["signature_validation_pipeline"]["future_crypto_fixture"]
        == "closure-local in _run_future_contract_pipeline"
    )
    envelope = build_envelope(_payload())
    normal = run_pipeline(
        raw_document(envelope), revocation_registry={"caprev_test1234": "NOT_REVOKED"}
    )
    ...
    future = _run_future_contract_pipeline(
        raw_document(envelope), revocation_registry={"caprev_test1234": "NOT_REVOKED"}
    )
    ...
    assert (
        validate_expiry_revocation_stage(
            other, future["crypto"], "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
        )["stage_result"]
        == "EXPIRY_REVOCATION_REJECTED"
    )
...
def test_snapshot_rejects_any_stage_trust_denial_diagnostic_or_context_mismatch():
    ...
    regressions = [
        ...
        (
            1,
            _stage_result(
                "HASHES_ACCEPTED", None, None, "PRODUCT_CAPABILITIES_UNSUPPORTED_SCHEMA", context
            ),
        ),
    ...
    for index, bad in regressions:
        ...
        evidence[index] = bad
        assert (
            create_validated_snapshot(envelope, *evidence)["stage_result"] == "SNAPSHOT_NOT_CREATED"
        )
    mismatched = copy.deepcopy(accepted)
    mismatched[2] = {**mismatched[2], "validation_context_id": "different"}
    assert (
        create_validated_snapshot(envelope, *mismatched)["stage_result"] == "SNAPSHOT_NOT_CREATED"
    )
    mismatched_hash = copy.deepcopy(accepted)
    mismatched_hash[1] = {**mismatched_hash[1], "signed_payload_hash": "0" * 64}
    assert (
        create_validated_snapshot(envelope, *mismatched_hash)["stage_result"]
        == "SNAPSHOT_NOT_CREATED"
    )
...
def test_registry_policy_fail_closed_for_malformed_input_types_without_exceptions():
    ...
    for mutate in malformed_cases:
        ...
        assert result["stage_result"] == "REGISTRY_REJECTED"
        assert result["diagnostic_reason_code"] in {
            "REGISTRY_INPUT_DOCUMENT_INVALID",
            "REGISTRY_REFERENCE_TYPE_INVALID",
        }
    for algorithms, keys in [
        (1, {"key"}),
        ({"alg"}, True),
        ({"alg": True}, {"key"}),
        ([[]], {"key"}),
        ...
        assert result["diagnostic_reason_code"] in {
            "REGISTRY_CONTAINER_INVALID",
            "MISSING_SIGNATURE_ALGORITHM_REGISTRY",
            "MISSING_VERIFICATION_KEY_REGISTRY",
        }
...
def test_defensive_stage_inputs_and_json_surrounding_whitespace_policy():
    ...
    assert (
        validate_canonical_hashes(envelope, {"stage_result": "STRUCTURE_ACCEPTED"})["stage_result"]
        == "HASHES_REJECTED"
    )
    assert (
        validate_registry_policy(envelope, {"stage_result": "HASHES_ACCEPTED"}, {"alg"}, {"key"})[
            "stage_result"
        ]
        == "REGISTRY_REJECTED"
    )
    assert (
        cryptographic_signature_verification(envelope, {"stage_result": "REGISTRY_ACCEPTED"})[
            "stage_result"
        ]
        == "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED"
    )
    assert (
        validate_expiry_revocation_stage(
            envelope,
            {"stage_result": "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED"},
            "2026-01-01T00:00:00Z",
            {"caprev_test1234": "NOT_REVOKED"},
        )["stage_result"]
        == "EXPIRY_REVOCATION_REJECTED"
    ...
    assert (
        create_validated_snapshot(envelope, {"stage_result": "STRUCTURE_ACCEPTED"}, {}, {}, {}, {})[
            "stage_result"
        ]
        == "SNAPSHOT_NOT_CREATED"
    )
...
def _accepted_future_evidence_chain(envelope: dict):
    ...
    return (
        ...
        future["registry"],
        future["crypto"],
        future["expiry_revocation"],
    )


def test_same_identity_mutations_change_document_fingerprint_and_reject_prior_evidence():
    envelope_a = build_envelope(_payload())
    structure_a, hashes_a, registry_a, crypto_a, expiry_a = _accepted_future_evidence_chain(
        envelope_a
    )
    fingerprint_a = _context_from_envelope(envelope_a)["document_fingerprint"]
    mutations = {
        "feature_flags_live_execution": lambda e: e["capability_payload"][
            "feature_flags"
        ].__setitem__("live_execution", True),
        "live_private_execution": lambda e: e["capability_payload"]["environment_capabilities"][
            "LIVE"
        ].__setitem__("private_execution_allowed", True),
        "capability_set": lambda e: e["capability_payload"].__setitem__(
            "capability_set",
            ["LIVE_VISIBLE_LOCKED_ONLY", "PAPER_LOCAL_SIMULATION", "LIVE_PRIVATE_EXECUTION"],
        ),
        "fail_closed_policy": lambda e: e["capability_payload"].__setitem__(
            "fail_closed_policy", "OPEN"
        ),
        "source": lambda e: e["capability_payload"].__setitem__("source", "signed"),
        "revocation_reference": lambda e: e["capability_payload"].__setitem__(
            "revocation_reference", "caprev_changed1"
        ),
        "signature_header_key_id": lambda e: e["signature_header"].__setitem__(
            "key_id", "other-key"
        ),
        "signature": lambda e: e.__setitem__(
            "signature", base64url_no_padding(b"changed-signature")
        ),
    }
    for name, mutate in mutations.items():
...
def test_every_transition_rejects_dirty_or_malformed_previous_evidence_metadata():
    ...
    transitions = [
        ...
        (
            lambda ev: validate_registry_policy(envelope, ev, {"alg"}, {"key"}),
            hashes,
            "REGISTRY_REJECTED",
        ),
        (
            lambda ev: cryptographic_signature_verification(envelope, ev),
            registry,
            "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED",
        ),
        (
            lambda ev: validate_expiry_revocation_stage(
                envelope, ev, "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
            ),
            crypto,
            "EXPIRY_REVOCATION_REJECTED",
        ),
    ...
    for call, accepted, rejected_result in transitions:
        ...
        for key, value in [
            ...
            bad[key] = value
            dirty_cases.append(bad)
        missing = copy.deepcopy(accepted)
        missing.pop("document_fingerprint")
        dirty_cases.append(missing)
...
def test_no_public_future_crypto_acceptance_producers_and_snapshot_prerequisites_exact():
    required = [
        ...
        "same_capabilities_id",
        "same_edition_id",
        "same_signed_payload_hash",
        "same_capability_set_hash",
        "same_payload_schema_version",
        "same_signature_schema_version",
        "no_diagnostic_reason",
        "no_mapped_non_valid_trust_state",
        "no_denial_code",
        "current_edition_policy_consistent",
        "canonical_hashes_revalidated",
        "final_document_unchanged",
        "verified_stage_evidence_id_chain",
        "crypto_acceptance_fixture_bound_to_registry_evidence",
        "expiry_evidence_bound_to_crypto_evidence",
        "evidence_ids_recalculated",
        "no_fabricated_accepted_stage_evidence",
        "stage_specific_private_issuer_attestation",
        "private_predecessor_object_chain_verified",
        "public_and_private_predecessor_links_match",
        "serialized_evidence_cannot_preserve_issuer_attestation",
        "future_crypto_private_fixture_attestation_verified",
        "fixture_marker_is_diagnostic_not_authoritative",
        "fully_rehashed_fabricated_chain_rejected",
        "immutable_issued_stage_evidence",
        "recursively_validated_predecessor_results",
        "recursively_validated_predecessor_contexts",
        "recursively_validated_predecessor_clean_metadata",
        "stage_specific_issuer_only",
        "no_generic_accepted_evidence_factory",
        "direct_wrapper_construction_rejected",
        "mutated_issued_predecessor_chain_rejected",
        ...
        "runtime_authority_objects_not_global",
        "raw_stage_specific_issuers_not_global",
        "closure_local_attestation_registry",
        "public_mapping_digest_bound_to_attestation",
        "object_attribute_tampering_fails_closed",
        "input_public_dict_copied_before_issue",
        "no_module_global_evidence_runtime",
        "no_module_global_future_fixture_token",
        "no_authority_bundle_factory_global",
        "no_global_stage_issuer_methods",
        "strong_current_evidence_ref_identity",
        "numeric_object_id_not_attestation",
        "future_fixture_authority_closure_local_unreturned",
        "no_standalone_future_crypto_issuer_global",
        "initializer_removed_from_globals_after_install",
        "future_crypto_acceptance_only_inside_future_pipeline_closure",
        "future_fixture_authority_not_runtime_argument_or_token",
    ]
    assert DATA["signature_validation_pipeline"]["snapshot_prerequisites"] == required
    public_future_producers = [
        name
        for name, value in globals().items()
        if callable(value)
        and not name.startswith("_")
    ...
    assert (
        DATA["signature_validation_pipeline"]["public_future_crypto_acceptance_producers_allowed"]
        is False
    )
...
def test_strict_json_whitespace_rejects_non_json_surrounding_whitespace():
    ...
    for prefix in [" ", "\t", "\n", "\r", " \t\r\n"]:
        assert (
            validate_raw_document_structure(prefix + raw + prefix)["stage_result"]
            == "STRUCTURE_ACCEPTED"
        )
    for bad_prefix in ["\v", "\f", "\u00a0", "\u2003", "\ufeff"]:
        assert (
            validate_raw_document_structure(bad_prefix + raw)["stage_result"]
            == "STRUCTURE_REJECTED"
        )
    assert validate_raw_document_structure(raw + " \n {}")["stage_result"] == "STRUCTURE_REJECTED"
    policy = DATA["raw_json_parse_policy"]
    assert policy["legal_surrounding_whitespace_codepoints"] == [
        "U+0020",
        "U+0009",
        "U+000A",
        "U+000D",
...
def test_fabricated_accepted_stage_dicts_cannot_create_snapshot():
    ...
    fabricated_hashes = _stage_result(
        "HASHES_ACCEPTED", context=context, predecessor_stage_evidence_id="f" * 64
    )
    fabricated_registry = _stage_result(
        "REGISTRY_ACCEPTED", context=context, predecessor_stage_evidence_id="e" * 64
    )
    fabricated_crypto = dict(crypto)
    fabricated_crypto.pop("fixture_availability")
    fabricated_expiry = _stage_result(
        "EXPIRY_REVOCATION_ACCEPTED", context=context, predecessor_stage_evidence_id="d" * 64
    )
    variants = [
        (
        ...
        (
            structure,
            {**hashes, "predecessor_stage_evidence_id": "a" * 64},
            registry,
            crypto,
            expiry,
        ),
        (
            structure,
            hashes,
            {**registry, "predecessor_stage_evidence_id": "b" * 64},
            crypto,
            expiry,
        ),
        ...
        (
            ...
            hashes,
            registry,
            {k: v for k, v in crypto.items() if k != "fixture_availability"},
            expiry,
        ),
        (structure, hashes, registry, crypto, {**expiry, "predecessor_stage_evidence_id": None}),
        (
            structure,
            {**hashes, "stage_evidence_id": registry["stage_evidence_id"]},
            registry,
            crypto,
            expiry,
        ),
        (
            ...
            hashes,
            registry,
            {**crypto, "stage_result": "CRYPTOGRAPHIC_VERIFICATION_NOT_IMPLEMENTED_IN_M0_4"},
            expiry,
        ),
    ]
...
def test_stage_evidence_chain_contract_matches_interpreter_and_pure_policy_is_not_stage_evidence():
    ...
    mutated = dict(hashes)
    mutated["stage_result"] = "HASHES_REJECTED"
    assert (
        create_validated_snapshot(envelope, structure, mutated, registry, crypto, expiry)[
            "stage_result"
        ]
        == "SNAPSHOT_NOT_CREATED"
    )
    ...
    pure = _evaluate_expiry_and_revocation_policy(
        envelope, "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
    )
    assert set(pure) == {
        "policy_result",
...
def test_no_public_arbitrary_stage_result_or_future_acceptance_producer():
    ...
    assert all(
        "stage_result" not in inspect.signature(fn).parameters for fn in public_functions.values()
    )
    ...
    assert all(
        result["stage_result"] != "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED"
        for result in normal.values()
    )
    context = _context_from_envelope(envelope)
    assert (
        _stage_result("CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED", context=context)[
            "stage_result"
        ]
        == "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED"
    )
...
def _fabricated_public_evidence(
    result_code: str,
    context: dict,
    predecessor_id: str | None,
    fixture_availability: str | None = None,
) -> dict:
    evidence = _base_stage_evidence(
        result_code,
        context=context,
        predecessor_stage_evidence_id=predecessor_id,
        fixture_availability=fixture_availability,
    )
    evidence["stage_evidence_id"] = _evidence_id(evidence)
    return evidence
...
def test_fully_rehashed_fabricated_chain_cannot_create_snapshot():
    ...
    registry = _fabricated_public_evidence(
        "REGISTRY_ACCEPTED", context, hashes["stage_evidence_id"]
    )
    crypto = _fabricated_public_evidence(
        "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED",
        context,
        registry["stage_evidence_id"],
        "TEST_FIXTURE_FUTURE_ONLY",
    )
    expiry = _fabricated_public_evidence(
        "EXPIRY_REVOCATION_ACCEPTED", context, crypto["stage_evidence_id"]
    )
    for evidence in [structure, hashes, registry, crypto, expiry]:
    ...
    assert (
        create_validated_snapshot(envelope, structure, hashes, registry, crypto, expiry)[
            "stage_result"
        ]
        == "SNAPSHOT_NOT_CREATED"
    )
...
def test_transition_level_private_predecessor_attestation_is_required():
    ...
    assert (
        validate_canonical_hashes(envelope, forged_structure)["stage_result"] == "HASHES_REJECTED"
    )
    ...
    assert (
        validate_canonical_hashes(envelope, forged_bad_structure)["stage_result"]
        == "HASHES_REJECTED"
    )
    forged_hash = _fabricated_public_evidence("HASHES_ACCEPTED", context, "b" * 64)
    assert (
        validate_registry_policy(envelope, forged_hash, {"alg"}, {"key"})["stage_result"]
        == "REGISTRY_REJECTED"
    )
    forged_registry = _fabricated_public_evidence("REGISTRY_ACCEPTED", context, "c" * 64)
    assert (
        cryptographic_signature_verification(envelope, forged_registry)["stage_result"]
        == "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED"
    )
    assert (
        validate_expiry_revocation_stage(
            envelope, dict(crypto), "2026-01-01T00:00:00Z", {"caprev_test1234": "NOT_REVOKED"}
        )["stage_result"]
        == "EXPIRY_REVOCATION_REJECTED"
    )
    ...
    assert (
        cryptographic_signature_verification(envelope, forged_private_ref)["stage_result"]
        == "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED"
    )
    ...
    assert (
        cryptographic_signature_verification(envelope, forged_public_id)["stage_result"]
        == "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED"
    )
    other = build_envelope(_payload("cap-other"))
    assert (
        create_validated_snapshot(other, structure, hashes, registry, crypto, expiry)[
            "stage_result"
        ]
        == "SNAPSHOT_NOT_CREATED"
    )
...
def test_mutated_issued_structure_chain_cannot_reach_registry_accepted():
    ...
    assert (
        validate_registry_policy(envelope, forged_hashes, {"alg"}, {"key"})["stage_result"]
        == "REGISTRY_REJECTED"
    )
...
def test_mutated_issued_predecessor_context_cannot_reach_successor_acceptance():
    ...
    assert (
        create_validated_snapshot(envelope, structure, hashes, registry, crypto, forged_expiry)[
            "stage_result"
        ]
        == "SNAPSHOT_NOT_CREATED"
    )
...
def test_mutated_issued_predecessor_diagnostics_cannot_reach_successor_acceptance():
    ...
    for key, value in dirty_fields:
        ...
        forged["stage_evidence_id"] = _evidence_id(forged)
        assert (
            cryptographic_signature_verification(envelope, forged)["stage_result"]
            == "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_REJECTED"
        )
    forged_expiry = dict(expiry)
...
def test_no_module_global_runtime_issuer_token_registry_exists():
    ...
    assert (
        DATA["stage_evidence_producer_attestation_contract"]["issuer_token_values_exposed_in_json"]
        is False
    )
...
def test_direct_wrapper_construction_cannot_create_valid_attestation():
    public = _fabricated_public_evidence(
        "STRUCTURE_ACCEPTED", _context_from_envelope(build_envelope(_payload())), None
    )
    try:
        _IssuedStageEvidence(
            public,
            issuer_token=object(),
            producer_stage_id="DOCUMENT_STRUCTURE_VALIDATION",
...
def test_accepted_chain_cannot_be_built_by_calling_raw_issuers():
    ...
    chain.append(
        _fabricated_public_evidence("HASHES_ACCEPTED", context, chain[-1]["stage_evidence_id"])
    )
    chain.append(
        _fabricated_public_evidence("REGISTRY_ACCEPTED", context, chain[-1]["stage_evidence_id"])
    )
    chain.append(
        _fabricated_public_evidence(
            "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED",
            context,
            chain[-1]["stage_evidence_id"],
            "TEST_FIXTURE_FUTURE_ONLY",
        )
    )
    chain.append(
        _fabricated_public_evidence(
            "EXPIRY_REVOCATION_ACCEPTED", context, chain[-1]["stage_evidence_id"]
        )
    )
    assert create_validated_snapshot(envelope, *chain)["stage_result"] == "SNAPSHOT_NOT_CREATED"
...
def test_direct_wrapper_construction_variants_do_not_create_attestation():
    ...
    object.__setattr__(
        raw,
        "_public_fields",
        {**dict(public), "issuer_token": object(), "construction_authority": object()},
    )
    assert not _issued_chain_valid(raw)


def _initialize_closure_local_interpreter() -> None:
    global _issued_chain_valid
    global _complete_clean_evidence
    global _validate_parsed_document_structure
    global validate_raw_document_structure
    global validate_canonical_hashes
    global validate_registry_policy
    global cryptographic_signature_verification
    global validate_expiry_revocation_stage
    global create_validated_snapshot
    global run_pipeline
    global _run_future_contract_pipeline
    ...
    def seal(public: dict, predecessor):
        evidence = _IssuedStageEvidence(public)
        context = {
            key: evidence.get(key) for key in DATA["validation_context_contract"]["context_fields"]
        }
    ...
    def issued_chain_valid(evidence, context: dict | None = None) -> bool:
        ...
        if meta["stage_id"] != stage_id or meta["stage_result"] != expected_result:
            return False
        if (
            public.get("evidence_schema_version")
            != DATA["stage_evidence_chain_contract"]["evidence_schema_version"]
        ):
            return False
        if not _evidence_id_is_valid(public):
            return False
        public_context = {
            key: public.get(key) for key in DATA["validation_context_contract"]["context_fields"]
        }
        if context is None:
            context = public_context
        if meta["context"] != public_context or public_context != {
            key: context.get(key) for key in DATA["validation_context_contract"]["context_fields"]
        }:
            return False
        if (
            public.get("diagnostic_reason_code") is not None
            or public.get("mapped_trust_state") is not None
            or public.get("mapped_denial_code") is not None
        ):
            return False
        if not isinstance(public.get("document_fingerprint"), str) or not HEX64.fullmatch(
            public["document_fingerprint"]
        ):
            return False
        for key in [
        ...
        return (
            ...
            and public.get("predecessor_stage_evidence_id")
            == predecessor_ref.get("stage_evidence_id")
            and issued_chain_valid(predecessor_ref, context)
        )

    def complete_clean_evidence(
        evidence, expected_stage_id: str, expected_result: str, context: dict, predecessor=None
    ) -> bool:
        if not issued_chain_valid(evidence, context):
            return False
        meta = issued_registry.get(id(evidence))
        if (
            dict(evidence).get("stage_id") != expected_stage_id
            or dict(evidence).get("stage_result") != expected_result
        ):
            return False
        if predecessor is None:
            if expected_stage_id == "DOCUMENT_STRUCTURE_VALIDATION":
                return (
                    meta["predecessor_ref"] is None
                    and evidence.get("predecessor_stage_evidence_id") is None
                )
            return True
        return (
            isinstance(predecessor, _IssuedStageEvidence)
            and meta["predecessor_ref"] is predecessor
            and evidence.get("predecessor_stage_evidence_id")
            == predecessor.get("stage_evidence_id")
            and issued_chain_valid(predecessor, context)
        )

    def issue_structure(context: dict):
        return seal(
            _base_stage_evidence(
                "STRUCTURE_ACCEPTED", context=context, predecessor_stage_evidence_id=None
            ),
            None,
        )

    def issue_hashes(context: dict, structure_evidence):
        if not complete_clean_evidence(
            structure_evidence,
            "DOCUMENT_STRUCTURE_VALIDATION",
            "STRUCTURE_ACCEPTED",
            context,
        ...
        return seal(
            _base_stage_evidence(
                "HASHES_ACCEPTED",
                context=context,
                predecessor_stage_evidence_id=structure_evidence["stage_evidence_id"],
            ),
            structure_evidence,
        )
    ...
    def issue_registry(context: dict, hash_evidence):
        if not complete_clean_evidence(
            hash_evidence, "CANONICAL_HASH_VALIDATION", "HASHES_ACCEPTED", context
        ):
        ...
        return seal(
            _base_stage_evidence(
                "REGISTRY_ACCEPTED",
                context=context,
                predecessor_stage_evidence_id=hash_evidence["stage_evidence_id"],
            ),
            hash_evidence,
        )
    ...
    def issue_future_crypto(context: dict, registry_evidence):
        if not complete_clean_evidence(
            registry_evidence, "REGISTRY_POLICY_VALIDATION", "REGISTRY_ACCEPTED", context
        ):
            raise ValueError(
                "future crypto accepted evidence requires issued registry predecessor chain"
            )
        return seal(
            _base_stage_evidence(
                "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED",
                context=context,
                predecessor_stage_evidence_id=registry_evidence["stage_evidence_id"],
                fixture_availability="TEST_FIXTURE_FUTURE_ONLY",
            ),
            registry_evidence,
        )
    ...
    def issue_expiry(context: dict, crypto_evidence):
        ...
        return seal(
            _base_stage_evidence(
                "EXPIRY_REVOCATION_ACCEPTED",
                context=context,
                predecessor_stage_evidence_id=crypto_evidence["stage_evidence_id"],
            ),
            crypto_evidence,
        )
    ...
    def issue_snapshot(context: dict, expiry_evidence):
        ...
        return seal(
            _base_stage_evidence(
                "SNAPSHOT_CREATED",
                context=context,
                predecessor_stage_evidence_id=expiry_evidence["stage_evidence_id"],
            ),
            expiry_evidence,
        )
    ...
    def parsed_structure(envelope: dict, version_registry=DEFAULT_VERSION_REGISTRY, schemas=None):
        ...
        try:
            if not isinstance(envelope, dict) or set(envelope) != set(ENVELOPE):
                return _stage_result("STRUCTURE_REJECTED")
            ...
            if not isinstance(payload, dict) or not isinstance(header, dict):
                return _stage_result("STRUCTURE_REJECTED")
            if "schema_registry" in payload or "schema_registry" in envelope:
                return _stage_result(
                    "STRUCTURE_REJECTED",
                    "PAYLOAD_SCHEMA_REGISTRY_OVERRIDE_FORBIDDEN",
                    "UNSUPPORTED_SCHEMA",
            ...
            if set(payload) != set(CANONICAL_PAYLOAD) or set(header) != set(HEADER):
                return _stage_result("STRUCTURE_REJECTED")
            if (
                not isinstance(version_registry, dict)
                or set(version_registry) != {"payload", "signature"}
                or not version_registry["payload"]
                or not version_registry["signature"]
            ):
                return _stage_result(
                    "STRUCTURE_REJECTED",
                    "MISSING_SCHEMA_VERSION_REGISTRY",
            ...
            if payload["schema_version"] not in version_registry["payload"]:
                return _stage_result(
                    "STRUCTURE_REJECTED",
                    "UNKNOWN_CAPABILITY_PAYLOAD_SCHEMA_VERSION",
                    "UNSUPPORTED_SCHEMA",
            ...
            if header["signature_schema_version"] not in version_registry["signature"]:
                return _stage_result(
                    "STRUCTURE_REJECTED",
                    "UNKNOWN_SIGNATURE_SCHEMA_VERSION",
                    "UNSUPPORTED_SCHEMA",
                    "PRODUCT_CAPABILITIES_UNSUPPORTED_SCHEMA",
                )
            reject_numbers_in_signed_payload(payload)
            for field, contract in schemas["capability_payload_schema"]["field_contracts"].items():
                if not validate_field(payload[field], contract):
                    return _stage_result("STRUCTURE_REJECTED")
            for field, contract in schemas["signature_header_schema"]["field_contracts"].items():
                if not validate_field(header[field], contract):
                    return _stage_result("STRUCTURE_REJECTED")
            if not validate_nested_environment_capabilities(payload["environment_capabilities"]):
                return _stage_result("STRUCTURE_REJECTED")
            if not validate_feature_flags(payload["feature_flags"]):
                return _stage_result("STRUCTURE_REJECTED")
            if not _current_edition_policy_consistent(envelope):
                return _stage_result(
                    "STRUCTURE_REJECTED",
                    "EDITION_POLICY_MISMATCH",
                    "UNSUPPORTED_SCHEMA",
                    "PRODUCT_CAPABILITIES_UNSUPPORTED_SCHEMA",
                )
    ...
    def raw_structure(raw_document, version_registry=DEFAULT_VERSION_REGISTRY):
        ...
        try:
            return parsed_structure(
                parse_raw_json_without_duplicates(raw_document), version_registry
            )
        except Exception:
            return _stage_result("STRUCTURE_REJECTED")
    ...
    def canonical_hashes(envelope: dict, structure_result: Mapping) -> dict:
        try:
            if (
                not isinstance(structure_result, Mapping)
                or structure_result.get("stage_result") != "STRUCTURE_ACCEPTED"
            ):
                return _stage_result("HASH_VALIDATION_NOT_REACHED")
            context = _context_from_envelope(envelope)
            if not complete_clean_evidence(
                structure_result, "DOCUMENT_STRUCTURE_VALIDATION", "STRUCTURE_ACCEPTED", context
            ):
                return _stage_result(
                    "HASHES_REJECTED",
                    "PREVIOUS_STAGE_EVIDENCE_INVALID",
                    ...
                    "PRODUCT_CAPABILITIES_UNSUPPORTED_SCHEMA",
                    context,
                    structure_result.get("stage_evidence_id")
                    if isinstance(structure_result, Mapping)
                    else None,
                )
            payload = envelope["capability_payload"]
            if payload["capability_set"] != sorted(
                payload["capability_set"], key=utf16_code_unit_sort_key
            ):
                return _stage_result(
                    "HASHES_REJECTED",
                    context=context,
                    predecessor_stage_evidence_id=structure_result.get("stage_evidence_id"),
                )
            if payload["capability_set_hash"] != capability_set_hash(payload["capability_set"]):
                return _stage_result(
                    "HASHES_REJECTED",
                    context=context,
                    predecessor_stage_evidence_id=structure_result.get("stage_evidence_id"),
                )
            if envelope["signed_payload_hash"] != sha_bytes(
                jcs_current_schema_canonicalize(payload)
            ):
                return _stage_result(
                    "HASHES_REJECTED",
                    context=context,
    ...
    def registry_policy(
        envelope: dict,
        hash_result: Mapping,
        recognized_algorithm_ids: set[str] | None,
        recognized_key_ids: set[str] | None,
    ) -> dict:
        contract = DATA["signature_contract"]["signature_verification_registry_contract"]

        def rejected(reason: str, context: dict | None = None) -> dict:
            return _stage_result(
                contract["rejected_stage_result"],
                reason,
                contract["reason_code_to_trust_state"].get(reason),
                contract["reason_code_to_denial_code"].get(reason),
                context,
                hash_result.get("stage_evidence_id") if isinstance(hash_result, Mapping) else None,
            )

        try:
            if (
                not isinstance(hash_result, Mapping)
                or hash_result.get("stage_result") != "HASHES_ACCEPTED"
            ):
            ...
            context = _context_from_envelope(envelope)
            if not complete_clean_evidence(
                hash_result, "CANONICAL_HASH_VALIDATION", "HASHES_ACCEPTED", context
            ):
        ...
        if not _valid_registry_container(recognized_algorithm_ids) or not _valid_registry_container(
            recognized_key_ids
        ):
            return rejected("REGISTRY_CONTAINER_INVALID", context)
        if not recognized_algorithm_ids:
            return rejected("MISSING_SIGNATURE_ALGORITHM_REGISTRY", context)
        if not recognized_key_ids:
    ...
    def future_crypto(registry_evidence: Mapping) -> dict:
        context = (
            {
                key: registry_evidence.get(key)
                for key in DATA["validation_context_contract"]["context_fields"]
            }
            if isinstance(registry_evidence, Mapping)
            else {}
        )
        if not complete_clean_evidence(
            registry_evidence, "REGISTRY_POLICY_VALIDATION", "REGISTRY_ACCEPTED", context
            ...
            return _stage_result(
                ...
                registry_evidence.get("stage_evidence_id")
                if isinstance(registry_evidence, Mapping)
                else None,
            )
    ...
    def expiry_stage(
        envelope: dict, crypto_result: Mapping, current_utc: str, revocation_registry: dict | None
    ) -> dict:
        try:
            if (
                not isinstance(crypto_result, Mapping)
                or crypto_result.get("stage_result")
                != "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED"
            ):
            ...
            decision = _evaluate_expiry_and_revocation_policy(
                envelope, current_utc, revocation_registry
            )
            return (
                issue_expiry(context, crypto_result)
                if decision["policy_result"] == "EXPIRY_REVOCATION_ACCEPTED"
                else _stage_result(
                    decision["policy_result"],
                    decision.get("diagnostic_reason_code"),
                    decision.get("mapped_trust_state"),
                    decision.get("mapped_denial_code"),
                    context,
                    crypto_result["stage_evidence_id"],
                )
            )
        except Exception:
            return _stage_result(
    ...
    def snapshot(
        envelope: dict,
        structure_result: Mapping,
        hash_result: Mapping,
        registry_result: Mapping,
        crypto_result: Mapping,
        expiry_revocation_result: Mapping,
    ) -> dict:
        try:
            context = _context_from_envelope(envelope)
            evidences = [
            ...
            for evidence, (stage_id, result), predecessor in zip(evidences, expected, predecessors):
                if not complete_clean_evidence(evidence, stage_id, result, context, predecessor):
                    return _stage_result(
                        "SNAPSHOT_NOT_CREATED", "PREVIOUS_STAGE_EVIDENCE_INVALID", context=context
                    )
            if (
                crypto_result.get("fixture_availability")
                != DATA["stage_evidence_chain_contract"]["future_crypto_fixture_marker"]
            ):
                return _stage_result(
                    "SNAPSHOT_NOT_CREATED", "PREVIOUS_STAGE_EVIDENCE_INVALID", context=context
                )
            if not _current_edition_policy_consistent(
                envelope
            ) or not _canonical_hashes_revalidated(envelope):
                return _stage_result(
                    "SNAPSHOT_NOT_CREATED", "DOCUMENT_IDENTITY_MISMATCH", context=context
                )
    ...
    def future_pipeline(
        ...
        *,
        ...
        registry = registry_policy(document or {}, hashes, algorithms, keys)
        crypto = (
            future_crypto(registry)
            if registry.get("stage_result") == "REGISTRY_ACCEPTED"
            else crypto_verify(document or {}, registry)
        )
        expiry = expiry_stage(document or {}, crypto, current_utc, revocation_registry)
        snap = snapshot(document or {}, structure, hashes, registry, crypto, expiry)
        return {
...
def test_no_global_evidence_runtime_token_or_callable_issuer_attributes_escape():
    forbidden_global_names = {
        "_EVIDENCE_RUNTIME",
        "_make_evidence_runtime",
        "_FUTURE_CRYPTO_FIXTURE_TOKEN",
        "_future_crypto_accepted_evidence",
        "future_crypto",
        "issue_future_crypto",
        "_initialize_closure_local_interpreter",
    }
    ...
    fabricated.append(
        _fabricated_public_evidence("HASHES_ACCEPTED", context, fabricated[-1]["stage_evidence_id"])
    )
    fabricated.append(
        _fabricated_public_evidence(
            "REGISTRY_ACCEPTED", context, fabricated[-1]["stage_evidence_id"]
        )
    )
    fabricated.append(
        _fabricated_public_evidence(
            "CRYPTOGRAPHIC_SIGNATURE_VERIFICATION_ACCEPTED",
            context,
            fabricated[-1]["stage_evidence_id"],
            "TEST_FIXTURE_FUTURE_ONLY",
        )
    )
    fabricated.append(
        _fabricated_public_evidence(
            "EXPIRY_REVOCATION_ACCEPTED", context, fabricated[-1]["stage_evidence_id"]
        )
    )
    assert (
        create_validated_snapshot(envelope, *fabricated)["stage_result"] == "SNAPSHOT_NOT_CREATED"
    )
...
def test_strong_evidence_reference_and_no_numeric_id_only_attestation():
    ...
    assert (
        create_validated_snapshot(
            envelope, issued[0], public_copy, issued[2], issued[3], issued[4]
        )["stage_result"]
        == "SNAPSHOT_NOT_CREATED"
    )
    deep = copy.deepcopy(issued[2])
    deep["stage_evidence_id"] = _evidence_id(deep)
    roundtrip = json.loads(json.dumps(dict(issued[3])))
    roundtrip["stage_evidence_id"] = _evidence_id(roundtrip)
    assert (
        create_validated_snapshot(envelope, issued[0], issued[1], deep, issued[3], issued[4])[
            "stage_result"
        ]
        == "SNAPSHOT_NOT_CREATED"
    )
    assert (
        create_validated_snapshot(envelope, issued[0], issued[1], issued[2], roundtrip, issued[4])[
            "stage_result"
        ]
        == "SNAPSHOT_NOT_CREATED"
    )
    tampered = issued[0]
    public = dict(tampered)
    public["stage_result"] = "STRUCTURE_REJECTED"
    public["stage_evidence_id"] = _evidence_id(public)
