...
def validate_frozen_receipt_parity(selection, canonical):
    ...
    assert receipt["exact_authenticated_preimage_fields"] == [
        field for field in evidence["fields"] if field != "authentication_tag_or_signature"
    ]
...
def test_discovery_is_complete_and_honest():
    ...
    assert set(items) == {
        "production freshness CAS backend",
        "authoritative freshness history backend",
        "durable finalization receipt store",
        "dedicated local freshness authority adapter",
        "freshness authority signing custody",
        "CHA freshness proposer custody",
        "trusted proposer verification lineage",
        "TEST/PRODUCTION separation",
        "independent revoked-history evidence",
        "production/local rollback model",
    }
...
def test_postgresql_selection_is_transactional_not_process_local():
    value = load()
    selection = value["production_local_selection"]
...
def test_atomic_authority_transaction_retains_decision_and_receipt():
    ...
    for required in (
        "exact current generation/digest/complete head set",
        "exact N+1",
        "immutable document, decision, receipt",
        "commit exactly once",
    ):
...
def test_receipt_is_exact_and_never_caller_authority():
    ...
    assert (
        receipt["domain"]
        == "cryptohunter.account-genesis.freshness-finalization-receipt-authentication.v1"
    )
...
def test_internal_database_metadata_cannot_leak_into_receipt_v1_preimage():
    ...
    assert (
        ...
        in separation["internal_postgresql_authority_fields_may_include"]
    )
...
def test_recovered_receipt_is_byte_compatible_with_frozen_v1_verifier():
    ...
    assert (
        value["receipt"]["lost_response_recovered_receipt_accepted_by_frozen_v1_verifier"] is True
    )
...
def test_custody_roles_and_numeric_versions_are_distinct():
    ...
    assert {"RootProof issuer", "history attestation", "CHA proposer"} <= set(
        custody["forbidden_alias_roles"]
    )
...
def test_crash_matrix_has_exact_recovery_and_no_second_successor():
    ...
    assert {row["cut"] for row in matrix} == {
        "before CAS",
        "CAS transaction committed / receipt response lost",
        "history committed / receipt not returned",
        "receipt committed / process crash",
        "CAS committed / local caller response lost",
        "restart before local final commit",
    }
...
def test_readiness_remains_fail_closed():
    ...
    assert (
        readiness["FRESHNESS_AUTHORITY_PRODUCTION_LOCAL_SIGNING_CUSTODY_FOUNDATION_IMPLEMENTED"]
        is True
    )
