...
def validate(v: dict) -> None:
    ...
    assert not any(
        a[k]
        for k in (
            "root_proof_issuer_equals_CHA",
            "root_proof_issuer_equals_FreshnessAuthority",
            "root_proof_issuer_equals_Catalog_authority",
            "root_proof_issuer_equals_local_projection",
        )
    )
    ...
    assert (
        e["reusable"]
        is e["transferable_between_trust_domains"]
        is e["caller_may_choose_or_replace"]
        is False
    )
    ...
    assert (
        replay["model"] == "C_STATEFUL_SINGLE_BIND_AT_ISSUANCE_PLUS_IMMUTABLE_EXACT_OPERATION_PROOF"
    )
    assert (
        replay["one_proof_more_than_one_operation"]
        is replay["one_proof_more_than_one_account"]
        is replay["parallel_proofs_same_entitlement"]
        is False
    )
    assert (
        replay["mutable_proof_consumed_state"]
        is replay["global_consumption_serialization_required"]
        is False
    )
    ...
    assert (
        keys["REVOKED"]["issue"]
        is keys["REVOKED"]["proof_alone_is_ultimate_historical_trust"]
        is False
    )
    crypto = v["crypto"]
    ...
    assert (
        ...
        crypto["digest"],
        crypto["signature"],
        crypto["signature_encoding"],
        crypto["digest_encoding"],
    ) == (
        "RFC 8785 / JCS over signed_payload only",
        "SHA-256",
        "Ed25519",
    ...
    assert (
        accepted["success_is_committed"]
        is accepted["success_is_freshness"]
        is accepted["success_is_publication"]
        is False
    )
    ...
    assert (
        status["FreshnessAuthority_implementation_allowed"]
        is status["CryptoHunterAccountAuthority_implementation_allowed"]
        is False
    )
...
def test_cross_artifact_contracts_and_m03_implementation() -> None:
    ...
    freshness = load(
        DOCS / "m05_account_genesis_freshness_authority_implementation_readiness_contract.json"
    )
    admission = load(DOCS / "m05_account_genesis_root_proof_admission_binding_contract.json")
    assert (
        physical["selected_or_blocked_protocol"]["selection"]
    ...
    assert (
        freshness["implementation_scope"][
            "FreshnessAuthority_implementation_allowed_after_iteration"
        ]
        is True
    )
...
MUTATIONS = {
    ...
    "freshness_credential_reused_without_distinct_role": (
        "authority_model.root_proof_issuer_equals_FreshnessAuthority",
        True,
    ),
    ...
    "candidate_carried_issuer_key_accepted": (
        "authority_model.trust_rule",
        "candidate key allowed",
    ),
    "test_proof_accepted_in_production": (
        "bootstrap_entitlement.test_production_separation",
        "shared",
    ),
    "acct_a_proof_accepted_for_acct_b": (
        "replay_and_consumption.one_proof_more_than_one_account",
        True,
    ),
    "operation_o1_proof_accepted_for_o2": (
        "replay_and_consumption.one_proof_more_than_one_operation",
        True,
    ),
    "request_r1_proof_accepted_for_r2": (
        "root_proof_object.signed_payload_fields",
        ["schema_version"],
    ),
    "root_proof_id_treated_as_operation_identity": (
        "identity.root_proof_id_is_logical_operation_identity",
        True,
    ),
    "caller_selects_genuine_root_proof_id": ("identity.caller_selected", True),
    "same_proof_authorizes_two_account_ids": (
        "root_proof_object.signed_payload_fields",
        ["logical_operation_id"],
    ),
    "same_entitlement_authorizes_two_operations": (
        "bootstrap_entitlement.cardinality",
        "unlimited",
    ),
    "stateless_proof_without_cardinality_authority": ("replay_and_consumption.model", "STATELESS"),
    "process_local_mutex_as_global_replay_protection": (
        "concurrency.process_local_mutex_sufficient",
        True,
    ),
    "crash_allows_second_proof_or_account": (
        "cross_authority_atomicity.post_issuance_callback",
        True,
    ),
    ...
    "wrong_bootstrap_entitlement_accepted": (
        "root_proof_object.signed_payload_fields",
        ["account_id"],
    ),
...
def validate_issuer_admission_parity(issuer: dict, admission: dict) -> None:
    ...
    assert (
        consumption["post_issuance_CONSUMED_state"]
        == replay["mutable_proof_consumed_state"]
        is False
    )
    assert (
        consumption["post_issuance_issuer_callback"]
        == issuer["cross_authority_atomicity"]["post_issuance_callback"]
        is False
    )
    ...
    assert (
        admission["operation_binding"]["current_candidate_binds_canonical_genesis_request"] is True
    )
    assert (
        consumption["account_bound"]
        is consumption["operation_bound"]
        is consumption["request_fingerprint_bound"]
        is True
    )
    assert admission["replay"]["same_proof_same_logical_operation_retry"].startswith(
        "IDEMPOTENT_SAME_PROOF"
    )
    ...
    assert (
        admission["result"]["production_status"]
        == "PRODUCTION_ISSUER_ENTITLEMENT_AND_CLAIMANT_AUTHORITY_NOT_AVAILABLE"
    )
    assert (
        issuer["implementation_status"]["RootProofIssuer_implementation_allowed_by_this_artifact"]
        is False
    )


AUTHORIZATION_MUTATIONS = {
    "issuer_consumption_model_vs_admission_not_frozen": (
        "admission",
        "frozen_inputs.root_proof_consumption_semantics",
        "NOT_FROZEN",
    ),
    "admission_allows_second_operation": (
        "admission",
        "consumption_semantics.max_logical_operations",
        2,
    ),
    "admission_requires_post_issuance_consumed": (
        "admission",
        "consumption_semantics.post_issuance_CONSUMED_state",
        True,
    ),
    "public_entitlement_id_is_authorization": (
        "issuer",
        "claimant_authorization.public_entitlement_id_is_authorization",
        True,
    ),
    "wrong_claimant_with_handle_binds": (
        "issuer",
        "claimant_authorization.authorization_rule",
        "handle wins",
    ),
    "unauthenticated_issuance_requester": (
        "issuer",
        "issuance_request.requester_authentication",
        "NONE",
    ),
    "unauthenticated_initial_binding_tuple": (
        "issuer",
        "initial_binding_authenticity.caller_supplied_tuple_alone_sufficient",
        True,
    ),
    "wrong_cha_requester_key": ("issuer", "initial_binding_authenticity.key_rule", "any key"),
    "issuance_request_cross_domain_replay": (
        "issuer",
        "issuance_request.cross_protocol_replay",
        True,
    ),
    "entitlement_subject_changed_before_bind": (
        "issuer",
        "bootstrap_entitlement.claimant_key_binding",
        "mutable after lookup",
    ),
    "unauthorized_first_writer_wins": (
        "issuer",
        "concurrency.unauthorized_first_bind_possible",
        True,
    ),
    "caller_selected_entitlement_alias": (
        "issuer",
        "bootstrap_entitlement.alias_rule",
        "caller selects target",
    ),
    "bind_before_claimant_authorization": (
        "issuer",
        "bind_authorization_order",
        ["1_BIND", "2_AUTHORIZE"],
    ),
    "lost_response_different_tuple_returns_old_proof": (
        "issuer",
        "lost_response.different_attempt_id",
        "RETURN_OLD_PROOF",
    ),
    "test_request_against_production_entitlement": (
        "issuer",
        "bootstrap_entitlement.test_production_separation",
        "TEST MAY AUTHORIZE PRODUCTION",
    ),
}
...
def validate_authorization(value: dict) -> None:
    ...
    assert (
        request["requester_credential_model"]
        == "DISTINCT_CHA_ROOT_PROOF_ISSUANCE_REQUESTER_CREDENTIAL"
    )
    ...
    assert (
        request["domain_literal"] == "CRYPTOHUNTER_ACCOUNT_GENESIS_ROOT_PROOF_ISSUANCE_REQUEST_V1"
    )
    ...
    assert value["bind_authorization_order"][:4] == [
        "1_RESOLVE_AUTHORITATIVE_ENTITLEMENT",
        "2_AUTHENTICATE_ISSUANCE_REQUESTER",
        "3_VERIFY_ENTITLEMENT_CLAIMANT_AUTHORIZATION",
        "4_VERIFY_GENUINE_INITIAL_BINDING_EXACT_TUPLE",
    ]
    assert value["lost_response"]["different_attempt_id"].startswith(
        "FAIL_CLOSED / NOT_EXACT_RETRY"
    )
...
ROLE_EVIDENCE_MUTATIONS = {
    "freshness_proposer_as_issuance_requester": (
        "issuer",
        "freshness_proposer_role_parity.freshness_proposer_accepted_as_root_proof_requester",
        True,
    ),
    "issuance_requester_as_freshness_proposer": (
        "freshness",
        "proposer_authentication_boundary.proposer_key_role",
        "also root-proof requester",
    ),
    "same_key_material_two_aliases": (
        "issuer",
        "issuance_request.key_material_reuse_with_freshness_proposer",
        True,
    ),
    "requester_namespace_aliases_freshness_proposer": (
        "issuer",
        "freshness_proposer_role_parity.key_id_namespace_shared",
        True,
    ),
    "same_custody_role_is_sufficient": (
        "issuer",
        "issuance_request.custody_reuse_with_freshness_proposer",
        True,
    ),
    "revoked_proposer_accepted_as_requester": (
        "issuer",
        "issuance_request.lifecycle.REVOKED",
        "may authenticate new request",
    ),
    "verify_only_requester_authenticates_new_request": (
        "issuer",
        "issuance_request.lifecycle.VERIFY_ONLY",
        "may authenticate new request",
    ),
    "prepared_root_proof_id_only": ("admission", "validation_output.fields", ["root_proof_id"]),
    "evidence_omits_root_proof_signed_digest": (
        "admission",
        "validation_output.fields",
        ["schema_version"],
    ),
    "evidence_omits_initial_binding_digest_reference": (
        "admission",
        "validation_output.fields",
        ["root_proof_signed_payload_digest_sha256"],
    ),
    "evidence_omits_entitlement_generation": (
        "admission",
        "validation_output.fields",
        ["initial_binding_digest_sha256"],
    ),
    "evidence_omits_claimant_principal_key": (
        "admission",
        "validation_output.fields",
        ["entitlement_generation"],
    ),
    "evidence_omits_requester_key_identity": (
        "admission",
        "validation_output.fields",
        ["provisioning_principal_id"],
    ),
    "evidence_omits_issuer_history_decision": (
        "admission",
        "validation_output.fields",
        ["cha_root_proof_issuance_requester_credential_id"],
    ),
    "current_issuer_state_substitutes_history": (
        "admission",
        "admission_recovery.current_issuer_state_substitutes_retained_history",
        True,
    ),
    "caller_reconstructs_evidence_after_crash": (
        "admission",
        "admission_recovery.caller_may_reconstruct_missing_evidence",
        True,
    ),
    "evidence_o1_acct_a_reused_o2_acct_b": (
        "admission",
        "admission_recovery.continue_PREPARED_only_if",
        "may mismatch operation/account",
    ),
    "validation_record_claims_committed": (
        "admission",
        "validation_output.implies_COMMITTED",
        True,
    ),
    "validation_record_claims_freshness": (
        "admission",
        "validation_output.implies_freshness",
        True,
    ),
    "admission_final_owner_not_found": (
        "admission",
        "authority_boundary.genesis_final_decision_owner",
        "NOT_FOUND",
    ),
    "operation_reason_root_proof_timing_blocked": (
        "operation",
        "result.reason",
        "root-proof timing semantics remain blocked",
    ),
    "validation_output_design_blocked": ("admission", "validation_output.owner", "DESIGN_BLOCKED"),
    "admission_evidence_not_found": (
        "admission",
        "existing_candidate_inventory.AccountGenesis_admission_evidence.object",
        "NOT_FOUND",
    ),
}
...
def validate_role_and_prepared_evidence(
    ...
    assert (
        proposer["proposer_key_role"]
        == "authenticate origin of new candidate as CryptoHunterAccountAuthority only"
    )
    assert (
        request["requester_credential_model"]
        == "DISTINCT_CHA_ROOT_PROOF_ISSUANCE_REQUESTER_CREDENTIAL"
    ...
    assert (
        parity["key_material_shared"]
        is parity["custody_shared"]
        is parity["key_id_namespace_shared"]
        is False
    )
    ...
    assert (
        len(
            {
                request["domain_literal"],
                issuer["crypto"]["domain_literal"],
                issuer["claimant_authorization"]["domain_literal"],
                proposer["domain_literal"],
            }
        )
        == 4
    )
    ...
    required = {
        ...
        "root_proof_id",
        "root_proof_signed_payload_digest_sha256",
        "root_proof_signature_or_immutable_signed_proof_reference",
        "root_proof_issuer_id",
        "issuer_key_id",
        "issuer_key_version",
        "bootstrap_entitlement_id",
        "entitlement_generation",
        "bound_decision_identity_or_reference",
        "provisioning_principal_id",
        "claimant_key_id",
        "claimant_key_version",
        "cha_root_proof_issuance_requester_credential_id",
        "cha_root_proof_issuance_requester_key_version",
        "issuer_retained_history_decision_reference",
        "lifecycle_checkpoint_reference",
        "validation_policy_profile_version",
        "validation_outcome",
    }
    ...
    assert recovery["continue_PREPARED_only_if"].startswith(
        "durable RootProofAdmissionEvidenceV1 exactly matches"
    )
    assert (
        admission["existing_candidate_inventory"]["AccountGenesis_admission_evidence"]["object"]
        == "RootProofAdmissionEvidenceV1"
    )
    assert (
        admission["authority_boundary"]["genesis_final_decision_owner"]
    ...
    assert (
        topology["root_proof_validation_role"]["validation_result_is_final_genesis_decision"]
        is False
    )
    assert "accepted root-proof validation evidence and provenance" in {
        row["record"] for row in physical["authoritative_records"]
    }
    assert (
        "accepted root-proof validation evidence/reference with exact provenance"
        in physical["local_transaction_contract"]["PREPARED_atomic_records"]
    )
    reason = operation["result"]["reason"]
    assert "root-proof timing semantics remain blocked" not in reason
    assert (
        operation["result"]["root_proof_timing_status"]
        == "FROZEN_AFTER_INITIAL_BINDING_BEFORE_PREPARED"
    )
    assert (
        readiness["implementation_scope"][
            "FreshnessAuthority_implementation_allowed_after_iteration"
        ]
        is True
    )
...
def test_freshness_role_and_prepared_evidence_cross_artifact_contract() -> None:
    ...
    assert set(issuer["key_role_redteam_mutations"]) | set(
        admission["admission_evidence_redteam_mutations"]
    ) == set(ROLE_EVIDENCE_MUTATIONS)
...
ROTATION_RECOVERY_MUTATIONS = {
    "send_before_durable_attempt": (
        "physical",
        "root_proof_issuance_attempt_recovery.persist_before_send",
        False,
    ),
    "crash_loses_signed_request": (
        "issuer",
        "root_proof_issuance_attempt.fields",
        ["issuance_attempt_id"],
    ),
    "requester_verify_only_cannot_recover_bound": (
        "issuer",
        "credential_rotation_recovery.EXACT_ALREADY_BOUND_RECOVERY.requester_eligibility",
        "ACTIVE only",
    ),
    "requester_verify_only_creates_unbound_bind": (
        "issuer",
        "credential_rotation_recovery.NEW_BIND_AUTHORIZATION.requester_eligibility",
        "ACTIVE or VERIFY_ONLY",
    ),
    "claimant_verify_only_prevents_bound_recovery": (
        "issuer",
        "credential_rotation_recovery.EXACT_ALREADY_BOUND_RECOVERY.claimant_eligibility",
        "ACTIVE only",
    ),
    "claimant_verify_only_claims_unbound": (
        "issuer",
        "credential_rotation_recovery.NEW_BIND_AUTHORIZATION.claimant_eligibility",
        "ACTIVE or VERIFY_ONLY",
    ),
    "current_requester_substitutes_historical": (
        "issuer",
        "credential_rotation_recovery.current_key_substitution",
        True,
    ),
    "current_claimant_substitutes_historical": (
        "issuer",
        "credential_rotation_recovery.claimant_rotation",
        "use current claimant silently",
    ),
    "reconstruct_missing_requester_signature": (
        "issuer",
        "root_proof_issuance_attempt.fields",
        ["claimant_authorization_signature_bytes"],
    ),
    "reconstruct_missing_claimant_signature": (
        "issuer",
        "root_proof_issuance_attempt.fields",
        ["requester_signature_bytes"],
    ),
    "new_attempt_without_authoritative_reconciliation": (
        "issuer",
        "lost_response_reconciliation.authoritative_read_required",
        False,
    ),
    "supersede_without_authoritative_unbound": (
        "issuer",
        "lost_response_reconciliation.AUTHORITATIVELY_UNBOUND",
        "supersede without read",
    ),
    "prefer_newest_over_bound_winner": (
        "issuer",
        "lost_response_reconciliation.BOUND_TO_ANOTHER_DURABLE_ATTEMPT_SAME_OPERATION",
        "prefer newest",
    ),
    "adopt_bound_different_tuple": (
        "issuer",
        "lost_response_reconciliation.BOUND_TO_DIFFERENT_OPERATION_ACCOUNT_REQUEST",
        "ADOPT",
    ),
    "verify_only_recovery_is_new_authorization": (
        "issuer",
        "credential_rotation_recovery.EXACT_ALREADY_BOUND_RECOVERY.creates_new_bound_decision",
        True,
    ),
    "revoked_requester_signature_alone": (
        "issuer",
        "credential_rotation_recovery.REVOKED_historical.signature_alone_sufficient",
        True,
    ),
    "revoked_claimant_signature_alone": (
        "issuer",
        "credential_rotation_recovery.REVOKED_historical.missing_evidence",
        "ACCEPT signature",
    ),
    "admission_omits_winning_attempt": ("admission", "validation_output.fields", ["root_proof_id"]),
    "attempt_id_is_operation_id": (
        "issuer",
        "root_proof_issuance_attempt.identity_separation.is_logical_operation_identity",
        True,
    ),
    "process_memory_reconstructs_attempt": (
        "issuer",
        "root_proof_issuance_attempt.reconstruction_sources_forbidden",
        ["caller input"],
    ),
}
...
def validate_rotation_recovery(issuer: dict, admission: dict, physical: dict) -> None:
    ...
    assert rotation["operations_are_distinct"] == [
        "NEW_BIND_AUTHORIZATION",
        "EXACT_ALREADY_BOUND_RECOVERY",
    ]
    new = rotation["NEW_BIND_AUTHORIZATION"]
    assert new["authoritative_precondition"].endswith("authoritatively UNBOUND")
    assert (
        new["requester_eligibility"]
        == new["claimant_eligibility"]
        == new["issuer_signing_key_eligibility"]
        == "ACTIVE only"
    )
    ...
    required = {
        ...
        "claimant_key_id",
        "claimant_key_version",
        "root_proof_issuance_request_signed_payload",
        "canonical_signed_payload_bytes_or_immutable_reference",
        "signed_payload_digest_sha256",
        "requester_signature_bytes",
        "claimant_authorization_signature_bytes",
        "issuance_request_domain_and_profile_version",
        "claimant_authorization_domain_and_profile_version",
        "attempt_state",
    }
    ...
    assert set(attempt["reconstruction_sources_forbidden"]) == {
        "caller input",
        "current requester key",
        "current claimant key",
    ...
    assert reconcile["AUTHORITATIVELY_UNBOUND"].startswith(
        "retry original only if original requester and claimant remain ACTIVE"
    )
    assert (
        "actual authoritative BOUND winner"
        in reconcile["BOUND_TO_ANOTHER_DURABLE_ATTEMPT_SAME_OPERATION"]
    )
    ...
    assert {
        "winning_root_proof_issuance_attempt_id",
        "winning_root_proof_issuance_attempt_digest_sha256",
    } <= fields
    assert admission["validation_output"]["winning_attempt_rule"].startswith(
        "must reference exact actual issuer BOUND winner"
    )
    assert admission["admission_recovery"]["winning_attempt"].startswith(
        "actual authoritative BOUND attempt controls"
    )
    ...
    assert (
        physical["result"]["selected_protocol"]
        == "INITIAL_BINDING -> PREPARED -> EXTERNAL_FRESHNESS_CAS -> LOCAL_FINAL_COMMIT"
    )
    ...
    assert by_case["H_BOUND_OLD_ATTEMPT_NEWER_LOCAL_EXISTS"]["winner"].startswith(
        "old actual authoritative BOUND"
    )
...
ATTEMPT_IDENTITY_MUTATIONS = {
    "caller_selects_issuance_attempt_id": (
        "issuer",
        "root_proof_issuance_attempt_identity.issuance_attempt_id.caller_selected",
        True,
    ),
    "replacement_reuses_old_attempt_id": (
        "issuer",
        "root_proof_issuance_attempt_identity.replacement.new_issuance_attempt_id_required",
        False,
    ),
    "same_attempt_id_different_immutable_payload": (
        "issuer",
        "root_proof_issuance_attempt_identity.issuance_attempt_id.same_id_unequal_payload",
        "ACCEPT",
    ),
    "attempt_digest_includes_mutable_attempt_state": (
        "issuer",
        "root_proof_issuance_attempt_identity.mutable_fields_excluded",
        ["retry_counters"],
    ),
    "legal_state_transition_changes_attempt_digest": (
        "issuer",
        "root_proof_issuance_attempt_identity.conformance_vector.state_changes_digest",
        True,
    ),
    "attempt_digest_is_attempt_id_only": (
        "issuer",
        "root_proof_issuance_attempt_identity.digest.meaning",
        "hash ID only",
    ),
    "attempt_digest_is_request_digest_only": (
        "issuer",
        "root_proof_issuance_attempt_identity.digest.meaning",
        "request digest only",
    ),
    "attempt_digest_omits_initial_binding_digest": (
        "issuer",
        "root_proof_issuance_attempt_identity.immutable_fields",
        ["issuance_attempt_id"],
    ),
    "attempt_digest_omits_entitlement_generation": (
        "issuer",
        "root_proof_issuance_attempt_identity.immutable_fields",
        ["initial_binding_digest_sha256"],
    ),
    "attempt_digest_omits_requester_key_version": (
        "issuer",
        "root_proof_issuance_attempt_identity.immutable_fields",
        ["entitlement_generation"],
    ),
    "attempt_digest_omits_claimant_key_version": (
        "issuer",
        "root_proof_issuance_attempt_identity.immutable_fields",
        ["requester_key_version"],
    ),
    "attempt_digest_omits_requester_signature": (
        "issuer",
        "root_proof_issuance_attempt_identity.immutable_fields",
        ["claimant_key_version"],
    ),
    "attempt_digest_omits_claimant_signature": (
        "issuer",
        "root_proof_issuance_attempt_identity.immutable_fields",
        ["requester_signature_base64url"],
    ),
    "attempt_digest_uses_noncanonical_process_specific_serialization": (
        "issuer",
        "root_proof_issuance_attempt_identity.digest.canonicalization",
        "native JSON",
    ),
    "attempt_digest_has_no_object_domain_separator": (
        "issuer",
        "root_proof_issuance_attempt_identity.digest.preimage",
        "JCS(payload)",
    ),
    "attempt_digest_reuses_root_proof_domain": (
        "issuer",
        "root_proof_issuance_attempt_identity.digest.domain_literal",
        "CRYPTOHUNTER_ACCOUNT_GENESIS_ROOT_PROOF_V1",
    ),
    "admission_evidence_uses_mutable_row_digest": (
        "admission",
        "validation_output.winning_attempt_digest_semantics.exact_meaning",
        "mutable row digest",
    ),
    "admission_evidence_digest_not_equal_winning_attempt_identity_digest": (
        "admission",
        "validation_output.winning_attempt_digest_semantics.consistency",
        "need not match",
    ),
    "supersession_rewrites_old_immutable_attempt_payload": (
        "issuer",
        "root_proof_issuance_attempt_identity.replacement.old_immutable_payload_rewritten",
        True,
    ),
    "current_attempt_digest_substituted_for_authoritative_winner_digest": (
        "admission",
        "validation_output.winning_attempt_rule",
        "use current attempt",
    ),
}
...
def validate_attempt_identity(issuer: dict, admission: dict, physical: dict) -> None:
    ...
    assert (
        attempt_id["issuer"]
        == "CryptoHunterAccountAuthority local AccountGenesis authority boundary"
    )
    assert attempt_id["representation"] == "rpa_<canonical lowercase UUIDv7>"
    assert (
        attempt_id["caller_selected"]
        is attempt_id["process_selected_unauthenticated"]
        is attempt_id["reusable"]
        is False
    )
    ...
    required = {
        ...
        "claimant_key_id",
        "claimant_key_version",
        "root_proof_issuance_request_signed_payload_digest_sha256",
        "root_proof_issuance_request_canonical_bytes_reference",
        "requester_signature_base64url",
        "claimant_authorization_signature_base64url",
        "issuance_request_domain_and_profile_version",
        "claimant_authorization_domain_and_profile_version",
    }
    assert set(identity["immutable_fields"]) == required
    assert set(identity["mutable_fields_excluded"]) == {
        "attempt_state",
        "wall_clock_timestamps",
        "retry_counters",
        "diagnostic_text",
        "transport_error_strings",
        "current_process_metadata",
    }
    digest = identity["digest"]
    assert digest["meaning"].startswith(
        "SHA-256 digest of RootProofIssuanceAttemptIdentityPayloadV1 only"
    )
    assert (
        digest["canonicalization"],
        digest["text_encoding"],
        digest["algorithm"],
        digest["representation"],
    ) == ("RFC 8785 / JCS", "UTF-8", "SHA-256", "lowercase 64-character hexadecimal")
    assert (
        digest["domain_literal"]
        == "CRYPTOHUNTER_ACCOUNT_GENESIS_ROOT_PROOF_ISSUANCE_ATTEMPT_IDENTITY_V1"
    )
    assert (
        digest["preimage"]
        == "ASCII(domain_literal) || 0x00 || UTF8(JCS(RootProofIssuanceAttemptIdentityPayloadV1))"
    )
    assert digest["domain_literal"] not in {
        issuer["crypto"]["domain_literal"],
        issuer["issuance_request"]["domain_literal"],
        issuer["claimant_authorization"]["domain_literal"],
    }
    replacement = identity["replacement"]
    assert (
        replacement["authoritative_UNBOUND_required"]
        is replacement["old_id_and_digest_preserved"]
        is replacement["new_issuance_attempt_id_required"]
        is replacement["new_immutable_digest_required"]
        is True
    )
    ...
    assert semantics["exact_meaning"].startswith(
        "SHA-256 immutable RootProofIssuanceAttemptIdentityPayloadV1 digest"
    )
    assert set(semantics["forbidden_meanings"]) == {
        "mutable attempt row digest",
        "attempt_state digest",
        "issuance_attempt_id hash",
        "request digest alone",
    ...
    recovery = physical["root_proof_issuance_attempt_recovery"]
    assert recovery["persist_before_send"] is True
    assert recovery["identity_contract"].startswith(
        "authority-issued rpa_<canonical lowercase UUIDv7>"
    )
    assert recovery["replacement"].startswith(
        "authoritative UNBOUND first creates a new durable reservation with NOT_YET_DEFINED digest"
    )
    assert (
        physical["result"]["selected_protocol"]
        == "INITIAL_BINDING -> PREPARED -> EXTERNAL_FRESHNESS_CAS -> LOCAL_FINAL_COMMIT"
...
END_TO_END_ATTEMPT_MUTATIONS = {
    "issuance_request_omits_attempt_id": (
        "issuer",
        "issuance_request.signed_payload_fields",
        ["schema_version"],
    ),
    "requester_signature_does_not_cover_attempt_id": (
        "issuer",
        "issuance_request.attempt_id_binding.requester_signature_covers",
        False,
    ),
    "claimant_signature_does_not_cover_attempt_id": (
        "issuer",
        "issuance_request.attempt_id_binding.claimant_signature_covers",
        False,
    ),
    "issuer_bound_history_omits_attempt_id": (
        "issuer",
        "issuer_bound_attempt_linkage.bound_history_field",
        "NONE",
    ),
    "same_external_request_two_attempt_ids_treated_exact_retry": (
        "issuer",
        "issuer_bound_attempt_linkage.different_attempt_id_same_other_fields",
        "EXACT_RETRY",
    ),
    "admission_attempt_id_differs_from_issuer_bound_attempt_id": (
        "admission",
        "validation_output.winning_attempt_end_to_end_consistency.admission_winning_attempt_id",
        "MAY DIFFER",
    ),
    "admission_digest_for_attempt_B_with_issuer_bound_attempt_A": (
        "admission",
        "validation_output.winning_attempt_end_to_end_consistency.immutable_digest",
        "any local digest",
    ),
    "two_CHA_processes_create_two_current_attempts_same_tuple": (
        "issuer",
        "local_current_attempt_fence.crash_two_current_same_tuple",
        True,
    ),
    "process_local_mutex_used_as_creation_fence": (
        "issuer",
        "local_current_attempt_fence.global_serialization",
        "process-local mutex",
    ),
    "current_attempt_fence_not_durable": (
        "physical",
        "root_proof_issuance_attempt_recovery.reservation_atomicity",
        "best effort",
    ),
    "replacement_switch_not_atomic": (
        "issuer",
        "local_current_attempt_fence.replacement_reservation_switch_atomic",
        False,
    ),
    "replacement_reuses_old_signed_request_with_new_attempt_id": (
        "issuer",
        "local_current_attempt_fence.AUTHORITATIVELY_UNBOUND_REPLACEMENT_RESERVATION",
        "rewrite old request",
    ),
    "delayed_old_attempt_wins_but_newest_local_selected": (
        "issuer",
        "local_current_attempt_fence.AUTHORITATIVELY_BOUND",
        "newest local wins",
    ),
    "new_attempt_wins_but_old_local_attempt_selected": (
        "issuer",
        "attempt_creation_concurrency_matrix",
        [],
    ),
    "issuer_history_cannot_distinguish_attempts_but_PREPARED_allowed": (
        "issuer",
        "issuer_bound_attempt_linkage.ambiguous_history",
        "ALLOW PREPARED",
    ),
    "exact_retry_identity_ignores_attempt_id": (
        "issuer",
        "issuer_bound_attempt_linkage.exact_retry_fields",
        ["logical_operation_id"],
    ),
    "caller_can_override_bound_attempt_id": (
        "issuer",
        "issuer_bound_attempt_linkage.caller_may_override_bound_attempt_id",
        True,
    ),
    "current_pointer_overrides_authoritative_bound_winner": (
        "admission",
        "validation_output.winning_attempt_end_to_end_consistency.current_pointer_is_authoritative",
        True,
    ),
    "crash_leaves_two_current_attempts": (
        "physical",
        "root_proof_issuance_attempt_recovery.crash_two_current_attempts_same_tuple",
        "ALLOWED",
    ),
    "equivalent_request_content_substitutes_different_attempt_identity": (
        "admission",
        "validation_output.winning_attempt_end_to_end_consistency.signed_request_identity",
...
def validate_end_to_end_attempt_binding(issuer: dict, admission: dict, physical: dict) -> None:
    ...
    assert (
        binding["required"]
        is binding["requester_signature_covers"]
        is binding["claimant_signature_covers"]
        is True
    )
    ...
    assert (
        linkage["requester_signature_covers_attempt_id"]
        is linkage["claimant_signature_covers_attempt_id"]
        is linkage["root_proof_signed_payload_covers_attempt_id"]
        is True
    )
    ...
    assert set(linkage["winner_selection_forbidden"]) == {
        "newest local record",
        "process identity",
        "timestamps",
        "insertion order",
        "current attempt pointer",
        "equivalent request content",
    }
    ...
    assert set(fence["exact_idempotency_key_fields"]) == {
        "environment",
        "trust_domain",
        "product_scope",
        "logical_operation_id",
    ...
    assert set(fence["forbidden_idempotency_inputs"]) == {
        "wall-clock time",
        "thread id",
        "process id",
        "caller-generated random token",
    }
    assert fence["EXACT_CURRENT_RESERVED"].startswith(
        "return same authority-owned issuance_attempt_id"
    )
    assert "NOT_YET_DEFINED" in fence["EXACT_CURRENT_RESERVED"]
    assert fence["EXACT_CURRENT_IMMUTABLE"].startswith(
        "return same issuance_attempt_id, same DEFINED immutable digest"
    )
    assert fence["CURRENT_OUTCOME_AMBIGUOUS"].startswith(
        "no replacement before authoritative reconciliation"
    )
    assert fence["AUTHORITATIVELY_UNBOUND_REPLACEMENT_RESERVATION"].startswith(
        "after authenticated authoritative UNBOUND reconciliation"
    )
    assert fence["AUTHORITATIVELY_BOUND"].startswith(
        "issuer_history.bound_issuance_attempt_id wins"
    )
    assert fence["crash_two_current_same_tuple"] is False
    assert (
        fence["replacement_reservation_switch_atomic"]
        is fence["superseded_history_retained"]
        is True
    )
    matrix = issuer["attempt_creation_concurrency_matrix"]
    assert len(matrix) == 5
    assert matrix[0]["result"].startswith(
        "one authority-owned rpa_id and one RootProofIssuanceAttemptReservationV1 installed"
    )
    assert matrix[0]["immutable_digest_status"] == "NOT_YET_DEFINED"
    assert (
        matrix[0]["requester_signature_present"] is matrix[0]["claimant_signature_present"] is False
    )
    ...
    assert (
        consistency["admission_winning_attempt_id"]
        == "MUST equal issuer_history.bound_issuance_attempt_id"
    )
    assert consistency["local_attempt_id"] == "MUST equal admission and issuer history ID"
    assert consistency["immutable_digest"].startswith(
        "MUST resolve the local immutable identity payload for that exact ID"
    )
    assert consistency["signed_request_identity"].startswith(
        "MUST exactly match issuer-retained signed RootProofIssuanceRequest identity"
    )
    assert consistency["current_pointer_is_authoritative"] is False

    recovery = physical["root_proof_issuance_attempt_recovery"]
    assert recovery["reservation_atomicity"].startswith(
        "current fence + RootProofIssuanceAttemptReservationV1 creation/install are one"
    )
    assert recovery["immutable_finalization_atomicity"].startswith(
        "later local authority transaction verifies exact current non-superseded reservation"
    )
    assert recovery["replacement_reservation_switch_atomicity"].startswith(
        "old supersession + durable exact authenticated authoritative-UNBOUND evidence"
    )
    assert recovery["replacement_immutable_finalization_atomicity"].startswith(
        "later reuse immutable_finalization_atomicity"
    )
    assert recovery["crash_two_current_attempts_same_tuple"] == "FORBIDDEN"
    assert recovery["authority_winner"].startswith(
        "issuer_history.bound_issuance_attempt_id overrides"
    )
    assert recovery["persist_before_send"] is True
    assert (
        physical["result"]["selected_protocol"]
...
class SemanticCurrentAttemptAuthority:
    ...
    def replace_after_authoritative_unbound(
        ...
        exact = {
            ...
            "canonical_genesis_request_fingerprint_sha256": old_request[
                "canonical_genesis_request_fingerprint_sha256"
            ],
        ...
        assert all(evidence[field] == value for field, value in exact.items()), (
            "FAIL_CLOSED / EVIDENCE_MISMATCH"
        )
        assert evidence["authoritative_state_revision"] == self.authority_revision, (
            "FAIL_CLOSED / STALE_EVIDENCE"
        )
        ...
        if current["attempt_id"] != expected_old_id:
            if (
                current["exact_key"] == replacement_key
                and current["reconciliation_evidence_digest"]
        ...
        old_history["replacement_authorization_evidence_reference"] = evidence[
            "authority_authenticated_evidence_reference"
        ]
        old_history["replacement_authorization_evidence_digest_sha256"] = evidence[
            "authority_authenticated_evidence_digest_sha256"
        ]
        ...
        self.current_attempt_by_operation[slot] = {
            ...
            "reconciliation_evidence_reference": evidence[
                "authority_authenticated_evidence_reference"
            ],
    ...
    def external_send_eligible(self, request: dict) -> bool:
        ...
        return (
            current["state"] == "SIGNED_IMMUTABLE_DURABLE_NOT_SENT"
            and current["digest"] is not None
        )
...
def reconciliation_evidence_fixture(request: dict, old_id: str, **overrides: object) -> dict:
    evidence = {
        ...
        "canonical_genesis_request_fingerprint_sha256": request[
            "canonical_genesis_request_fingerprint_sha256"
        ],
...
def test_current_attempt_authority_isolation_and_ambiguous_change() -> None:
    ...
    operation_2 = semantic_authorization_fixture(
        logical_operation_id="ago_018f3e70-7b6b-7c21-8b9a-0123456789ab"
    )
    ...
    for change in variants:
        ...
        assert derive_exact_idempotency_key(contract, changed) != derive_exact_idempotency_key(
            contract, base
        )
...
def test_end_to_end_attempt_binding_contract() -> None:
    ...
    assert set(issuer["end_to_end_attempt_binding_redteam_mutations"]) == set(
        END_TO_END_ATTEMPT_MUTATIONS
    )
...
def test_end_to_end_attempt_binding_mutations_rejected(name: str) -> None:
    ...
    assert values[artifact] != original
    with pytest.raises(AssertionError):
        validate_end_to_end_attempt_binding(
            values["issuer"], values["admission"], values["physical"]
        )


CAS_LOST_RESPONSE_MUTATIONS = {
    "idempotency_key_uses_field_names_only": (
        "local_current_attempt_fence.idempotency_key_derivation",
        "tuple of field names",
    ),
    "idempotency_key_ignores_field_values": (
        "local_current_attempt_fence.field_names_only_key_allowed",
        True,
    ),
    "all_operations_share_one_current_attempt_slot": (
        "local_current_attempt_fence.global_singleton_current_attempt_registry",
        True,
    ),
    "different_operation_reuses_existing_attempt": (
        "local_current_attempt_fence.different_operation_slots_isolated",
        False,
    ),
    "different_account_reuses_existing_attempt": (
        "local_current_attempt_fence.different_account_may_converge",
        True,
    ),
    "changed_request_fingerprint_treated_exact_same": (
        "local_current_attempt_fence.exact_idempotency_key_fields",
        ["logical_operation_id"],
    ),
    "changed_entitlement_generation_treated_exact_same": (
        "local_current_attempt_fence.exact_idempotency_key_fields",
        ["canonical_genesis_request_fingerprint_sha256"],
    ),
    "changed_requester_key_version_treated_exact_same": (
        "local_current_attempt_fence.exact_idempotency_key_fields",
        ["entitlement_generation"],
    ),
    "changed_claimant_key_version_treated_exact_same": (
        "local_current_attempt_fence.exact_idempotency_key_fields",
        ["requester_key_version"],
    ),
    "changed_credentials_create_second_current_before_reconciliation": (
        "local_current_attempt_fence.changed_exact_authorization_may_bypass_current_fence",
        True,
    ),
    "process_local_dictionary_singleton_claimed_as_authority": (
        "local_current_attempt_fence.process_local_dictionary_is_authority",
        True,
    ),
    "candidate_loser_remains_externally_sendable": (
        "local_current_attempt_fence.candidate_loser_externally_sendable",
        True,
    ),
    "lost_response_ignores_attempt_id": (
        "lost_response.recovery_requires_fields",
        ["logical_operation_id"],
    ),
    "lost_response_different_attempt_id_returns_old_proof": (
        "lost_response.different_attempt_id",
        "RETURN OLD PROOF",
    ),
    "bound_recovery_does_not_compare_bound_attempt_id": (
        "lost_response.requested_attempt_id_must_equal",
        "no comparison",
    ),
    "semantic_equivalence_substitutes_attempt_identity": (
        "lost_response.semantic_equivalence_substitutes_attempt_identity",
        True,
    ),
    "current_pointer_attempt_id_substitutes_bound_attempt_id": (
        "lost_response.current_pointer_substitutes_bound_attempt_id",
        True,
    ),
}
...
def validate_cas_and_lost_response(contract: dict) -> None:
    ...
    assert fence["operation_slot_key_fields"] == [
        "environment",
        "trust_domain",
        "logical_operation_id",
    ]
    ...
    assert {
        ...
        "initial_binding_reference",
        "initial_binding_digest_sha256",
        "signed_root_proof_issuance_request_identity",
    } <= required
...
def test_lost_response_same_attempt_succeeds_and_different_attempt_rejects() -> None:
    ...
    assert (
        recover_lost_response(contract, attempt_a, attempt_a) == "BYTE_IDENTICAL_PROOF_AND_HISTORY"
    )
...
def test_cas_and_lost_response_contract() -> None:
    contract = load()
    validate_cas_and_lost_response(contract)
    assert set(contract["cas_and_lost_response_redteam_mutations"]) == set(
        CAS_LOST_RESPONSE_MUTATIONS
    )
...
RESERVATION_SEQUENCING_MUTATIONS = {
    "process_supplies_candidate_rpa_id_to_authority": (
        "root_proof_issuance_attempt_reservation.process_proposes_candidate_rpa_id",
        True,
    ),
    "rpa_id_not_durable_before_requester_signature": (
        "root_proof_issuance_attempt_reservation.purpose",
        "sign then persist ID",
    ),
    "rpa_id_not_durable_before_claimant_signature": (
        "signed_attempt_finalization.ordered_steps",
        ["obtain claimant authorization signature", "load reservation"],
    ),
    "immutable_attempt_persist_required_in_same_tx_as_first_id_mint": (
        "root_proof_issuance_attempt_reservation.NO_CURRENT_atomic_transaction",
        ["persist immutable attempt"],
    ),
    "local_transaction_spans_external_claimant_signing": (
        "root_proof_issuance_attempt_reservation.transaction_excludes",
        ["external issuer calls"],
    ),
    "crash_after_id_mint_before_signature_remints_new_id": (
        "reservation_crash_recovery.reservation_durable_no_requester_signature",
        "remint",
    ),
    "crash_after_requester_signature_before_claimant_remints_new_id": (
        "reservation_crash_recovery.requester_signature_claimant_missing",
        "remint",
    ),
    "crash_after_both_signatures_before_attempt_persist_remints_new_id": (
        "reservation_crash_recovery.both_signatures_attempt_not_persisted",
        "remint",
    ),
    "losing_process_can_sign_non_authoritative_rpa_id": (
        "signed_attempt_finalization.signing_non_reserved_id_forbidden",
        False,
    ),
    "losing_process_candidate_digest_exists_before_authority_selection": (
        "root_proof_issuance_attempt_reservation.immutable_attempt_digest_present",
        True,
    ),
    "reservation_rpa_id_differs_from_signed_request_rpa_id": (
        "signed_attempt_finalization.completion_equality_fields",
        ["requester_key_version"],
    ),
    "reservation_requester_key_version_differs_from_completed_attempt": (
        "signed_attempt_finalization.completion_equality_fields",
        ["issuance_attempt_id"],
    ),
    "reservation_claimant_key_version_differs_from_completed_attempt": (
        "signed_attempt_finalization.completion_equality_fields",
        ["requester_key_version"],
    ),
    "reservation_entitlement_generation_differs_from_completed_attempt": (
        "signed_attempt_finalization.completion_equality_fields",
        ["claimant_key_version"],
    ),
    "external_send_allowed_from_reserved_only_state": (
        "signed_attempt_finalization.external_send_eligible_only_after",
        "RESERVED_AWAITING_SIGNATURES",
    ),
    "placeholder_attempt_digest_created_at_reservation": (
        "root_proof_issuance_attempt_reservation.placeholder_digest_forbidden",
        False,
    ),
    "pre_send_reservation_superseded_without_proof_send_was_impossible": (
        "pre_send_reservation_supersession.requires",
        [],
    ),
    "current_pointer_requires_completed_digest_before_it_can_reference_reserved_id": (
        "root_proof_issuance_attempt_reservation.current_pointer_may_reference_reserved_without_digest",
        False,
    ),
}
...
def validate_reservation_sequencing(issuer: dict, physical: dict) -> None:
    ...
    assert reservation["purpose"].startswith(
        "durably establish authority-owned issuance_attempt_id before any"
    )
    required = {
        "schema_version",
        "environment",
    ...
    assert (
        reservation["signatures_present"]
        is reservation["immutable_attempt_digest_present"]
        is False
    )
    ...
    assert reservation["authority_id_factory"].startswith(
        "inside durable CHA reservation transaction"
    )
    assert reservation["exact_retry"].startswith(
        "returns same durable authority-issued issuance_attempt_id"
    )
    assert reservation["current_pointer_may_reference_reserved_without_digest"] is True
    assert reservation["NO_CURRENT_atomic_transaction"] == [
        "derive per-operation slot from actual values",
        "derive exact idempotency identity",
        "verify no incompatible current/reservation",
        "mint authority-owned rpa_id",
        "persist RootProofIssuanceAttemptReservationV1",
        "install current_root_proof_issuance_attempt_id",
    ]
    assert set(reservation["transaction_excludes"]) == {
        "requester signing",
        "claimant signing",
        "remote calls",
        "non-local HSM/network interaction",
        "external issuer calls",
    }
    ...
    assert finalization["immutable_digest_defined_at"].startswith(
        "after exact request and both signatures"
    )
    ...
    assert crash["reservation_durable_no_requester_signature"].startswith(
        "recover same reserved rpa_id"
    )
    assert crash["requester_signature_claimant_missing"].startswith("recover same reserved rpa_id")
    assert crash["both_signatures_attempt_not_persisted"].startswith(
    ...
    assert set(supersession["requires"]) == {
        "proof no external send was possible",
        "proof no completed externally-sendable attempt exists",
        "proof no issuer BOUND decision can exist",
    }
    ...
    assert parity["two_step_local_model"] == [
...
def test_reservation_sequencing_contract() -> None:
    ...
    assert set(issuer["reservation_sequencing_redteam_mutations"]) == set(
        RESERVATION_SEQUENCING_MUTATIONS
    )
...
TWO_STEP_RECONCILIATION_MUTATIONS = {
    "exact_reserved_retry_requires_immutable_digest": (
        "issuer",
        "local_current_attempt_fence.EXACT_CURRENT_RESERVED",
        "digest required",
    ),
    "placeholder_digest_returned_for_reserved_retry": (
        "issuer",
        "current_attempt_resolution.reserved_semantics",
        "return zero digest",
    ),
    "reserved_current_lookup_treated_as_corrupt_because_digest_missing": (
        "issuer",
        "current_attempt_resolution.missing_digest_field",
        "NOT_YET_DEFINED is corrupt",
    ),
    "reservation_and_immutable_finalization_forced_same_transaction": (
        "issuer",
        "immutable_finalization_atomicity.transaction",
        "same reservation transaction",
    ),
    "physical_current_fence_claims_immutable_creation_same_tx": (
        "physical",
        "root_proof_issuance_attempt_recovery.reservation_atomicity",
        "fence + immutable persist same transaction",
    ),
    "initial_race_claims_digest_exists_before_signing": (
        "issuer",
        "attempt_creation_concurrency_matrix",
        [],
    ),
    "validator_requires_digest_for_RESERVED_AWAITING_SIGNATURES": (
        "issuer",
        "current_attempt_resolution.digest_status_union",
        ["DEFINED"],
    ),
    "validator_requires_old_one_step_current_attempt_atomicity": (
        "physical",
        "root_proof_issuance_attempt_recovery.immutable_finalization_atomicity",
        "same first transaction",
    ),
    "two_finalizers_same_rpa_unequal_payload_both_accept": (
        "issuer",
        "immutable_finalization_atomicity.same_rpa_unequal_identity",
        "ACCEPT",
    ),
    "stale_superseded_reservation_can_finalize": (
        "issuer",
        "immutable_finalization_atomicity.stale_superseded_may_finalize",
        True,
    ),
    "exact_duplicate_finalization_mints_new_digest_identity": (
        "issuer",
        "immutable_finalization_atomicity.same_rpa_identical_identity",
        "mint new digest",
    ),
    "pre_send_superseded_reservation_required_to_have_digest": (
        "issuer",
        "pre_send_reservation_supersession.historical_digest_semantics",
        "digest required",
    ),
}
...
def validate_two_step_reconciliation(issuer: dict, admission: dict, physical: dict) -> None:
    ...
    assert resolution["digest_status_union"] == [
        "NOT_YET_DEFINED",
        "DEFINED(<lowercase 64-character SHA-256 hex>)",
    ]
    assert resolution["reserved_semantics"].startswith("NOT_YET_DEFINED is authoritative and valid")
    assert set(resolution["forbidden_not_yet_defined_representations"]) == {
        "empty string",
        "zero digest",
        "all-zero digest",
        "synthetic placeholder hash",
        "ambiguous null",
    }
    assert (
        resolution["missing_digest_field"]
        == "MALFORMED / FAIL_CLOSED; distinct from explicit NOT_YET_DEFINED"
    )

    finalization = issuer["immutable_finalization_atomicity"]
    assert finalization["transaction"].startswith(
        "later local authority transaction distinct from reservation transaction"
    )
    ...
    assert outputs["after_both_signatures_before_finalization"]["digest_status"].startswith(
        "NOT_YET_DEFINED"
    )
    assert (
        outputs["after_immutable_finalization"]["digest_status"] == "DEFINED(same immutable digest)"
    )
    assert issuer["pre_send_reservation_supersession"]["historical_digest_semantics"].startswith(
        "superseded proven-unsendable reservation may permanently retain NOT_YET_DEFINED"
    )

    recovery = physical["root_proof_issuance_attempt_recovery"]
    assert "current_attempt_fence_atomicity" not in recovery
    assert recovery["reservation_atomicity"].startswith(
        "current fence + RootProofIssuanceAttemptReservationV1 creation/install"
    )
    assert recovery["immutable_finalization_atomicity"].startswith(
        "later local authority transaction"
    )
    ...
    assert (
        "winning_root_proof_issuance_attempt_digest_sha256"
        in admission["validation_output"]["fields"]
    )
    assert (
        physical["result"]["selected_protocol"]
        == "INITIAL_BINDING -> PREPARED -> EXTERNAL_FRESHNESS_CAS -> LOCAL_FINAL_COMMIT"
...
def test_immutable_finalization_race_and_superseded_history() -> None:
    ...
    old = {
        "attempt_id": "rpa_old",
        "state": "SUPERSEDED_PRE_SEND_PROVEN_UNSENDABLE",
        "digest": None,
    }
...
def test_two_step_reconciliation_contract() -> None:
    ...
    assert set(issuer["two_step_reconciliation_redteam_mutations"]) == set(
        TWO_STEP_RECONCILIATION_MUTATIONS
    )
...
REPLACEMENT_TWO_STEP_MUTATIONS = {
    "replacement_switch_persists_immutable_digest_in_first_transaction": (
        "issuer",
        "replacement_reservation_transition.immutable_attempt_present",
        True,
    ),
    "replacement_switch_requires_signed_request_before_new_rpa_reservation": (
        "issuer",
        "replacement_reservation_transition.signing_after_commit",
        False,
    ),
    "replacement_new_rpa_not_durable_before_requester_signature": (
        "issuer",
        "replacement_reservation_transition.requester_signature_present",
        True,
    ),
    "replacement_new_rpa_not_durable_before_claimant_signature": (
        "issuer",
        "replacement_reservation_transition.claimant_signature_present",
        True,
    ),
    "replacement_initial_digest_status_defined": (
        "issuer",
        "replacement_reservation_transition.immutable_attempt_digest_status",
        "DEFINED",
    ),
    "replacement_transaction_includes_requester_signing": (
        "physical",
        "root_proof_issuance_attempt_recovery.replacement_reservation_transaction_includes.requester_signing",
        True,
    ),
    "replacement_transaction_includes_claimant_signing": (
        "physical",
        "root_proof_issuance_attempt_recovery.replacement_reservation_transaction_includes.claimant_signing",
        True,
    ),
    "replacement_transaction_includes_immutable_attempt_persistence": (
        "physical",
        "root_proof_issuance_attempt_recovery.replacement_reservation_transaction_includes.immutable_attempt_persistence",
        True,
    ),
    "replacement_transaction_includes_external_send": (
        "physical",
        "root_proof_issuance_attempt_recovery.replacement_reservation_transaction_includes.external_send",
        True,
    ),
    "concurrent_replacements_install_two_current_reservations": (
        "issuer",
        "replacement_concurrency.two_current_replacements_possible",
        True,
    ),
    "losing_replacement_reservation_can_be_signed": (
        "issuer",
        "replacement_concurrency.loser_may_sign",
        True,
    ),
    "old_superseded_attempt_can_finalize_after_replacement_switch": (
        "issuer",
        "immutable_finalization_atomicity.stale_superseded_may_finalize",
        True,
    ),
    "crash_after_replacement_reservation_remints_new_rpa": (
        "issuer",
        "replacement_crash_recovery.R2_REPLACEMENT_RESERVED_BEFORE_REQUESTER_SIGNATURE_CRASH",
        "remint",
    ),
    "crash_between_replacement_signatures_remints_new_rpa": (
        "issuer",
        "replacement_crash_recovery.R3_REQUESTER_SIGNED_BEFORE_CLAIMANT_CRASH",
        "remint",
    ),
    "replacement_mutates_old_immutable_attempt_credentials": (
        "issuer",
        "replacement_concurrency.old_attempt_credentials_mutated",
        True,
    ),
    "replacement_current_pointer_switch_occurs_only_after_digest_exists": (
        "issuer",
        "replacement_reservation_transition.current_switch_requires_digest",
        True,
    ),
}
...
def validate_replacement_two_step(issuer: dict, physical: dict) -> None:
    ...
    assert (
        transition["requester_signature_present"]
        is transition["claimant_signature_present"]
        is transition["immutable_attempt_present"]
        is transition["external_send_eligible"]
        is False
    )
    assert transition["signing_after_commit"] is True
    assert (
        transition["immutable_finalization_after_signatures"]
        == "reuse immutable_finalization_atomicity"
    )
    assert transition["current_switch_requires_digest"] is False
    assert set(transition["old_history_never_rewritten"]) == {
        "old rpa_id",
        "old signed request",
        "old signatures",
        "old immutable digest if defined",
        "old send/outcome history",
        "authoritative reconciliation evidence",
    }

    crash = issuer["replacement_crash_recovery"]
    assert crash["R1_UNBOUND_PROVEN_BEFORE_TX_CRASH"].startswith(
        "old remains authoritative current"
    )
    assert crash["R2_REPLACEMENT_RESERVED_BEFORE_REQUESTER_SIGNATURE_CRASH"].startswith(
        "new rpa_id current; NOT_YET_DEFINED; recover same ID"
    )
    assert crash["R3_REQUESTER_SIGNED_BEFORE_CLAIMANT_CRASH"].startswith(
        "same new rpa_id; NOT_YET_DEFINED"
    )
    assert crash["R4_BOTH_SIGNATURES_BEFORE_FINALIZATION_CRASH"].startswith(
        "same replacement reservation authoritative current"
    )
    assert crash["R5_FINALIZATION_COMMITTED_BEFORE_SEND_CRASH"].startswith(
        "same new rpa_id and same DEFINED"
    )
    assert crash["R6_OLD_STALE_FINALIZATION"].startswith("FAIL_CLOSED")
    concurrency = issuer["replacement_concurrency"]
    assert (
        concurrency["two_current_replacements_possible"]
        is concurrency["loser_may_sign"]
        is concurrency["loser_externally_sendable"]
        is False
    )
    ...
    matrix = issuer["attempt_creation_concurrency_matrix"]
    reserved = next(
        row for row in matrix if row["step"] == "AUTHORITATIVE_UNBOUND_REPLACEMENT_RESERVATION"
    )
    ...
    assert (
        reserved["requester_signature_present"]
        is reserved["claimant_signature_present"]
        is reserved["external_send_eligible"]
        is False
    )
...
def test_replacement_reservation_finalization_and_race() -> None:
    ...
    replacement_request = semantic_authorization_fixture(
        requester_key_version=2, claimant_key_version=2
    )
    evidence = reconciliation_evidence_fixture(request, old_id)
    new_id = authority.replace_after_authoritative_unbound(
    ...
    assert (
        authority.resolve_current(replacement_request)["immutable_attempt_digest_status"]
        == "NOT_YET_DEFINED"
    )
...
def test_replacement_two_step_contract() -> None:
    ...
    assert set(issuer["replacement_two_step_redteam_mutations"]) == set(
        REPLACEMENT_TWO_STEP_MUTATIONS
    )
...
RECONCILIATION_EVIDENCE_MUTATIONS = {
    "replacement_without_reconciliation_evidence": (
        "issuer",
        "replacement_reservation_transition.evidence_required",
        False,
    ),
    "caller_boolean_unbound_authorizes_replacement": (
        "issuer",
        "replacement_reservation_transition.caller_assertion_sufficient",
        True,
    ),
    "method_name_unbound_is_treated_as_evidence": (
        "issuer",
        "replacement_reservation_transition.evidence_object",
        "method name",
    ),
    "timeout_treated_as_unbound": (
        "issuer",
        "reconciliation_evidence_contract.non_authoritative_outcomes",
        ["connection error"],
    ),
    "not_found_treated_as_unbound": (
        "issuer",
        "reconciliation_evidence_contract.non_authoritative_outcomes",
        ["timeout"],
    ),
    "missing_history_treated_as_unbound": (
        "issuer",
        "reconciliation_evidence_contract.non_authoritative_outcomes",
        ["timeout", "NOT_FOUND"],
    ),
    "unauthenticated_unbound_snapshot_accepted": (
        "issuer",
        "reconciliation_evidence_contract.authentication_required",
        False,
    ),
    "reconciliation_wrong_old_rpa_id_accepted": (
        "issuer",
        "replacement_reservation_transition.evidence_exact_binding_fields",
        ["environment"],
    ),
    "reconciliation_wrong_entitlement_generation_accepted": (
        "issuer",
        "replacement_reservation_transition.evidence_exact_binding_fields",
        ["old_issuance_attempt_id"],
    ),
    "reconciliation_wrong_operation_accepted": (
        "issuer",
        "replacement_reservation_transition.evidence_exact_binding_fields",
        ["old_issuance_attempt_id", "entitlement_generation"],
    ),
    "reconciliation_wrong_account_accepted": (
        "issuer",
        "replacement_reservation_transition.evidence_exact_binding_fields",
        ["logical_operation_id"],
    ),
    "reconciliation_wrong_request_fingerprint_accepted": (
        "issuer",
        "replacement_reservation_transition.evidence_exact_binding_fields",
        ["account_id"],
    ),
    "reconciliation_wrong_initial_binding_accepted": (
        "issuer",
        "replacement_reservation_transition.evidence_exact_binding_fields",
        ["canonical_genesis_request_fingerprint_sha256"],
    ),
    "reconciliation_wrong_environment_accepted": (
        "issuer",
        "replacement_reservation_transition.evidence_exact_binding_fields",
        ["trust_domain"],
    ),
    "reconciliation_wrong_trust_domain_accepted": (
        "issuer",
        "replacement_reservation_transition.evidence_exact_binding_fields",
        ["environment"],
    ),
    "reconciliation_wrong_product_scope_accepted": (
        "issuer",
        "replacement_reservation_transition.evidence_exact_binding_fields",
        ["environment", "trust_domain"],
    ),
    "stale_unbound_evidence_after_later_bound_accepted": (
        "issuer",
        "reconciliation_evidence_contract.stale_after_later_bound_authorizes",
        True,
    ),
    "replacement_commit_omits_reconciliation_evidence_reference": (
        "issuer",
        "replacement_reservation_transition.durable_replacement_binding_fields",
        ["old_issuance_attempt_id", "new_issuance_attempt_id"],
    ),
    "crash_reconstructs_reconciliation_evidence_from_current_lookup": (
        "issuer",
        "reconciliation_evidence_contract.forbidden_reconstruction_sources",
        ["caller"],
    ),
    "concurrent_replacement_accepts_incompatible_evidence": (
        "issuer",
        "replacement_concurrency.incompatible_evidence_after_switch",
        "ACCEPT",
    ),
    "superseded_history_loses_unbound_evidence_provenance": (
        "issuer",
        "reconciliation_evidence_contract.durable_relation",
        "NONE",
    ),
}
...
def validate_reconciliation_evidence(issuer: dict, physical: dict) -> None:
    ...
    assert (
        evidence["positive_result"]
        is evidence["absence_never_proves_unbound"]
        is evidence["authentication_required"]
        is True
    )
    assert set(evidence["non_authoritative_outcomes"]) == {
        "timeout",
        "connection error",
        "missing response",
        "NOT_FOUND",
        "absent local proof",
        "absent local receipt",
        "empty history",
        "caller boolean/string UNBOUND",
        "process memory",
        "cached non-authoritative projection",
        "stale issuer snapshot",
    }
    ...
    assert (
        evidence["anti_staleness_model"] == "CURRENT_AUTHORITY_REVISION_AND_RETAINED_HISTORY_FENCE"
    )
    assert evidence["stale_after_later_bound_authorizes"] is False
    assert set(evidence["forbidden_reconstruction_sources"]) == {
        "caller",
        "process memory",
        "current issuer lookup",
        "current credentials",
    }
    assert (
        evidence["durable_relation"]
        == "old_issuance_attempt_id --superseded_by_authoritative_unbound_evidence--> new_issuance_attempt_id"
    )
    transition = issuer["replacement_reservation_transition"]
    ...
    assert set(transition["durable_replacement_binding_fields"]) == {
        "old_issuance_attempt_id",
    ...
    concurrency = issuer["replacement_concurrency"]
    assert (
        concurrency["same_evidence_converges"]
        is concurrency["evidence_identity_must_match_current_replacement"]
        is True
    )
    ...
    assert (
        recovery["replacement_reservation_transaction_includes"][
            "authenticated_unbound_evidence_binding"
        ]
        is True
    )
...
def test_reconciliation_evidence_state_machine() -> None:
    ...
    invalid_evidence = [
        ...
        reconciliation_evidence_fixture(
            request, old_id, canonical_genesis_request_fingerprint_sha256="ff" * 32
        ),
    ...
    assert (
        authority.replace_after_authoritative_unbound(request, replacement, old_id, valid) == new_id
    )
    incompatible = reconciliation_evidence_fixture(
        request, old_id, authority_authenticated_evidence_digest_sha256="d" * 64
    )
    assert (
        authority.replace_after_authoritative_unbound(request, replacement, old_id, incompatible)
    ...
    assert (
        history["attempt_id"],
        history["replacement_authorization_evidence_digest_sha256"],
        current["attempt_id"],
    ) == (old_id, "c" * 64, new_id)
...
def test_reconciliation_evidence_contract_and_mutations() -> None:
    ...
    assert set(issuer["reconciliation_evidence_redteam_mutations"]) == set(
        RECONCILIATION_EVIDENCE_MUTATIONS
    )
