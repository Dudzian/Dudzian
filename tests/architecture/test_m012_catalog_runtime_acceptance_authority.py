...
def _payload(
    *, server_time: int | None = None, tick: str = "0.01", extra: object = None
) -> dict[str, object]:
...
def authority(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "__init__",
        lambda storage, **kwargs: setattr(storage, "_catalog_test_values", secrets),
    )
    monkeypatch.setattr(
        KeyringSecretStorage,
        "get_secret",
...
def test_core_invokes_release_producer_and_restart_preserves_history(authority) -> None:
    ...
    assert validate_canonical_catalog_context_graph(
        ...
        accepted_source_catalog_snapshots_by_id={
            first.accepted_source_catalog_snapshot_id: first.to_mapping()
        },
        instruments_by_id={},
        instrument_history_by_id={},
        source_producer_membership_authority=membership,
    )
    grant = membership.resolve_historical(
        first.accepted_source_producer_membership_id,
        first.source_producer_generation,
        first.source_producer_membership_fingerprint,
        _BinanceSpotCatalogProducer.identity,
        first.effective_at_utc,
    )
    ...
    restarted = CatalogRuntimeAcceptanceAuthority(
        SourceProducerMembershipAuthority(carrier), runtime._receipts
    )
    ...
    values.append(
        _payload(server_time=int(datetime.now(timezone.utc).timestamp() * 1000) - 500, tick="0.1")
    )
    second = restarted.fetch_catalog_once("core_release_1_46_binance_spot")
    assert (
        second is not None
        and second.previous_snapshot_id == first.accepted_source_catalog_snapshot_id
    )
...
def test_orphan_genuine_receipt_does_not_create_catalog_fact(authority) -> None:
    ...
    with _sqlite(carrier) as db:
        assert db.execute(
            "SELECT COUNT(*) FROM catalog_snapshot_admission_receipts"
        ).fetchone() == (0,)
...
def test_catalog_valid_prefix_rollback_is_rejected_by_finalization_closure(authority) -> None:
    ...
    with _sqlite(carrier) as db:
        db.execute(
            ...
            "WHERE accepted_source_catalog_snapshot_id=?",
        ...
        db.execute("INSERT INTO source_metadata_authority_head VALUES(?,?,?,?)", metadata_head_one)
...
@pytest.mark.parametrize(
    "bad", [None, True, 1, [], {}, {"symbols": []}, {"serverTime": float("nan"), "symbols": []}]
)
...
def test_concurrent_revoke_committed_before_acceptance_fence_wins(authority, monkeypatch) -> None:
    ...
    worker = Thread(
        target=lambda: result.append(runtime.fetch_catalog_once("core_release_1_46_binance_spot"))
    )
    worker.start()
...
def test_metadata_mutation_with_recomputed_public_fingerprint_is_corruption(authority) -> None:
    ...
    with _sqlite(carrier) as db:
        (canonical,) = db.execute(
            "SELECT canonical_record FROM source_metadata_versions"
        ).fetchone()
...
def test_metadata_exact_relation_and_introduction_time_are_enforced(authority, mutation) -> None:
    ...
    with _sqlite(carrier) as db:
        ...
        if mutation == "wrong_tuple":
            db.execute(
                "UPDATE source_metadata_versions SET venue_symbol=?,canonical_record=?,metadata_record_digest=?",
                ("ETHUSDT", canonical, digest),
            )
        else:
            db.execute(
...
def test_snapshot_shadow_columns_are_exact_bound(authority, column, value) -> None:
    ...
    with _sqlite(carrier) as db:
        db.execute(
            f"UPDATE accepted_catalog_snapshots SET {column}=? WHERE snapshot_sequence=1", (value,)
        )
    with pytest.raises(ValueError, match="CORRUPT_CATALOG_AUTHORITY"):
        runtime.snapshots()
    with pytest.raises(ValueError, match="CORRUPT_CATALOG_AUTHORITY"):
...
def test_introduced_by_shadow_is_digest_bound_even_if_later_snapshot_references_metadata(
    authority,
) -> None:
    runtime, _membership, values, carrier = authority
    first = runtime.fetch_catalog_once("core_release_1_46_binance_spot")
...
def test_supersession_before_acceptance_denies_and_old_snapshot_stays_historical(
...
def test_acceptance_writer_lock_commits_before_concurrent_supersession(
    authority, monkeypatch
) -> None:
    runtime, membership, _values, _carrier = authority
    assert membership.admit_release_grant("core_release_1_45_binance_spot_generation_2") is not None
    ...
    accept_thread = Thread(
        target=lambda: accepted.append(runtime.fetch_catalog_once("core_release_1_46_binance_spot"))
    )
    accept_thread.start()
    assert locked.wait(5)
    supersede_thread = Thread(
        target=lambda: terminal.append(
            membership.admit_release_event("core_release_1_45_supersede_binance_spot")
        )
    )
    ...
    assert validate_accepted_source_snapshot_membership_evidence(
        accepted[0].to_mapping(), membership
    )
