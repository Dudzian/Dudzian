...
from bot_core.execution.m07_fill_validation import (
    FILL_V1_FIELDS, FILL_V2_FIELDS, FILL_V2_FINGERPRINT_DOMAIN,
    FILL_V2_FINGERPRINT_FIELDS, M07FillValidationError,
    canonical_fill_fingerprint, canonical_fill_v2_fingerprint, validate_structural_fill,
)
...
def asset(code="BNB", namespace="binance"):
    return {"venue_asset_code": code, "canonical_display_code": code,
            "asset_namespace": namespace, "mapping_status": "EXACT"}


def fill(**changes):
    raw = {"fill_id": f"fill_{U}", "order_id": f"ord_{U}", "environment": "PAPER",
           ...
           "execution_route_id": f"xroute_{U}", "venue_trade_id": "trade-é", "side": "BUY",
           "executed_quantity": "0.8", "execution_price": "100",
           "executed_at_utc": "2025-01-01T00:00:00Z", "fee_kind": "CHARGE",
           "fee_quantity": "0.01", "fee_asset_reference": asset(), "fill_fingerprint_sha256": ""}
    raw.update(changes)
...
def test_exact_contract_golden_vector_and_independent_oracle():
    ...
    encoded = unicodedata.normalize("NFC", json.dumps(projection, sort_keys=True, separators=(",", ":"), ensure_ascii=False))
...
@pytest.mark.parametrize("snapshot", ["", "foo", "snapshot_1", "wcat_1", "ascat", "ascat_", "ASCat_1", 1])
...
def test_prefix_is_derived_from_catalog_contract():
    path = Path(__file__).parents[2] / "docs/architecture/cryptohunter_product_architecture/exchange_accounts_and_instruments.json"
...
def test_none_fee_and_closed_shapes():
    assert validate_structural_fill(fill(fee_kind="NONE", fee_quantity="0", fee_asset_reference=None))
    for bad in [fill(fee_kind="NONE", fee_quantity="0.1", fee_asset_reference=None),
                fill(fee_kind="CHARGE", fee_quantity="0", fee_asset_reference=asset())]: reject(bad)
    raw=fill(); raw["extra"]=1; reject(raw)
    raw=fill(); del raw["side"]; reject(raw)


@pytest.mark.parametrize(("field", "value"), [("instrument_metadata_version", True), ("exchange_id", 1),
    ("executed_quantity", 1.0), ("fee_asset_reference", []), ("executed_quantity", "NaN"),
    ("execution_price", "Infinity"), ("executed_quantity", "+1"), ("executed_quantity", "01"),
    ("execution_price", "1."), ("fee_quantity", "-0"), ("executed_at_utc", "2025-01-01T00:00:00+00:00")])
...
def test_asset_exact_types_and_mapping_status():
    ...
    bad=asset(); bad["mapping_status"]=1; reject(fill(fee_asset_reference=bad))


def test_every_input_is_bound_and_nfc_is_canonicalized():
    raw=fill(); expected=raw["fill_fingerprint_sha256"]
    for field in FILL_V2_FINGERPRINT_FIELDS:
        changed=deepcopy(raw)
        value=changed[field]
        changed[field] = ({**value, "venue_asset_code":"ETH"} if isinstance(value,dict) else
                          value + "x" if isinstance(value,str) else value + 1)
...
def test_cross_version_isolation_and_legacy_regression():
    v2=fill(); old_hash=canonical_fill_fingerprint(v2)
    v2["fill_fingerprint_sha256"]=old_hash; reject(v2)
    legacy=fill(); del legacy["accepted_source_catalog_snapshot_id"]
    legacy["exchange_id"]="binance"; legacy["fill_fingerprint_sha256"]=canonical_fill_fingerprint(legacy)
    assert set(legacy)==FILL_V1_FIELDS and validate_structural_fill(legacy)
    upgraded=dict(legacy, accepted_source_catalog_snapshot_id="ascat_1"); reject(upgraded)
    downgraded=fill(); del downgraded["accepted_source_catalog_snapshot_id"]; reject(downgraded)
...
def test_implementation_status_is_deterministic_and_preserves_blockers():
    ...
    rendered = ("# M0.7 structural Full Fill v2 implementation status\n\n"
