...
from bot_core.instruments.source_producer_membership import (
    JsonlMembershipCarrier, SourceProducerMembershipAuthority,
...
def _process_admit_same_grant(path, barrier, results):
    ...
    barrier.wait()
    results.put(authority.admit_release_grant("core_release_1_45_binance_spot", trusted_core_now_utc="2026-09-14T00:00:00Z") is not None)
...
def test_public_resolver_boundary_does_not_accept_history_mapping(tmp_path):
    ...
    assert authority.admit_release_grant(fake, trusted_core_now_utc="2026-09-15T00:00:00Z") is None
...
def test_append_only_revoke_preserves_grant_and_old_snapshot(tmp_path):
    ...
    assert authority.admit_release_event("core_release_1_45_revoke_binance_spot", trusted_core_now_utc="2026-09-15T01:00:00Z") is not None
    with sqlite3.connect(authority._carrier.path) as connection:
...
def test_append_only_supersession_derives_generation_two_current(tmp_path):
    ...
    old = snapshot(grant1)
    grant2 = authority.admit_release_grant("core_release_1_45_binance_spot_generation_2", trusted_core_now_utc="2026-09-15T01:00:00Z")
    ...
    assert authority.admit_release_event("core_release_1_45_supersede_binance_spot", trusted_core_now_utc="2026-09-15T01:00:00Z") is not None
    restored = SourceProducerMembershipAuthority(JsonlMembershipCarrier(authority._carrier.path))
...
def test_runtime_plugin_config_factory_and_self_description_cannot_admit(tmp_path, claim):
    ...
    assert authority.admit_release_grant(claim, trusted_core_now_utc="2026-09-15T00:00:00Z") is None
    assert authority.admit_release_event(claim, trusted_core_now_utc="2026-09-15T00:00:00Z") is None
...
def test_clean_acknowledged_revoke_tail_deletion_is_rollback(tmp_path):
    ...
    authority.admit_release_event("core_release_1_45_revoke_binance_spot", trusted_core_now_utc="2026-09-15T01:00:00Z")
...
def test_clean_acknowledged_supersession_tail_deletion_is_rollback(tmp_path):
    ...
    authority.admit_release_grant("core_release_1_45_binance_spot_generation_2", trusted_core_now_utc="2026-09-15T01:00:00Z")
    authority.admit_release_event("core_release_1_45_supersede_binance_spot", trusted_core_now_utc="2026-09-15T01:00:00Z")
...
def test_same_grant_concurrent_threads_is_one_physical_record(tmp_path):
    ...
    def admit():
        barrier.wait()
        return authority.admit_release_grant("core_release_1_45_binance_spot", trusted_core_now_utc="2026-09-14T00:00:00Z")
...
def test_same_event_concurrent_threads_is_one_physical_event(tmp_path):
    ...
    def admit():
        barrier.wait()
        return authority.admit_release_event("core_release_1_45_revoke_binance_spot", trusted_core_now_utc="2026-09-15T01:00:00Z")
...
def test_revoke_vs_supersede_race_commits_exactly_one_terminal(tmp_path):
    ...
    def admit(name):
        barrier.wait()
        return authority.admit_release_event(name, trusted_core_now_utc="2026-09-15T01:00:00Z")
...
def test_late_backdated_event_is_non_retroactive_and_scheduled_cutoff_works(tmp_path):
    ...
    event = authority.admit_release_event("core_release_1_45_late_revoke_binance_spot", trusted_core_now_utc="2026-09-15T04:00:00Z")
...
def test_scheduled_future_grant_requires_both_admission_and_effective_time(tmp_path):
    ...
    grant = authority.admit_release_grant(
        ...
        trusted_core_now_utc="2025-12-31T23:00:00Z",
    )
    assert grant is not None
