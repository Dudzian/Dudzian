...
def validate(d: dict) -> None:
    ...
    assert d["account_device_ordering"]["ATOMIC_ACCOUNT_PLUS_FIRST_DEVICE"]["authority_protocol"] == "NOT_FOUND"
    ...
    assert d["concurrency"]["distinct_account_same_subject"]["closure_status"].startswith("DESIGN_BLOCKED")
    ...
    assert len(d["mandatory_redteam_mutations"]) == 15
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda d: d["authentication_requirements"].update(
            {"M0.11_carrier_is_root_proof": True}
        ),
...
def test_cross_artifact_status_parity() -> None:
    ...
    for filename in (
        ...
        for key in ("CryptoHunterAccountAuthority", "WorkspaceAuthority", "FullFillAuthority", "M0.8"):
            if key in statuses:
...
def test_pre_account_root_evidence_rejection_breaks_semantic_parity() -> None:
    ...
    root = json.loads(ROOT_RECONCILIATION.read_text(encoding="utf-8"))
    genesis["account_root_provisioning_evidence"]["before_account_genesis"] = (
        "NOT_AUTHORIZED"
    )
