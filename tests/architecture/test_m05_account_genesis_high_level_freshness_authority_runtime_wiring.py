...
def test_preparation_unknown_is_sticky_when_exact_retry_is_unavailable(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ProductionLocalFreshnessAuthority, "_request_preparation_once", lambda *_: next(outcomes)
    )
...
def test_preparation_unknown_can_only_be_resolved_by_exact_prepared_retry(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ProductionLocalFreshnessAuthority,
        "_compare_and_advance",
        lambda self, value, document, receipt: SimpleNamespace(
            outcome=FreshnessAuthorityOutcome.CAS_ACCEPTED,
            prepared=value,
...
def test_native_serialization_failure_retries_exact_evidence_in_new_attempt(monkeypatch, tmp_path):
    ...
    def _attempt(self, value, document, receipt):
        ...
        return FreshnessAuthorityResult(
            FreshnessAuthorityOutcome.ALREADY_ACCEPTED_EXACT, 7, receipt
        )

    monkeypatch.setattr(
        ProductionLocalFreshnessAuthority, "_request_preparation", lambda *_: prepared
    )
...
def test_bounded_native_serialization_failures_end_outcome_unknown(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ProductionLocalFreshnessAuthority,
        "_compare_and_advance_once",
        lambda *args: calls.append(args[1:]) or runtime_module._NativeSerializationFailure(),
    )
