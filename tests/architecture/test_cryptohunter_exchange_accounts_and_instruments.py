...
def unique_str_list(value):
    return (
        ...
        and all(is_nonempty_str(v) for v in value)
...
def validate_exchange_registry_entry(entry):
    ...
    if entry["capability_discovery_policy"] not in {
        "STATIC_BUILD_TIME",
...
def validate_account(account, *, operation="UPDATE_ACCOUNT", validation_context=None):
    ...
    ok, denial = closed_schema(
        account,
        fields,
        fields + ["legacy_record", "readiness_confirmed", "private_connection_active"],
    ...
    if account["connection_state"] not in DATA["exchange_account_contract"]["connection_states"]:
        return deny("EXCHANGE_ACCOUNT_NOT_FOUND")
    if (
        account["execution_authorization"]
        not in DATA["exchange_account_contract"]["execution_authorizations"]
    ):
        return deny("EXCHANGE_ACCOUNT_NOT_FOUND")
    if (
        account["external_account_identity_state"]
        not in DATA["external_account_identity_contract"]["states"]
    ):
    ...
    for field in [
        ...
        if account.get(field) is not None and not is_nonempty_str(account.get(field)):
            return deny("ACCOUNT_READINESS_BLOCKED")
    if not parse_ts(account.get("created_at_utc")) or (
        account.get("retired_at_utc") is not None and not parse_ts(account["retired_at_utc"])
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    if (
        account["lifecycle_state"] in {"DRAFT", "ACTIVE", "DISABLED"}
        and account.get("retired_at_utc") is not None
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    if account["lifecycle_state"] == "RETIRED" and (
        account.get("retired_at_utc") is None
        or not ts_le(account["created_at_utc"], account["retired_at_utc"])
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    if "readiness_confirmed" in account and type(account["readiness_confirmed"]) is not bool:
        return deny("ACCOUNT_READINESS_BLOCKED")
    if (
        "private_connection_active" in account
        and type(account["private_connection_active"]) is not bool
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    if account.get("account_capability_snapshot_id") is not None:
        snapshots = (
            validation_context.get("account_capability_snapshots_by_id")
            if isinstance(validation_context, dict)
            else None
        )
        previous = (
            validation_context.get("previous_account_capability_snapshots_by_id")
            if isinstance(validation_context, dict)
            else None
        )
        snapshot = (
            snapshots.get(account["account_capability_snapshot_id"])
        ...
        if not ok:
            return deny("ACCOUNT_READINESS_BLOCKED")
    if (
        operation not in {"QUARANTINE_LEGACY_LIVE_RECORD", "RETIRE_ACCOUNT"}
...
def validate_credential_profile_binding(profile, account, lineage_context=None):
    ...
    if (
        ...
        not in DATA["credential_profile_contract"]["credential_purposes"]
        or profile["lifecycle_state"] != "ACTIVE"
    ):
        return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
    perms = profile.get("permission_snapshot")
    if not unique_str_list(perms) or not set(perms) <= set(
        DATA["credential_profile_contract"]["permission_registry"]
    ):
        return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
    ...
    active_ids = (
        active_by_account.get(account["exchange_account_id"])
        if isinstance(active_by_account, dict)
        else None
    )
...
def validate_credential_profile(profile):
    ...
    for field in ["credential_profile_id", "exchange_account_id", "exchange_id"]:
        if not is_nonempty_str(profile.get(field)):
            return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
    if (
        profile.get("environment_scope") not in DATA["environment_registry"]
        or profile.get("credential_purpose")
        not in DATA["credential_profile_contract"]["credential_purposes"]
    ):
        return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
    if (
    ...
    locator = (
        reference[len("secure-store://") :]
        if isinstance(reference, str) and reference.startswith("secure-store://")
        else ""
    )
    if (
        not is_nonempty_str(reference)
        or not locator
        or any(char.isspace() for char in reference)
        or any(char in reference for char in "?#=")
        or any(marker in reference.lower() for marker in forbidden_payload_markers)
    ):
        return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
    if profile.get("public_key_identifier") is not None and not is_nonempty_str(
        profile.get("public_key_identifier")
    ):
        return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
    permissions = profile.get("permission_snapshot")
    if not unique_str_list(permissions) or not set(permissions) <= set(
        DATA["credential_profile_contract"]["permission_registry"]
    ):
        return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
    ...
    if profile["lifecycle_state"] == "ACTIVE" and profile.get("retired_at_utc") is not None:
        return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
    if profile["lifecycle_state"] == "RETIRED" and (
        not parse_ts(profile.get("retired_at_utc"))
...
def validate_credential_lineage(current, context):
    ...
    while cursor is not None:
        ...
        if (
            not validate_credential_profile(predecessor)[0]
            or predecessor.get("credential_profile_id") != cursor
        ):
            return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
        for field in ["exchange_account_id", "exchange_id", "environment_scope"]:
            if predecessor.get(field) != current[field]:
                return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
        if predecessor.get("lifecycle_state") != "RETIRED" or not parse_ts(
            predecessor.get("retired_at_utc")
        ):
            return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
        if successor.get("rotated_from_credential_profile_id") != cursor or not ts_le(
            predecessor["retired_at_utc"], successor["created_at_utc"]
        ):
            return deny("CREDENTIAL_PROFILE_NOT_ACTIVE")
...
def validate_external_account_identity(identity, account, *, reconnect=False):
    ...
    ok, denial = closed_schema(
        identity, fields, fields + ["revalidated_at_utc"], "EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED"
    )
    if not ok:
        return False, denial
    ...
    if not is_nonempty_str(identity["venue_account_identifier"]):
        return deny("EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED")
    if identity["subaccount_identifier"] is not None and not isinstance(
        identity["subaccount_identifier"], str
    ):
        return deny("EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED")
    if identity["subaccount_identifier"] == "":
        return deny("EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED")
    if not is_nonempty_str(identity["account_type"]) or not is_nonempty_str(
        identity["adapter_version_source"]
    ):
        return deny("EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED")
    if not unique_str_list(identity["observed_permission_set"]):
        return deny("EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED")
    if not set(identity["observed_permission_set"]) <= set(
        DATA["credential_profile_contract"]["permission_registry"]
    ):
        return deny("EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED")
...
def validate_account_capability_snapshot(snapshot, account, previous_by_id=None):
    ...
    for field in [
        ...
        if not is_nonempty_str(snapshot.get(field)):
            return deny("ACCOUNT_READINESS_BLOCKED")
    if snapshot["exchange_account_id"] != account.get("exchange_account_id") or any(
        snapshot.get(field) != account.get(field)
        for field in ["exchange_id", "environment", "market_type"]
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    ...
    if (
        ex is None
        or ex.get("status") != "ENABLED"
        or snapshot["adapter_family_id"] != ex.get("adapter_family_id")
        or snapshot["environment"] == "LIVE"
    ):
        return deny(
            "LIVE_BLOCKED_BY_EDITION"
            if snapshot["environment"] == "LIVE"
            else "ACCOUNT_READINESS_BLOCKED"
        )
    if snapshot["environment"] not in ex.get("supported_environments", []) or snapshot[
        "market_type"
    ] not in ex.get("supported_market_types", []):
        return deny("ACCOUNT_READINESS_BLOCKED")
    if (
        type(snapshot.get("version")) is not int
        or snapshot["version"] <= 0
        or snapshot.get("status") not in contract["statuses"]
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    if snapshot.get("previous_snapshot_id") is not None and not is_nonempty_str(
    ...
    allowed_for_market = set(
        DATA["allowed_market_instrument_type_pairs"].get(snapshot["market_type"], [])
    )
    if (
        not unique_str_list(types)
        or not set(types) <= set(DATA["instrument_type_registry"])
        or not set(types) <= set(ex["supported_instrument_types"])
        or not set(types) <= allowed_for_market
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    if not (
        ts_le(snapshot.get("observed_at_utc"), snapshot.get("effective_at_utc"))
        and ts_lt(snapshot.get("effective_at_utc"), snapshot.get("stale_after_utc"))
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    definition = contract["content_hash_definition"]
    if (
        not isinstance(snapshot.get("content_hash"), str)
        or not HEX64.fullmatch(snapshot["content_hash"])
        or snapshot["content_hash"] != hash_payload(definition, snapshot)
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    ...
    if snapshot["version"] == 1:
        return (
            (True, {"restricts_only": True, "extends_product_capabilities": False})
            if snapshot["previous_snapshot_id"] is None
            else deny("ACCOUNT_READINESS_BLOCKED")
        )
    visited = {snapshot["account_capability_snapshot_id"]}
    cursor = snapshot["previous_snapshot_id"]
    expected = snapshot["version"] - 1
    while cursor is not None:
        if not is_nonempty_str(cursor) or cursor in visited:
            return deny("ACCOUNT_READINESS_BLOCKED")
        visited.add(cursor)
        predecessor = previous_by_id.get(cursor)
        if (
            not closed_schema(predecessor, fields, fields, "ACCOUNT_READINESS_BLOCKED")[0]
            or predecessor.get("account_capability_snapshot_id") != cursor
        ):
            return deny("ACCOUNT_READINESS_BLOCKED")
        if predecessor.get("version") != expected or any(
            predecessor.get(field) != snapshot[field]
            for field in ["exchange_account_id", "exchange_id", "environment", "market_type"]
        ):
            return deny("ACCOUNT_READINESS_BLOCKED")
        if (
            type(predecessor.get("version")) is not int
            or predecessor.get("status") not in contract["statuses"]
            or predecessor.get("adapter_family_id") != ex.get("adapter_family_id")
            or not is_nonempty_str(predecessor.get("adapter_version"))
        ):
            return deny("ACCOUNT_READINESS_BLOCKED")
        if predecessor.get("environment") not in ex.get(
            "supported_environments", []
        ) or predecessor.get("market_type") not in ex.get("supported_market_types", []):
            return deny("ACCOUNT_READINESS_BLOCKED")
        if not unique_str_list(predecessor.get("observed_permission_set")) or not set(
            predecessor["observed_permission_set"]
        ) <= set(DATA["credential_profile_contract"]["permission_registry"]):
            return deny("ACCOUNT_READINESS_BLOCKED")
        predecessor_market_types = set(
            DATA["allowed_market_instrument_type_pairs"].get(predecessor["market_type"], [])
        )
        if (
            not unique_str_list(predecessor.get("supported_instrument_types"))
            or not set(predecessor["supported_instrument_types"])
            <= set(ex["supported_instrument_types"])
            or not set(predecessor["supported_instrument_types"]) <= predecessor_market_types
        ):
            return deny("ACCOUNT_READINESS_BLOCKED")
        if not (
            ts_le(predecessor.get("observed_at_utc"), predecessor.get("effective_at_utc"))
            and ts_lt(predecessor.get("effective_at_utc"), predecessor.get("stale_after_utc"))
        ):
            return deny("ACCOUNT_READINESS_BLOCKED")
        if predecessor.get("content_hash") != hash_payload(definition, predecessor):
            return deny("ACCOUNT_READINESS_BLOCKED")
        cursor = predecessor.get("previous_snapshot_id")
        expected -= 1
    return (
        (True, {"restricts_only": True, "extends_product_capabilities": False})
        if expected == 0
...
def validate_instrument_record(
    inst, now="2026-06-01T00:00:00Z", *, require_tradable=False, require_fresh=True
):
    ...
    if inst["market_type"] not in ex["supported_market_types"]:
        return deny("MARKET_TYPE_UNSUPPORTED")
    if inst["instrument_type"] not in ex["supported_instrument_types"] or inst[
        "instrument_type"
    ] not in DATA["allowed_market_instrument_type_pairs"].get(inst["market_type"], []):
        return deny("INSTRUMENT_TYPE_UNSUPPORTED")
    if (
        not is_nonempty_str(inst["venue_symbol"])
        or inst["venue_symbol"].strip() != inst["venue_symbol"]
    ):
    ...
    if type(inst["metadata_version"]) is not int or inst["metadata_version"] <= 0:
        return deny("INSTRUMENT_METADATA_INVALID")
    if not (
        ts_le(inst["observed_at_utc"], inst["effective_at_utc"])
        and ts_lt(inst["effective_at_utc"], inst["stale_after_utc"])
    ):
        return deny("INSTRUMENT_METADATA_INVALID")
    ...
    if itype in {"SPOT_PAIR", "MARGIN_PAIR"}:
        ...
        if settlement is not None and settlement.get("venue_asset_code") != inst[
            "quote_asset_reference"
        ].get("venue_asset_code"):
            return deny("INSTRUMENT_METADATA_INVALID")
    elif itype == "PERPETUAL_CONTRACT":
        if (
            not is_canonical_decimal(inst.get("contract_size"), positive=True)
            or inst.get("settlement_asset_reference") is None
            or not is_nonempty_str(inst.get("contract_value_currency"))
            or inst.get("derivative_settlement_type") not in {"LINEAR", "INVERSE"}
            or any(
                inst.get(field) is not None
        ...
        if (
            ...
            or not is_nonempty_str(inst.get("contract_value_currency"))
            or inst.get("derivative_settlement_type") not in {"LINEAR", "INVERSE"}
            or not parse_ts(inst.get("expiry_at_utc"))
            or any(inst.get(field) is not None for field in ["strike_price", "option_side"])
        ):
            return deny("INSTRUMENT_METADATA_INVALID")
    elif itype == "OPTION":
        if (
            ...
            or inst.get("derivative_settlement_type") not in {"LINEAR", "INVERSE"}
            or not parse_ts(inst.get("expiry_at_utc"))
            or not is_canonical_decimal(inst.get("strike_price"), positive=True)
            or inst.get("option_side") not in {"CALL", "PUT"}
        ):
            return deny("INSTRUMENT_METADATA_INVALID")
    return True, None
...
def validate_instrument_history_map(
    instrument_history_by_id, current_instruments_by_id, now="2026-06-01T00:00:00Z"
):
    if not isinstance(instrument_history_by_id, dict) or not isinstance(
    ...
    for instrument_id, history in instrument_history_by_id.items():
        ...
        for record in history:
            ...
            if (
                ...
                and tuple(current.get(field) for field in identity_fields) != identity_tuple
            ):
                return deny("INSTRUMENT_IDENTITY_COLLISION")
        ...
        if current is not None:
            ...
            if tuple(current.get(field) for field in identity_fields) != identity_tuple:
                return deny("INSTRUMENT_IDENTITY_COLLISION")
            if (
                type(current.get("metadata_version")) is not int
                or current["metadata_version"] <= versions[-1]
            ):
                return deny("INSTRUMENT_METADATA_INVALID")
    return True, None


def validate_instrument_catalog_snapshot(
    snapshot,
    instruments,
    previous_catalogs_by_id=None,
    now="2026-06-01T00:00:00Z",
    instrument_history_by_id=None,
    catalogs_by_id=None,
):
    ...
    if snapshot["market_type"] not in ex.get("supported_market_types", []):
        return deny("MARKET_TYPE_UNSUPPORTED")
    if snapshot["adapter_family_id"] != ex["adapter_family_id"] or not is_nonempty_str(
    ...
    if not unique_str_list(snapshot["instrument_ids"]):
        return deny("CATALOG_SNAPSHOT_INVALID")
    if not (
        ts_le(snapshot["observed_at_utc"], snapshot["effective_at_utc"])
        and ts_lt(snapshot["effective_at_utc"], snapshot["stale_after_utc"])
        and ts_lt(now, snapshot["stale_after_utc"])
    ):
    ...
    for instrument_id in snapshot["instrument_ids"]:
        ...
        if not ok:
            return False, denial
        trusted_catalogs = (
            {snapshot.get("catalog_snapshot_id"): snapshot}
            if catalogs_by_id is None
            else catalogs_by_id
        )
        if (
            resolve_catalog_member(
                snapshot,
                instrument_id,
                instruments,
            ...
            return deny("CATALOG_SNAPSHOT_INVALID")
    if snapshot["content_hash"] != hash_payload(
...
def validate_catalog_node(snapshot):
    ...
    if not closed_schema(snapshot, fields, fields, "CATALOG_SNAPSHOT_INVALID")[0]:
        return False
    if any(
        not is_nonempty_str(snapshot.get(field))
        for field in [
            "catalog_snapshot_id",
            "exchange_id",
            "environment",
    ...
    if snapshot.get("previous_snapshot_id") is not None and not is_nonempty_str(
        ...
        return False
    if not HEX64.fullmatch(snapshot["content_hash"]) or not unique_str_list(
        snapshot.get("instrument_ids")
    ):
        return False
    ex = exchange_entry(snapshot["exchange_id"])
    if (
...
def validate_universe_record(univ):
    ...
    if not ok:
        return False, denial
    if not is_nonempty_str(univ.get("trading_universe_id")) or not is_nonempty_str(
        univ.get("exchange_account_id")
    ):
    ...
    if univ.get("lifecycle_state") not in DATA["trading_universe_contract"]["lifecycle_states"]:
        return deny("TRADING_UNIVERSE_INVALID")
    if (
        not univ.get("instrument_ids")
        or not unique_str_list(univ.get("instrument_ids"))
        or not univ.get("source_catalog_snapshot_ids")
        or not unique_str_list(univ.get("source_catalog_snapshot_ids"))
    ):
        return deny("TRADING_UNIVERSE_INVALID")
    if univ.get("previous_version_id") is not None and not is_nonempty_str(
        univ.get("previous_version_id")
    ):
        return deny("TRADING_UNIVERSE_INVALID")
    ...
    for field in ["activated_at_utc", "retired_at_utc"]:
        if univ.get(field) is not None and not parse_ts(univ[field]):
            return deny("TRADING_UNIVERSE_INVALID")
    if univ["lifecycle_state"] == "ACTIVE" and (
        univ.get("activated_at_utc") is None or univ.get("retired_at_utc") is not None
    ):
        return deny("TRADING_UNIVERSE_INVALID")
...
def validate_universe_lineage(univ, previous_universes_by_id):
    ...
    if univ["version"] == 1:
        return (
            ...
            else deny("TRADING_UNIVERSE_VERSION_CONFLICT")
        )
    if univ["previous_version_id"] is None:
        return deny("TRADING_UNIVERSE_VERSION_CONFLICT")
    visited = {univ["trading_universe_id"]}
    cursor = univ["previous_version_id"]
    expected = univ["version"] - 1
    while cursor is not None:
        if not is_nonempty_str(cursor) or cursor in visited:
            return deny("TRADING_UNIVERSE_VERSION_CONFLICT")
        visited.add(cursor)
        predecessor = previous_universes_by_id.get(cursor)
        if (
            not validate_universe_record(predecessor)[0]
            or predecessor.get("trading_universe_id") != cursor
        ):
            return deny("TRADING_UNIVERSE_VERSION_CONFLICT")
        if (
            predecessor.get("exchange_account_id") != univ["exchange_account_id"]
            or predecessor.get("version") != expected
        ):
            return deny("TRADING_UNIVERSE_VERSION_CONFLICT")
        if predecessor.get("content_hash") != hash_payload(
            DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], predecessor
        ):
            return deny("TRADING_UNIVERSE_VERSION_CONFLICT")
        if predecessor.get("lifecycle_state") not in {"ACTIVE", "RETIRED"}:
            return deny("TRADING_UNIVERSE_VERSION_CONFLICT")
        cursor = predecessor.get("previous_version_id")
        expected -= 1
...
def enforce_operation_states(operation, account):
    ...
    for field, axis in [
        ("lifecycle_state", "allowed_lifecycle_states"),
        ("connection_state", "allowed_connection_states"),
        ("execution_authorization", "allowed_authorization_states"),
    ]:
        allowed = spec[axis]
        if allowed and account.get(field) not in allowed:
            return deny(
                "EXCHANGE_ACCOUNT_RETIRED"
                if field == "lifecycle_state" and account.get(field) == "RETIRED"
                else "ACCOUNT_READINESS_BLOCKED"
            )
    return True, None


def validate_trading_universe_version(
    univ,
    account,
    instruments,
    catalogs,
    previous_universes_by_id=None,
    ...
    if (
        not validate_context(validation_context)
        or not isinstance(univ, dict)
    ...
    trusted_account = (
        validation_context["accounts_by_id"].get(account_id)
        if is_nonempty_str(account_id)
        else None
    )
    trusted_universe = (
        validation_context["universes_by_id"].get(universe_id)
        if is_nonempty_str(universe_id)
        else None
    )
    bindings = [
        (account, trusted_account),
        (univ, trusted_universe),
    ...
    if (
        trusted_account is None
        or trusted_universe is None
        or any(argument != trusted for argument, trusted in bindings)
    ):
        return deny("ACCOUNT_READINESS_BLOCKED")
    ready, readiness = validate_account_activation_readiness(
        trusted_account, validation_context, now=now
    )
    if not ready:
    ...
    if not isinstance(active_universes, list) or any(
        not isinstance(item, dict) for item in active_universes
    ):
        return deny("TRADING_UNIVERSE_INVALID")
    ...
    active_union.update(
        {
            uid: item
            for uid, item in validation_context["universes_by_id"].items()
    ...
    for active in active_union.values():
        ...
        if (
            ...
            and active["exchange_account_id"] == account["exchange_account_id"]
        ):
    ...
    if (
        ...
        or not unique_str_list(univ["source_catalog_snapshot_ids"])
    ):
        return deny("CATALOG_SNAPSHOT_INVALID")
    ...
    if univ["content_hash"] != hash_payload(
        DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], univ
    ):
        return deny("TRADING_UNIVERSE_INVALID")
...
def audit_result(operation, ok, denial=None):
    ...
    if (
        denial not in spec["denial_codes"]
        or not event
        or not schema
        or denial not in schema["allowed_denial_codes"]
    ):
        contract_denial = "CONTRACT_VALIDATION_MAPPING_ERROR"
        return False, {
            "denial": contract_denial,
            "audit_event": "CONTRACT_VALIDATION_MAPPING_REJECTED",
        }
...
CONTEXT_MAPS = {
    ...
    "readiness_by_account_id",
    "active_profile_ids_by_account_id",
}
CONTEXT_LISTS = {"active_universes"}
MAP_ID_FIELDS = {
    "accounts_by_id": "exchange_account_id",
    "credential_profiles_by_id": "credential_profile_id",
    "universes_by_id": "trading_universe_id",
    "catalogs_by_id": "catalog_snapshot_id",
    "instruments_by_id": "instrument_id",
    "previous_profiles_by_id": "credential_profile_id",
    "previous_catalogs_by_id": "catalog_snapshot_id",
    "previous_universes_by_id": "trading_universe_id",
    "account_capability_snapshots_by_id": "account_capability_snapshot_id",
    "previous_account_capability_snapshots_by_id": "account_capability_snapshot_id",
}


def validate_account_structure(account):
    fields = DATA["exchange_account_contract"]["record_fields"]
    if not closed_schema(
        account,
        fields,
        fields + ["legacy_record", "readiness_confirmed", "private_connection_active"],
        "EXCHANGE_ACCOUNT_NOT_FOUND",
    ...
    if (
        ex is None
        or account["environment"] not in ex.get("supported_environments", [])
        or account["market_type"] not in ex.get("supported_market_types", [])
    ):
        return False
    if (
        account["lifecycle_state"] not in DATA["exchange_account_contract"]["lifecycle_states"]
        or account["connection_state"] not in DATA["exchange_account_contract"]["connection_states"]
        or account["execution_authorization"]
        not in DATA["exchange_account_contract"]["execution_authorizations"]
        or account["external_account_identity_state"]
        not in DATA["external_account_identity_contract"]["states"]
    ):
        return False
    if not isinstance(account.get("display_name"), str) or not parse_ts(
        account.get("created_at_utc")
    ):
        return False
    ...
    for field in [
        ...
        if account.get(field) is not None and not is_nonempty_str(account[field]):
            return False
    return all(
        field not in account or type(account[field]) is bool
        for field in ["legacy_record", "readiness_confirmed", "private_connection_active"]
    )
...
def validate_identity_structure(identity, account):
    ...
    if not closed_schema(
        identity, fields, fields + ["revalidated_at_utc"], "EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED"
    )[0] or not isinstance(account, dict):
        return False
    ...
    if any(
        ...
        return False
    if (
        not is_nonempty_str(identity.get("venue_account_identifier"))
        or identity.get("subaccount_identifier") == ""
        or (
            identity.get("subaccount_identifier") is not None
            and not isinstance(identity.get("subaccount_identifier"), str)
        )
    ):
        return False
    if (
        not is_nonempty_str(identity.get("account_type"))
        or not is_nonempty_str(identity.get("adapter_version_source"))
        or not parse_ts(identity.get("verification_timestamp"))
    ):
        return False
    if not unique_str_list(identity.get("observed_permission_set")) or not set(
        identity["observed_permission_set"]
    ) <= set(DATA["credential_profile_contract"]["permission_registry"]):
        return False
    return (
        identity.get("revalidated_at_utc") is None
        or parse_ts(identity.get("revalidated_at_utc")) is not None
    )


def validate_catalog_structure(snapshot, previous_catalogs_by_id):
    return (
        validate_catalog_node(snapshot)
        and validate_catalog_lineage(snapshot, previous_catalogs_by_id)[0]
    )
...
def validate_capability_account_binding(snapshot, accounts_by_id, previous_by_id):
    if (
        ...
        or not isinstance(accounts_by_id, dict)
        or not isinstance(previous_by_id, dict)
    ):
        return False
    account = accounts_by_id.get(snapshot.get("exchange_account_id"))
    return (
        isinstance(account, dict)
        and validate_account_capability_snapshot(snapshot, account, previous_by_id)[0]
    )


def validate_target_history_catalog_bindings(
    instrument_id, history, catalogs_by_id, previous_catalogs_by_id
):
    if (
        not is_nonempty_str(instrument_id)
        or not isinstance(history, list)
        or not isinstance(catalogs_by_id, dict)
        or not isinstance(previous_catalogs_by_id, dict)
    ):
        return False
...
def resolve_catalog_member(
    ...
    if (
        ...
        or not is_nonempty_str(instrument_id)
        or not isinstance(instruments_by_id, dict)
        or not isinstance(instrument_history_by_id, dict)
    ):
        return None
    lineage = (
        {}
        if previous_catalogs_by_id is None and catalog.get("previous_snapshot_id") is None
        else previous_catalogs_by_id
    )
    ...
    if (
        exchange is None
        or catalog["environment"] not in DATA["environment_registry"]
    ...
    if not isinstance(history, list):
        return None
    trusted_catalogs = (
        {catalog.get("catalog_snapshot_id"): catalog} if catalogs_by_id is None else catalogs_by_id
    )
    ...
    if current is not None:
        if (
            ...
            or not validate_instrument_record(current, require_fresh=False)[0]
        ):
            return None
        if (
        ...
        if (
            ...
            or (versions and current_version <= versions[-1])
        ):
            return None
        if bound(current):
            return current
...
def validate_universe_source_membership(
    ...
    *,
    ...
    if not validate_universe_record(universe)[0] or not all(
        isinstance(item, dict)
        for item in [accounts_by_id, catalogs_by_id, instruments_by_id, instrument_history_by_id]
    ):
        return False
    if any(
        not is_nonempty_str(key)
        or not isinstance(value, dict)
        or value.get("exchange_account_id") != key
        or not validate_account_structure(value)
...
def validate_historical_instrument_catalog_bindings(
    instrument_history_by_id, catalogs_by_id, previous_catalogs_by_id
):
    if not all(
        isinstance(item, dict)
        for item in [instrument_history_by_id, catalogs_by_id, previous_catalogs_by_id]
    ):
        return False
    all_catalogs = {**previous_catalogs_by_id, **catalogs_by_id}
    for instrument_id, history in instrument_history_by_id.items():
...
def validate_catalog_instrument_graph(
    catalogs_by_id, previous_catalogs_by_id, instruments_by_id, instrument_history_by_id
):
    if not all(
        isinstance(item, dict)
...
def validate_account_owned_bindings(account, ctx):
    ...
    active_profiles = [
        ...
        if profile.get("exchange_account_id") == account_id
        and profile.get("lifecycle_state") == "ACTIVE"
    ]
    if profile_id is None:
        ...
        if (
            not validate_profile_account_binding(profile, ctx["accounts_by_id"])
            or profile.get("lifecycle_state") != "ACTIVE"
            or active_ids != [profile_id]
            or profile_id in ctx["previous_profiles_by_id"]
            or len(active_profiles) != 1
        ):
            return False
    ...
    if capability_id is not None:
        ...
        if (
            snapshot is None
            or capability_id in ctx["previous_account_capability_snapshots_by_id"]
            or not validate_capability_account_binding(
                snapshot, ctx["accounts_by_id"], ctx["previous_account_capability_snapshots_by_id"]
            )
        ):
            return False
    return True
...
def validate_context(ctx):
    ...
    if any(
        ...
        for name in CONTEXT_MAPS
        - {
            "readiness_by_account_id",
            "active_profile_ids_by_account_id",
            "instrument_history_by_id",
        }
        for key, value in ctx[name].items()
    ):
        return False
    for name, id_field in MAP_ID_FIELDS.items():
        if any(
            not is_nonempty_str(key) or value.get(id_field) != key
            for key, value in ctx[name].items()
        ):
            return False
    disjoint_pairs = [
    ...
    if any(not validate_account_structure(record) for record in ctx["accounts_by_id"].values()):
        return False
    if any(
        not validate_account_owned_bindings(account, ctx)
        for account in ctx["accounts_by_id"].values()
    ):
        return False
    if any(
        not validate_credential_profile(record)[0]
        for name in ["credential_profiles_by_id", "previous_profiles_by_id"]
        for record in ctx[name].values()
    ):
        return False
    if any(
        record.get("lifecycle_state") != "RETIRED"
        for record in ctx["previous_profiles_by_id"].values()
    ):
        return False
    for profile in list(ctx["credential_profiles_by_id"].values()) + list(
        ctx["previous_profiles_by_id"].values()
    ):
    ...
    for account_id, identity in ctx["external_identity_snapshots_by_account_id"].items():
        ...
        if not validate_identity_structure(identity, account):
            return False
    if any(
        not validate_universe_record(record)[0]
        or record.get("content_hash")
        != hash_payload(
            DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], record
        )
        for name in ["universes_by_id", "previous_universes_by_id"]
        for record in ctx[name].values()
    ):
        return False
    if canonical:
    ...
    for map_name in [
        ...
        for snapshot in ctx[map_name].values():
            if not validate_capability_account_binding(
                snapshot, ctx["accounts_by_id"], ctx["previous_account_capability_snapshots_by_id"]
            ):
                return False
    if any(
        not isinstance(key, str) or type(value) is not bool
        for key, value in ctx["readiness_by_account_id"].items()
    ):
        return False
    if any(
        account_id not in ctx["accounts_by_id"] for account_id in ctx["readiness_by_account_id"]
    ):
        return False
    if not all(
        validate_universe_record(item)[0]
        and item.get("content_hash")
        == hash_payload(DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], item)
    ...
    for item in ctx["active_universes"]:
        if (
            ...
            or ctx["universes_by_id"].get(item.get("trading_universe_id")) != item
        ):
            return False
    if any(
        not is_nonempty_str(account_id) or not ids or not unique_str_list(ids)
        for account_id, ids in ctx["active_profile_ids_by_account_id"].items()
    ):
        return False
    for account_id, profile_ids in ctx["active_profile_ids_by_account_id"].items():
        ...
        for profile_id in profile_ids:
            ...
            if (
                ...
                or profile.get("exchange_account_id") != account_id
                or profile.get("lifecycle_state") != "ACTIVE"
            ):
                return False
    ...
    for profile in ctx["credential_profiles_by_id"].values():
        if profile.get("lifecycle_state") == "ACTIVE":
            active_profiles_by_account.setdefault(profile["exchange_account_id"], []).append(
    ...
    for account_id, ids in active_profiles_by_account.items():
        account = ctx["accounts_by_id"].get(account_id)
        if (
            ctx["active_profile_ids_by_account_id"].get(account_id) != ids
            or not isinstance(account, dict)
            or account.get("active_credential_profile_id") != ids[0]
        ):
            return False
    if any(
        account_id not in active_profiles_by_account
        for account_id in ctx["active_profile_ids_by_account_id"]
    ):
        return False
    return True
...
def validate_request_shape(operation, request):
    ...
    if (
        ...
        or set(request) & forbidden
        or set(request) != set(spec["required_inputs"])
    ):
    ...
    for field in [f for f in request if f.endswith("_id") and f != "request_id"]:
        if not is_nonempty_str(request[field]):
            return False, spec["denial_codes"][0]
    if "source_catalog_snapshot_ids" in request and not unique_str_list(
        request["source_catalog_snapshot_ids"]
    ):
        return False, "CATALOG_SNAPSHOT_INVALID"
    if "content_hash" in request and (
        not isinstance(request["content_hash"], str) or not HEX64.fullmatch(request["content_hash"])
    ):
        return False, "TRADING_UNIVERSE_INVALID"
    type_denial = {
        "VERIFY_EXTERNAL_IDENTITY": "EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED",
        "QUARANTINE_LEGACY_LIVE_RECORD": "ACCOUNT_READINESS_BLOCKED",
    }.get(operation, spec["denial_codes"][0])
    string_fields = {
        "portfolio_id",
        "exchange_id",
        "environment",
        ...
        "binding_intent",
        "expected_adapter_version_source",
        "adapter_family_id",
        "adapter_version",
        "retirement_reason",
        "target_lifecycle_state",
        "target_connection_state",
        "target_execution_authorization",
        "quarantine_reason",
    }
...
def validate_operation_request(operation, request, *, validation_context):
    ...
    if handler is None:
        return audit_result(
            operation, False, DATA["operation_validation_matrix"][operation]["denial_codes"][0]
        )
...
def handle_update_account(request, ctx):
    ...
    ok, denial = closed_schema(
        request["expected_identity_fields"],
        expected_fields,
        expected_fields,
        "EXCHANGE_ACCOUNT_IDENTITY_IMMUTABLE",
    )
    if not ok:
        return False, denial
    ...
    if (
        ...
        and result["external_account_identity_state"] == "VERIFIED"
    ):
...
def handle_bind_credential_profile(request, ctx):
    ...
    if (
        account.get("exchange_account_id") != request["exchange_account_id"]
        or profile.get("credential_profile_id") != request["credential_profile_id"]
    ):
        return False, "CREDENTIAL_PROFILE_NOT_FOUND"
    ok, denial = validate_account(
        account, operation="BIND_CREDENTIAL_PROFILE", validation_context=ctx
    )
    if not ok:
        return False, denial
...
def validate_trusted_identity_uniqueness(exchange_account_id, ctx):
    ...
    snapshots = (
        ctx.get("external_identity_snapshots_by_account_id") if isinstance(ctx, dict) else None
    )
    if not isinstance(accounts, dict) or not isinstance(snapshots, dict):
        return deny("EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED")
    account = accounts.get(exchange_account_id)
    snapshot = snapshots.get(exchange_account_id)
    ...
    for other_id, other_snapshot in snapshots.items():
        ...
        if (
            other_id == exchange_account_id
            or not isinstance(other_account, dict)
            or other_account.get("lifecycle_state") != "ACTIVE"
            or not isinstance(other_snapshot, dict)
            or other_snapshot.get("state") != "VERIFIED"
        ):
            continue
...
def validate_account_activation_readiness(account, ctx, now="2026-06-01T00:00:00Z"):
    ...
    if readiness.get(account_id) is not True or account.get("readiness_confirmed") is not True:
        return deny("ACCOUNT_READINESS_BLOCKED")
    ok, denial = validate_account(
        account, operation="ACTIVATE_TRADING_UNIVERSE", validation_context=ctx
    )
    if not ok:
        return False, denial
    ok, denial = enforce_operation_states("ACTIVATE_TRADING_UNIVERSE", account)
    if not ok or account.get("lifecycle_state") != "ACTIVE" or account.get("environment") == "LIVE":
        return False, denial or (
            "LIVE_BLOCKED_BY_EDITION"
            if account.get("environment") == "LIVE"
            else "ACCOUNT_READINESS_BLOCKED"
        )
    ...
    if snapshot_id is not None:
        ...
        ok, denial = validate_account_capability_snapshot(
            snapshot, account, ctx["previous_account_capability_snapshots_by_id"]
        )
        if (
            not ok
            or snapshot.get("status") != "VALID"
            or not ts_lt(now, snapshot.get("stale_after_utc"))
        ):
            return deny("ACCOUNT_READINESS_BLOCKED")
    ...
    if account["environment"] == "TESTNET":
        ...
        if not ok:
            return deny(
                profile_denial
                if profile_denial == "WITHDRAWAL_PERMISSION_FORBIDDEN"
                else "ACCOUNT_READINESS_BLOCKED"
            )
        identity = ctx["external_identity_snapshots_by_account_id"].get(
            account["exchange_account_id"]
        )
        permission_sources = [
            set(profile["permission_snapshot"]),
            set(capability_snapshot["observed_permission_set"]),
        ...
        if "READ_ACCOUNT" not in effective_permissions:
            return deny("ACCOUNT_READINESS_BLOCKED")
        if (
            account["execution_authorization"] == "ORDER_ENTRY_ALLOWED"
            and "PLACE_ORDERS" not in effective_permissions
        ):
            return deny("ACCOUNT_READINESS_BLOCKED")
    else:
    ...
    return True, {
        ...
        "capability_instrument_types": set(capability_snapshot["supported_instrument_types"])
        if capability_snapshot
        else None,
        "extends_product_capabilities": False,
    }


def handle_verify_external_identity(request, ctx):
    if type(request["reconnect"]) is not bool or not is_nonempty_str(
        request["expected_adapter_version_source"]
    ):
        return False, "EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED"
    account = ctx["accounts_by_id"].get(request["exchange_account_id"])
    if account is None:
    ...
    if account.get("environment") == "LIVE":
        return False, "LIVE_BLOCKED_BY_EDITION"
    ok, denial = validate_account(
        account, operation="VERIFY_EXTERNAL_IDENTITY", validation_context=ctx
    )
    if not ok:
        return False, denial
    ...
    if snapshot.get("adapter_version_source") != request["expected_adapter_version_source"]:
        return False, "EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED"
    ok, denial = validate_external_account_identity(
        snapshot, account, reconnect=request["reconnect"] is True
    )
    if not ok:
        return False, denial
...
def handle_activate_trading_universe(request, ctx):
    ...
    if universe is None:
        return False, "TRADING_UNIVERSE_INVALID"
    if (
        universe.get("trading_universe_id") != request["trading_universe_id"]
        or universe.get("exchange_account_id") != request["exchange_account_id"]
    ):
        return False, "TRADING_UNIVERSE_INVALID"
    if universe.get("content_hash") != request["content_hash"] or sorted(
        universe.get("source_catalog_snapshot_ids", [])
    ) != sorted(request["source_catalog_snapshot_ids"]):
        return False, "TRADING_UNIVERSE_INVALID"
    source_catalogs = {
        cid: ctx["catalogs_by_id"].get(cid) for cid in request["source_catalog_snapshot_ids"]
    }
    if (
        account is None
        or account.get("exchange_account_id") != request["exchange_account_id"]
        or any(
            not isinstance(cat, dict) or cat.get("catalog_snapshot_id") != cid
            for cid, cat in source_catalogs.items()
        )
    ):
        return (
            False,
            "EXCHANGE_ACCOUNT_NOT_FOUND" if account is None else "CATALOG_SNAPSHOT_INVALID",
        )
    return validate_trading_universe_version(
        ...
        ctx["instruments_by_id"],
        ctx["catalogs_by_id"],
        ctx["previous_universes_by_id"],
        ctx["active_universes"],
        ctx["previous_catalogs_by_id"],
...
def handle_retire_account(request, ctx):
    ...
    if account.get("private_connection_active") is True or account.get("connection_state") not in {
        "DISCONNECTED",
        "BLOCKED",
    }:
        return False, "ACCOUNT_READINESS_BLOCKED"
...
def handle_retire_instrument(request, ctx):
    ...
    for universe in ctx["active_universes"]:
        if isinstance(universe, dict) and inst["instrument_id"] in universe.get(
            "instrument_ids", []
        ):
...
def handle_quarantine_legacy_live_record(request, ctx):
    ...
    if account.get("exchange_account_id") != request["exchange_account_id"]:
        return False, "EXCHANGE_ACCOUNT_NOT_FOUND"
    if (
        request["legacy_record"] is not True
        or account.get("legacy_record") is not True
        or account.get("environment") != "LIVE"
    ):
        return False, "LIVE_BLOCKED_BY_EDITION"
...
def asset(code="BTC", status="EXACT"):
    return {
        ...
        "asset_namespace": "generic_testnet_venue",
...
def sample_account(**overrides):
    data = {
        "exchange_account_id": "xacc_test_1",
        "portfolio_id": "port_1",
        ...
        "external_subaccount_reference": None,
...
def direct_universe_result(
    ...
    **context_overrides,
    ...
    ctx["previous_universes_by_id"] = (
        {} if previous_universes_by_id is None else previous_universes_by_id
    )
    ctx["active_universes"] = [] if active_universes is None else active_universes
    ctx["previous_catalogs_by_id"] = (
        {} if previous_catalogs_by_id is None else previous_catalogs_by_id
    )
...
def sample_profile(**overrides):
    data = {
        ...
        "secure_store_reference": "secure-store://profile/cred_1",
...
def sample_capability_snapshot(account=None, **overrides):
    ...
    data = {
        "account_capability_snapshot_id": "caps_1",
        ...
        "stale_after_utc": "2026-12-01T00:00:00Z",
        "adapter_family_id": "generic_testnet_adapter_family",
        "adapter_version": "1",
        "content_hash": "",
    }
    data.update(overrides)
    data["content_hash"] = overrides.get("content_hash") or hash_payload(
        DATA["account_capability_snapshot_contract"]["content_hash_definition"], data
    )
    return data
...
def sample_catalog(inst=None, **overrides):
    ...
    data.update(overrides)
    data["content_hash"] = overrides.get("content_hash") or hash_payload(
        DATA["deterministic_hash_contracts"]["catalog_snapshot_content_hash"], data
    )
    return data
...
def sample_universe(account=None, inst=None, catalog=None, **overrides):
    ...
    data = {
        ...
        "activated_at_utc": "2026-01-02T00:00:00Z",
        "retired_at_utc": None,
        "previous_version_id": None,
        "source_catalog_snapshot_ids": [catalog["catalog_snapshot_id"]],
        "content_hash": "",
        "creation_reason": "INITIAL",
    }
    data.update(overrides)
    data["content_hash"] = overrides.get("content_hash") or hash_payload(
        DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], data
    )
    return data


def context(
    account=None, profile=None, identity=None, inst=None, catalog=None, universe=None, **overrides
):
    account = dict(account or sample_account())
    capability_snapshots = {}
    if (
        account.get("exchange_id") == "generic_testnet_venue"
        and account.get("environment") == "TESTNET"
        and account.get("account_capability_snapshot_id") is None
    ):
        account["account_capability_snapshot_id"] = "caps_1"
        capability_snapshots["caps_1"] = sample_capability_snapshot(account)
    if (
        account.get("environment") == "TESTNET"
        and account.get("active_credential_profile_id") is None
    ):
    ...
    bound_profile = account.get("active_credential_profile_id") == profile.get(
        "credential_profile_id"
    )
    data = {
        "accounts_by_id": {account["exchange_account_id"]: account},
        "credential_profiles_by_id": (
            {profile["credential_profile_id"]: profile} if bound_profile else {}
        ),
        "external_identity_snapshots_by_account_id": {account["exchange_account_id"]: identity},
        "universes_by_id": {universe["trading_universe_id"]: universe},
        "catalogs_by_id": {catalog["catalog_snapshot_id"]: catalog},
        "instruments_by_id": {inst["instrument_id"]: inst},
        "instrument_history_by_id": {},
        "previous_profiles_by_id": {},
        "previous_catalogs_by_id": {},
        "previous_universes_by_id": {},
        "account_capability_snapshots_by_id": capability_snapshots,
        "previous_account_capability_snapshots_by_id": {},
        "active_universes": [],
        "active_profile_ids_by_account_id": (
            {account["exchange_account_id"]: [profile["credential_profile_id"]]}
            if bound_profile
            else {}
        ),
        "readiness_by_account_id": {account["exchange_account_id"]: True},
    }
    data.update(overrides)
    return data
...
def request_for(operation, **overrides):
    base = {
        ...
        "UPDATE_ACCOUNT": {
            "request_id": "r2",
            "exchange_account_id": "xacc_test_1",
            "expected_identity_fields": identity_fields(sample_account()),
            "mutable_patch": {"display_name": "Updated"},
        },
        "BIND_CREDENTIAL_PROFILE": {
            "request_id": "r3",
            "exchange_account_id": "xacc_test_1",
            "credential_profile_id": "cred_1",
            "binding_intent": "ACTIVATE",
        },
        "VERIFY_EXTERNAL_IDENTITY": {
            "request_id": "r4",
            "exchange_account_id": "xacc_test_1",
            "expected_adapter_version_source": "generic_testnet_adapter_family/1",
            "reconnect": False,
        },
        ...
        "ACTIVATE_TRADING_UNIVERSE": {
            ...
            "trading_universe_id": "univ_1",
            "exchange_account_id": "xacc_test_1",
            "source_catalog_snapshot_ids": ["cat_1"],
            "content_hash": sample_universe().get("content_hash"),
        },
        "RETIRE_ACCOUNT": {
            "request_id": "r7",
            "exchange_account_id": "xacc_test_1",
            "retirement_reason": "operator",
        },
        "RETIRE_INSTRUMENT": {
            "request_id": "r8",
            "instrument_id": "instr_btcusdt_spot_testnet",
            "catalog_snapshot_id": "cat_1",
            "retirement_reason": "delisted",
        },
        "QUARANTINE_LEGACY_LIVE_RECORD": {
            "request_id": "r9",
            "exchange_account_id": "xacc_live",
            "legacy_record": True,
            "target_lifecycle_state": "DISABLED",
            "target_connection_state": "BLOCKED",
            "target_execution_authorization": "BLOCKED_BY_POLICY",
...
def test_status_and_documentation_sync():
    assert (
        M04["status"] == "closed"
        and "Status: `closed`" in M04_MD.read_text(encoding="utf-8")
        and "Status M0.4 — closed" in ARCH.read_text(encoding="utf-8")
    )
    assert DATA["status"] == "closed"
    assert "Status: `closed`" in MD.read_text(encoding="utf-8")
    assert "Status M0.5 — closed" in ARCH.read_text(encoding="utf-8")
    assert "request carries transport IDs hashes lists only" in json.dumps(
        DATA["operation_dispatch_policy"]
    )
...
def test_request_id_fields_reject_embedded_records():
    for operation, field in [
        ("UPDATE_ACCOUNT", "exchange_account_id"),
        ("BIND_CREDENTIAL_PROFILE", "credential_profile_id"),
        ("ACTIVATE_TRADING_UNIVERSE", "trading_universe_id"),
    ]:
...
def test_create_rejects_active_requested_lifecycle():
    ok, out = validate_operation_request(
        "CREATE_ACCOUNT",
        request_for("CREATE_ACCOUNT", requested_lifecycle_state="ACTIVE"),
        validation_context=context(),
    )
    assert (
        not ok
        and out["denial"] == "ACCOUNT_READINESS_BLOCKED"
        and out["audit_event"] != "EXCHANGE_ACCOUNT_CREATED"
    )
...
def test_update_rejects_request_record_id_mismatch():
    ...
    ok, out = validate_operation_request(
        "UPDATE_ACCOUNT",
        request_for("UPDATE_ACCOUNT", exchange_account_id="xacc_requested"),
        validation_context=context(account=acct),
    )
...
def test_live_update_cannot_preserve_operational_state():
    ...
    req = request_for(
        "UPDATE_ACCOUNT",
        exchange_account_id="xacc_live",
        expected_identity_fields=identity_fields(live),
    )
    ok, out = validate_operation_request(
        "UPDATE_ACCOUNT", req, validation_context=context(account=live)
    )
    assert not ok and out["denial"] == "EXCHANGE_ACCOUNT_NOT_FOUND"
...
def test_verify_external_identity_runs_real_validator():
    ...
    ok, out = validate_operation_request(
        ...
        req,
        validation_context=context(
            account=sample_account(connection_state="SYNCHRONIZING"), identity=bad
        ),
    )
    assert (
        not ok
        and out["denial"] == "EXCHANGE_ACCOUNT_NOT_FOUND"
        and out["audit_event"] != "EXTERNAL_ACCOUNT_IDENTITY_VERIFIED"
    )
...
def test_refresh_catalog_runs_real_validator():
    ...
    ok, out = validate_operation_request(
        "REFRESH_INSTRUMENT_CATALOG",
        req,
        validation_context=context(inst=inst, catalog=bad_catalog),
    )
...
def test_retire_account_runs_real_validator():
    ...
    ok, out = validate_operation_request(
        "RETIRE_ACCOUNT", request_for("RETIRE_ACCOUNT"), validation_context=context(account=acct)
    )
...
def test_retire_instrument_runs_real_validator():
    ...
    ok, out = validate_operation_request(
        "RETIRE_INSTRUMENT",
        request_for("RETIRE_INSTRUMENT"),
        validation_context=context(inst=inst, catalog=cat, active_universes=[active]),
    )
...
def test_quarantine_rejects_active_target():
    ...
    ok, out = validate_operation_request(
        "QUARANTINE_LEGACY_LIVE_RECORD",
        request_for("QUARANTINE_LEGACY_LIVE_RECORD", target_lifecycle_state="ACTIVE"),
        validation_context=context(account=live),
    )
    assert not ok and out["denial"] == "EXCHANGE_ACCOUNT_NOT_FOUND"
...
def test_quarantine_rejects_online_target():
    ...
    ok, out = validate_operation_request(
        "QUARANTINE_LEGACY_LIVE_RECORD",
        request_for("QUARANTINE_LEGACY_LIVE_RECORD", target_connection_state="ONLINE"),
        validation_context=context(account=live),
    )
    assert not ok and out["denial"] == "EXCHANGE_ACCOUNT_NOT_FOUND"
...
def test_quarantine_rejects_order_entry_target():
    ...
    ok, out = validate_operation_request(
        "QUARANTINE_LEGACY_LIVE_RECORD",
        request_for(
            "QUARANTINE_LEGACY_LIVE_RECORD", target_execution_authorization="ORDER_ENTRY_ALLOWED"
        ),
        validation_context=context(account=live),
    )
    assert not ok and out["denial"] == "EXCHANGE_ACCOUNT_NOT_FOUND"
...
def test_quarantine_checks_legacy_flag_and_reason():
    ...
    assert (
        validate_operation_request(
            "QUARANTINE_LEGACY_LIVE_RECORD",
            request_for("QUARANTINE_LEGACY_LIVE_RECORD", legacy_record=False),
            validation_context=context(account=live),
        )[1]["denial"]
        == "EXCHANGE_ACCOUNT_NOT_FOUND"
    )
    assert (
        validate_operation_request(
            "QUARANTINE_LEGACY_LIVE_RECORD",
            request_for("QUARANTINE_LEGACY_LIVE_RECORD", quarantine_reason=""),
            validation_context=context(account=live),
        )[1]["denial"]
        == "EXCHANGE_ACCOUNT_NOT_FOUND"
    )
    assert (
        validate_operation_request(
            "QUARANTINE_LEGACY_LIVE_RECORD",
            request_for("QUARANTINE_LEGACY_LIVE_RECORD"),
            validation_context=context(account=sample_account(exchange_account_id="xacc_live")),
        )[1]["denial"]
        == "LIVE_BLOCKED_BY_EDITION"
    )
...
def test_denial_audit_schema_has_nonempty_allowed_codes():
    ...
    for schema in DATA["audit_event_schemas"]:
        assert len(schema["required_identifier_fields"]) == len(
            set(schema["required_identifier_fields"])
        )
        ...
        if schema["outcome_class"] == "DENIAL":
            ...
            assert (
                schema["allowed_denial_codes"] == []
                and schema["denial_code"] == "nullable_must_be_null_on_success"
            )
...
def test_nested_catalog_wrong_type_fails_closed():
    for bad_context in [
        ...
        [],
        {"catalogs_by_id": []},
        {"catalogs_by_id": {"cat_1": []}},
        {"catalogs_by_id": {"cat_1": {"missing": "fields"}}},
    ]:
        ok, out = validate_operation_request(
            "REFRESH_INSTRUMENT_CATALOG",
            request_for("REFRESH_INSTRUMENT_CATALOG"),
            validation_context=bad_context,
        )
        assert not ok and out["denial"] in DENIALS
...
def test_catalog_lineage_rejects_two_node_cycle():
    ...
    assert (
        validate_instrument_catalog_snapshot(
            cat_a,
            {inst["instrument_id"]: inst},
            {"cat_b": cat_b, "cat_a": cat_a},
            instrument_history_by_id={},
        )[1]
...
def test_catalog_lineage_rejects_multinode_cycle():
    ...
    assert (
        validate_instrument_catalog_snapshot(
            cat_a,
            {inst["instrument_id"]: inst},
            {"cat_b": cat_b, "cat_c": cat_c, "cat_a": cat_a},
            instrument_history_by_id={},
        )[1]
        == "CATALOG_SNAPSHOT_INVALID"
    )
...
def test_unknown_operation_and_no_caller_result_controls():
    assert validate_operation_request(
        "NO_SUCH_OP", {"request_id": "r"}, validation_context=context()
    )[1] == {"denial": "UNKNOWN_OPERATION", "audit_event": "UNKNOWN_OPERATION_REJECTED"}
...
def test_exact_venue_symbol_and_decimal_fullmatch():
    ...
    assert validate_instrument_record(sample_instrument(venue_symbol="BtcUsdt"))[0]
    assert (
        validate_instrument_record(sample_instrument(venue_symbol=" BTCUSDT"))[1]
...
def test_json_markdown_sync_for_final_fix():
    ...
    for phrase in [
        ...
        "brak default-success",
        "pełny handler",
        "visited set",
        "literal bool readiness",
        "nested fail-closed",
    ]:
        assert phrase in md
    assert DATA["status"] == "closed"
    ...
    for operation in OPERATIONS:
        for field in CONTEXT_MAPS | CONTEXT_LISTS:
            for bad in (None, 1, "bad", [] if field in CONTEXT_MAPS else {}):
                ...
                ctx[field] = bad
                ok, out = validate_operation_request(
                    operation, request_for(operation), validation_context=ctx
                )
                assert not ok and out["denial"] in DENIALS
...
def test_verify_rejects_non_boolean_reconnect():
    for value in (0, 1, "true", "yes", [], {}, None):
        ok, out = validate_operation_request(
            "VERIFY_EXTERNAL_IDENTITY",
            request_for("VERIFY_EXTERNAL_IDENTITY", reconnect=value),
            validation_context=context(),
        )
        assert not ok and out["denial"] == "EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED"
...
def test_verify_non_boolean_reconnect_cannot_bypass_revalidation():
    ...
    for value in (1, "true", [], {}):
        ok, out = validate_operation_request(
            "VERIFY_EXTERNAL_IDENTITY",
            request_for("VERIFY_EXTERNAL_IDENTITY", reconnect=value),
            validation_context=context(identity=snapshot),
        )
...
def test_create_rejects_non_string_external_account_reference():
    for value in ({}, [], 1, True, ""):
        assert (
            validate_operation_request(
                "CREATE_ACCOUNT",
                request_for("CREATE_ACCOUNT", external_account_reference=value),
                validation_context=context(),
            )[0]
            is False
        )
...
def test_create_rejects_non_string_external_subaccount_reference():
    for value in ({}, [], 1, True, ""):
        assert (
            validate_operation_request(
                "CREATE_ACCOUNT",
                request_for("CREATE_ACCOUNT", external_subaccount_reference=value),
                validation_context=context(),
            )[0]
            is False
        )
...
def test_catalog_rejects_exchange_unsupported_environment():
    ...
    ok, out = validate_operation_request(
        "REFRESH_INSTRUMENT_CATALOG",
        request_for("REFRESH_INSTRUMENT_CATALOG", environment="PAPER"),
        validation_context=ctx,
    )
    assert not ok and out["denial"] == "UNKNOWN_EXCHANGE_ID"
...
def test_catalog_rejects_exchange_unsupported_market_type():
    ...
    ok, out = validate_operation_request(
        "REFRESH_INSTRUMENT_CATALOG",
        request_for("REFRESH_INSTRUMENT_CATALOG", market_type="MARGIN"),
        validation_context=ctx,
    )
    assert not ok and out["denial"] == "UNKNOWN_EXCHANGE_ID"
...
def test_universe_rejects_instrument_not_in_source_catalogs():
    ...
    member = sample_instrument(
        instrument_id="instr_member", catalog_snapshot_id="cat_other", venue_symbol="MEMBER"
    )
    ...
    other_catalog = sample_catalog(
        member, catalog_snapshot_id="cat_other", instrument_ids=[member["instrument_id"]]
    )
    ok, denial = direct_universe_result(
        universe,
        account,
        {member["instrument_id"]: member, source_inst["instrument_id"]: source_inst},
        {catalog["catalog_snapshot_id"]: catalog, "cat_other": other_catalog},
    )
...
def test_universe_rejects_instrument_from_different_catalog():
    ...
    assert (
        direct_universe_result(
            universe, account, {inst["instrument_id"]: inst}, {"cat_1": catalog}
        )[0]
        is False
    )
...
def test_universe_accepts_valid_catalog_with_predecessor_lineage():
    ...
    assert direct_universe_result(
        universe,
        account,
        {inst["instrument_id"]: inst},
        {"cat_1": current},
        previous_catalogs_by_id={"cat_old": old},
        instrument_history_by_id={inst["instrument_id"]: [historical]},
    )[0]
...
def test_universe_rejects_broken_source_catalog_lineage():
    account = sample_account()
    inst = sample_instrument()
    catalog = sample_catalog(inst, previous_snapshot_id="missing")
    universe = sample_universe(account, inst, catalog)
    assert (
        direct_universe_result(
            universe, account, {inst["instrument_id"]: inst}, {"cat_1": catalog}
...
def test_runtime_denials_are_declared_for_every_handler():
    ...
    scenarios += [
        ("CREATE_ACCOUNT", request_for("CREATE_ACCOUNT", environment="LIVE"), context()),
        (
            "REFRESH_INSTRUMENT_CATALOG",
            request_for("REFRESH_INSTRUMENT_CATALOG", environment="PAPER"),
            context(),
...
def test_runtime_denial_event_schema_matches_actual_denial():
    ...
    for operation, spec in DATA["operation_validation_matrix"].items():
        for denial, event in spec["denial_event_by_code"].items():
            assert (
                denial in schemas[event]["allowed_denial_codes"]
                and event != "UNKNOWN_OPERATION_REJECTED"
            )
...
def test_activate_unsupported_environment_has_valid_audit_mapping():
    ...
    assert (
        spec["denial_event_by_code"]["EXCHANGE_ENVIRONMENT_UNSUPPORTED"]
        == "TRADING_UNIVERSE_REJECTED"
    )
...
def test_audit_result_never_pairs_denial_with_unrelated_event():
    ...
    assert not ok and out == {
        "denial": "CONTRACT_VALIDATION_MAPPING_ERROR",
        "audit_event": "CONTRACT_VALIDATION_MAPPING_REJECTED",
    }
...
def assert_universe_denied(**overrides):
    account = sample_account()
    inst = sample_instrument()
    catalog = sample_catalog(inst)
    universe = sample_universe(account, inst, catalog, **overrides)
    ok, denial = direct_universe_result(
        universe, account, {inst["instrument_id"]: inst}, {catalog["catalog_snapshot_id"]: catalog}
    )
    assert not ok
    return denial
...
def test_universe_rejects_duplicate_instrument_ids():
    assert (
        assert_universe_denied(instrument_ids=["instr_btcusdt_spot_testnet"] * 2)
        == "ACCOUNT_READINESS_BLOCKED"
    )
...
def versioned_universe(version=2, previous_version_id="univ_1", **overrides):
    ...
    cat = sample_catalog(inst)
    values = {
        "trading_universe_id": f"univ_{version}",
        "version": version,
...
def test_universe_rejects_previous_version_number_gap():
    current = versioned_universe(version=3, previous_version_id="univ_1")
    old = versioned_universe(version=1, previous_version_id=None, trading_universe_id="univ_1")
    assert (
        validate_universe_lineage(current, {"univ_1": old})[1]
        == "TRADING_UNIVERSE_VERSION_CONFLICT"
    )


def test_universe_rejects_predecessor_id_mismatch():
    current = versioned_universe()
    old = versioned_universe(version=1, previous_version_id=None, trading_universe_id="spoof")
    assert (
        validate_universe_lineage(current, {"univ_1": old})[1]
        == "TRADING_UNIVERSE_VERSION_CONFLICT"
    )
...
def test_universe_rejects_multinode_version_cycle():
    current = versioned_universe(version=3, previous_version_id="univ_2")
    two = versioned_universe(version=2, previous_version_id="univ_1")
    one = versioned_universe(version=1, previous_version_id="univ_3")
    assert (
        validate_universe_lineage(current, {"univ_2": two, "univ_1": one})[1]
        == "TRADING_UNIVERSE_VERSION_CONFLICT"
    )
...
def test_universe_rejects_other_active_version_same_account():
    ...
    inst = sample_instrument()
    cat = sample_catalog(inst)
    current = sample_universe(account, inst, cat)
    active = sample_universe(account, inst, cat, trading_universe_id="univ_other")
    assert (
        direct_universe_result(
            current,
            account,
            ...
            universes_by_id={
                current["trading_universe_id"]: current,
                active["trading_universe_id"]: active,
            },
        )[1]
        == "TRADING_UNIVERSE_VERSION_CONFLICT"
    )
...
def test_universe_allows_active_version_of_other_account():
    ...
    cat = sample_catalog(inst)
    current = sample_universe(account, inst, cat)
    other = sample_universe(
        sample_account(exchange_account_id="xacc_other"),
        inst,
        cat,
        trading_universe_id="univ_other",
        exchange_account_id="xacc_other",
    )
    other_account = sample_account(
        exchange_account_id="xacc_other", active_credential_profile_id=None
    )
    assert direct_universe_result(
        current,
        account,
        {inst["instrument_id"]: inst},
        {"cat_1": cat},
        active_universes=[other],
        universes_by_id={
            current["trading_universe_id"]: current,
            other["trading_universe_id"]: other,
        },
        accounts_by_id={
            account["exchange_account_id"]: context(account=account)["accounts_by_id"][
                account["exchange_account_id"]
            ],
            "xacc_other": other_account,
        },
    )[0]


def operation_result(operation, account):
    return validate_operation_request(
        operation, request_for(operation), validation_context=context(account=account)
    )
...
def test_activate_rejects_disconnected_account():
    assert (
        operation_result(
            "ACTIVATE_TRADING_UNIVERSE", sample_account(connection_state="DISCONNECTED")
        )[1]["denial"]
        == "ACCOUNT_READINESS_BLOCKED"
    )
...
def test_activate_rejects_connecting_account():
    assert (
        operation_result(
            "ACTIVATE_TRADING_UNIVERSE", sample_account(connection_state="CONNECTING")
        )[0]
        is False
    )
...
def test_activate_rejects_synchronizing_account():
    assert (
        operation_result(
            "ACTIVATE_TRADING_UNIVERSE", sample_account(connection_state="SYNCHRONIZING")
        )[0]
        is False
    )
...
def test_bind_rejects_online_account():
    assert (
        operation_result("BIND_CREDENTIAL_PROFILE", sample_account(connection_state="ONLINE"))[1][
            "denial"
        ]
        == "ACCOUNT_READINESS_BLOCKED"
    )
...
def test_verify_rejects_disallowed_connection_state():
    assert (
        operation_result("VERIFY_EXTERNAL_IDENTITY", sample_account(connection_state="ONLINE"))[1][
            "denial"
        ]
        == "ACCOUNT_READINESS_BLOCKED"
    )
...
def test_operation_state_matrix_is_enforced_dynamically():
    cases = [
        (
            "ACTIVATE_TRADING_UNIVERSE",
            "connection_state",
            ["DISCONNECTED", "CONNECTING", "SYNCHRONIZING", "BLOCKED"],
        ),
        (
            "BIND_CREDENTIAL_PROFILE",
            "connection_state",
            ["ONLINE", "CONNECTING", "SYNCHRONIZING", "DEGRADED"],
        ),
        ("VERIFY_EXTERNAL_IDENTITY", "lifecycle_state", ["DRAFT", "RETIRED"]),
    ]
    for operation, field, values in cases:
        for value in values:
            account_values = {field: value}
            if field != "connection_state":
                account_values["connection_state"] = "DISCONNECTED"
            ok, out = operation_result(operation, sample_account(**account_values))
            assert (
                not ok
                and out["denial"] in DATA["operation_validation_matrix"][operation]["denial_codes"]
            )


def test_context_map_key_record_id_mismatches():
    cases = [
        ("accounts_by_id", sample_account(), "false"),
        ("credential_profiles_by_id", sample_profile(), "false"),
        ("universes_by_id", sample_universe(), "false"),
        ("catalogs_by_id", sample_catalog(), "false"),
        ("instruments_by_id", sample_instrument(), "false"),
    ]
    for field, record, key in cases:
        ctx = context()
        ctx[field] = {key: record}
        ok, out = validate_operation_request(
            "CREATE_ACCOUNT", request_for("CREATE_ACCOUNT"), validation_context=ctx
        )
        assert not ok and out["denial"] in DENIALS
...
def test_context_rejects_account_map_key_record_id_mismatch():
    ctx = context()
    ctx["accounts_by_id"] = {"false": sample_account()}
    assert (
        validate_operation_request(
            "CREATE_ACCOUNT", request_for("CREATE_ACCOUNT"), validation_context=ctx
        )[0]
...
def test_context_rejects_profile_map_key_record_id_mismatch():
    ctx = context()
    ctx["credential_profiles_by_id"] = {"false": sample_profile()}
    assert (
        validate_operation_request(
            "CREATE_ACCOUNT", request_for("CREATE_ACCOUNT"), validation_context=ctx
        )[0]
...
def test_context_rejects_universe_map_key_record_id_mismatch():
    ctx = context()
    ctx["universes_by_id"] = {"false": sample_universe()}
    assert (
        validate_operation_request(
            "CREATE_ACCOUNT", request_for("CREATE_ACCOUNT"), validation_context=ctx
        )[0]
...
def test_context_rejects_catalog_map_key_record_id_mismatch():
    ctx = context()
    ctx["catalogs_by_id"] = {"false": sample_catalog()}
    assert (
        validate_operation_request(
            "CREATE_ACCOUNT", request_for("CREATE_ACCOUNT"), validation_context=ctx
        )[0]
...
def test_context_rejects_instrument_map_key_record_id_mismatch():
    ctx = context()
    ctx["instruments_by_id"] = {"false": sample_instrument()}
    assert (
        validate_operation_request(
            "CREATE_ACCOUNT", request_for("CREATE_ACCOUNT"), validation_context=ctx
        )[0]
...
def test_verify_cannot_use_account_stored_under_false_id():
    ctx = context()
    ctx["accounts_by_id"] = {"xacc_test_1": sample_account(exchange_account_id="xacc_spoof")}
    assert (
        validate_operation_request(
            "VERIFY_EXTERNAL_IDENTITY",
            request_for("VERIFY_EXTERNAL_IDENTITY"),
            validation_context=ctx,
        )[0]
        is False
    )
...
def test_refresh_cannot_use_catalog_stored_under_false_id():
    ctx = context()
    ctx["catalogs_by_id"] = {"cat_1": sample_catalog(catalog_snapshot_id="cat_spoof")}
    assert (
        validate_operation_request(
            "REFRESH_INSTRUMENT_CATALOG",
            request_for("REFRESH_INSTRUMENT_CATALOG"),
...
def test_retire_cannot_use_record_stored_under_false_id():
    ctx = context()
    ctx["instruments_by_id"] = {
        "instr_btcusdt_spot_testnet": sample_instrument(instrument_id="instr_spoof")
    }
    assert (
        validate_operation_request(
            "RETIRE_INSTRUMENT", request_for("RETIRE_INSTRUMENT"), validation_context=ctx
        )[0]
        is False
    )
...
def test_credential_lineage_rejects_predecessor_id_mismatch():
    ...
    assert (
        ...
        == "CREDENTIAL_PROFILE_NOT_ACTIVE"
    )


def test_bind_rejects_two_active_profiles_same_account():
    ctx = context(active_profile_ids_by_account_id={"xacc_test_1": ["cred_1", "cred_2"]})
    acct = sample_account(connection_state="DISCONNECTED")
    ctx = context(
        account=acct, active_profile_ids_by_account_id={"xacc_test_1": ["cred_1", "cred_2"]}
    )
    assert (
        validate_operation_request(
            "BIND_CREDENTIAL_PROFILE",
            request_for("BIND_CREDENTIAL_PROFILE"),
            validation_context=ctx,
        )[0]
        is False
    )
...
def test_bind_allows_active_profile_on_other_account():
    ...
    ctx = context(
        account=acct,
        active_profile_ids_by_account_id={"xacc_test_1": ["cred_1"], "xacc_other": ["cred_other"]},
    )
    ctx["accounts_by_id"]["xacc_other"] = sample_account(
        exchange_account_id="xacc_other", active_credential_profile_id="cred_other"
    )
    ctx["credential_profiles_by_id"]["cred_other"] = sample_profile(
        credential_profile_id="cred_other", exchange_account_id="xacc_other"
    )
    assert validate_operation_request(
        "BIND_CREDENTIAL_PROFILE", request_for("BIND_CREDENTIAL_PROFILE"), validation_context=ctx
    )[0]
...
def test_non_rotated_profile_still_checks_active_uniqueness():
    ...
    acct = sample_account(connection_state="DISCONNECTED")
    assert (
        validate_credential_profile_binding(
            profile,
            acct,
            {
                "previous_profiles_by_id": {},
                "active_profile_ids_by_account_id": {"xacc_test_1": ["cred_1", "cred_2"]},
            },
        )[0]
        is False
    )
...
def test_catalog_lineage_rejects_predecessor_identity_and_record_errors():
    ...
    variants = [
        sample_catalog(inst, catalog_snapshot_id="cat_spoof"),
        {"catalog_snapshot_id": "cat_old"},
        sample_catalog(inst, catalog_snapshot_id="cat_old", content_hash="0" * 64),
        sample_catalog(inst, catalog_snapshot_id="cat_old", adapter_family_id="wrong"),
        sample_catalog(inst, catalog_snapshot_id="cat_old", observed_at_utc="bad"),
    ]
    for predecessor in variants:
        assert (
            validate_catalog_lineage(current, {"cat_old": predecessor})[1]
            == "CATALOG_SNAPSHOT_INVALID"
        )
...
def test_catalog_lineage_rejects_invalid_predecessor_hash():
    assert (
        catalog_lineage_result(
            sample_catalog(catalog_snapshot_id="cat_old", content_hash="0" * 64)
        )[0]
        is False
    )
...
def test_catalog_lineage_rejects_invalid_predecessor_adapter():
    assert (
        catalog_lineage_result(
            sample_catalog(catalog_snapshot_id="cat_old", adapter_family_id="spoof")
        )[0]
        is False
    )
...
def test_catalog_lineage_rejects_invalid_predecessor_timestamp():
    assert (
        catalog_lineage_result(
            sample_catalog(catalog_snapshot_id="cat_old", observed_at_utc="bad")
        )[0]
        is False
    )
...
def test_update_rejects_malformed_mutable_field_types():
    for field in [
        ...
        for value in ({}, []):
            ok, out = validate_operation_request(
                "UPDATE_ACCOUNT",
                request_for("UPDATE_ACCOUNT", mutable_patch={field: value}),
                validation_context=context(),
            )
...
def test_dynamic_adversarial_matrix_has_complete_audit_mapping():
    scenarios = [
        ...
        (
            "BIND_CREDENTIAL_PROFILE",
            sample_account(lifecycle_state="RETIRED", connection_state="DISCONNECTED"),
        ),
        (
            "VERIFY_EXTERNAL_IDENTITY",
            sample_account(lifecycle_state="DRAFT", connection_state="DISCONNECTED"),
        ),
    ]
    schemas = {s["event_name"]: s for s in DATA["audit_event_schemas"]}
    for operation, account in scenarios:
        ok, out = operation_result(operation, account)
        spec = DATA["operation_validation_matrix"][operation]
        assert not ok and out["denial"] in spec["denial_codes"]
        event = spec["denial_event_by_code"][out["denial"]]
        assert (
            out["audit_event"] == event
            and event in DATA["audit_event_registry"]
            and out["denial"] in schemas[event]["allowed_denial_codes"]
            and event != "UNKNOWN_OPERATION_REJECTED"
        )
...
def test_verify_request_cannot_embed_identity_snapshot():
    req = request_for("VERIFY_EXTERNAL_IDENTITY")
    req["identity_snapshot"] = sample_identity()
    assert (
        validate_operation_request("VERIFY_EXTERNAL_IDENTITY", req, validation_context=context())[0]
        is False
    )


def test_verify_uses_trusted_context_snapshot():
    acct = sample_account(connection_state="SYNCHRONIZING")
    ctx = context(account=acct, identity=sample_identity())
    assert validate_operation_request(
        "VERIFY_EXTERNAL_IDENTITY", request_for("VERIFY_EXTERNAL_IDENTITY"), validation_context=ctx
    )[0]


def test_verify_trusted_mismatch_cannot_be_overridden_by_request():
    acct = sample_account(connection_state="SYNCHRONIZING")
    ctx = context(account=acct, identity=sample_identity(state="MISMATCH"))
    req = request_for("VERIFY_EXTERNAL_IDENTITY")
    assert (
        validate_operation_request("VERIFY_EXTERNAL_IDENTITY", req, validation_context=ctx)[1][
            "denial"
        ]
    ...
    req["state"] = "VERIFIED"
    assert (
        validate_operation_request("VERIFY_EXTERNAL_IDENTITY", req, validation_context=ctx)[0]
        is False
    )
...
def test_verify_trusted_unavailable_cannot_be_overridden_by_request():
    acct = sample_account(connection_state="SYNCHRONIZING")
    ctx = context(account=acct, identity=sample_identity(state="UNAVAILABLE"))
    assert (
        validate_operation_request(
            "VERIFY_EXTERNAL_IDENTITY",
            request_for("VERIFY_EXTERNAL_IDENTITY"),
        ...
        == "EXTERNAL_ACCOUNT_IDENTITY_UNAVAILABLE"
    )
...
def test_verify_missing_trusted_snapshot_fails_closed():
    acct = sample_account(connection_state="SYNCHRONIZING")
    ctx = context(account=acct)
    ctx["external_identity_snapshots_by_account_id"] = {}
    assert (
        validate_operation_request(
            "VERIFY_EXTERNAL_IDENTITY",
            request_for("VERIFY_EXTERNAL_IDENTITY"),
...
def test_universe_rejects_partial_source_catalog():
    acct = sample_account()
    inst = sample_instrument()
    cat = sample_catalog(inst, status="PARTIAL")
    univ = sample_universe(acct, inst, cat)
    assert (
        direct_universe_result(
            univ, acct, {inst["instrument_id"]: inst}, {cat["catalog_snapshot_id"]: cat}
        )[1]
        == "CATALOG_SNAPSHOT_INVALID"
    )
...
def test_universe_accepts_only_valid_source_catalogs():
    acct = sample_account()
    inst = sample_instrument()
    cat = sample_catalog(inst, status="VALID")
    univ = sample_universe(acct, inst, cat)
    assert direct_universe_result(
        univ, acct, {inst["instrument_id"]: inst}, {cat["catalog_snapshot_id"]: cat}
    )[0]
...
def test_partial_catalog_never_emits_trading_universe_activated():
    inst = sample_instrument()
    cat = sample_catalog(inst, status="PARTIAL")
    univ = sample_universe(catalog=cat)
    ctx = context(inst=inst, catalog=cat, universe=univ)
    ok, out = validate_operation_request(
        "ACTIVATE_TRADING_UNIVERSE",
        request_for("ACTIVATE_TRADING_UNIVERSE", content_hash=univ["content_hash"]),
        validation_context=ctx,
    )
    assert not ok and out["audit_event"] != "TRADING_UNIVERSE_ACTIVATED"
...
def test_refresh_and_activation_catalog_status_policies_are_distinct():
    ...
    assert "PARTIAL" in policy["REFRESH_INSTRUMENT_CATALOG"] and policy[
        "ACTIVATE_TRADING_UNIVERSE"
    ] == ["VALID"]
...
def test_update_rejects_blocked_by_kill_switch_when_not_allowed():
    ok, out = validate_operation_request(
        "UPDATE_ACCOUNT",
        request_for(
            "UPDATE_ACCOUNT", mutable_patch={"execution_authorization": "BLOCKED_BY_KILL_SWITCH"}
        ),
        validation_context=context(),
    )
    assert not ok and out["denial"] == "ACCOUNT_READINESS_BLOCKED"
...
def test_update_rejects_blocked_by_lease_when_not_allowed():
    ok, out = validate_operation_request(
        "UPDATE_ACCOUNT",
        request_for(
            "UPDATE_ACCOUNT", mutable_patch={"execution_authorization": "BLOCKED_BY_LEASE"}
        ),
        validation_context=context(),
    )
    assert not ok and out["denial"] == "ACCOUNT_READINESS_BLOCKED"
...
def test_update_enforces_result_state_not_only_input_state():
    assert (
        validate_operation_request(
            ...
            request_for(
                "UPDATE_ACCOUNT",
                mutable_patch={"execution_authorization": "BLOCKED_BY_KILL_SWITCH"},
            ),
            validation_context=context(),
        )[0]
        is False
...
def test_every_nonempty_operation_state_axis_is_enforced():
    for operation, spec in DATA["operation_validation_matrix"].items():
        for axis, field in [
            ...
            ("allowed_connection_states", "connection_state"),
            ("allowed_authorization_states", "execution_authorization"),
        ]:
...
def derivative_instrument(instrument_type, market_type, **overrides):
    data = sample_instrument(
        ...
        base_asset_reference=asset("BTC"),
        quote_asset_reference=asset("USD"),
    )
    data["base_asset_reference"]["asset_namespace"] = "paper_simulated_venue"
    data["quote_asset_reference"]["asset_namespace"] = "paper_simulated_venue"
    data.update(overrides)
    return data
...
def settlement(code="USD"):
    ref = asset(code)
    ref["asset_namespace"] = "paper_simulated_venue"
    return ref


def test_instrument_metadata_version_rejects_bool():
    assert (
        validate_instrument_record(sample_instrument(metadata_version=True))[1]
        == "INSTRUMENT_METADATA_INVALID"
    )
...
def test_instrument_none_minimum_with_maximum_fails_closed():
    assert (
        validate_instrument_record(sample_instrument(min_quantity=None, max_quantity="1"))[1]
        == "INSTRUMENT_METADATA_INVALID"
    )
...
def test_instrument_notional_range_is_validated():
    assert (
        validate_instrument_record(sample_instrument(min_notional="10", max_notional="1"))[1]
        == "INSTRUMENT_METADATA_INVALID"
    )
...
def test_instrument_wrong_decimal_types_never_raise():
    for value in [None, 1, True, {}, []]:
        assert validate_instrument_record(sample_instrument(price_tick=value))[0] is False
...
def test_perpetual_requires_contract_fields():
    inst = derivative_instrument(
        "PERPETUAL_CONTRACT", "PERPETUAL", settlement_asset_reference=settlement()
    )
...
def test_delivery_future_requires_expiry():
    inst = derivative_instrument(
        ...
        settlement_asset_reference=settlement(),
    )
    assert validate_instrument_record(inst)[0] is False
...
def test_option_requires_strike_and_side():
    inst = derivative_instrument(
        ...
        contract_value_currency="USD",
        derivative_settlement_type="LINEAR",
        expiry_at_utc="2026-10-01T00:00:00Z",
        settlement_asset_reference=settlement(),
    )
    assert validate_instrument_record(inst)[0] is False
...
def test_instrument_adversarial_types_never_raise():
    for field in DATA["instrument_contract"]["record_fields"]:
        ...
        for value in [{}, []]:
            assert validate_instrument_record(sample_instrument(**{field: value}))[0] is False


def test_universe_lineage_rejects_predecessor_hash_mismatch():
    current = versioned_universe()
    old = versioned_universe(version=1, previous_version_id=None)
...
def active_identity_result(**changes):
    ...
    inst = sample_instrument()
    cat = sample_catalog(inst)
    current = sample_universe(acct, inst, cat)
    active = dict(current)
    active.update(changes)
    return direct_universe_result(
        current, acct, {inst["instrument_id"]: inst}, {"cat_1": cat}, active_universes=[active]
    )
...
def test_exact_idempotent_reactivation_policy_is_explicit():
    assert (
        active_identity_result()[0]
        and "exactly equal" in DATA["trading_universe_contract"]["idempotent_reactivation_policy"]
    )
...
def test_active_profile_rejects_retired_timestamp():
    assert (
        validate_credential_profile(sample_profile(retired_at_utc="2026-02-01T00:00:00Z"))[0]
        is False
    )
...
def test_retired_profile_requires_retired_timestamp():
    assert (
        validate_credential_profile(sample_profile(lifecycle_state="RETIRED", retired_at_utc=None))[
            0
        ]
        is False
    )
...
def test_profile_rejects_plaintext_secret_reference():
    assert (
        validate_credential_profile(
            sample_profile(secure_store_reference="secure-store://plaintext-secret")
        )[0]
        is False
    )
...
def test_profile_requires_secure_store_reference_shape():
    for value in ["", "file://secret", None, {}, []]:
        assert validate_credential_profile(sample_profile(secure_store_reference=value))[0] is False
...
def test_context_rejects_malformed_active_universe_record():
    ctx = context(active_universes=[{"trading_universe_id": "univ_bad"}])
    assert validate_context(ctx) is False


def create_with_existing(existing, **request_overrides):
    ctx = context(account=existing)
    return validate_operation_request(
        "CREATE_ACCOUNT", request_for("CREATE_ACCOUNT", **request_overrides), validation_context=ctx
    )


def test_create_allows_distinct_subaccounts_same_scope():
    existing = sample_account(
        external_account_reference="venue-account", external_subaccount_reference="sub-a"
    )
    assert create_with_existing(
        existing, external_account_reference="venue-account", external_subaccount_reference="sub-b"
    )[0]
...
def test_create_allows_multiple_draft_accounts_same_exchange_scope():
    ...
    assert create_with_existing(
        existing, external_account_reference=None, external_subaccount_reference=None
    )[0]
...
def test_verified_same_external_account_and_subaccount_collides():
    ...
    ok, out = create_with_existing(
        existing, external_account_reference="venue-account", external_subaccount_reference="sub-a"
    )
...
def test_account_collision_policy_matches_markdown_and_json():
    assert "different subaccounts" in " ".join(
        DATA["exchange_account_contract"]["multi_account_collision_policy"]
    )
    assert "różne subkonta" in MD.read_text(encoding="utf-8")


def trusted_collision_context(other_subaccount=None, target_subaccount=None):
    target = sample_account(connection_state="SYNCHRONIZING")
    ctx = context(account=target, identity=sample_identity(subaccount_identifier=target_subaccount))
    other = sample_account(exchange_account_id="xacc_other")
    ctx["accounts_by_id"]["xacc_other"] = other
    ctx["external_identity_snapshots_by_account_id"]["xacc_other"] = sample_identity(
        subaccount_identifier=other_subaccount
    )
    return ctx
...
def test_verify_rejects_duplicate_trusted_identity_on_other_active_account():
    ok, out = validate_operation_request(
        "VERIFY_EXTERNAL_IDENTITY",
        request_for("VERIFY_EXTERNAL_IDENTITY"),
        validation_context=trusted_collision_context(),
    )
...
def test_verify_allows_distinct_trusted_subaccounts():
    assert validate_operation_request(
        "VERIFY_EXTERNAL_IDENTITY",
        request_for("VERIFY_EXTERNAL_IDENTITY"),
        validation_context=trusted_collision_context("sub-b", "sub-a"),
    )[0]


def test_verify_rejects_empty_subaccount_identifier():
    acct = sample_account(connection_state="SYNCHRONIZING")
    ctx = context(account=acct, identity=sample_identity(subaccount_identifier=""))
    assert (
        validate_operation_request(
            "VERIFY_EXTERNAL_IDENTITY",
            request_for("VERIFY_EXTERNAL_IDENTITY"),
...
def test_verify_root_account_uses_null_subaccount():
    acct = sample_account(connection_state="SYNCHRONIZING")
    ctx = context(account=acct, identity=sample_identity(subaccount_identifier=None))
    assert validate_operation_request(
        "VERIFY_EXTERNAL_IDENTITY", request_for("VERIFY_EXTERNAL_IDENTITY"), validation_context=ctx
    )[0]
...
def test_create_external_references_are_not_identity_authority():
    existing = sample_account(
        external_account_reference="spoof", external_subaccount_reference="same"
    )
    assert create_with_existing(
        existing, external_account_reference="spoof", external_subaccount_reference="same"
    )[0]


def test_spoofed_external_reference_cannot_bypass_trusted_collision():
    ctx = trusted_collision_context()
    ctx["accounts_by_id"]["xacc_test_1"]["external_account_reference"] = "different"
    ok, out = validate_operation_request(
        "VERIFY_EXTERNAL_IDENTITY", request_for("VERIFY_EXTERNAL_IDENTITY"), validation_context=ctx
    )
    assert not ok and out["denial"] == "EXCHANGE_ACCOUNT_IDENTITY_COLLISION"
...
def test_duplicate_trusted_identity_denial_has_valid_audit_mapping():
    spec = DATA["operation_validation_matrix"]["VERIFY_EXTERNAL_IDENTITY"]
    event = spec["denial_event_by_code"]["EXCHANGE_ACCOUNT_IDENTITY_COLLISION"]
    schemas = {s["event_name"]: s for s in DATA["audit_event_schemas"]}
...
def partial_refresh_result(**catalog_overrides):
    inst = sample_instrument()
    cat = sample_catalog(inst, status="PARTIAL", **catalog_overrides)
    ctx = context(inst=inst, catalog=cat)
    return validate_operation_request(
        "REFRESH_INSTRUMENT_CATALOG",
        request_for("REFRESH_INSTRUMENT_CATALOG"),
...
def test_all_operation_request_fields_wrong_types_never_raise():
    fields = {
        "CREATE_ACCOUNT": ["requested_lifecycle_state", "environment", "market_type"],
        "BIND_CREDENTIAL_PROFILE": ["binding_intent"],
        "VERIFY_EXTERNAL_IDENTITY": ["expected_adapter_version_source", "reconnect"],
        "UPDATE_ACCOUNT": ["expected_identity_fields", "mutable_patch"],
        "RETIRE_ACCOUNT": ["retirement_reason"],
        "QUARANTINE_LEGACY_LIVE_RECORD": [
            "legacy_record",
            "target_lifecycle_state",
            "target_connection_state",
    ...
    for operation, names in fields.items():
        for field in names:
            bad_values = (
                [[], {}, 1, "true"]
                if field in {"legacy_record", "reconnect"}
                else (
                    [[], True, "bad"]
                    if field in {"expected_identity_fields", "mutable_patch"}
                    else [[], {}, True]
                )
            )
...
def test_create_malformed_lifecycle_never_raises():
    for bad in [[], {}, True]:
        assert (
            validate_operation_request(
                "CREATE_ACCOUNT",
                request_for("CREATE_ACCOUNT", requested_lifecycle_state=bad),
                validation_context=context(),
            )[0]
            is False
        )
...
def test_quarantine_malformed_states_never_raise():
    for field in [
        ...
        req = request_for("QUARANTINE_LEGACY_LIVE_RECORD")
        req[field] = []
        assert (
            validate_operation_request(
                "QUARANTINE_LEGACY_LIVE_RECORD", req, validation_context=context()
            )[0]
            is False
        )
...
def test_bind_malformed_profile_lifecycle_never_raises():
    acct = sample_account(connection_state="DISCONNECTED")
    ctx = context(account=acct, profile=sample_profile(lifecycle_state=[]))
    assert (
        validate_operation_request(
            "BIND_CREDENTIAL_PROFILE",
            request_for("BIND_CREDENTIAL_PROFILE"),
...
def test_malformed_enum_values_return_controlled_denial():
    assert (
        validate_operation_request(
            "CREATE_ACCOUNT",
            request_for("CREATE_ACCOUNT", market_type={}),
            validation_context=context(),
        )[0]
        is False
    )
...
def test_known_operation_malformed_input_never_uses_unknown_operation_event():
    ok, out = validate_operation_request(
        "CREATE_ACCOUNT",
        request_for("CREATE_ACCOUNT", environment=[]),
        validation_context=context(),
    )
    assert not ok and out["audit_event"] != "UNKNOWN_OPERATION_REJECTED"
...
def test_profile_rejects_empty_secure_store_locator():
    assert (
        validate_credential_profile(sample_profile(secure_store_reference="secure-store://"))[0]
        is False
    )
...
def test_profile_rejects_secure_store_query():
    assert (
        validate_credential_profile(
            sample_profile(secure_store_reference="secure-store://profile?x")
        )[0]
        is False
    )
...
def test_profile_rejects_secure_store_payload_assignment():
    assert (
        validate_credential_profile(
            sample_profile(secure_store_reference="secure-store://value=x")
        )[0]
        is False
    )
...
def test_profile_rejects_api_key_payload_marker():
    assert (
        validate_credential_profile(
            sample_profile(secure_store_reference="secure-store://api_key")
        )[0]
        is False
    )
...
def test_profile_lifecycle_wrong_type_never_raises():
    for bad in [[], {}, True, None]:
        assert validate_credential_profile(sample_profile(lifecycle_state=bad))[0] is False


def test_retired_profile_must_not_precede_creation():
    assert (
        validate_credential_profile(
            sample_profile(lifecycle_state="RETIRED", retired_at_utc="2025-01-01T00:00:00Z")
        )[0]
        is False
    )
...
def test_draft_account_rejects_retired_timestamp():
    assert (
        validate_account(
            sample_account(lifecycle_state="DRAFT", retired_at_utc="2026-02-01T00:00:00Z")
        )[0]
        is False
    )
...
def test_disabled_account_rejects_retired_timestamp():
    assert (
        validate_account(
            sample_account(lifecycle_state="DISABLED", retired_at_utc="2026-02-01T00:00:00Z")
        )[0]
        is False
    )
...
def test_retired_account_timestamp_not_before_creation():
    assert (
        validate_account(
            sample_account(lifecycle_state="RETIRED", retired_at_utc="2025-01-01T00:00:00Z")
        )[0]
        is False
    )
...
def test_update_cannot_add_retired_timestamp_to_active_account():
    req = request_for("UPDATE_ACCOUNT", mutable_patch={"retired_at_utc": "2026-02-01T00:00:00Z"})
    assert (
        validate_operation_request("UPDATE_ACCOUNT", req, validation_context=context())[0] is False
    )


def test_update_retirement_requires_complete_consistent_transition():
    bad = request_for("UPDATE_ACCOUNT", mutable_patch={"lifecycle_state": "RETIRED"})
    assert (
        validate_operation_request("UPDATE_ACCOUNT", bad, validation_context=context())[0] is False
    )
    good = request_for(
        "UPDATE_ACCOUNT",
        mutable_patch={"lifecycle_state": "RETIRED", "retired_at_utc": "2026-02-01T00:00:00Z"},
    )
    assert (
        validate_operation_request("UPDATE_ACCOUNT", good, validation_context=context())[0] is False
    )  # UPDATE matrix excludes RETIRED


def test_update_cannot_set_external_identity_verified():
    req = request_for(
        "UPDATE_ACCOUNT", mutable_patch={"external_account_identity_state": "VERIFIED"}
    )
    ok, out = validate_operation_request("UPDATE_ACCOUNT", req, validation_context=context())
    assert not ok and out["denial"] == "EXCHANGE_ACCOUNT_IDENTITY_IMMUTABLE"
    ...
    for state in DATA["external_account_identity_contract"]["states"]:
        assert (
            validate_operation_request(
                "UPDATE_ACCOUNT",
                request_for(
                    "UPDATE_ACCOUNT", mutable_patch={"external_account_identity_state": state}
                ),
                validation_context=context(),
            )[0]
            is False
...
def test_update_cannot_clear_external_identity_state():
    assert (
        validate_operation_request(
            "UPDATE_ACCOUNT",
            request_for("UPDATE_ACCOUNT", mutable_patch={"external_account_identity_state": None}),
            validation_context=context(),
        )[0]
        is False
    )
...
def test_update_identity_state_denial_has_valid_audit_mapping():
    spec = DATA["operation_validation_matrix"]["UPDATE_ACCOUNT"]
    event = spec["denial_event_by_code"]["EXCHANGE_ACCOUNT_IDENTITY_IMMUTABLE"]
    schemas = {s["event_name"]: s for s in DATA["audit_event_schemas"]}
    assert "EXCHANGE_ACCOUNT_IDENTITY_IMMUTABLE" in schemas[event]["allowed_denial_codes"]
...
def test_update_cannot_bind_missing_credential_profile():
    assert (
        validate_operation_request(
            "UPDATE_ACCOUNT",
            request_for(
                "UPDATE_ACCOUNT", mutable_patch={"active_credential_profile_id": "cred_missing"}
            ),
            validation_context=context(),
        )[0]
        is False
...
def test_update_cannot_bind_foreign_credential_profile():
    assert (
        validate_operation_request(
            "UPDATE_ACCOUNT",
            request_for(
                "UPDATE_ACCOUNT", mutable_patch={"active_credential_profile_id": "cred_foreign"}
            ),
            validation_context=context(),
        )[0]
        is False
...
def test_update_cannot_bind_withdraw_profile():
    assert (
        validate_operation_request(
            "UPDATE_ACCOUNT",
            request_for(
                "UPDATE_ACCOUNT", mutable_patch={"active_credential_profile_id": "cred_withdraw"}
            ),
            validation_context=context(),
        )[0]
        is False
...
def test_update_cannot_clear_active_credential_profile():
    acct = sample_account(active_credential_profile_id="cred_1")
    assert (
        validate_operation_request(
            "UPDATE_ACCOUNT",
            request_for("UPDATE_ACCOUNT", mutable_patch={"active_credential_profile_id": None}),
            validation_context=context(account=acct),
        )[0]
        is False
    )
...
def test_only_bind_operation_can_change_active_profile():
    assert "active_credential_profile_id" not in DATA["exchange_account_contract"][
        "caller_mutable_patch_fields"
    ] and DATA["exchange_account_contract"]["credential_binding_authority"].startswith(
        "BIND_CREDENTIAL_PROFILE"
    )
...
def test_update_credential_binding_denial_has_valid_audit_mapping():
    ok, out = validate_operation_request(
        "UPDATE_ACCOUNT",
        request_for("UPDATE_ACCOUNT", mutable_patch={"active_credential_profile_id": "cred_1"}),
        validation_context=context(),
    )
    spec = DATA["operation_validation_matrix"]["UPDATE_ACCOUNT"]
    assert not ok and out["denial"] in spec["denial_codes"]
...
def update_identity_context(target_sub=None, other_sub=None):
    target = sample_account(lifecycle_state="DISABLED", exchange_account_id="xacc_target")
    other = sample_account(exchange_account_id="xacc_other")
    ctx = context(account=target, identity=sample_identity(subaccount_identifier=target_sub))
    ctx["accounts_by_id"]["xacc_other"] = other
    ctx["external_identity_snapshots_by_account_id"]["xacc_other"] = sample_identity(
        subaccount_identifier=other_sub
    )
    return target, ctx


def test_update_rejects_duplicate_trusted_identity_when_activating_account():
    target, ctx = update_identity_context()
    req = request_for(
        "UPDATE_ACCOUNT",
        exchange_account_id="xacc_target",
        expected_identity_fields=identity_fields(target),
        mutable_patch={"lifecycle_state": "ACTIVE"},
    )
    ok, out = validate_operation_request("UPDATE_ACCOUNT", req, validation_context=ctx)
    assert not ok and out["denial"] == "EXCHANGE_ACCOUNT_IDENTITY_COLLISION"


def test_update_allows_distinct_trusted_subaccount():
    target, ctx = update_identity_context("sub-a", "sub-b")
    req = request_for(
        "UPDATE_ACCOUNT",
        exchange_account_id="xacc_target",
        expected_identity_fields=identity_fields(target),
    ...
    assert validate_operation_request("UPDATE_ACCOUNT", req, validation_context=ctx)[0]


def test_update_duplicate_identity_uses_trusted_snapshots_only():
    target, ctx = update_identity_context()
    target["external_account_reference"] = "spoof"
    req = request_for(
        "UPDATE_ACCOUNT",
        exchange_account_id="xacc_target",
        expected_identity_fields=identity_fields(target),
    ...
    assert validate_operation_request("UPDATE_ACCOUNT", req, validation_context=ctx)[0] is False
...
def test_verify_and_update_use_same_identity_uniqueness_helper():
    ...
    assert "validate_trusted_identity_uniqueness" in inspect.getsource(
        handle_verify_external_identity
    ) and "validate_trusted_identity_uniqueness" in inspect.getsource(handle_update_account)
...
def test_capability_snapshot_full_valid_record():
    assert validate_account_capability_snapshot(sample_capability_snapshot(), sample_account(), {})[
        0
    ]
...
def test_capability_snapshot_rejects_spoofed_map_key():
    snap = sample_capability_snapshot()
    ctx = context(account_capability_snapshots_by_id={"spoof": snap})
    assert validate_context(ctx) is False


def test_capability_snapshot_rejects_account_scope_mismatch():
    assert (
        validate_account_capability_snapshot(
            sample_capability_snapshot(exchange_account_id="xacc_other"), sample_account(), {}
        )[0]
        is False
    )
...
def test_capability_snapshot_rejects_unknown_permission():
    assert (
        validate_account_capability_snapshot(
            sample_capability_snapshot(observed_permission_set=["UNKNOWN"]), sample_account(), {}
        )[0]
        is False
    )
...
def test_capability_snapshot_rejects_unknown_instrument_type():
    assert (
        validate_account_capability_snapshot(
            sample_capability_snapshot(supported_instrument_types=["UNKNOWN"]), sample_account(), {}
        )[0]
        is False
    )
...
def test_capability_snapshot_rejects_invalid_hash():
    assert (
        validate_account_capability_snapshot(
            sample_capability_snapshot(content_hash="0" * 64), sample_account(), {}
        )[0]
        is False
    )
...
def test_capability_snapshot_rejects_invalid_timestamps():
    assert (
        validate_account_capability_snapshot(
            sample_capability_snapshot(observed_at_utc="bad"), sample_account(), {}
        )[0]
        is False
    )
...
def test_capability_snapshot_rejects_version_bool():
    assert (
        validate_account_capability_snapshot(
            sample_capability_snapshot(version=True), sample_account(), {}
        )[0]
        is False
    )
...
def capability_version(version, snapshot_id, previous_id):
    return sample_capability_snapshot(
        account_capability_snapshot_id=snapshot_id,
        version=version,
...
def test_capability_snapshot_rejects_lineage_cycle():
    current = capability_version(2, "caps_2", "caps_1")
    old = capability_version(1, "caps_1", "caps_2")
    assert (
        validate_account_capability_snapshot(
            current, sample_account(), {"caps_1": old, "caps_2": current}
        )[0]
        is False
    )
...
def test_capability_snapshot_rejects_non_contiguous_version():
    current = capability_version(3, "caps_3", "caps_1")
    old = capability_version(1, "caps_1", None)
    assert (
        validate_account_capability_snapshot(current, sample_account(), {"caps_1": old})[0] is False
    )
...
def test_update_cannot_bind_arbitrary_capability_snapshot():
    assert (
        validate_operation_request(
            "UPDATE_ACCOUNT",
            request_for(
                "UPDATE_ACCOUNT", mutable_patch={"account_capability_snapshot_id": "caps_1"}
            ),
            validation_context=context(),
        )[0]
        is False
...
def test_stale_capability_snapshot_only_restricts():
    ok, result = validate_account_capability_snapshot(
        sample_capability_snapshot(status="STALE"), sample_account(), {}
    )
    assert ok and result["restricts_only"]


def test_capability_snapshot_never_extends_product_capabilities():
    assert (
        validate_account_capability_snapshot(sample_capability_snapshot(), sample_account(), {})[1][
            "extends_product_capabilities"
        ]
        is False
    )
...
def test_capability_snapshot_never_enables_live():
    account = sample_account(exchange_id="paper_simulated_venue", environment="LIVE")
    snap = sample_capability_snapshot(account, adapter_family_id="paper_simulation_adapter_family")
    assert validate_account_capability_snapshot(snap, account, {})[0] is False


def refresh_status(status):
    inst = sample_instrument(trading_status=status)
    cat = sample_catalog(inst)
    ctx = context(inst=inst, catalog=cat)
    return validate_operation_request(
        "REFRESH_INSTRUMENT_CATALOG",
...
def activate_status(status):
    inst = sample_instrument(trading_status=status)
    cat = sample_catalog(inst)
    univ = sample_universe(inst=inst, catalog=cat)
    ctx = context(inst=inst, catalog=cat, universe=univ)
    return validate_operation_request(
        "ACTIVATE_TRADING_UNIVERSE",
        request_for("ACTIVATE_TRADING_UNIVERSE", content_hash=univ["content_hash"]),
...
def test_universe_non_trading_status_never_emits_activation_success():
    for status in ["HALTED", "SUSPENDED", "DELISTED", "UNKNOWN"]:
        assert activate_status(status)[1]["audit_event"] != "TRADING_UNIVERSE_ACTIVATED"
...
def profile_timeline(profile_id, created, retired=None, rotated=None, lifecycle="RETIRED"):
    return sample_profile(
        ...
        created_at_utc=created,
        retired_at_utc=retired,
        rotated_from_credential_profile_id=rotated,
...
def activation_with_context(ctx):
    ...
    return validate_operation_request(
        ...
        request_for(
            ...
            exchange_account_id=universe["exchange_account_id"],
            source_catalog_snapshot_ids=universe["source_catalog_snapshot_ids"],
...
def test_activation_rejects_unverified_trusted_identity_snapshot():
    assert (
        activation_with_context(context(identity=sample_identity(state="UNVERIFIED")))[0] is False
    )
...
def test_activation_rejects_snapshot_account_scope_mismatch():
    ctx = context()
    ctx["external_identity_snapshots_by_account_id"]["xacc_test_1"]["market_type"] = "PERPETUAL"
    assert activation_with_context(ctx)[0] is False
...
def test_activation_rejects_duplicate_trusted_identity():
    ...
    assert activation_with_context(ctx)[1]["denial"] == "EXCHANGE_ACCOUNT_IDENTITY_COLLISION"
...
def test_account_verified_string_without_snapshot_is_not_authority():
    ctx = context()
    assert ctx["accounts_by_id"]["xacc_test_1"]["external_account_identity_state"] == "VERIFIED"
    ctx["external_identity_snapshots_by_account_id"] = {}
    assert activation_with_context(ctx)[0] is False
...
def test_activation_identity_denials_have_valid_audit_mapping():
    ...
    schemas = {s["event_name"]: s for s in DATA["audit_event_schemas"]}
    for denial in [
        "EXTERNAL_ACCOUNT_IDENTITY_UNVERIFIED",
        "EXTERNAL_ACCOUNT_IDENTITY_MISMATCH",
        "EXCHANGE_ACCOUNT_IDENTITY_COLLISION",
    ]:
        assert denial in schemas[spec["denial_event_by_code"][denial]]["allowed_denial_codes"]


def capability_activation_context(status="VALID", **overrides):
    ctx = context()
    account = ctx["accounts_by_id"]["xacc_test_1"]
    snap = sample_capability_snapshot(account, status=status, **overrides)
    account["account_capability_snapshot_id"] = snap["account_capability_snapshot_id"]
    ctx["account_capability_snapshots_by_id"] = {snap["account_capability_snapshot_id"]: snap}
    return ctx
...
def test_activation_rejects_missing_bound_capability_snapshot():
    ctx = context()
    ctx["accounts_by_id"]["xacc_test_1"]["account_capability_snapshot_id"] = "caps_missing"
    ctx["account_capability_snapshots_by_id"] = {}
    assert activation_with_context(ctx)[0] is False
...
def test_activation_rejects_expired_valid_capability_snapshot():
    assert (
        activation_with_context(
            capability_activation_context(stale_after_utc="2026-06-01T00:00:00Z")
        )[0]
        is False
    )
...
def test_activation_rejects_capability_lineage_cycle():
    ...
    current = capability_version(2, "caps_2", "caps_1")
    old = capability_version(1, "caps_1", "caps_2")
    account["account_capability_snapshot_id"] = "caps_2"
    ctx["account_capability_snapshots_by_id"] = {"caps_2": current}
    ctx["previous_account_capability_snapshots_by_id"] = {"caps_1": old, "caps_2": current}
    assert activation_with_context(ctx)[0] is False


def test_adapter_snapshot_required_exchange_requires_snapshot():
    ctx = context()
    ctx["accounts_by_id"]["xacc_test_1"]["account_capability_snapshot_id"] = None
    ctx["account_capability_snapshots_by_id"] = {}
    assert activation_with_context(ctx)[0] is False
...
def paper_activation_context():
    ...
    return context(
        account=account,
        identity=sample_identity(exchange_id="paper_simulated_venue", environment="PAPER"),
        inst=inst,
        catalog=cat,
        universe=univ,
    )
...
def test_capability_snapshot_cannot_enable_live():
    live = sample_account(exchange_account_id="xacc_live", environment="LIVE")
    snap = sample_capability_snapshot(live)
    assert validate_account_capability_snapshot(snap, live, {})[0] is False


def test_capability_snapshot_cannot_bypass_readiness():
    ctx = capability_activation_context()
    ctx["accounts_by_id"]["xacc_test_1"]["readiness_confirmed"] = False
    assert activation_with_context(ctx)[0] is False


def test_capability_activation_denials_have_valid_audit_mapping():
    spec = DATA["operation_validation_matrix"]["ACTIVATE_TRADING_UNIVERSE"]
    event = spec["denial_event_by_code"]["ACCOUNT_READINESS_BLOCKED"]
    schemas = {s["event_name"]: s for s in DATA["audit_event_schemas"]}
    assert "ACCOUNT_READINESS_BLOCKED" in schemas[event]["allowed_denial_codes"]


def test_capability_rejects_exchange_unsupported_environment():
    assert (
        validate_account_capability_snapshot(
            sample_capability_snapshot(environment="PAPER"), sample_account(), {}
        )[0]
        is False
    )
...
def test_capability_rejects_exchange_unsupported_market_type():
    assert (
        validate_account_capability_snapshot(
            sample_capability_snapshot(market_type="MARGIN"), sample_account(), {}
        )[0]
        is False
    )
...
def test_capability_rejects_disabled_exchange():
    entry = exchange_entry("generic_testnet_venue")
    old = entry["status"]
    entry["status"] = "DISABLED"
    try:
        assert (
            validate_account_capability_snapshot(
                sample_capability_snapshot(), sample_account(), {}
            )[0]
            is False
        )
    finally:
...
def test_capability_rejects_market_instrument_type_mismatch():
    assert (
        validate_account_capability_snapshot(
            sample_capability_snapshot(supported_instrument_types=["PERPETUAL_CONTRACT"]),
            sample_account(),
            {},
        )[0]
        is False
...
def test_capability_predecessor_rejects_unsupported_environment():
    current = capability_version(2, "caps_2", "caps_1")
    old = capability_version(1, "caps_1", None)
    old["environment"] = "PAPER"
    old["content_hash"] = hash_payload(
        DATA["account_capability_snapshot_contract"]["content_hash_definition"], old
    )
    assert (
        validate_account_capability_snapshot(current, sample_account(), {"caps_1": old})[0] is False
    )
...
def test_malformed_current_capability_context_fails_closed():
    ctx = context()
    ctx["account_capability_snapshots_by_id"] = {"bad": {"account_capability_snapshot_id": "bad"}}
    assert (
        validate_operation_request(
            "CREATE_ACCOUNT", request_for("CREATE_ACCOUNT"), validation_context=ctx
        )[0]
...
def test_malformed_previous_capability_context_fails_closed():
    ctx = context()
    ctx["previous_account_capability_snapshots_by_id"] = {
        "bad": {"account_capability_snapshot_id": "bad"}
    }
    assert (
        validate_operation_request(
            "CREATE_ACCOUNT", request_for("CREATE_ACCOUNT"), validation_context=ctx
...
def catalog_with_records(records):
    mapping = {key: value for key, value in records}
    first = records[0][1]
    cat = sample_catalog(first, instrument_ids=[key for key, _ in records])
    return cat, mapping


def test_catalog_rejects_two_ids_with_same_instrument_identity():
    one = sample_instrument(instrument_id="instr_one")
    two = sample_instrument(instrument_id="instr_two")
    cat, mapping = catalog_with_records([("instr_one", one), ("instr_two", two)])
    assert (
        validate_instrument_catalog_snapshot(cat, mapping, {}, instrument_history_by_id={})[1]
        == "INSTRUMENT_IDENTITY_COLLISION"
    )
    ctx = multi_catalog_context()
    second = ctx["instruments_by_id"]["instr_ethusdt_spot_testnet"]
    second["venue_symbol"] = "BTCUSDT"
    assert (
        validate_catalog_instrument_graph(
            ctx["catalogs_by_id"],
            ctx["previous_catalogs_by_id"],
            ctx["instruments_by_id"],
            ctx["instrument_history_by_id"],
    ...
    ok, out = activation_with_context(ctx)
    assert not ok and out == {
        "denial": "ACCOUNT_READINESS_BLOCKED",
        "audit_event": "TRADING_UNIVERSE_REJECTED",
    }


def test_catalog_rejects_same_id_with_changed_identity():
    current = sample_instrument(instrument_id="instr_same", venue_symbol="ONE", metadata_version=2)
    historical = sample_instrument(
        instrument_id="instr_same", venue_symbol="TWO", metadata_version=1
    )
    cat = sample_catalog(current, instrument_ids=["instr_same"])
    assert (
        validate_instrument_catalog_snapshot(
            cat, {"instr_same": current}, {}, instrument_history_by_id={"instr_same": [historical]}
        )[1]
        == "INSTRUMENT_IDENTITY_COLLISION"
    )
    ctx = history_only_member_context()
    ctx["instrument_history_by_id"]["instr_ghost"][0]["venue_symbol"] = "BTCUSDT"
    assert (
        validate_catalog_instrument_graph(
            ctx["catalogs_by_id"],
            ctx["previous_catalogs_by_id"],
...
def test_catalog_allows_exactly_distinct_venue_symbols():
    one = sample_instrument(instrument_id="instr_one", venue_symbol="ONE")
    two = sample_instrument(instrument_id="instr_two", venue_symbol="TWO")
    cat, mapping = catalog_with_records([("instr_one", one), ("instr_two", two)])
    assert validate_instrument_catalog_snapshot(cat, mapping, {}, instrument_history_by_id={})[0]


def test_catalog_treats_symbol_case_as_exact():
    one = sample_instrument(instrument_id="instr_upper", venue_symbol="BTCUSDT")
    two = sample_instrument(instrument_id="instr_lower", venue_symbol="btcusdt")
    cat, mapping = catalog_with_records([("instr_upper", one), ("instr_lower", two)])
    assert validate_instrument_catalog_snapshot(cat, mapping, {}, instrument_history_by_id={})[0]


def test_display_symbol_does_not_create_identity():
    one = sample_instrument(instrument_id="instr_one", venue_symbol="ONE", display_symbol="SAME")
    two = sample_instrument(instrument_id="instr_two", venue_symbol="TWO", display_symbol="SAME")
    cat, mapping = catalog_with_records([("instr_one", one), ("instr_two", two)])
    assert validate_instrument_catalog_snapshot(cat, mapping, {}, instrument_history_by_id={})[0]
...
def test_instrument_identity_collision_has_valid_audit_mapping():
    ...
    for operation in ["REFRESH_INSTRUMENT_CATALOG", "ACTIVATE_TRADING_UNIVERSE"]:
        spec = DATA["operation_validation_matrix"][operation]
        event = spec["denial_event_by_code"]["INSTRUMENT_IDENTITY_COLLISION"]
        assert "INSTRUMENT_IDENTITY_COLLISION" in schemas[event]["allowed_denial_codes"]
...
def direct_activation_from_context(ctx, validation_context_marker=True):
    ...
    return validate_trading_universe_version(
        ...
        ctx["active_universes"],
        ctx["previous_catalogs_by_id"],
        validation_context=supplied,
    )


def test_direct_universe_validator_requires_validation_context():
    ctx = context()
    assert direct_activation_from_context(ctx, False) == (False, "ACCOUNT_READINESS_BLOCKED")


def test_direct_universe_validator_rejects_missing_trusted_identity():
    ctx = context()
    ctx["external_identity_snapshots_by_account_id"] = {}
    assert direct_activation_from_context(ctx)[0] is False


def test_direct_universe_validator_rejects_unverified_identity():
    ctx = context(identity=sample_identity(state="UNVERIFIED"))
    assert direct_activation_from_context(ctx)[0] is False
...
def test_direct_universe_validator_rejects_duplicate_identity():
    ctx = trusted_collision_context()
    ctx["accounts_by_id"]["xacc_test_1"]["connection_state"] = "ONLINE"
    assert direct_activation_from_context(ctx)[1] == "EXCHANGE_ACCOUNT_IDENTITY_COLLISION"
...
def test_direct_universe_validator_rejects_expired_capability():
    assert (
        direct_activation_from_context(
            capability_activation_context(stale_after_utc="2026-06-01T00:00:00Z")
        )[0]
        is False
    )
...
def test_direct_and_dispatched_activation_share_same_readiness_result():
    ctx = capability_activation_context("STALE")
    direct = direct_activation_from_context(ctx)
    dispatched = activation_with_context(ctx)
    assert direct[0] is dispatched[0] is False and direct[1] == dispatched[1]["denial"]


def test_no_direct_activation_success_bypasses_readiness():
    ctx = context()
    ctx["readiness_by_account_id"]["xacc_test_1"] = False
    assert direct_activation_from_context(ctx)[0] is False


def test_testnet_activation_rejects_null_active_credential_profile_id():
    ctx = context()
    ctx["accounts_by_id"]["xacc_test_1"]["active_credential_profile_id"] = None
    assert activation_with_context(ctx)[0] is False
...
def test_testnet_activation_rejects_foreign_active_profile():
    ctx = context()
    ctx["credential_profiles_by_id"]["cred_1"]["exchange_account_id"] = "other"
    assert activation_with_context(ctx)[0] is False


def test_testnet_activation_rejects_profile_scope_mismatch():
    ctx = context()
    ctx["credential_profiles_by_id"]["cred_1"]["environment_scope"] = "PAPER"
    assert activation_with_context(ctx)[0] is False


def test_testnet_activation_rejects_inactive_profile():
    ctx = context(
        profile=sample_profile(lifecycle_state="RETIRED", retired_at_utc="2026-02-01T00:00:00Z")
    )
    assert activation_with_context(ctx)[0] is False
...
def test_testnet_activation_rejects_active_profile_map_mismatch():
    ctx = context()
    ctx["active_profile_ids_by_account_id"]["xacc_test_1"] = ["other"]
    assert activation_with_context(ctx)[0] is False


def test_testnet_activation_rejects_withdraw_profile():
    ctx = context(profile=sample_profile(permission_snapshot=["READ_ACCOUNT", "WITHDRAW"]))
    assert activation_with_context(ctx)[0] is False


def test_testnet_activation_rejects_invalid_profile_lineage():
    ctx = context(profile=sample_profile(rotated_from_credential_profile_id="missing"))
    assert activation_with_context(ctx)[0] is False
...
def order_entry_context(profile_permissions, capability_permissions, identity_permissions):
    profile = sample_profile(
        credential_purpose="ORDER_ENTRY", permission_snapshot=profile_permissions
    )
    identity = sample_identity(observed_permission_set=identity_permissions)
    ctx = context(profile=profile, identity=identity)
    account = ctx["accounts_by_id"]["xacc_test_1"]
    account["execution_authorization"] = "ORDER_ENTRY_ALLOWED"
    snap = sample_capability_snapshot(account, observed_permission_set=capability_permissions)
    ctx["account_capability_snapshots_by_id"] = {"caps_1": snap}
    return ctx
...
def test_order_entry_rejects_profile_without_place_orders():
    assert (
        activation_with_context(
            order_entry_context(
                ["READ_ACCOUNT"], ["READ_ACCOUNT", "PLACE_ORDERS"], ["READ_ACCOUNT", "PLACE_ORDERS"]
            )
        )[0]
        is False
    )
...
def test_order_entry_rejects_capability_without_place_orders():
    assert (
        activation_with_context(
            order_entry_context(
                ["READ_ACCOUNT", "PLACE_ORDERS"], ["READ_ACCOUNT"], ["READ_ACCOUNT", "PLACE_ORDERS"]
            )
        )[0]
        is False
    )
...
def test_order_entry_rejects_identity_without_place_orders():
    assert (
        activation_with_context(
            order_entry_context(
                ["READ_ACCOUNT", "PLACE_ORDERS"], ["READ_ACCOUNT", "PLACE_ORDERS"], ["READ_ACCOUNT"]
            )
        )[0]
        is False
    )
...
def test_missing_read_account_blocks_testnet_readiness():
    ctx = context(identity=sample_identity(observed_permission_set=[]))
    assert activation_with_context(ctx)[0] is False


def test_withdraw_in_capability_blocks_readiness():
    ctx = capability_activation_context(observed_permission_set=["READ_ACCOUNT", "WITHDRAW"])
    assert activation_with_context(ctx)[0] is False


def test_withdraw_in_identity_blocks_readiness():
    ctx = context(identity=sample_identity(observed_permission_set=["READ_ACCOUNT", "WITHDRAW"]))
    assert activation_with_context(ctx)[0] is False


def test_permissions_never_extend_product_capabilities():
    ok, details = validate_account_activation_readiness(
        context()["accounts_by_id"]["xacc_test_1"], context()
    )
    assert ok and details["extends_product_capabilities"] is False


def test_activation_rejects_empty_capability_instrument_types():
    assert (
        activation_with_context(capability_activation_context(supported_instrument_types=[]))[0]
        is False
    )
...
def test_bound_static_paper_snapshot_restricts_instrument_types():
    ctx = paper_activation_context()
    account = ctx["accounts_by_id"]["xacc_paper"]
    snap = sample_capability_snapshot(
        account, supported_instrument_types=[], adapter_family_id="paper_simulation_adapter_family"
    )
    account["account_capability_snapshot_id"] = "caps_1"
    ctx["account_capability_snapshots_by_id"] = {"caps_1": snap}
    assert activation_with_context(ctx)[0] is False
...
def history_context(current=None, history=None):
    current = current or sample_instrument(metadata_version=3)
    cat = sample_catalog(current)
    univ = sample_universe(inst=current, catalog=cat)
    ctx = context(inst=current, catalog=cat, universe=univ)
    ctx["instrument_history_by_id"] = {
        current["instrument_id"]: history
        or [
            sample_instrument(metadata_version=1, trading_status="DELISTED"),
            sample_instrument(metadata_version=2, display_symbol="BTC-USDT"),
        ]
    }
    return ctx
...
def test_context_rejects_history_key_record_id_mismatch():
    ctx = history_context()
    ctx["instrument_history_by_id"] = {
        "wrong": ctx["instrument_history_by_id"]["instr_btcusdt_spot_testnet"]
    }
...
def test_context_rejects_malformed_instrument_history_record():
    ctx = history_context()
    ctx["instrument_history_by_id"]["instr_btcusdt_spot_testnet"] = [{}]
    assert not validate_context(ctx)


def test_context_rejects_duplicate_history_metadata_version():
    ctx = history_context(
        history=[sample_instrument(metadata_version=1), sample_instrument(metadata_version=1)]
    )
    assert not validate_context(ctx)
...
def test_catalog_rejects_historical_identity_rewrite_fields(field, value):
    current = sample_instrument(metadata_version=2)
    historical = sample_instrument(metadata_version=1)
    historical[field] = value
    cat = sample_catalog(current)
    assert (
        validate_instrument_catalog_snapshot(
            cat,
            {current["instrument_id"]: current},
            {},
            instrument_history_by_id={current["instrument_id"]: [historical]},
        )[1]
        == "INSTRUMENT_IDENTITY_COLLISION"
    )


def test_catalog_rejects_historical_venue_symbol_rewrite():
    test_catalog_rejects_historical_identity_rewrite_fields("venue_symbol", "NEW")
...
def test_catalog_rejects_historical_market_type_rewrite():
    test_catalog_rejects_historical_identity_rewrite_fields("market_type", "MARGIN")
...
def test_dispatcher_rejects_historical_identity_rewrite():
    ctx = history_context()
    ctx["instrument_history_by_id"]["instr_btcusdt_spot_testnet"][0]["venue_symbol"] = "OLD"
    assert activation_with_context(ctx)[0] is False


def test_direct_catalog_validator_rejects_historical_identity_rewrite():
    current = sample_instrument(metadata_version=2)
    old = sample_instrument(metadata_version=1, venue_symbol="OLD")
    cat = sample_catalog(current)
    assert (
        validate_instrument_catalog_snapshot(
            cat,
            {current["instrument_id"]: current},
            {},
            instrument_history_by_id={current["instrument_id"]: [old]},
        )[0]
        is False
    )
...
def raw_direct_activation(
    ...
    *,
    ...
    instruments=None,
    catalogs=None,
    previous_catalogs=None,
    previous_universes=None,
    active_universes=None,
):
...
def test_direct_activation_rejects_account_not_equal_to_context_record():
    ctx = context()
    foreign = dict(ctx["accounts_by_id"]["xacc_test_1"], display_name="foreign")
    assert raw_direct_activation(ctx, account=foreign)[0] is False


def test_direct_activation_rejects_paper_account_against_testnet_context():
    ctx = context()
    paper = dict(
        ctx["accounts_by_id"]["xacc_test_1"],
        exchange_id="paper_simulated_venue",
        environment="PAPER",
    )
    assert raw_direct_activation(ctx, account=paper)[0] is False


def test_direct_activation_rejects_instruments_not_bound_to_context():
    ctx = context()
    foreign = {**ctx["instruments_by_id"]}
    foreign[next(iter(foreign))] = dict(next(iter(foreign.values())), display_symbol="foreign")
    assert raw_direct_activation(ctx, instruments=foreign)[0] is False


def test_direct_activation_rejects_catalogs_not_bound_to_context():
    ctx = context()
    foreign = {**ctx["catalogs_by_id"]}
    key = next(iter(foreign))
    foreign[key] = dict(foreign[key], adapter_version="foreign")
    assert raw_direct_activation(ctx, catalogs=foreign)[0] is False


def test_direct_activation_rejects_universe_not_bound_to_context():
    ctx = context()
    foreign = dict(next(iter(ctx["universes_by_id"].values())), creation_reason="foreign")
    assert raw_direct_activation(ctx, universe=foreign)[0] is False
...
def test_readiness_rejects_missing_context_field():
    ctx = context()
    account = ctx["accounts_by_id"]["xacc_test_1"]
    del ctx["instrument_history_by_id"]
    assert validate_account_activation_readiness(account, ctx)[0] is False


def test_readiness_rejects_account_not_bound_to_context():
    ctx = context()
    account = dict(ctx["accounts_by_id"]["xacc_test_1"], display_name="spoof")
    assert validate_account_activation_readiness(account, ctx)[0] is False


@pytest.mark.parametrize("state", ["DRAFT", "DISABLED"])
def test_readiness_rejects_nonactive_account(state):
    ctx = context(account=sample_account(lifecycle_state=state))
    account = ctx["accounts_by_id"]["xacc_test_1"]
    assert validate_account_activation_readiness(account, ctx)[0] is False


def test_readiness_rejects_draft_account():
    test_readiness_rejects_nonactive_account("DRAFT")


def test_readiness_rejects_disabled_account():
    test_readiness_rejects_nonactive_account("DISABLED")


def test_readiness_rejects_disconnected_account():
    ctx = context(account=sample_account(connection_state="DISCONNECTED"))
    assert (
        validate_account_activation_readiness(ctx["accounts_by_id"]["xacc_test_1"], ctx)[0] is False
    )


def test_readiness_rejects_blocked_authorization():
    ctx = context(account=sample_account(execution_authorization="BLOCKED_BY_POLICY"))
    assert (
        validate_account_activation_readiness(ctx["accounts_by_id"]["xacc_test_1"], ctx)[0] is False
    )
...
def test_readiness_rejects_account_readiness_confirmed_false():
    ctx = context(account=sample_account(readiness_confirmed=False))
    assert (
        validate_account_activation_readiness(ctx["accounts_by_id"]["xacc_test_1"], ctx)[0] is False
    )
...
def test_readiness_rejects_account_readiness_confirmed_missing():
    ctx = context()
    account = ctx["accounts_by_id"]["xacc_test_1"]
    del account["readiness_confirmed"]
    assert validate_account_activation_readiness(account, ctx)[0] is False
...
def test_readiness_accepts_only_complete_operable_account():
    ctx = context()
    assert validate_account_activation_readiness(ctx["accounts_by_id"]["xacc_test_1"], ctx)[0]


def test_direct_activation_missing_instrument_history_map_fails_closed():
    ctx = context()
    del ctx["instrument_history_by_id"]
    assert raw_direct_activation(ctx)[0] is False
...
def test_direct_activation_malformed_context_never_raises(bad):
    assert (
        validate_trading_universe_version({}, {}, {}, {}, {}, [], {}, validation_context=bad)[0]
        is False
    )
...
def withdrawal_result(source):
    if source == "profile":
        ...
        ctx = context(
            identity=sample_identity(observed_permission_set=["READ_ACCOUNT", "WITHDRAW"])
        )
...
def test_withdrawal_sources_preserve_exact_denial_and_audit(source):
    ok, out = withdrawal_result(source)
    assert not ok and out == {
        "denial": "WITHDRAWAL_PERMISSION_FORBIDDEN",
        "audit_event": "TRADING_UNIVERSE_REJECTED",
    }
...
def direct_catalog(history):
    ...
    cat = sample_catalog(inst)
    return validate_instrument_catalog_snapshot(
        cat, {inst["instrument_id"]: inst}, {}, instrument_history_by_id=history
    )
...
def test_direct_catalog_requires_explicit_history_map():
    ...
    assert (
        validate_instrument_catalog_snapshot(
            sample_catalog(inst), {inst["instrument_id"]: inst}, {}
        )[0]
        is False
    )


def test_direct_catalog_rejects_none_history():
    assert direct_catalog(None)[0] is False


def test_direct_catalog_rejects_string_history():
    assert direct_catalog("history")[0] is False


def test_direct_catalog_rejects_list_history():
    assert direct_catalog([])[0] is False


def test_direct_catalog_rejects_non_list_history_entry():
    assert direct_catalog({"instr_btcusdt_spot_testnet": {}})[0] is False


def test_direct_catalog_rejects_non_dict_history_record():
    assert direct_catalog({"instr_btcusdt_spot_testnet": ["record"]})[0] is False


@pytest.mark.parametrize("bad", [None, False, 1, "history", [], {"id": None}, {"id": [None]}])
def test_direct_catalog_malformed_history_never_raises(bad):
    assert direct_catalog(bad)[0] is False


def test_direct_catalog_empty_trusted_history_map_is_valid_for_new_instrument():
    assert direct_catalog({})[0]


def history_without_current(*records):
    ctx = context()
    ctx["instruments_by_id"] = {}
    ctx["instrument_history_by_id"] = {records[0]["instrument_id"]: list(records)}
    return ctx


def test_context_rejects_identity_change_between_history_versions():
    one = sample_instrument(metadata_version=1)
    two = sample_instrument(metadata_version=2, venue_symbol="OTHER")
    assert not validate_context(history_without_current(one, two))
...
def test_context_rejects_history_identity_change_without_current_record(field, value):
    one = sample_instrument(metadata_version=1)
    two = sample_instrument(metadata_version=2)
    two[field] = value
    assert not validate_context(history_without_current(one, two))


def test_context_rejects_history_symbol_change_without_current_record():
    test_context_rejects_history_identity_change_without_current_record("venue_symbol", "OTHER")


def test_context_rejects_history_exchange_change_without_current_record():
    test_context_rejects_history_identity_change_without_current_record(
        "exchange_id", "paper_simulated_venue"
    )


def test_context_rejects_history_environment_change_without_current_record():
    test_context_rejects_history_identity_change_without_current_record("environment", "PAPER")


def test_context_rejects_history_market_change_without_current_record():
    test_context_rejects_history_identity_change_without_current_record("market_type", "MARGIN")


def version_history_context(current_version, history_versions=(1, 2)):
    current = sample_instrument(metadata_version=current_version)
    history = [sample_instrument(metadata_version=v) for v in history_versions]
    return history_context(current=current, history=history)


def test_context_rejects_current_version_equal_to_history_version():
    assert not validate_context(version_history_context(2))


def test_context_rejects_current_version_lower_than_history_version():
    assert not validate_context(version_history_context(1))


def test_context_rejects_current_version_rollback():
    assert not validate_context(version_history_context(1, (2, 3)))


def test_activation_rejects_current_version_equal_to_history():
    assert activation_with_context(version_history_context(2))[0] is False


def test_activation_rejects_current_version_lower_than_history():
    assert activation_with_context(version_history_context(1))[0] is False


def test_context_accepts_current_version_greater_than_complete_history():
    assert validate_context(version_history_context(3))


def test_context_allows_display_symbol_change_across_history():
    ctx = version_history_context(3)
    ctx["instrument_history_by_id"]["instr_btcusdt_spot_testnet"][1]["display_symbol"] = "OLD"
    assert validate_context(ctx)


def test_context_allows_trading_status_change_across_history():
    ctx = version_history_context(3)
    ctx["instrument_history_by_id"]["instr_btcusdt_spot_testnet"][0]["trading_status"] = "DELISTED"
    assert validate_context(ctx)
...
def stale_history(status="DELISTED"):
    return sample_instrument(
        ...
        observed_at_utc="2020-01-01T00:00:00Z",
        effective_at_utc="2020-01-02T00:00:00Z",
        stale_after_utc="2020-02-01T00:00:00Z",
    )


def test_context_accepts_structurally_valid_stale_historical_record():
    assert validate_context(history_context(history=[stale_history()]))


def test_context_accepts_old_delisted_historical_record():
    assert validate_context(history_context(history=[stale_history("DELISTED")]))


def test_context_accepts_old_halted_historical_record():
    assert validate_context(history_context(history=[stale_history("HALTED")]))


def test_current_catalog_rejects_stale_current_instrument():
    inst = sample_instrument(stale_after_utc="2020-01-01T00:00:00Z")
    assert (
        validate_instrument_catalog_snapshot(
            sample_catalog(inst), {inst["instrument_id"]: inst}, {}, instrument_history_by_id={}
        )[0]
        is False
    )
...
def test_activation_rejects_stale_current_instrument():
    ...
    cat = sample_catalog(inst)
    univ = sample_universe(inst=inst, catalog=cat)
    assert activation_with_context(context(inst=inst, catalog=cat, universe=univ))[0] is False


def test_historical_staleness_does_not_grant_execution():
    assert (
        validate_instrument_record(stale_history(), require_fresh=False)[0]
        and validate_instrument_record(stale_history(), require_fresh=True)[0] is False
    )
...
def multi_catalog_context():
    ...
    cat2 = sample_catalog(
        second, catalog_snapshot_id="cat_2", instrument_ids=[second["instrument_id"]]
    )
...
def test_activation_rejects_foreign_source_catalog():
    ctx = multi_catalog_context()
    req = request_for("ACTIVATE_TRADING_UNIVERSE", source_catalog_snapshot_ids=["cat_2"])
    assert (
        validate_operation_request("ACTIVATE_TRADING_UNIVERSE", req, validation_context=ctx)[0]
        is False
    )
...
def test_activation_rejects_tampered_catalog_record():
    ctx = multi_catalog_context()
    ctx["catalogs_by_id"]["cat_1"]["adapter_version"] = "tampered"
    assert activation_with_context(ctx)[0] is False
...
def test_unrelated_catalog_does_not_extend_universe_membership():
    ctx = multi_catalog_context()
    universe = ctx["universes_by_id"]["univ_1"]
    universe["instrument_ids"].append("instr_ethusdt_spot_testnet")
...
def direct_history_result(current_version, history):
    ...
    return validate_instrument_catalog_snapshot(
        ...
        {current["instrument_id"]: current},
        {},
        instrument_history_by_id={current["instrument_id"]: history},
    )


def test_direct_catalog_rejects_duplicate_history_versions():
    assert direct_history_result(
        3, [sample_instrument(metadata_version=1), sample_instrument(metadata_version=1)]
    ) == (False, "INSTRUMENT_METADATA_INVALID")


def test_direct_catalog_rejects_out_of_order_history_versions():
    assert direct_history_result(
        3, [sample_instrument(metadata_version=2), sample_instrument(metadata_version=1)]
    ) == (False, "INSTRUMENT_METADATA_INVALID")


def test_direct_catalog_rejects_current_version_equal_to_history():
    assert direct_history_result(
        2, [sample_instrument(metadata_version=1), sample_instrument(metadata_version=2)]
    ) == (False, "INSTRUMENT_METADATA_INVALID")


def test_direct_catalog_rejects_current_version_lower_than_history():
    assert direct_history_result(1, [sample_instrument(metadata_version=2)]) == (
        False,
        "INSTRUMENT_METADATA_INVALID",
    )
...
def test_direct_catalog_rejects_current_version_rollback():
    assert direct_history_result(
        2, [sample_instrument(metadata_version=1), sample_instrument(metadata_version=3)]
    ) == (False, "INSTRUMENT_METADATA_INVALID")
...
def test_direct_catalog_rejects_identity_change_between_history_versions():
    assert direct_history_result(
        3,
        [
            sample_instrument(metadata_version=1),
            sample_instrument(metadata_version=2, venue_symbol="OTHER"),
        ],
    ) == (False, "INSTRUMENT_IDENTITY_COLLISION")


def test_direct_catalog_rejects_identity_change_against_current_record():
    assert direct_history_result(
        2, [sample_instrument(metadata_version=1, venue_symbol="OTHER")]
    ) == (False, "INSTRUMENT_IDENTITY_COLLISION")


def test_direct_catalog_accepts_current_version_above_complete_history():
    assert direct_history_result(
        3, [sample_instrument(metadata_version=1), sample_instrument(metadata_version=2)]
    ) == (True, None)
...
def test_direct_catalog_allows_historical_display_symbol_change():
    assert direct_history_result(
        2, [sample_instrument(metadata_version=1, display_symbol="OLD")]
    ) == (True, None)


def test_direct_catalog_allows_historical_trading_status_change():
    assert direct_history_result(
        2, [sample_instrument(metadata_version=1, trading_status="DELISTED")]
    ) == (True, None)
...
def malformed_unrelated(map_name, bad):
    ctx = context()
    if map_name == "accounts_by_id":
        ctx[map_name]["xacc_bad"] = bad if bad != {} else {"exchange_account_id": "xacc_bad"}
    elif map_name == "credential_profiles_by_id":
        ctx[map_name]["cred_bad"] = bad if bad != {} else {"credential_profile_id": "cred_bad"}
    elif map_name == "external_identity_snapshots_by_account_id":
        ctx["accounts_by_id"]["xacc_other"] = sample_account(exchange_account_id="xacc_other")
        ctx[map_name]["xacc_other"] = bad if bad != {} else {"state": "UNVERIFIED"}
    elif map_name == "universes_by_id":
        ctx[map_name]["univ_bad"] = bad if bad != {} else {"trading_universe_id": "univ_bad"}
    elif map_name == "catalogs_by_id":
        ctx[map_name]["cat_bad"] = bad if bad != {} else {"catalog_snapshot_id": "cat_bad"}
    elif map_name == "instruments_by_id":
        ctx[map_name]["instr_bad"] = bad if bad != {} else {"instrument_id": "instr_bad"}
    elif map_name == "previous_profiles_by_id":
        ctx[map_name]["cred_bad"] = bad if bad != {} else {"credential_profile_id": "cred_bad"}
    elif map_name == "previous_catalogs_by_id":
        ctx[map_name]["cat_bad"] = bad if bad != {} else {"catalog_snapshot_id": "cat_bad"}
    else:
        ctx[map_name]["univ_bad"] = bad if bad != {} else {"trading_universe_id": "univ_bad"}
    return ctx
...
def test_context_rejects_malformed_unrelated_account():
    assert not validate_context(malformed_unrelated("accounts_by_id", {}))


def test_context_rejects_malformed_unrelated_credential_profile():
    assert not validate_context(malformed_unrelated("credential_profiles_by_id", {}))


def test_context_rejects_malformed_unrelated_external_identity():
    assert not validate_context(
        malformed_unrelated("external_identity_snapshots_by_account_id", {})
    )


def test_context_rejects_malformed_unrelated_universe():
    assert not validate_context(malformed_unrelated("universes_by_id", {}))


def test_context_rejects_malformed_unrelated_catalog():
    assert not validate_context(malformed_unrelated("catalogs_by_id", {}))


def test_context_rejects_malformed_unrelated_instrument():
    assert not validate_context(malformed_unrelated("instruments_by_id", {}))


def test_context_rejects_malformed_unrelated_previous_profile():
    assert not validate_context(malformed_unrelated("previous_profiles_by_id", {}))


def test_context_rejects_malformed_unrelated_previous_catalog():
    assert not validate_context(malformed_unrelated("previous_catalogs_by_id", {}))


def test_context_rejects_malformed_unrelated_previous_universe():
    assert not validate_context(malformed_unrelated("previous_universes_by_id", {}))
...
def test_context_accepts_structurally_valid_nonoperable_historical_records():
    ctx = context()
    ctx["accounts_by_id"]["xacc_disabled"] = sample_account(
        exchange_account_id="xacc_disabled",
        lifecycle_state="DISABLED",
        connection_state="DISCONNECTED",
        external_account_identity_state="UNVERIFIED",
    )
    ctx["external_identity_snapshots_by_account_id"]["xacc_disabled"] = sample_identity(
        state="UNVERIFIED"
    )
    old = stale_history("DELISTED")
    ctx["instrument_history_by_id"] = {old["instrument_id"]: [old]}
    ctx["instruments_by_id"][old["instrument_id"]]["metadata_version"] = 2
    assert validate_context(ctx)
...
def second_active_profile_context(same_account=True):
    ctx = context()
    account_id = "xacc_test_1" if same_account else "xacc_other"
    if not same_account:
        ctx["accounts_by_id"][account_id] = sample_account(
            exchange_account_id=account_id, active_credential_profile_id="cred_2"
        )
        ctx["active_profile_ids_by_account_id"][account_id] = ["cred_2"]
    ctx["credential_profiles_by_id"]["cred_2"] = sample_profile(
        credential_profile_id="cred_2", exchange_account_id=account_id
    )
    return ctx


def test_context_rejects_hidden_second_active_profile_same_account():
    assert not validate_context(second_active_profile_context())


def test_activation_rejects_hidden_second_active_profile_same_account():
    assert activation_with_context(second_active_profile_context())[0] is False
...
def test_context_rejects_retired_profile_in_active_index():
    ...
    ctx["accounts_by_id"]["xacc_test_1"]["active_credential_profile_id"] = "cred_1"
    assert not validate_context(ctx)
...
def test_context_rejects_duplicate_profile_id_across_current_and_previous():
    ctx = context()
    ctx["previous_profiles_by_id"]["cred_1"] = dict(ctx["credential_profiles_by_id"]["cred_1"])
    assert not validate_context(ctx)
...
def test_active_profile_on_other_account_does_not_conflict():
    assert validate_context(second_active_profile_context(False))


def hidden_active_universe_context(other_account=False):
    ctx = context()
    target = ctx["universes_by_id"]["univ_1"]
    account_id = "xacc_other" if other_account else target["exchange_account_id"]
    if other_account:
        ctx["accounts_by_id"][account_id] = sample_account(
            exchange_account_id=account_id, active_credential_profile_id=None
        )
...
def test_context_rejects_active_universe_not_bound_to_universe_map():
    ctx = context()
    ctx["active_universes"] = [sample_universe(trading_universe_id="missing")]
    assert not validate_context(ctx)


def test_context_rejects_tampered_active_universe_copy():
    ctx = context()
    copy = dict(ctx["universes_by_id"]["univ_1"], creation_reason="tampered")
    copy["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], copy
    )
    ctx["active_universes"] = [copy]
    assert not validate_context(ctx)


def test_context_rejects_duplicate_active_universe_entries():
    ctx = context()
    item = ctx["universes_by_id"]["univ_1"]
    ctx["active_universes"] = [item, item]
    assert not validate_context(ctx)


def test_activation_rejects_hidden_second_active_universe_same_account():
    ok, out = activation_with_context(hidden_active_universe_context())
    assert not ok and out["denial"] == "TRADING_UNIVERSE_VERSION_CONFLICT"


def test_direct_activation_rejects_hidden_second_active_universe_same_account():
    assert raw_direct_activation(hidden_active_universe_context()) == (
        False,
        "TRADING_UNIVERSE_VERSION_CONFLICT",
    )
...
def test_dispatch_and_direct_return_same_universe_conflict():
    ctx = hidden_active_universe_context()
    direct = raw_direct_activation(ctx)
    dispatched = activation_with_context(ctx)
    assert (
        direct[1] == dispatched[1]["denial"] == "TRADING_UNIVERSE_VERSION_CONFLICT"
        and dispatched[1]["audit_event"] == "TRADING_UNIVERSE_REJECTED"
    )
...
def test_context_rejects_duplicate_universe_id_across_current_and_previous():
    ctx = context()
    ctx["previous_universes_by_id"]["univ_1"] = dict(ctx["universes_by_id"]["univ_1"])
    assert not validate_context(ctx)
...
def test_context_rejects_duplicate_durable_id_across_current_previous(current, previous, different):
    ctx = context()
    key = next(iter(ctx[current]))
    record = dict(ctx[current][key])
    if different:
        record[next(iter(record))] = key
    ctx[previous][key] = record
    assert not validate_context(ctx)
...
def test_context_allows_disjoint_current_previous_ids(current, previous):
    ...
    elif current == "universes_by_id":
        record = sample_universe(trading_universe_id="univ_old")
    else:
        record = sample_capability_snapshot(
            account=ctx["accounts_by_id"]["xacc_test_1"], account_capability_snapshot_id="caps_old"
        )
    ctx[previous][
        next(
            value
...
def test_context_rejects_profile_with_missing_account():
    ctx = context()
    ctx["credential_profiles_by_id"]["cred_1"]["exchange_account_id"] = "missing"
    assert not validate_context(ctx)
...
def test_context_rejects_current_instrument_catalog_scope_mismatch():
    ctx = context()
    ctx["instruments_by_id"]["instr_btcusdt_spot_testnet"]["source_adapter_family_id"] = "other"
    assert not validate_context(ctx)
...
def test_context_rejects_current_catalog_with_unresolved_member():
    ...
    catalog["instrument_ids"] = ["missing"]
    catalog["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["catalog_snapshot_content_hash"], catalog
    )
    assert not validate_context(ctx)
...
def test_context_rejects_universe_with_missing_account():
    ctx = context()
    universe = ctx["universes_by_id"]["univ_1"]
    universe["exchange_account_id"] = "missing"
    universe["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], universe
    )
    assert not validate_context(ctx)
...
def test_context_rejects_universe_with_missing_source_catalog():
    ctx = context()
    universe = ctx["universes_by_id"]["univ_1"]
    universe["source_catalog_snapshot_ids"] = ["missing"]
    universe["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], universe
    )
    assert not validate_context(ctx)
...
def test_context_rejects_universe_with_unresolved_instrument():
    ctx = context()
    universe = ctx["universes_by_id"]["univ_1"]
    universe["instrument_ids"] = ["missing"]
    universe["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], universe
    )
    assert not validate_context(ctx)
...
def test_context_accepts_structurally_valid_nonoperable_history():
    test_context_accepts_resolvable_historical_catalog_member()


@pytest.mark.parametrize("bad", [False, 0, "", []])
def test_direct_catalog_rejects_falsey_previous_maps(bad):
    inst = sample_instrument()
    assert validate_instrument_catalog_snapshot(
        sample_catalog(inst), {inst["instrument_id"]: inst}, bad, instrument_history_by_id={}
    ) == (False, "CATALOG_SNAPSHOT_INVALID")


def test_direct_catalog_rejects_false_previous_map():
    test_direct_catalog_rejects_falsey_previous_maps(False)


def test_direct_catalog_rejects_zero_previous_map():
    test_direct_catalog_rejects_falsey_previous_maps(0)


def test_direct_catalog_rejects_empty_string_previous_map():
    test_direct_catalog_rejects_falsey_previous_maps("")


def test_direct_catalog_rejects_list_previous_map():
    test_direct_catalog_rejects_falsey_previous_maps([])


@pytest.mark.parametrize("bad", [False, 0, "", []])
def test_direct_capability_rejects_falsey_previous_maps(bad):
    assert validate_account_capability_snapshot(
        sample_capability_snapshot(), sample_account(), bad
    ) == (False, "ACCOUNT_READINESS_BLOCKED")


def test_direct_capability_rejects_false_previous_map():
    test_direct_capability_rejects_falsey_previous_maps(False)


def test_direct_capability_rejects_zero_previous_map():
    test_direct_capability_rejects_falsey_previous_maps(0)


def test_direct_capability_rejects_empty_string_previous_map():
    test_direct_capability_rejects_falsey_previous_maps("")


def test_direct_capability_rejects_list_previous_map():
    test_direct_capability_rejects_falsey_previous_maps([])
...
def test_direct_universe_rejects_non_dict_previous_map(bad):
    ctx = context()
    universe = ctx["universes_by_id"]["univ_1"]
    assert validate_trading_universe_version(
        universe,
        ctx["accounts_by_id"]["xacc_test_1"],
        ctx["instruments_by_id"],
        ctx["catalogs_by_id"],
...
def test_empty_dict_is_valid_for_first_version_without_predecessor():
    ...
    assert (
        ...
        and validate_account_capability_snapshot(
            sample_capability_snapshot(), sample_account(), {}
        )[0]
    )
...
def test_context_rejects_dangling_account_active_profile_id():
    assert_context_and_activation_blocked(
        unrelated_account_context(active_credential_profile_id="missing")
    )
...
def test_context_rejects_account_bound_to_foreign_profile():
    ctx = unrelated_account_context(active_credential_profile_id="cred_foreign")
    ctx["credential_profiles_by_id"]["cred_foreign"] = sample_profile(
        credential_profile_id="cred_foreign", exchange_account_id="xacc_test_1"
    )
    assert_context_and_activation_blocked(ctx)


def test_context_rejects_account_profile_scope_mismatch():
    ctx = unrelated_account_context(active_credential_profile_id="cred_other")
    ctx["credential_profiles_by_id"]["cred_other"] = sample_profile(
        credential_profile_id="cred_other",
        exchange_account_id="xacc_other",
...
def test_context_rejects_active_index_when_account_pointer_is_null():
    ctx = unrelated_account_context()
    ctx["active_profile_ids_by_account_id"]["xacc_other"] = ["cred_1"]
    assert_context_and_activation_blocked(ctx)


def test_context_rejects_dangling_account_capability_snapshot_id():
    assert_context_and_activation_blocked(
        unrelated_account_context(account_capability_snapshot_id="missing")
    )


def test_context_rejects_account_capability_bound_from_previous_map():
    ctx = unrelated_account_context(account_capability_snapshot_id="caps_old")
    snapshot = sample_capability_snapshot(
        ctx["accounts_by_id"]["xacc_other"], account_capability_snapshot_id="caps_old"
    )
    ctx["previous_account_capability_snapshots_by_id"]["caps_old"] = snapshot
    assert_context_and_activation_blocked(ctx)


def test_context_rejects_account_capability_scope_mismatch():
    ctx = unrelated_account_context(account_capability_snapshot_id="caps_other")
    snapshot = sample_capability_snapshot(
        ctx["accounts_by_id"]["xacc_other"],
        account_capability_snapshot_id="caps_other",
        environment="PAPER",
    )
    ctx["account_capability_snapshots_by_id"]["caps_other"] = snapshot
    assert_context_and_activation_blocked(ctx)


def test_context_accepts_null_optional_bindings_without_active_records():
    assert validate_context(unrelated_account_context())
...
def test_context_rejects_unrelated_dangling_previous_profile():
    test_context_rejects_previous_profile_with_missing_account()
...
def exact_history_catalog_context():
    ctx = context()
    current = ctx["instruments_by_id"]["instr_btcusdt_spot_testnet"]
    ...
    ctx["previous_catalogs_by_id"]["cat_old"] = old_catalog
    ctx["instrument_history_by_id"] = {old["instrument_id"]: [old]}
    return ctx
...
def test_context_rejects_catalog_listing_current_instrument_from_other_catalog():
    ctx = multi_catalog_context()
    cat2 = ctx["catalogs_by_id"]["cat_2"]
    cat2["instrument_ids"] = ["instr_btcusdt_spot_testnet"]
    cat2["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["catalog_snapshot_content_hash"], cat2
    )
    assert not validate_context(ctx)
...
def test_context_rejects_previous_catalog_listing_current_instrument_from_other_catalog():
    ctx = context()
    old = sample_catalog(catalog_snapshot_id="cat_old")
    ctx["previous_catalogs_by_id"]["cat_old"] = old
    assert not validate_context(ctx)
...
def test_context_rejects_catalog_member_scope_or_adapter_mismatch(field, value):
    ctx = exact_history_catalog_context()
    ctx["instrument_history_by_id"]["instr_btcusdt_spot_testnet"][0][field] = value
    assert not validate_context(ctx)


def test_context_rejects_catalog_member_exchange_mismatch():
    test_context_rejects_catalog_member_scope_or_adapter_mismatch(
        "exchange_id", "paper_simulated_venue"
    )


def test_context_rejects_catalog_member_environment_mismatch():
    test_context_rejects_catalog_member_scope_or_adapter_mismatch("environment", "PAPER")


def test_context_rejects_catalog_member_market_mismatch():
    test_context_rejects_catalog_member_scope_or_adapter_mismatch("market_type", "MARGIN")


def test_context_rejects_catalog_member_adapter_mismatch():
    test_context_rejects_catalog_member_scope_or_adapter_mismatch(
        "source_adapter_family_id", "other"
    )


def test_context_accepts_current_member_bound_to_exact_catalog():
    assert validate_context(context())


def test_context_accepts_historical_member_bound_to_exact_catalog():
    assert validate_context(exact_history_catalog_context())


def test_context_accepts_same_instrument_across_sequential_exact_catalog_records():
    assert validate_context(exact_history_catalog_context())
...
def universe_cross_catalog_context():
    ctx = multi_catalog_context()
    universe = ctx["universes_by_id"]["univ_1"]
    universe["instrument_ids"] = ["instr_ethusdt_spot_testnet"]
    universe["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], universe
    )
    return ctx
...
def test_context_rejects_universe_instrument_outside_source_catalogs():
    assert not validate_context(universe_cross_catalog_context())


def test_context_rejects_universe_instrument_bound_to_different_catalog():
    test_context_rejects_universe_instrument_outside_source_catalogs()


def test_context_rejects_universe_using_unrelated_catalog_membership():
    test_context_rejects_universe_instrument_outside_source_catalogs()


def historical_universe_context():
    ctx = exact_history_catalog_context()
    old_universe = sample_universe(
        trading_universe_id="univ_old",
        version=1,
        lifecycle_state="RETIRED",
        instrument_ids=["instr_btcusdt_spot_testnet"],
        source_catalog_snapshot_ids=["cat_old"],
        retired_at_utc="2026-02-01T00:00:00Z",
    )
    ctx["previous_universes_by_id"]["univ_old"] = old_universe
    return ctx


def test_context_rejects_previous_universe_with_missing_account():
    ctx = historical_universe_context()
    u = ctx["previous_universes_by_id"]["univ_old"]
    u["exchange_account_id"] = "missing"
    u["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], u
    )
    assert not validate_context(ctx)
...
def test_context_rejects_previous_universe_with_missing_source_catalog():
    ctx = historical_universe_context()
    u = ctx["previous_universes_by_id"]["univ_old"]
    u["source_catalog_snapshot_ids"] = ["missing"]
    u["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["trading_universe_content_hash"], u
    )
    assert not validate_context(ctx)
...
def test_context_accepts_universe_with_exact_source_membership():
    assert validate_context(context())


def test_context_accepts_valid_historical_universe_membership():
    assert validate_context(historical_universe_context())


def test_activation_rejects_unrelated_invalid_universe_in_context():
    assert_context_and_activation_blocked(universe_cross_catalog_context())
...
def catalog_without_target_context(replacement_id=None):
    ...
    catalog = ctx["catalogs_by_id"]["cat_1"]
    if replacement_id:
        other = sample_instrument(
            instrument_id=replacement_id, venue_symbol="OTHER", display_symbol="OTHER/USDT"
        )
...
def test_current_universe_rejects_target_absent_from_catalog_member_ids():
    ctx = catalog_without_target_context()
    catalog = ctx["catalogs_by_id"]["cat_1"]
    iid = "instr_btcusdt_spot_testnet"
    assert (
        resolve_catalog_member(
            catalog, iid, ctx["instruments_by_id"], ctx["instrument_history_by_id"]
        )
        is None
    )
    assert (
        validate_universe_source_membership(
            ctx["universes_by_id"]["univ_1"],
            ctx["accounts_by_id"],
            ctx["catalogs_by_id"],
            ctx["instruments_by_id"],
            ctx["instrument_history_by_id"],
    ...
    assert (
        not ok
        and out
        == {"denial": "ACCOUNT_READINESS_BLOCKED", "audit_event": "TRADING_UNIVERSE_REJECTED"}
        and out["audit_event"] != "TRADING_UNIVERSE_ACTIVATED"
    )


def test_current_catalog_with_other_member_does_not_grant_target_membership():
    ctx = catalog_without_target_context("instr_other")
    catalog = ctx["catalogs_by_id"]["cat_1"]
    iid = "instr_btcusdt_spot_testnet"
    assert (
        resolve_catalog_member(
...
def test_historical_universe_rejects_member_absent_from_source_catalog_ids():
    ctx = historical_universe_context()
    catalog = ctx["previous_catalogs_by_id"]["cat_old"]
    catalog["instrument_ids"] = []
    catalog["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["catalog_snapshot_content_hash"], catalog
    )
...
def test_malformed_history_metadata_version_is_total_and_fail_closed(bad_version):
    ctx = exact_history_catalog_context()
    record = ctx["instrument_history_by_id"]["instr_btcusdt_spot_testnet"][0]
    record["metadata_version"] = bad_version
    catalog = ctx["previous_catalogs_by_id"]["cat_old"]
    assert (
        resolve_catalog_member(
            catalog,
...
def test_mixed_history_metadata_versions_are_total_and_fail_closed():
    ctx = exact_history_catalog_context()
    first = ctx["instrument_history_by_id"]["instr_btcusdt_spot_testnet"][0]
    second = dict(first, metadata_version="bad")
    ctx["instrument_history_by_id"][first["instrument_id"]] = [first, second]
    catalog = ctx["previous_catalogs_by_id"]["cat_old"]
    assert (
        resolve_catalog_member(
            catalog,
...
def history_only_member_context(*, catalog_id="cat_1", listed=True):
    ctx = context()
    base = ctx["instruments_by_id"]["instr_btcusdt_spot_testnet"]
    ghost = dict(
        base,
        instrument_id="instr_ghost",
        venue_symbol="GHOST",
...
def test_context_rejects_dangling_historical_instrument_catalog_reference():
    ctx = history_only_member_context(catalog_id="cat_missing", listed=False)
    assert_context_and_activation_blocked(ctx)


def test_context_rejects_unlisted_historical_instrument_member():
    ctx = history_only_member_context(listed=False)
    assert_context_and_activation_blocked(ctx)
...
def test_context_rejects_historical_instrument_catalog_scope_or_adapter_mismatch(field, value):
    ctx = history_only_member_context()
    ctx["instrument_history_by_id"]["instr_ghost"][0][field] = value
    assert_context_and_activation_blocked(ctx)
...
def test_direct_membership_rejects_current_version_not_above_history_max(
    history_versions, current_version
):
    ctx = exact_history_catalog_context()
    iid = "instr_btcusdt_spot_testnet"
    template = ctx["instrument_history_by_id"][iid][0]
    ctx["instrument_history_by_id"][iid] = [
        dict(template, metadata_version=version) for version in history_versions
    ]
...
def test_direct_membership_accepts_current_version_above_complete_history():
    ...
    template = ctx["instrument_history_by_id"][iid][0]
    ctx["instrument_history_by_id"][iid] = [
        dict(template, metadata_version=1),
        dict(template, metadata_version=2),
    ]
    ctx["instruments_by_id"][iid]["metadata_version"] = 3
    assert (
        resolve_catalog_member(
            ctx["catalogs_by_id"]["cat_1"],
            iid,
            ctx["instruments_by_id"],
            ctx["instrument_history_by_id"],
            ctx["previous_catalogs_by_id"],
        )
        == ctx["instruments_by_id"][iid]
    )
    assert (
        validate_catalog_instrument_graph(
            ctx["catalogs_by_id"],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("content_hash", "bad"),
        ("observed_at_utc", "bad"),
        ("effective_at_utc", "bad"),
        ("stale_after_utc", "bad"),
        ("adapter_version", ""),
        ("previous_snapshot_id", False),
        ("environment", "LIVE"),
        ("market_type", "OPTIONS"),
    ],
)
...
def orphan_current_instrument_context():
    ctx = context()
    orphan = sample_instrument(
        instrument_id="instr_orphan", venue_symbol="ORPHAN", display_symbol="ORPHAN/USDT"
    )
    ctx["instruments_by_id"][orphan["instrument_id"]] = orphan
    return ctx


def test_orphan_current_instrument_invalidates_complete_trusted_graph():
    ctx = orphan_current_instrument_context()
    assert validate_context(ctx) is False
    ok, out = activation_with_context(ctx)
    assert (
        not ok
...
def test_resolver_rejects_malformed_current_metadata_version_without_exception(bad_version):
    ctx = context()
    current = ctx["instruments_by_id"]["instr_btcusdt_spot_testnet"]
    current["metadata_version"] = bad_version
    catalog = ctx["catalogs_by_id"]["cat_1"]
    assert (
        resolve_catalog_member(
            catalog,
...
def test_resolver_rejects_malformed_catalog_without_exception(mutation):
    ctx = context()
    catalog = dict(ctx["catalogs_by_id"]["cat_1"])
    iid = "instr_btcusdt_spot_testnet"
    if mutation == "instrument_ids_wrong_type":
        catalog["instrument_ids"] = "bad"
    elif mutation == "duplicate_ids":
        catalog["instrument_ids"] = [iid, iid]
    elif mutation == "empty_id":
        catalog["instrument_ids"] = [""]
    elif mutation == "minimal":
        catalog = {"instrument_ids": [iid]}
    else:
        catalog.pop(mutation, None)
    assert (
        resolve_catalog_member(
            catalog, iid, ctx["instruments_by_id"], ctx["instrument_history_by_id"]
        )
...
def test_resolver_rejects_out_of_order_history_without_exception(versions):
    ...
    template = ctx["instrument_history_by_id"][iid][0]
    ctx["instrument_history_by_id"][iid] = [
        dict(template, metadata_version=version) for version in versions
    ]
    catalog = ctx["previous_catalogs_by_id"]["cat_old"]
    assert (
        resolve_catalog_member(
...
def test_resolver_rejects_malformed_history_record_with_valid_version(mutation):
    ctx = exact_history_catalog_context()
    iid = "instr_btcusdt_spot_testnet"
    record = ctx["instrument_history_by_id"][iid][0]
    if mutation == "missing_field":
        del record["display_symbol"]
    elif mutation == "different_id":
        record["instrument_id"] = "different"
    else:
        record["venue_symbol"] = "DIFFERENT"
    assert (
        resolve_catalog_member(
            ctx["previous_catalogs_by_id"]["cat_old"],
            iid,
            ctx["instruments_by_id"],
            ctx["instrument_history_by_id"],
        )
...
def test_reverse_binding_and_resolver_success_regressions():
    assert validate_context(context())
    assert validate_context(exact_history_catalog_context())
    ordered = exact_history_catalog_context()
    iid = "instr_btcusdt_spot_testnet"
    first = ordered["instrument_history_by_id"][iid][0]
    ordered["instrument_history_by_id"][iid] = [
        dict(first, metadata_version=1),
        dict(first, metadata_version=2),
    ]
    ordered["instruments_by_id"][iid]["metadata_version"] = 3
    assert validate_context(ordered)
    assert validate_context(historical_universe_context())
...
def test_resolver_catalog_snapshot_id_is_total(bad_id):
    ctx = context()
    catalog = dict(ctx["catalogs_by_id"]["cat_1"])
    catalog["catalog_snapshot_id"] = bad_id
    assert (
        resolve_catalog_member(
            catalog,
            "instr_btcusdt_spot_testnet",
            ctx["instruments_by_id"],
            ctx["instrument_history_by_id"],
            {},
        )
...
def test_catalog_previous_snapshot_id_is_total(bad_id):
    ctx = context()
    catalog = dict(ctx["catalogs_by_id"]["cat_1"])
    catalog["previous_snapshot_id"] = bad_id
...
def test_source_catalog_member_closure_rejects_additional_unresolved_member():
    ctx = context()
    catalog = ctx["catalogs_by_id"]["cat_1"]
    catalog["instrument_ids"].append("instr_missing")
    catalog["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["catalog_snapshot_content_hash"], catalog
    )
    assert (
...
@pytest.mark.parametrize(
    "mutation", ["missing_field", "scope", "adapter", "dangling_history", "bad_version"]
)
def test_source_catalog_member_closure_rejects_additional_malformed_member(mutation):
    ctx = context()
    catalog = ctx["catalogs_by_id"]["cat_1"]
    extra = sample_instrument(
        instrument_id="instr_extra", venue_symbol="EXTRA", display_symbol="EXTRA/USDT"
    )
    if mutation == "missing_field":
        extra.pop("display_symbol")
    elif mutation == "scope":
        extra["environment"] = "PAPER"
    elif mutation == "adapter":
        extra["source_adapter_family_id"] = "wrong"
    elif mutation == "bad_version":
        extra["metadata_version"] = "bad"
    else:
        extra["catalog_snapshot_id"] = "cat_missing"
        ctx["instrument_history_by_id"]["instr_extra"] = [extra]
        extra = None
    if extra is not None:
        ctx["instruments_by_id"]["instr_extra"] = extra
    catalog["instrument_ids"].append("instr_extra")
    catalog["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["catalog_snapshot_content_hash"], catalog
    )
    assert (
...
def catalog_lineage_context():
    ...
    current["previous_snapshot_id"] = "cat_old"
    current["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["catalog_snapshot_content_hash"], current
    )
    return ctx
...
MALFORMED_PREDECESSORS = [
    "instrument_ids_type",
    "instrument_ids_duplicate",
    "instrument_ids_empty",
    "catalog_id",
    "previous_id",
    "status",
    "adapter_version",
...
def test_catalog_lineage_rejects_malformed_predecessor_totally(mutation):
    ctx = catalog_lineage_context()
    previous = ctx["previous_catalogs_by_id"]["cat_old"]
    if mutation == "instrument_ids_type":
        previous["instrument_ids"] = "bad"
    elif mutation == "instrument_ids_duplicate":
        previous["instrument_ids"] = ["instr_btcusdt_spot_testnet"] * 2
    elif mutation == "instrument_ids_empty":
        previous["instrument_ids"] = [""]
    elif mutation == "catalog_id":
        previous["catalog_snapshot_id"] = []
    elif mutation == "previous_id":
        previous["previous_snapshot_id"] = set()
    elif mutation == "status":
        previous["status"] = "BOGUS"
    elif mutation == "adapter_version":
        previous["adapter_version"] = ""
    elif mutation == "content_hash":
        previous["content_hash"] = "bad"
    elif mutation == "timestamps":
        previous["observed_at_utc"] = "bad"
    elif mutation == "environment":
        previous["environment"] = "LIVE"
    else:
        previous["market_type"] = "OPTIONS"
    current = ctx["catalogs_by_id"]["cat_1"]
    assert validate_catalog_lineage(current, ctx["previous_catalogs_by_id"]) == (
        False,
        "CATALOG_SNAPSHOT_INVALID",
    )
...
def test_total_catalog_and_graph_closure_success_regressions():
    ...
    assert validate_context(historical_universe_context())
    ctx = catalog_lineage_context()
    assert validate_catalog_lineage(
        ctx["catalogs_by_id"]["cat_1"], ctx["previous_catalogs_by_id"]
    ) == (True, None)
...
def test_direct_membership_is_total_for_malformed_source_instrument_ids(bad_members):
    ctx = context()
    ctx["catalogs_by_id"]["cat_1"]["instrument_ids"] = bad_members
...
def test_direct_membership_is_total_for_malformed_trusted_maps(target, bad):
    ctx = context()
    if target == "catalog_value":
        ctx["catalogs_by_id"]["cat_1"] = bad
    elif target == "catalog_key":
        ctx["catalogs_by_id"] = {1: ctx["catalogs_by_id"]["cat_1"]}
    elif target == "previous_value":
        ctx["previous_catalogs_by_id"]["cat_old"] = bad
    elif target == "instrument_value":
        ctx["instruments_by_id"]["instr_bad"] = bad
    else:
        ctx["instrument_history_by_id"]["instr_bad"] = bad
    assert direct_membership(ctx) is False
...
def test_direct_rejects_unrelated_empty_history():
    ctx = context()
    ctx["instrument_history_by_id"]["instr_ghost"] = []
    assert_unrelated_graph_rejected(ctx)


def unrelated_history_context():
    ctx = history_only_member_context()
    record = ctx["instrument_history_by_id"]["instr_ghost"][0]
    return ctx, record


def test_direct_rejects_unrelated_out_of_order_history():
    ctx, record = unrelated_history_context()
    ctx["instrument_history_by_id"]["instr_ghost"] = [
        dict(record, metadata_version=2),
        dict(record, metadata_version=1),
    ]
    assert_unrelated_graph_rejected(ctx)
...
def test_direct_rejects_unrelated_history_identity_rewrite():
    ...
    ctx["instrument_history_by_id"]["instr_ghost"] = [
        dict(record, metadata_version=1),
        dict(record, metadata_version=2, venue_symbol="REWRITE"),
    ]
    assert_unrelated_graph_rejected(ctx)
...
def test_direct_rejects_unrelated_current_history_rollback(current_version):
    ...
    current = sample_instrument(
        instrument_id="instr_extra",
        venue_symbol="EXTRA",
        display_symbol="EXTRA/USDT",
        metadata_version=current_version,
    )
    ctx["instruments_by_id"]["instr_extra"] = current
    ctx["instrument_history_by_id"]["instr_extra"] = [
        dict(current, metadata_version=1),
        dict(current, metadata_version=2),
    ]
    catalog["instrument_ids"].append("instr_extra")
    catalog["content_hash"] = hash_payload(
        DATA["deterministic_hash_contracts"]["catalog_snapshot_content_hash"], catalog
...
@pytest.mark.parametrize(
    "mutation", ["missing_display", "bad_version", "missing_catalog", "bad_adapter", "scope"]
)
def test_direct_rejects_unrelated_malformed_current_instrument(mutation):
    ctx = context()
    record = sample_instrument(
        instrument_id="instr_extra", venue_symbol="EXTRA", display_symbol="EXTRA/USDT"
    )
    if mutation == "missing_display":
        record.pop("display_symbol")
    elif mutation == "bad_version":
    ...
    ctx["instruments_by_id"]["instr_extra"] = record
    assert_unrelated_graph_rejected(ctx)


@pytest.mark.parametrize(
    "mutation", ["status", "members", "hash", "timestamp", "unresolved", "key_mismatch"]
)
def test_direct_rejects_unrelated_malformed_catalog(mutation):
    ctx = multi_catalog_context()
    catalog = ctx["catalogs_by_id"]["cat_2"]
    if mutation == "status":
        catalog["status"] = "BOGUS"
    elif mutation == "members":
        catalog["instrument_ids"] = None
    elif mutation == "hash":
        catalog["content_hash"] = "bad"
    elif mutation == "timestamp":
        catalog["observed_at_utc"] = "bad"
        ...
        ctx["catalogs_by_id"]["wrong"] = ctx["catalogs_by_id"].pop("cat_2")
    assert_unrelated_graph_rejected(ctx)


def test_direct_rejects_current_previous_catalog_id_collision():
    ctx = context()
    ctx["previous_catalogs_by_id"]["cat_1"] = dict(ctx["catalogs_by_id"]["cat_1"])
    assert_unrelated_graph_rejected(ctx)


def history_in_second_current_catalog_context():
    ctx = exact_history_catalog_context()
    ctx["catalogs_by_id"]["cat_old"] = ctx["previous_catalogs_by_id"].pop("cat_old")
    return ctx
