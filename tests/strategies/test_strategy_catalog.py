...
def test_describe_multi_strategy_configuration_filters_blocked_entries(
    ...
    assert summary["blocked_suspension_reasons"] == {
        "schedule:scalp-run": "Strategia 'scalping' nie jest dostępna."
    }
    assert summary["guard_reason_summary"] == {
        "strategies": {
            "total": 1,
    ...
    assert summary["guard_reason_details"] == {
        "scalping": {
            "strategies": [
                {
                    "name": "scalper",
    ...
    assert summary["guard_reason_detail_summary"] == {
        "scalping": {
            "strategies": {
                "total": 1,
                "by_reason": {"Strategia 'scalping' nie jest dostępna.": 1},
    ...
    assert summary["guard_reason_detail_category_summary"] == {
        "strategies": {
            "total": 1,
            "by_capability": {"scalping": 1},
            "by_reason": {"Strategia 'scalping' nie jest dostępna.": 1},
    ...
    assert summary["guard_reason_detail_capability_reason_summary"] == {
        "scalping": {
    ...
    assert summary["guard_reason_detail_reason_summary"] == {
        "Strategia 'scalping' nie jest dostępna.": {
            "total": 5,
            "by_capability": {"scalping": 5},
            "by_category": {
    ...
    assert summary["guard_reason_detail_reason_details"] == {
        "Strategia 'scalping' nie jest dostępna.": {
            "total": 5,
