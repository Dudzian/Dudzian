...
OS_IDENTITIES = (
    "os_freshness_crypto_verifier", "os_freshness_runtime", "os_freshness_outsider",
)
...
def isolated_peer_cluster():
    ...
    for identity in OS_IDENTITIES:
        try:
            ...
            _run(["useradd", "--system", "--no-create-home", "--shell", "/usr/sbin/nologin", identity])
    ...
    port = _free_port()
    try:
        _as("postgres", [str(bindir / "initdb"), "-D", str(data), "--auth-local=reject", "--auth-host=reject"])
        ...
        _as("postgres", [
            str(bindir / "pg_ctl"), "-D", str(data), "-l", str(base / "postgres.log"),
            "-w", "start",
        ])
        ...
        with psycopg.connect(gate_admin, autocommit=True) as conn:
            ...
            conn.execute("GRANT CREATE, USAGE ON SCHEMA freshness_authority TO freshness_function_owner")
            conn.execute("CREATE TABLE freshness_authority.probe_state (value integer NOT NULL)")
            conn.execute("ALTER TABLE freshness_authority.probe_state OWNER TO freshness_schema_owner")
            conn.execute("GRANT SELECT,INSERT,UPDATE,DELETE ON freshness_authority.probe_state TO freshness_function_owner")
            for name, expected in (
                ...
                """)
                conn.execute(f"ALTER FUNCTION freshness_authority.{name}() OWNER TO freshness_function_owner")
                conn.execute(f"REVOKE ALL ON FUNCTION freshness_authority.{name}() FROM PUBLIC")
            conn.execute("REVOKE CREATE ON SCHEMA freshness_authority FROM freshness_function_owner")
            conn.execute("GRANT USAGE ON SCHEMA freshness_authority TO freshness_crypto_verifier, freshness_runtime")
            conn.execute("GRANT EXECUTE ON FUNCTION freshness_authority.prepare_verified_freshness_candidate() TO freshness_crypto_verifier")
            conn.execute("GRANT EXECUTE ON FUNCTION freshness_authority.compare_and_advance() TO freshness_runtime")
        yield {"bindir": bindir, "data": data, "socket": socket_dir, "port": port, "admin": gate_admin}
    finally:
        if data.exists() and (data / "postmaster.pid").exists():
            _as("postgres", [str(bindir / "pg_ctl"), "-D", str(data), "-m", "immediate", "-w", "stop"], check=False)
...
def _psql(cluster, os_identity: str, db_role: str, query: str):
    return _as(os_identity, [
        str(cluster["bindir"] / "psql"), "-XAt", "-h", str(cluster["socket"]),
        "-p", str(cluster["port"]), "-d", "freshness_gate", "-U", db_role, "-c", query,
    ], check=False)


def test_new_connections_are_bound_to_kernel_peer_identity(isolated_peer_cluster):
    cluster = isolated_peer_cluster
    verifier = _psql(cluster, "os_freshness_crypto_verifier", "freshness_crypto_verifier", "SELECT session_user")
    ...
    assert _psql(cluster, "os_freshness_runtime", "freshness_crypto_verifier", "SELECT 1").returncode != 0
    assert _psql(cluster, "os_freshness_outsider", "freshness_crypto_verifier", "SELECT 1").returncode != 0
    assert _psql(cluster, "os_freshness_crypto_verifier", "freshness_runtime", "SELECT 1").returncode != 0

    assert _psql(cluster, "os_freshness_crypto_verifier", "freshness_crypto_verifier",
                 "SELECT * FROM freshness_authority.prepare_verified_freshness_candidate()").stdout.strip() == (
                     "freshness_crypto_verifier|freshness_function_owner"
                 )
    assert _psql(cluster, "os_freshness_runtime", "freshness_runtime",
                 "SELECT * FROM freshness_authority.compare_and_advance()").stdout.strip() == (
                     "freshness_runtime|freshness_function_owner"
                 )
...
def test_hba_catalog_roles_owners_acl_and_durability(isolated_peer_cluster):
    ...
    with psycopg.connect(cluster["admin"]) as conn:
        ...
        assert settings == {**settings, "fsync": "on", "synchronous_commit": "on", "listen_addresses": ""}

        roles = conn.execute(
            "SELECT rolname,rolcanlogin,rolinherit FROM pg_roles WHERE rolname=ANY(%s)", (list(AUTHORITY_ROLES),)
        ).fetchall()
        assert len(roles) == 6
        assert all(not inherit for _, _, inherit in roles)
        assert {name for name, login, _ in roles if login} == {"freshness_crypto_verifier", "freshness_runtime"}
        assert conn.execute(
            "SELECT count(*) FROM pg_auth_members m JOIN pg_roles p ON p.oid=m.roleid JOIN pg_roles c ON c.oid=m.member "
            "WHERE p.rolname=ANY(%s) OR c.rolname=ANY(%s)", (list(AUTHORITY_ROLES), list(AUTHORITY_ROLES))
        ).fetchone() == (0,)
        ...
        function_owner_oid = conn.execute("SELECT oid FROM pg_roles WHERE rolname='freshness_function_owner'").fetchone()[0]
        ...
        """, (list(AUTHORITY_ROLES),)).fetchall()
        assert {name for name, allowed in usage if allowed} == {
            "freshness_schema_owner", "freshness_function_owner",
            "freshness_crypto_verifier", "freshness_runtime",
        }
