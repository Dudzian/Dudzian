...
def test_build_smoke_summary_payload_includes_metadata(tmp_path: Path) -> None:
    ...
    assert payload["publish"]["required"] is True
    assert payload["publish"]["reason"] == "auto_publish_invoked"
...
def test_append_publish_context_populates_metadata() -> None:
    context: dict[str, str] = {}
    run_daily_trend._append_publish_context(
        context,
        {
            "status": "skipped",
            "exit_code": 0,
            "required": True,
            "reason": "dry_run",
            "raw_stdout": "auto publish dry run - nothing executed",
            "raw_stderr": "",
            "json_sync": {
                "status": "ok",
                "backend": "local",
                "location": "audit/log.jsonl",
                "metadata": {"version_id": "v5"},
            },
            "archive_upload": {
                "status": "ok",
                "backend": "s3",
                "location": "s3://bucket/report.zip",
                "metadata": {"ack_request_id": "req-9"},
            },
        },
    )

    assert context["paper_smoke_publish_status"] == "skipped"
    assert context["paper_smoke_publish_exit_code"] == "0"
    ...
    assert context["paper_smoke_publish_json_version_id"] == "v5"
    assert context["paper_smoke_publish_archive_backend"] == "s3"
    assert context["paper_smoke_publish_archive_location"] == "s3://bucket/report.zip"
    assert context["paper_smoke_publish_archive_ack_request_id"] == "req-9"


def test_append_publish_context_truncates_snippets() -> None:
    context: dict[str, str] = {}
    long_text = "x" * 600
    run_daily_trend._append_publish_context(
        context,
        {
            "status": "error",
            "exit_code": 2,
            "raw_stdout": long_text,
            "raw_stderr": long_text,
        },
    )

    stdout_snippet = context["paper_smoke_publish_stdout_snippet"]
    stderr_snippet = context["paper_smoke_publish_stderr_snippet"]
    assert len(stdout_snippet) <= 240
    assert stdout_snippet.endswith("…")
    assert len(stderr_snippet) <= 240
    assert stderr_snippet.endswith("…")
