...
from bot_core.ai.config_loader import load_risk_thresholds, reset_threshold_cache
...
def test_script_writes_threshold_configuration(tmp_path: Path) -> None:
    ...
    assert metadata.get("generated_from") == "stage6.autotrade.threshold_calibration"
    assert metadata.get("generated_at") is not None
    assert metadata.get("since") == "2024-01-01T00:00:00+00:00"
    assert metadata.get("until") == "2024-01-02T00:00:00+00:00"
    sample_counts = metadata.get("sample_counts")
    assert isinstance(sample_counts, Mapping)
    assert sample_counts.get("journal_events") is not None
    assert sample_counts.get("autotrade_entries") is not None
    approximation_entries = metadata.get("approximation_metrics_list")
    if approximation_entries is not None:
        assert isinstance(approximation_entries, list)
        assert any(
            isinstance(entry, Mapping)
            and entry.get("primary_exchange") == "binance"
            and entry.get("strategy") == "trend_following"
            and entry.get("metric") == "signal_after_adjustment"
            for entry in approximation_entries
        )
    approximation_global = metadata.get("approximation_global_metrics_list")
    assert isinstance(approximation_global, list)
    assert "signal_after_adjustment" in approximation_global

    assert metadata.get("max_freeze_events") == 1
    assert metadata.get("max_raw_values") == 1
    assert metadata.get("max_group_samples") == 0
    assert metadata.get("max_global_samples") == 0

    raw_freeze_truncated_groups = metadata.get("raw_freeze_events_truncated_groups")
    if raw_freeze_truncated_groups is not None:
        assert isinstance(raw_freeze_truncated_groups, int)
        assert raw_freeze_truncated_groups >= 0

    raw_values_truncated_groups = metadata.get("raw_values_truncated_groups")
    if raw_values_truncated_groups is not None:
        assert isinstance(raw_values_truncated_groups, int)
        assert raw_values_truncated_groups >= 0

    raw_values_omitted_total = metadata.get("raw_values_omitted_total")
    if raw_values_omitted_total is not None:
        assert isinstance(raw_values_omitted_total, int)
        assert raw_values_omitted_total >= 0

    group_samples_truncated = metadata.get("group_samples_truncated_metrics")
    if group_samples_truncated is not None:
        assert isinstance(group_samples_truncated, int)
        assert group_samples_truncated >= 0

    group_samples_omitted = metadata.get("group_samples_omitted_total")
    if group_samples_omitted is not None:
        assert isinstance(group_samples_omitted, int)
        assert group_samples_omitted >= 0

    global_samples_truncated = metadata.get("global_samples_truncated_metrics")
    if global_samples_truncated is not None:
        assert isinstance(global_samples_truncated, int)
        assert global_samples_truncated >= 0

    global_samples_omitted = metadata.get("global_samples_omitted_total")
    if global_samples_omitted is not None:
        assert isinstance(global_samples_omitted, int)
        assert global_samples_omitted >= 0

    clamped_metadata = metadata.get("clamped_values")
    if clamped_metadata is not None:
        assert isinstance(clamped_metadata, Mapping)
        for scope in ("group", "global"):
            scope_payload = clamped_metadata.get(scope)
            if scope_payload is None:
                continue
            assert isinstance(scope_payload, Mapping)
            for counter_key in (
                "regular",
                "absolute",
                "metrics_with_regular_clamp",
                "metrics_with_absolute_clamp",
            ):
                counter_value = scope_payload.get(counter_key)
    ...
    assert "risk_score" not in trend_thresholds

    reset_threshold_cache()
    loaded_config = load_risk_thresholds(config_path=output_config)
    auto_loaded = loaded_config.get("auto_trader")
    assert isinstance(auto_loaded, Mapping)

    loaded_signal_thresholds = auto_loaded.get("signal_thresholds")
    assert isinstance(loaded_signal_thresholds, Mapping)
    assert loaded_signal_thresholds.get("signal_after_adjustment") == signal_thresholds.get(
        "signal_after_adjustment"
    )

    loaded_map_cfg = auto_loaded.get("map_regime_to_signal")
    assert isinstance(loaded_map_cfg, Mapping)
    assert loaded_map_cfg.get("risk_score") == map_cfg.get("risk_score")

    loaded_strategy_thresholds = auto_loaded.get("strategy_signal_thresholds")
    assert isinstance(loaded_strategy_thresholds, Mapping)
    binance_loaded = loaded_strategy_thresholds.get("binance")
    assert isinstance(binance_loaded, Mapping)
    trend_loaded = binance_loaded.get("trend_following")
    assert isinstance(trend_loaded, Mapping)
    assert "risk_score" not in trend_loaded
...
def test_build_threshold_config_deduplicates_approximation_lists() -> None:
    report = {
        "groups": [],
        "global_summary": {"metrics": {}},
        "sources": {
            "approximation_metrics_list": [
                {
                    "primary_exchange": "Binance",
                    "strategy": "Trend_Following",
                ...
                },
                {
                    "primary_exchange": "BINANCE ",
                    "strategy": "TREND_FOLLOWING",
                    "metric": "signal_after_clamp",
                },
            ],
            "approximation_global_metrics_list": [
                "Signal_After_Adjustment",
                "signal_after_clamp",
                "signal_after_adjustment",
            ],
    ...
    assert [
        ...
        ("Binance", "Trend_Following", "signal_after_adjustment"),
        ("BINANCE", "TREND_FOLLOWING", "signal_after_clamp"),
    ]

    approximation_global = metadata.get("approximation_global_metrics_list")
    assert approximation_global == [
        "signal_after_adjustment",
        "signal_after_clamp",
    ]
