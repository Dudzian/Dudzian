...
def test_generate_report_samples_raw_freeze_events() -> None:
    ...
    assert raw_freeze_payload["requested_limit"] == 1
    assert "display_limit" not in raw_freeze_payload
    assert "display_overflow_summary" not in raw_freeze_payload
    ...
    assert global_raw["requested_limit"] == 1
    assert "display_limit" not in global_raw
    assert "display_overflow_summary" not in global_raw
    ...
    assert "display_limit" not in sources
    assert "display_overflow_summary" not in sources
...
def test_generate_report_limits_raw_freeze_events_with_display_limit() -> None:
    ...
    assert raw_payload["mode"] == "sample"
    assert raw_payload["limit"] == 1
    assert raw_payload["requested_limit"] == 5
    assert raw_payload["display_limit"] == 1
    ...
    assert global_raw["mode"] == "sample"
    assert global_raw["limit"] == 1
    assert global_raw["requested_limit"] == 5
    assert global_raw["display_limit"] == 1
    assert len(global_raw["events"]) == 1
    assert global_raw["overflow_summary"]["total"] == 2
    assert global_raw["display_overflow_summary"]["total"] == 2

    sources = report["sources"]["raw_freeze_events"]
    assert sources["mode"] == "sample"
    assert sources["limit"] == 1
    assert sources["requested_limit"] == 5
    assert sources["display_limit"] == 1
    assert sources["overflow_summary"]["total"] == 2
    assert sources["display_overflow_summary"]["total"] == 2
...
def test_generate_report_can_omit_raw_freeze_events_when_requested() -> None:
    ...
    sources = report["sources"]["raw_freeze_events"]
    assert sources == {"mode": "omit", "requested_limit": 5, "reason": "explicit_omit"}
...
def test_generate_report_records_no_samples_reason_when_sampling_disabled() -> None:
    ...
    sources = report["sources"]["raw_freeze_events"]
    assert sources == {"mode": "omit", "requested_limit": 3, "reason": "no_samples"}


def test_generate_report_includes_full_freeze_events_when_unbounded() -> None:
    journal_events = [
        {
            "symbol": "BTCUSDT",
            "primary_exchange": "binance",
    ...
    assert freeze_summary["auto"] == 2
    assert freeze_summary["manual"] == 1
    assert {item["status"]: item["count"] for item in freeze_summary["statuses"]} == {
        "auto_risk_freeze": 2,
        "risk_freeze": 1,
    }
    assert {item["reason"]: item["count"] for item in freeze_summary["reasons"]} == {
        "manual_override": 1,
        "risk_score_threshold": 2,
    }
    freeze_events = group["freeze_events"]
    ...
    assert sources == {"mode": "all", "reason": "explicit_unbounded"}
...
def test_generate_report_limits_freeze_events_when_max_set() -> None:
    ...
    raw_group = group["raw_freeze_events"]
    assert raw_group["mode"] == "limit"
    assert raw_group["limit"] == 2
    assert raw_group["requested_limit"] == 2
    assert "display_limit" not in raw_group
    assert "display_overflow_summary" not in raw_group
    assert raw_group["events"] == freeze_events["events"]
    raw_overflow = raw_group["overflow_summary"]
    assert raw_overflow["total"] == 1
    assert {item["status"]: item["count"] for item in raw_overflow["statuses"]} == {
        "auto_risk_freeze": 1
    }
    assert {item["reason"]: item["count"] for item in raw_overflow["reasons"]} == {
        "risk_score_threshold": 1
    }

    raw_global = report["global_summary"]["raw_freeze_events"]
    assert raw_global["mode"] == "limit"
    assert raw_global["limit"] == 2
    assert raw_global["requested_limit"] == 2
    assert "display_limit" not in raw_global
    assert "display_overflow_summary" not in raw_global
    assert raw_global["events"] == global_freeze["events"]
...
def test_generate_report_limits_freeze_events_when_max_zero() -> None:
    ...
    raw_group = group["raw_freeze_events"]
    assert raw_group["mode"] == "limit"
    assert raw_group["limit"] == 0
    assert raw_group["requested_limit"] == 0
    assert raw_group["events"] == []
    raw_overflow = raw_group["overflow_summary"]
    assert raw_overflow["total"] == 2
    assert {item["status"]: item["count"] for item in raw_overflow["statuses"]} == {
        "auto_risk_freeze": 1,
        "risk_freeze": 1,
    }
    ...
    raw_global = report["global_summary"]["raw_freeze_events"]
    assert raw_global["mode"] == "limit"
    assert raw_global["limit"] == 0
    assert raw_global["requested_limit"] == 0
    assert raw_global["events"] == []
    assert raw_global["overflow_summary"] == raw_overflow
    assert "display_overflow_summary" not in raw_global
...
def test_generate_report_samples_raw_freeze_events_with_sample_limit() -> None:
    journal_events = [
        {
            "symbol": "BTCUSDT",
            "primary_exchange": "binance",
    ...
    with patch("scripts.calibrate_autotrade_thresholds.load_risk_thresholds", return_value={}):
        report = _generate_report(
            ...
            percentiles=[0.5],
            suggestion_percentile=0.5,
            raw_freeze_events_sample_limit=2,
        )

    assert report["groups"]
    group = report["groups"][0]
    freeze_summary = group["freeze_summary"]
    assert freeze_summary["total"] == 4
    assert {item["status"]: item["count"] for item in freeze_summary["statuses"]} == {
        "auto_risk_freeze": 2,
        "risk_freeze": 2,
    }
    ...
    assert [event["status"] for event in raw_payload["events"]] == [
        "risk_freeze",
        "auto_risk_freeze",
    ]
    raw_overflow = raw_payload["overflow_summary"]
    assert raw_overflow["total"] == 2
    assert {item["status"]: item["count"] for item in raw_overflow["statuses"]} == {
        "auto_risk_freeze": 1,
        "risk_freeze": 1,
    ...
    global_raw = report["global_summary"]["raw_freeze_events"]
    assert global_raw["mode"] == "sample"
    assert global_raw["limit"] == 2
    assert [event["status"] for event in global_raw["events"]] == [
        "risk_freeze",
        "auto_risk_freeze",
    ]
    assert global_raw["overflow_summary"] == raw_overflow

    raw_sources = report["sources"]["raw_freeze_events"]
    assert raw_sources["mode"] == "sample"
    assert raw_sources["limit"] == 2
    assert raw_sources["requested_limit"] == 2


def test_generate_report_omits_raw_freeze_events_when_sample_limit_zero() -> None:
    journal_events = [
        {
            "symbol": "BTCUSDT",
            "primary_exchange": "binance",
    ...
    with patch("scripts.calibrate_autotrade_thresholds.load_risk_thresholds", return_value={}):
        report = _generate_report(
            ...
            percentiles=[0.5],
            suggestion_percentile=0.5,
            raw_freeze_events_sample_limit=0,
        )
...
def test_generate_report_can_omit_freeze_events() -> None:
    ...
    with patch("scripts.calibrate_autotrade_thresholds.load_risk_thresholds", return_value={}):
        report = _generate_report(
            ...
            suggestion_percentile=0.5,
            max_freeze_events=2,
            omit_freeze_events=True,
            omit_raw_freeze_events=True,
        )

    assert report["groups"]
    group = report["groups"][0]
    assert group["freeze_summary"]["total"] == 2
    assert "freeze_events" not in group

    global_summary = report["global_summary"]
    assert global_summary["freeze_summary"]["total"] == 2
    assert "freeze_events" not in global_summary

    sources = report["sources"]["freeze_events"]
    assert sources == {"mode": "omit"}


def test_generate_report_can_omit_raw_freeze_events_with_max_limit() -> None:
    journal_events = [
        {
            "symbol": "BTCUSDT",
            "primary_exchange": "binance",
    ...
    sources = report["sources"]["raw_freeze_events"]
    assert sources == {"mode": "omit", "requested_limit": 3, "reason": "explicit_omit"}


def test_generate_report_limits_raw_freeze_events_with_max_limit_and_display_limit() -> None:
    journal_events = [
        {
            "symbol": "BTCUSDT",
            "primary_exchange": "binance",
...
def test_main_accepts_raw_freeze_events_sample_limit(
    ...
    assert exit_code == 0
    assert captured["raw_freeze_events_sample_limit"] == 7
    assert captured["raw_freeze_events_mode"] == "sample"
    assert captured["limit_freeze_events"] is None
    assert captured["omit_raw_freeze_events"] is False
    assert captured["max_raw_freeze_events"] is None
...
def test_main_accepts_zero_raw_freeze_events_sample_limit(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    from scripts import calibrate_autotrade_thresholds as module
    ...
    assert exit_code == 0
    assert captured["raw_freeze_events_sample_limit"] == 0
    assert captured["raw_freeze_events_mode"] == "sample"
    assert captured["limit_freeze_events"] is None
    assert captured["omit_raw_freeze_events"] is False
    assert captured["max_raw_freeze_events"] is None
...
def test_main_accepts_omit_freeze_events(
    ...
    assert exit_code == 0
    assert captured["omit_freeze_events"] is True
    assert captured["max_freeze_events"] == 2
    assert captured["raw_freeze_events_mode"] == "omit"
