from __future__ import annotations

import json
import subprocess
import sys
from pathlib import Path

import yaml

REPO_ROOT = Path(__file__).resolve().parents[2]
SCRIPT = REPO_ROOT / "scripts/mock_runtime_preview.py"
SAFE_CONFIG = REPO_ROOT / "config/e2e/demo_paper.yml"


def _run(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, str(SCRIPT), *args],
        cwd=REPO_ROOT,
        text=True,
        capture_output=True,
        check=False,
    )


def _mutated_config(tmp_path: Path, dotted_path: str, value: object) -> Path:
    payload = yaml.safe_load(SAFE_CONFIG.read_text(encoding="utf-8"))
    current = payload
    segments = dotted_path.split(".")
    for segment in segments[:-1]:
        current = current[segment]
    current[segments[-1]] = value
    path = tmp_path / f"mutated_{dotted_path.replace('.', '_')}.yml"
    path.write_text(yaml.safe_dump(payload, sort_keys=False), encoding="utf-8")
    return path


def test_mock_runtime_preview_safe_config_json() -> None:
    result = _run(
        "--mode", "demo", "--config", str(SAFE_CONFIG), "--duration-seconds", "5", "--json"
    )
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["status"] == "ok"
    assert payload["mode"] == "demo"
    assert payload["exchange_io"] == "disabled"
    assert payload["order_execution"] == "disabled"
    assert payload["api_keys_required"] is False
    assert payload["live_mode_allowed"] is False
    assert payload["runtime_preview_started"] is True
    assert payload["simulated_lifecycle_max_sleep_seconds"] == 1
    assert payload["issues"] == []


def test_mock_runtime_preview_rejects_live_mode() -> None:
    result = _run(
        "--mode", "live", "--config", str(SAFE_CONFIG), "--duration-seconds", "5", "--json"
    )
    assert result.returncode == 2
    payload = json.loads(result.stdout)
    assert payload["status"] == "blocked"
    assert payload["reason"] == "mock_runtime_preview_forbids_live_mode"


def test_mock_runtime_preview_rejects_unsafe_execution_live_enabled(tmp_path: Path) -> None:
    cfg = _mutated_config(tmp_path, "execution.live.enabled", True)
    result = _run("--mode", "demo", "--config", str(cfg), "--duration-seconds", "5", "--json")
    assert result.returncode == 2
    payload = json.loads(result.stdout)
    assert "unsafe_config:execution.live.enabled" in payload["issues"]


def test_mock_runtime_preview_rejects_unsafe_trading_live_mode(tmp_path: Path) -> None:
    cfg = _mutated_config(tmp_path, "trading.enable_live_mode", True)
    result = _run("--mode", "demo", "--config", str(cfg), "--duration-seconds", "5", "--json")
    assert result.returncode == 2
    payload = json.loads(result.stdout)
    assert "unsafe_config:trading.enable_live_mode" in payload["issues"]


def test_mock_runtime_preview_rejects_unsafe_execution_default_mode(tmp_path: Path) -> None:
    cfg = _mutated_config(tmp_path, "execution.default_mode", "live")
    result = _run("--mode", "demo", "--config", str(cfg), "--duration-seconds", "5", "--json")
    assert result.returncode == 2
    payload = json.loads(result.stdout)
    assert "unsafe_config:execution.default_mode" in payload["issues"]


def test_mock_runtime_preview_rejects_disabled_paper_mode(tmp_path: Path) -> None:
    cfg = _mutated_config(tmp_path, "trading.enable_paper_mode", False)
    result = _run("--mode", "demo", "--config", str(cfg), "--duration-seconds", "5", "--json")
    assert result.returncode == 2
    payload = json.loads(result.stdout)
    assert "unsafe_config:trading.enable_paper_mode" in payload["issues"]


def test_mock_runtime_preview_rejects_disabled_force_paper_when_offline(tmp_path: Path) -> None:
    cfg = _mutated_config(tmp_path, "execution.force_paper_when_offline", False)
    result = _run("--mode", "demo", "--config", str(cfg), "--duration-seconds", "5", "--json")
    assert result.returncode == 2
    payload = json.loads(result.stdout)
    assert "unsafe_config:execution.force_paper_when_offline" in payload["issues"]


def test_mock_runtime_preview_allows_duration_86401() -> None:
    result = _run(
        "--mode", "demo", "--config", str(SAFE_CONFIG), "--duration-seconds", "86401", "--json"
    )
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["status"] == "ok"
    assert payload["bounded_duration_seconds"] == 86401


def test_mock_runtime_preview_allows_duration_300() -> None:
    result = _run(
        "--mode", "demo", "--config", str(SAFE_CONFIG), "--duration-seconds", "300", "--json"
    )
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["status"] == "ok"
    assert payload["bounded_duration_seconds"] == 300
    assert payload["simulated_lifecycle_max_sleep_seconds"] == 1


def test_mock_runtime_preview_allows_duration_86400() -> None:
    result = _run(
        "--mode", "demo", "--config", str(SAFE_CONFIG), "--duration-seconds", "86400", "--json"
    )
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["status"] == "ok"
    assert payload["bounded_duration_seconds"] == 86400
    assert payload["simulated_lifecycle_max_sleep_seconds"] == 1


def test_mock_runtime_preview_rejects_duration_259201_primary() -> None:
    result = _run(
        "--mode", "demo", "--config", str(SAFE_CONFIG), "--duration-seconds", "259201", "--json"
    )
    assert result.returncode == 2
    payload = json.loads(result.stdout)
    assert payload["status"] == "blocked"
    assert payload["reason"] == "duration_out_of_bounds"
    assert payload["max_duration_seconds"] == 259200
    assert payload["bounded_duration_seconds"] == 259201


def test_mock_runtime_preview_allows_duration_259200() -> None:
    result = _run(
        "--mode", "demo", "--config", str(SAFE_CONFIG), "--duration-seconds", "259200", "--json"
    )
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["status"] == "ok"
    assert payload["bounded_duration_seconds"] == 259200
    assert payload["simulated_lifecycle_max_sleep_seconds"] == 1


def test_mock_runtime_preview_rejects_duration_259201() -> None:
    result = _run(
        "--mode", "demo", "--config", str(SAFE_CONFIG), "--duration-seconds", "259201", "--json"
    )
    assert result.returncode == 2
    payload = json.loads(result.stdout)
    assert payload["status"] == "blocked"
    assert payload["reason"] == "duration_out_of_bounds"
    assert payload["max_duration_seconds"] == 259200
    assert payload["bounded_duration_seconds"] == 259201


def test_mock_runtime_preview_no_api_keys_required_without_secrets() -> None:
    result = _run(
        "--mode", "demo", "--config", str(SAFE_CONFIG), "--duration-seconds", "1", "--json"
    )
    assert result.returncode == 0
    payload = json.loads(result.stdout)
    assert payload["api_keys_required"] is False


def test_mock_runtime_preview_source_safety_contract() -> None:
    source = SCRIPT.read_text(encoding="utf-8")
    forbidden = (
        "ccxt",
        "create_order",
        "fetch_balance",
        "load_markets",
        "get_secret",
        "keychain",
        "os.environ",
        "getenv",
        "shell=True",
    )
    for token in forbidden:
        assert token not in source
