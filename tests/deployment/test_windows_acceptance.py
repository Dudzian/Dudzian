...
class ReviewedBoundary(AcceptanceBoundary):
    def __init__(
        self, tmp_path: Path, *, host: str = "Windows", elevated: bool = True,
        dependency: str | None = None, core_failure: bool = False,
...
def invoke(tmp_path: Path, boundary: ReviewedBoundary, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, Path]:
    ...
    run_acceptance(
        mode="github", revision="revision-1", core_output=core,
        evidence_output=evidence, boundary=boundary,
    )
    return core, evidence


def test_non_windows_host_rejected_without_artifacts(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.parametrize("reason", ["pywin32 is required", "project dependency check failed: broken"])
...
@pytest.mark.parametrize("item", [
    ...
    "WINDOWS_GRACEFUL_STOP", "WINDOWS_MANUAL_RESTART", "WINDOWS_CLEANUP",
])
...
def test_local_identity_cannot_satisfy_github_gate_but_github_scm_items_can(
    ...
    run_acceptance(
        mode="local", revision="revision-1", core_output=core,
        evidence_output=local_path, boundary=boundary,
    )
    local = json.loads(local_path.read_text(encoding="utf-8"))
    assert local["ci_provider"] == LOCAL_PROVIDER
    data = load_contract()
    local_blockers = blocking_items(
        "WINDOWS", data, [local], "revision-1", GITHUB_PROVIDER, "run-1",
    )
    ...
    blockers = blocking_items(
        "WINDOWS", data, [github], "revision-1", GITHUB_PROVIDER, "run-1",
    )
    for item in (
        "WINDOWS_SERVICE_INSTALLATION", "WINDOWS_SERVICE_START",
        "WINDOWS_GRACEFUL_STOP", "WINDOWS_MANUAL_RESTART",
...
def test_probe_installs_service_before_first_service_identity_acl() -> None:
    ...
    install = probe.index("& $PythonExecutable $harness acceptance-install-strict --username $identity")
...
def test_probe_ownership_record_is_created_after_absence_check_before_install() -> None:
    ...
    for field in (
        "run_token", "service_name", "service_identity", "python_executable", "harness",
    ):
...
def test_cleanup_only_same_service_name_without_matching_record_is_never_owned() -> None:
    ...
    cleanup = probe[cleanup_start:probe.index(
...
def test_preexisting_or_token_mismatched_service_is_untouched_and_cleanup_fails() -> None:
    ...
    ownership_test = probe[probe.index("function Test-BaseOwnershipRecord"):probe.index("function Add-PlannedGrant")]
...
def test_matching_owned_cleanup_removes_run_resources_and_record_before_pass() -> None:
    ...
    finally_block = probe[probe.rindex("} finally {"):probe.index("$result | ConvertTo-Json")]
    for operation in (
        "Stop-Service $service", "Remove-Item $marker", "Remove-ServiceGrant",
        "& $PythonExecutable $harness remove", "Remove-Item -LiteralPath $ownershipPath",
...
def test_partial_acl_timeout_uses_only_recorded_planned_grants() -> None:
    ...
    assert plan.index("Test-ServiceGrant") < plan.index("ownership.grants") < plan.index(
...
def test_intent_cleanup_only_preserves_foreign_marker_even_without_service() -> None:
    policy = cleanup_policy(
        _ownership_intent(), service_exists=False, health_marker_exists=True,
    )
    assert policy.remove_service is False
    assert policy.remove_health_marker is False
    assert policy.ownership_failure is True
...
def test_token_mismatch_cannot_authorize_service_or_marker_cleanup() -> None:
    ...
    with pytest.raises(OwnershipQualificationError, match="does not match this run"):
        qualify_service_creation(
            intent, run_token="b" * 64,
            python_executable=r"C:\Python\python.exe",
            harness=r"C:\repo\deployment\windows_test_service.py",
            strict_create_result="CREATED", observation=_canonical_observation(),
        )
    policy = cleanup_policy(intent, service_exists=True, health_marker_exists=True)
    assert policy.remove_service is False
    assert policy.remove_health_marker is False
...
def test_exact_sys_executable_is_propagated_even_when_path_python_differs(
    ...
    calls: list[list[str]] = []
    payload = {
        "WINDOWS_SERVICE_INSTALLATION": "PASS", "WINDOWS_SERVICE_START": "PASS",
        "WINDOWS_GRACEFUL_STOP": "PASS", "WINDOWS_MANUAL_RESTART": "PASS",
        "cleanup": "PASS",
    }
...
def test_timeout_invokes_cleanup_only_with_same_exact_interpreter(
    ...
    def timeout_then_cleanup(command: list[str], **kwargs: object) -> object:
        ...
        return type("Completed", (), {
            "returncode": 1, "stdout": json.dumps({"cleanup": "PASS"}), "stderr": "",
        })()
...
def test_second_artifact_publish_failure_rolls_back_first_new_output(
    ...
    def publisher(staged_core: Path, core: Path, staged_evidence: Path, evidence: Path) -> None:
        windows_acceptance.publish_artifacts(
            staged_core, core, staged_evidence, evidence, replace=failing_replace,
        )
    ...
    with pytest.raises(WindowsAcceptanceError, match="PUBLICATION_FAILED"):
        run_acceptance(
            mode="github", revision="revision-1", core_output=core,
            evidence_output=evidence, boundary=ReviewedBoundary(tmp_path), publisher=publisher,
        )
    assert moves == 2
...
def test_existing_outputs_are_preserved_and_run_fails_closed(
    ...
    with pytest.raises(WindowsAcceptanceError, match="OUTPUT_ALREADY_EXISTS"):
        run_acceptance(
            mode="github", revision="revision-1", core_output=core,
            evidence_output=evidence, boundary=ReviewedBoundary(tmp_path),
        )
