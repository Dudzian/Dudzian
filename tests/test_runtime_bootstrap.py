...
def test_bootstrap_environment_runs_local_health_checks_without_network_flag(
    ...
    config_path = _write_config_custom(
        ...
        environment_overrides={
            ...
            "adapter_settings": {"live_trading": {"base_url": "http://127.0.0.1:9999"}},
        },
    )
...
def test_live_checklist_blocks_on_missing_required_definition(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _stub_license_validation(monkeypatch, tmp_path)
    _disable_exchange_health(monkeypatch)
    ...
    assert "fraud_matrix" in str(exc.value)
    assert "brak definicji dokumentu" in str(exc.value)


def test_live_checklist_ignores_null_required_entries(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _stub_license_validation(monkeypatch, tmp_path)
    _disable_exchange_health(monkeypatch)
...
def test_live_checklist_blocks_on_duplicate_document_definition(
    ...
    assert "kyc_packet" in str(exc.value)
    assert "zduplikowana definicja dokumentu" in str(exc.value)
...
def test_live_checklist_invalid_signature_fallback_filters_optional_documents(
    ...
    tampered_signature_path = tmp_path / penetration_doc["signature_path"]
    signature_payload = json.loads(tampered_signature_path.read_text(encoding="utf-8"))
    signature_payload["signature"]["value"] = "A" + signature_payload["signature"]["value"]
    tampered_signature_path.write_text(
        json.dumps(signature_payload, ensure_ascii=False, indent=2),
...
def test_bootstrap_loads_tco_report_for_decision_engine(tmp_path: Path) -> None:
    ...
    tco_report_path = reports_dir / "baseline_tco.json"
    tco_report_payload = {
        "total": {"cost_bps": 9.5},
        "strategies": {
            "mean_reversion": {
    ...
    config_data["decision_engine"] = {
        ...
        "tco": {
            "reports": ["reports/baseline_tco.json"],
