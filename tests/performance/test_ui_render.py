...
def qml_engine(qt_app: "QGuiApplication") -> "QQmlEngine":  # pragma: no cover - infrastructure
    ...
    from PySide6.QtQml import QQmlEngine  # type: ignore[attr-defined]
...
def _load_component(engine: "QQmlEngine") -> "QQmlComponent":
    ...
    from PySide6.QtQml import QQmlComponent  # type: ignore[attr-defined]
...
def _create_component_instance(component: QQmlComponent) -> Any:
    instance = component.create()
    assert instance is not None
    return instance


def _dispose_component_instance(instance: Any) -> None:
...
def _render_component(instance: Any, properties: dict[str, Any]) -> float:
...
def test_feed_sla_panel_render_time_under_feed_spike(
    ...
    instance = _create_component_instance(component)
    try:
        latencies = [
            _render_component(
                instance,
        ...
        _dispose_component_instance(instance)
...
def test_sla_panels_render_across_telemetry_samples(
    ...
    try:
        for sample in feed_samples:
            ...
            latencies.extend(_render_component(instance, properties) for _ in range(3))
    finally:
        _dispose_component_instance(instance)
        qml_engine.collectGarbage()
