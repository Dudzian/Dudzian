...
def test_decision_orchestrator_activity_by_profile_tracks_counts(
    manual_profile: ManualProfile,
) -> None:
    other_profile = ManualProfile(
        name="alt-profile",
        max_positions=5,
        max_leverage=3.0,
        drawdown_limit=0.2,
    ...
    secondary_candidate_payload = {
        "strategy": "carry",
        "action": "enter",
        "risk_profile": other_profile.name,
        "symbol": "BTCUSDT",
        "notional": 200.0,
    }

    primary_candidate = DecisionCandidate.from_mapping(primary_candidate_payload)
    secondary_candidate = DecisionCandidate.from_mapping(secondary_candidate_payload)
    ...
    accepted_eval = DecisionEvaluation(
        ...
        risk_flags=(),
        stress_failures=(),
        model_expected_return_bps=10.0,
        model_success_probability=0.6,
        model_name="alpha",
        model_selection=None,
    )
    rejected_eval = DecisionEvaluation(
        ...
        risk_flags=(),
        stress_failures=(),
        model_expected_return_bps=5.0,
        model_success_probability=0.38,
        model_name="beta",
        model_selection=None,
    )
    ...
    engine = ThresholdRiskEngine(clock=lambda: datetime(2024, 7, 1, 9, 0, 0))
    engine.register_profile(manual_profile)
    engine.register_profile(other_profile)
    engine.attach_decision_orchestrator(orchestrator)

    account = _snapshot(2_000.0)
    first_order = _order(25_000.0)
    first_order.metadata = dict(first_order.metadata)
    first_order.metadata["decision_candidate"] = primary_candidate_payload
    engine.apply_pre_trade_checks(
        first_order,
        account=account,
        profile_name=manual_profile.name,
    ...
    second_order = _order(25_000.0)
    second_order.metadata = dict(second_order.metadata)
    second_order.metadata["decision_candidate"] = secondary_candidate_payload
    engine.apply_pre_trade_checks(
        second_order,
        account=account,
        profile_name=other_profile.name,
    )

    aggregate = engine.decision_orchestrator_activity()
    assert aggregate["attempts"] == 2
    assert aggregate["evaluated"] == 2
    assert aggregate["accepted"] == 1
    assert aggregate["rejected"] == 1

    per_profile = engine.decision_orchestrator_activity_by_profile()
    assert set(per_profile.keys()) == {manual_profile.name, other_profile.name}

    primary_stats = per_profile[manual_profile.name]
    assert primary_stats["attempts"] == 1
    assert primary_stats["evaluated"] == 1
    assert primary_stats["accepted"] == 1
    assert primary_stats["rejected"] == 0
    assert primary_stats["errors"] == 0
    assert primary_stats["duration_ms"]["count"] == 1

    secondary_stats = per_profile[other_profile.name]
    assert secondary_stats["attempts"] == 1
    assert secondary_stats["evaluated"] == 1
    assert secondary_stats["accepted"] == 0
    assert secondary_stats["rejected"] == 1
    assert secondary_stats["errors"] == 0
    assert secondary_stats["duration_ms"]["count"] == 1

    snapshot = engine.decision_orchestrator_activity_by_profile(reset=True)
    assert snapshot == per_profile
    assert engine.decision_orchestrator_activity_by_profile() == {}


def test_decision_orchestrator_activity_by_profile_separates_errors(
    manual_profile: ManualProfile,
) -> None:
    other_profile = ManualProfile(
        name="error-profile",
    ...
    failing_order = _order(25_000.0)
    failing_order.metadata = dict(failing_order.metadata)
    failing_order.metadata["decision_candidate"] = failing_candidate
    engine.apply_pre_trade_checks(
        failing_order,
        account=account,
        profile_name=manual_profile.name,
    ...
    per_profile = engine.decision_orchestrator_activity_by_profile()
    manual_stats = per_profile[manual_profile.name]
    error_stats = per_profile[other_profile.name]

    assert manual_stats["attempts"] == 1
    assert manual_stats["errors"] == 1
    assert manual_stats["error_reasons"] == {"evaluation_failed": 1}
    assert manual_stats["duration_ms"]["count"] == 1

    assert error_stats["attempts"] == 0
    assert error_stats["errors"] == 1
    assert error_stats["error_reasons"] == {"invalid_candidate_payload": 1}
    assert error_stats["duration_ms"]["count"] == 0
