...
def test_unconsumed_error_route_obeys_current_suppression_and_expiry(tmp_path):
    ...
    store.execute_escalation_route(
        intent_id=intent.intent_id, now_utc=at_expiry + timedelta(seconds=1)
    )
...
def test_historical_effect_finalizes_despite_later_suppression(tmp_path):
    ...
    store.execute_escalation_route(intent_id=intent.intent_id, now_utc=NOW + timedelta(seconds=303))
    assert adapter.external_effect_count == calls + 1
    assert (
...
def test_source_replay_never_rewrites_atomic_historical_time(tmp_path):
    ...
    committed = store.observe(
        alert_id="alert-001", evidence_set=evidence, now_utc=NOW + timedelta(seconds=1)
    )
    before = carrier.load_atomic_state()
    replay = store.observe(
        alert_id="alert-001", evidence_set=evidence, now_utc=NOW + timedelta(hours=1)
    )
