...
def _raw(
    ...
    *,
    ...
    scope = (
        {"market_data_route_id": f"mdr_{U}{scope_suffix}", "instrument_id": f"instr_{U}"}
    ...
    value = (
        {"last_data_at_utc": stamp, "sequence_state": "CURRENT"}
        if category == "MARKET_DATA_FRESHNESS"
        else {"path_state": "AVAILABLE"}
    )
    return {
        "observation_id": f"observation-{run[-4:]}-{second}",
        "category": category,
        "source_component": "core_host",
        "source_instance_id": run,
        "environment": environment,
        "scope": scope,
        "source_event_at_utc": stamp,
        ...
        "condition": condition,
        "reason_code": f"PROBE_{condition}",
        "value": value,
...
def _system():
    ...
    authority, publisher = ObservationAuthority.compose(
        carrier,
        policies=(_policy("MARKET_DATA_FRESHNESS"), _policy("EXECUTION_PATH_HEALTH")),
...
@pytest.mark.parametrize(
    "category,expected_type,severity",
    [
...
def test_failing_then_cross_runtime_ok_resolves_stable_identity(category, expected_type, severity):
    ...
    first = store.observe(
        alert_id="alert-1",
        evidence_set=SourceEvidenceSet((failing.acceptance_id,)),
        now_utc=datetime(2030, 1, 1, tzinfo=timezone.utc),
    )
    healthy = publish.publish(
        _raw(category, condition="OK", run=RUN_B, second=1), now_utc="2030-01-01T00:00:01Z"
    )
    second = store.observe(
        alert_id="ignored",
        evidence_set=SourceEvidenceSet((healthy.acceptance_id,)),
        now_utc=datetime(2030, 1, 1, 0, 0, 2, tzinfo=timezone.utc),
    )
    assert (first.alert_type, first.severity, second.lifecycle_state) == (
        expected_type,
        severity,
        "RESOLVED",
    )
...
def test_effective_condition_and_frozen_severity(category, condition, severity):
    ...
    alert = store.observe(
        alert_id="alert",
        evidence_set=SourceEvidenceSet((accepted.acceptance_id,)),
        now_utc=datetime(2030, 1, 1, tzinfo=timezone.utc),
    )
...
def test_unconsumed_old_runtime_is_stale_and_cannot_mutate_store(category):
    ...
    with pytest.raises(AlertStoreError, match="SOURCE_EVIDENCE_STALE"):
        store.observe(
            alert_id="forged",
            evidence_set=SourceEvidenceSet((old.acceptance_id,)),
            now_utc=datetime(2030, 1, 1, 0, 0, 2, tzinfo=timezone.utc),
        )
    ...
    with pytest.raises(AlertStoreError, match="ALERT_NOT_FOUND"):
        store.observe(
            alert_id="none",
            evidence_set=SourceEvidenceSet((new.acceptance_id,)),
            now_utc=datetime(2030, 1, 1, 0, 0, 2, tzinfo=timezone.utc),
        )
...
def test_acceptance_membership_not_fingerprint_and_committed_replay_survives_supersession():
    ...
    first = publish.publish(
        _raw("MARKET_DATA_FRESHNESS", condition="BLOCKED"), now_utc="2030-01-01T00:00:00Z"
    )
    original = store.observe(
        alert_id="alert",
        evidence_set=SourceEvidenceSet((first.acceptance_id,)),
        now_utc=datetime(2030, 1, 1, tzinfo=timezone.utc),
    )
    before = alerts.load().store_revision
    publish.publish(
        _raw("MARKET_DATA_FRESHNESS", condition="OK", run=RUN_B, second=1),
    ...
    with pytest.raises(AlertStoreError, match="SOURCE_EVIDENCE_UNACCEPTED"):
        store.observe(
            alert_id="forged",
            evidence_set=SourceEvidenceSet((first.content_fingerprint,)),
            now_utc=datetime(2030, 1, 1, 0, 0, 2, tzinfo=timezone.utc),
        )
...
def test_alert_carrier_failure_has_no_partial_decision_and_retry_succeeds():
    _, publish, _, alerts, store = _system()
    accepted = publish.publish(
        _raw("EXECUTION_PATH_HEALTH", condition="BLOCKED"), now_utc="2030-01-01T00:00:00Z"
    )
