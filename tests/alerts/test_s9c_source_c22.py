...
def test_later_same_second_runtime_cannot_invalidate_durable_source_decision(category):
    ...
    with pytest.raises(ValueError, match="SEMANTIC_TRANSACTION_TIME_COLLISION"):
        publisher.publish(
            _raw(category, condition="OK", run=RUN_B, second=10), now_utc="2030-01-01T00:00:10Z"
        )
    ...
    restored = AlertStore.restore(object(), S9CObservationSourceAuthority(authority), alert_carrier)
