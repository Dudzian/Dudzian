...
def test_delivery_recovery_preserves_intervening_operator_edge(tmp_path):
    ...
    with pytest.raises(AlertStoreError, match="CARRIER_COMMIT_FAILED"):
        store.request_delivery("alert-001", expected_alert_revision=1,
            expected_delivery_revision=0, attempt_id="interleaved", now_utc=NOW)
    request, proof = authorized(auth, authority, item, 1)
    store.mutate(alert_id="alert-001", proof=proof, request=request,
        expected_alert_revision=1, now_utc=NOW + timedelta(seconds=1))
    recovered = store.request_delivery("alert-001", expected_alert_revision=1,
        expected_delivery_revision=0, attempt_id="interleaved",
    ...
    assert [x.mutation_type for x in store.snapshot().mutation_history][-2:] == [
        "ACKNOWLEDGE", "DELIVERY_ATTEMPT"]
    assert len([x for x in store.snapshot().delivery_attempts if x.attempt_id == "interleaved"]) == 1
    ...
    assert store.request_delivery("alert-001", expected_alert_revision=1,
        expected_delivery_revision=0, attempt_id="interleaved",
        now_utc=NOW + timedelta(hours=1)) == recovered
...
def test_route_recovery_after_sibling_route_commit(tmp_path):
    ...
    carrier.fail_next = "DURING"
    with pytest.raises(AlertStoreError, match="CARRIER_COMMIT_FAILED"):
        store.execute_escalation_route(intent_id=first.intent_id,
            now_utc=NOW + timedelta(seconds=301))
    store.execute_escalation_route(intent_id=second.intent_id,
...
class MutableSequenceCarrier:
    def __init__(self, state):
        self.state = AtomicAlertAuthorityState(
            replace(state.alert_store_snapshot,
