...
def test_backdated_later_publication_cannot_invalidate_durable_decision(category):
    ...
    first = publisher.publish(
        _raw(category, condition="BLOCKED", second=10), now_utc="2030-01-01T00:00:10Z"
    )
    store.observe(
        alert_id="alert",
    ...
    with pytest.raises(ValueError, match="TRANSACTION_TIME_ROLLBACK"):
        publisher.publish(
            _raw(category, condition="OK", run=RUN_B, second=5), now_utc="2030-01-01T00:00:05Z"
        )
...
def test_new_source_edge_cannot_precede_acceptance_or_current_alert(category):
    ...
    successor = publisher.publish(
        _raw(category, condition="BLOCKED", run=RUN_B, second=11), now_utc="2030-01-01T00:00:11Z"
    )
    before = carrier.load().store_revision
    with pytest.raises(AlertStoreError, match="TIME_ROLLBACK"):
        store.observe(
            alert_id="ignored",
            evidence_set=SourceEvidenceSet((successor.acceptance_id,)),
            now_utc=datetime(2030, 1, 1, 0, 0, 9, tzinfo=timezone.utc),
        )
