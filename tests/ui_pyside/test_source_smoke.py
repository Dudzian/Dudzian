...
def test_block_c_read_only_binding_visible_source_contract() -> None:
    ...
    for token in BLOCK_C_READ_ONLY_BINDING_CONTROLLED_SOURCE_TOKENS:
        assert token in dashboard_source
...
def _block_c_safe_controlled_source(
    *,
    integration_fallback: str = '"blocked"',
    extra_panel_token: str = "",
) -> str:
    return (
...
def test_block_c_read_only_binding_visible_source_allows_safe_export_label() -> None:
    ...
    assert evidence["safe_fallbacks_present"] is True
    assert evidence["no_action_controls"] is True
    assert "integration gate: blocked" not in safe_source


def test_block_c_read_only_binding_visible_source_rejects_missing_blocked_fallback() -> None:
    unsafe_source = _block_c_safe_controlled_source().replace(
        'blockCReadOnlyBindingValue("integrationGateStatus", "blocked")',
        'blockCReadOnlyBindingValue("integrationGateStatus", "open")',
    )
    ...
    assert "safe_fallbacks_present" in evidence["failed_checks"]
...
def test_block_c_read_only_binding_visible_source_requires_integration_gate_fallback() -> None:
    ...
    assert evidence["integration_gate_blocked_fallback_present"] is False
    assert "safe_fallbacks_present" in evidence["failed_checks"]
    assert "integration_gate_blocked_fallback_present" in evidence["failed_checks"]
...
def test_block_c_read_only_binding_visible_source_blocks_action_controls(
    ...
    unsafe_source = _block_c_safe_controlled_source(extra_panel_token=forbidden_token)
...
def test_runtime_session_control_live_shape_evidence_complete() -> None:
    evidence = _build_runtime_session_control_live_shape_evidence(
        _runtime_session_control_green_audit()
    )
...
def test_runtime_session_control_live_shape_evidence_fail_closed() -> None:
    audit = _runtime_session_control_green_audit()
    audit["no_live_reconnect_visible"] = False
...
def test_runtime_session_control_qml_contract_source() -> None:
    main_window = (QML_SOURCE_ROOT / "MainWindow.qml").read_text(encoding="utf-8")

    for snippet in (
        'objectName: "runtimeSessionControlPanel"',
        'objectName: "runtimeSessionPanelLiveShapeCard"',
...
def test_frontend_live_parity_evidence_all_required_sections_present() -> None:
    ...
    assert evidence["frontend_live_parity_runtime_session_control_present"] is True
    assert evidence["runtime_session_control_live_shape_parity_complete"] is True
...
def test_ui_preview_8_0j_final_top_level_panel_registry_and_order_contract() -> None:
    ...
    expected_panels = (
        ...
        ("runtimeSessionControlPanel", "Runtime / Sesja", 13),
        ("helpGlossaryPanel", "Pomoc / Słownik", 14),
    )
