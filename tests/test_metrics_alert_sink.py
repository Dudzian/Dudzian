...
def _install_monotonic(monkeypatch: pytest.MonkeyPatch, start: float) -> dict[str, float]:
    monotonic_time: dict[str, float] = {"value": start}
...
def test_reduce_motion_incident_recovery_emits_alert(
    ...
    monotonic_time = _install_monotonic(monkeypatch, 50.0)
...
def test_reduce_motion_incident_realert_respects_cooldown(
    ...
    monotonic_time = _install_monotonic(monkeypatch, 0.0)
...
def test_overlay_budget_realert_after_cooldown(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monotonic_time = _install_monotonic(monkeypatch, 200.0)
...
def test_overlay_budget_escalates_after_duration(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monotonic_time = _install_monotonic(monkeypatch, 10.0)
...
def test_overlay_budget_recovery_includes_duration(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monotonic_time = _install_monotonic(monkeypatch, 500.0)
