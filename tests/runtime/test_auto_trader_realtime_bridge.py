...
def _build_daily_trend_controller(
...
def test_auto_trader_executes_realtime_controller_cycle() -> None:
    ...
    try:
        ...
        assert list(execution_service.ledger()), (
            "AutoTrader did not trigger execution via controller runner"
        )
        ...
        assert trader._last_signal == "buy", (
            "AutoTrader should expose last signal generated by the controller"
        )

        telemetry = trader.get_last_controller_cycle()
        assert telemetry is not None, (
            "AutoTrader should expose telemetry for the last controller cycle"
        )
        ...
        event = next(
            (item for item in emitter.events if item[0] == "auto_trader.controller_cycle"), None
        )
